...
test.describe("GL Post & Close", () => {
  ...
  test("create and post a journal entry", async ({
    page,
    glJournalPage,
    dashboardPage,
  }: any) => {
    ...
    // Fill in reference
    const refField = page
      .locator('[name="reference"], [name*="ref"], [name*="name"]')
      .first();
    ...
    if (await descField.isVisible()) {
      await descField.fill(
        `Test journal entry for GL post/close journey — ${ref}`,
      );
    }
  ...
  test("drill-down from GL to source journal entry", async ({
    ...
    const viewLink = page
      ...
      .filter({ hasText: /view|open|drill/i })
      .first();
