...
test.describe("Tenant Isolation", () => {
  ...
  test("data created by tenant A is visible to tenant A", async ({
    ...
    const customerField = page
      .locator('[name="customerId"], [name="customer"], [id*="customer"]')
      .first();
  ...
  test("tenant-switch mechanism is accessible", async ({
    ...
    const tenantSwitcher = page
      .locator('[class*="tenant"], [id*="tenant"], [aria-label*="tenant"]')
      .first();
    const userMenu = page
      .locator('[class*="user-menu"], [class*="avatar"], [class*="profile"]')
      .first();
    ...
    if (await tenantSwitcher.isVisible()) {
      ...
      await page.waitForTimeout(1_000);
      const switchOption = page
        .locator('button, a, [role="menuitem"]')
        .filter({ hasText: /switch|tenant|change/i })
        .first();
