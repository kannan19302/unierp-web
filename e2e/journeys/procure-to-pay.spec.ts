...
test.describe("Procure-to-Pay", () => {
  ...
  test("complete procure-to-pay flow", async ({
    page,
    purchaseOrderPage,
    goodsReceiptPage,
    invoicePage,
    ...
    const vendorField = page
      .locator(
        '[name="vendorId"], [name="vendor"], [id*="vendor"], [placeholder*="vendor"]',
      )
      .first();
    ...
    const poRefField = page
      .locator('[name*="reference"], [name*="name"]')
      .first();
    ...
    const newGrBtn = page
      .locator("a, button")
      .filter({ hasText: /new|create|add.?receipt|record.?receipt/i })
      .first();
    ...
    const grPoField = page
      .locator(
        '[name="purchaseOrderId"], [name*="order"], [placeholder*="order"]',
      )
      .first();
    ...
    const grItemField = page
      .locator('[name*="item"], [placeholder*="item"]')
      .first();
    if (await grItemField.isVisible()) {
      await grItemField.fill("IT Equipment");
    }
    const grQtyField = page
      .locator('[name*="quantity"], [type="number"]')
      .first();
    ...
    const grSaveBtn = page
      .locator('button[type="submit"], button')
      .filter({ hasText: /save|submit|record/i })
      .first();
    ...
    const vendInvField = page
      .locator('[name="vendorId"], [name="vendor"], [placeholder*="vendor"]')
      .first();
    ...
    const newPayBtn = page
      .locator("a, button")
      .filter({ hasText: /new|create|add.?payment|record.?payment/i })
      .first();
    ...
    const payInvField = page
      .locator(
        '[name="invoiceId"], [name*="invoice"], [placeholder*="invoice"]',
      )
      .first();
    if (await payInvField.isVisible()) {
      await payInvField.fill(vendInvRef);
    }
    const payAmountField = page
      .locator('[name="amount"], [type="number"]')
      .first();
    if (await payAmountField.isVisible()) {
      await payAmountField.fill("1000");
    }
    const payRefField = page
      .locator('[name="reference"], [name="ref"]')
      .first();
    if (await payRefField.isVisible()) {
      await payRefField.fill(`PAY-PTP-${Date.now()}`);
    }

    const paySaveBtn = page
      .locator('button[type="submit"], button')
      .filter({ hasText: /save|submit|record/i })
      .first();
