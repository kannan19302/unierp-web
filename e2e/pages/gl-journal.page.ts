...
export class GLJournalPage {
  ...
  constructor(page: Page) {
    ...
    this.newEntryButton = page
      .locator("a, button")
      .filter({ hasText: /new|create|add.?entry|add.?journal/i })
      .first();
    this.saveButton = page
      .locator('button[type="submit"], button')
      .filter({ hasText: /save|submit|create|draft/i })
      .first();
    this.postButton = page
      .locator("button")
      .filter({ hasText: /post|submit|record/i })
      .first();
  ...
  async createEntry(data: JournalEntryData) {
    ...
    if (data.date) {
      const field = this.page
        .locator('[name="date"], [name="entryDate"], [type="date"]')
        .first();
    ...
    if (data.reference) {
      const field = this.page
        .locator('[name="reference"], [name="ref"], [placeholder*="reference"]')
        .first();
    ...
    if (data.description) {
      const field = this.page
        .locator(
          '[name="description"], [name="notes"], [placeholder*="description"]',
        )
        .first();
    ...
    if (data.entries) {
      for (const entry of data.entries) {
        const acctField = this.page
          .locator('[name*="account"], [placeholder*="account"]')
          .first();
        ...
        if (entry.debit && (await debitField.isVisible()))
          await debitField.fill(String(entry.debit));
        if (entry.credit && (await creditField.isVisible()))
          await creditField.fill(String(entry.credit));
      }
    }
