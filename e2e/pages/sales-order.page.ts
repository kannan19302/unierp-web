...
export class SalesOrderPage {
  ...
  constructor(page: Page) {
    ...
    this.newOrderButton = page
      .locator("a, button")
      .filter({ hasText: /new|create|add.?order/i })
      .first();
  ...
  async createOrder(data: SalesOrderData) {
    ...
    if (data.items) {
      for (let i = 0; i < data.items.length; i++) {
        ...
        const itemField = this.page
          .locator(`[name*="item"], [name*="product"], [placeholder*="item"]`)
          .first();
        const qtyField = this.page
          .locator(`[name*="quantity"], [name*="qty"], [type="number"]`)
          .first();
        const rateField = this.page
          .locator(`[name*="rate"], [name*="price"], [name*="amount"]`)
          .first();
    ...
    if (data.deliveryDate) {
      const dateField = this.page
        .locator('[name="deliveryDate"], [name*="delivery"], [type="date"]')
        .first();
