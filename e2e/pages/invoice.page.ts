...
export class InvoicePage {
  ...
  constructor(page: Page) {
    ...
    this.newInvoiceButton = page
      .locator("a, button")
      .filter({ hasText: /new|create|add.?invoice/i })
      .first();
  ...
  async createInvoice(data: InvoiceData) {
    ...
    if (data.orderRef) {
      const field = this.page
        .locator('[name="orderId"], [name*="order"], [placeholder*="order"]')
        .first();
      if (await field.isVisible()) {
    ...
    if (data.items) {
      for (const item of data.items) {
        ...
        if (await amountField.isVisible())
          await amountField.fill(String(item.amount));
      }
    }

    if (data.dueDate) {
      const field = this.page
        .locator('[name="dueDate"], [type="date"]')
        .first();
