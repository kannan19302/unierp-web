...
const container = (max = 1100): React.CSSProperties => ({
  maxWidth: max,
  margin: "0 auto",
});
  ...
  return (
    <section style={section()}>
      <div style={{ ...container(760), textAlign: align }}>
        ...
        <div
          style={{ fontSize: "1.05rem", lineHeight: 1.7, opacity: 0.85 }}
          dangerouslySetInnerHTML={{
            __html:
              content ||
              "<p>Add your content in the inspector. HTML is supported.</p>",
          }}
        />
      </div>
    </section>
      ...
      <div style={{ ...container(maxWidth), textAlign: "center" }}>
        {url ? (
          <img
            src={url}
            alt={caption || ""}
            style={{ width: "100%", borderRadius: rounded ? 14 : 0 }}
          />
        ) : (
          <div
            style={{
        ...
        {caption && (
          <p style={{ marginTop: 12, opacity: 0.6, fontSize: "0.9rem" }}>
            {caption}
          </p>
        )}
      </div>
      ...
      <div style={container()}>
        ...
        {list.length === 0 ? (
          <div style={{ textAlign: "center", opacity: 0.5, padding: 40 }}>
            Add image URLs in the inspector.
          </div>
        ) : (
          <div
            style={{
...
export function ColumnsBlock({
  ...
  const shown = cols.length
    ? cols
    : [
        ["Column one", "Edit in inspector"],
        ["Column two", "Edit in inspector"],
      ];
  return (
    <section style={section()}>
      <div
...
export function LogoCloudBlock({
  ...
  const list: string[] = Array.isArray(logos)
    ? logos
    : String(logos || "")
        .split("\n")
        .map((s: any) => s.trim())
        ...
        >
          {(list.length ? list : fallback).map((l: any, i: any) =>
            list.length ? (
              ...
              <span
                key={i}
                style={{ fontSize: "1.3rem", fontWeight: 800, opacity: 0.35 }}
              >
                {l}
              </span>
            ),
  ...
  return (
    ...
    >
      <div style={{ ...container(760), textAlign: "center" }}>
        <h2 style={{ fontSize: "2.25rem", fontWeight: 800, marginBottom: 12 }}>
          {title}
        </h2>
        {subtitle && (
...
export function NavbarBlock({
  ...
  const cart = useCart();
  const list: { label: string; url: string }[] = Array.isArray(links)
    ? links
    : String(
        links || "Home=/\nProducts=/products\nAbout=/about\nContact=/contact",
      )
    ...
    >
      ...
      >
        <div style={{ maxWidth: 320 }}>
          ...
          {tagline && (
            <p style={{ opacity: 0.6, marginTop: 10, lineHeight: 1.6 }}>
              {tagline}
            </p>
          )}
        </div>
...
export function CartBlock({ title = "Your Cart", tenantSlug }: any) {
  ...
  const [state, setState] = useState<"idle" | "placing" | "done" | "error">(
    "idle",
  );
    ...
    return (
      <section style={section()}>
        ...
        >
          ...
          <h2 style={{ fontSize: "1.6rem", fontWeight: 800, margin: "8px 0" }}>
            Order placed!
          </h2>
          <p style={{ opacity: 0.8 }}>
            Confirmation <strong>{order?.orderNumber}</strong> · Total{" "}
  ...
  return (
    <section style={section()}>
      ...
      >
        <div>
          <h2 style={{ fontSize: "1.6rem", fontWeight: 700, marginBottom: 20 }}>
            {title}
          </h2>
          {cart.items.length === 0 ? (
        ...
        >
          ...
          <button
            type="submit"
            disabled={state === "placing" || cart.items.length === 0}
            style={{
              padding: "14px",
              borderRadius: 10,
              border: "none",
