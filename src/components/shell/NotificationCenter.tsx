...
export function NotificationCenter({ iconBtnStyle }: { iconBtnStyle: string }) {
  ...
  const load = useCallback(async () => {
    try {
      const data = await client.get<AppNotification[]>(
    ...
    <div className="relative" ref={containerRef}>
      ...
      {open && (
        ...
        >
          ...
          >
            <p className="ui-dropdown-header" style={{ padding: 0, margin: 0 }}>
              Notifications
            </p>
            {unreadCount > 0 && (
              <button
              ...
              items.map((n) => (
                ...
                >
                  ...
                  >
                    ...
                    <span className="ui-text-micro" style={{ opacity: 0.7 }}>
                      {timeAgo(n.createdAt)}
                    </span>
                  </span>
                  {n.status === "READ" && (
                    <Check
                      size={12}
                      style={{ opacity: 0.5, flexShrink: 0, marginTop: 4 }}
                    />
                  )}
                </button>
              ))
