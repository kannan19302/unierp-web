...
import { Card, PageHeader, Button, Spinner, ListPageTemplate, type ListColumn } from "@kannan19302/ui";
  ...
  const fetchData = async () => {
    ...
    try {
      const [requirementsData, reportsData] = await Promise.all([
        client.get<Requirement[] | { data?: Requirement[] }>(
          "/advanced-hr/skills/requirements",
        ),
        client.get<GapReport[] | { data?: GapReport[] }>(
          "/advanced-hr/skills/gap-analysis",
        ),
      ]);
      setRequirements(
      ...
      setReports(
        Array.isArray(reportsData) ? reportsData : reportsData.data || [],
      );
    } catch {
    } finally {
      setLoading(false);
                ...
                <Card key={req.id} padding="sm">
                  ...
                  <div className={styles.s10}>
                    ...
                    <span className="font-bold">
                      Required: Lvl {req.requiredLevel}
                    </span>
                  </div>
                </Card>
              ))
                    ...
                    render: (_: any, row: any) => {
                      ...
                      return rep.gapsCount === 0 ? (
                        <span className={styles.s11}>
                          <Check size={14} /> Meets Requirements
                        </span>
                      ) : (
