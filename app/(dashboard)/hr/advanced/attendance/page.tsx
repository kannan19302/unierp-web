...
import { Card, PageHeader, StatusBadge, Button, ListPageTemplate, type ListColumn } from "@kannan19302/ui";
...
export default function AttendancePage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [empRes, attRes] = await Promise.all([
        ...
        client.get<AttendanceRecord[]>(
          `/api/v1/advanced-hr/attendance${selectedEmployee ? `?employeeId=${selectedEmployee}` : ""}`,
        ),
      ]);
      setEmployees(empRes);
      setRecords(attRes);
    } catch {
      setError("API connection issue");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCheckIn = async () => {
    ...
    try {
      await client.post("/api/v1/advanced-hr/attendance/check-in", {
  ...
  const handleCheckOut = async () => {
    ...
    try {
      await client.post("/api/v1/advanced-hr/attendance/check-out", {
        employeeId: selectedEmployee,
      });
  ...
  const handleSimulateRFID = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!simCode) {
      setMsg("Enter an employee code");
      return;
    }
    setSimulating(true);
    try {
      const res = await client.post(
        "/api/v1/advanced-hr/attendance/biometric",
        { employeeCode: simCode, actionType: simAction },
      );
      if (res) {
    ...
    <RouteGuard permission="hr.attendance.read">
      <div className="ui-stack-6 ui-animate-in">
        ...
        <div className={styles.layout}>
          ...
          <Card padding="md">
            <h4 className={styles.simulatorTitle}>
              <Cpu size={16} /> Biometric/RFID Simulator
            </h4>
            <p className={styles.simulatorDescription}>
              Simulate a physical RFID terminal scanner tap. This calls the
              backend integration webhook `/attendance/biometric`.
            </p>
