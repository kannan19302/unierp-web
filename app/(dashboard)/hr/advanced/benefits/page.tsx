...
export default function BenefitsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to load benefits data";
      setLoadError(message);
      notifyError("Failed to load benefits data", message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateScheme = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to create benefit scheme";
      setMsg(message);
      notifyError("Failed to create benefit scheme", message);
    }
  };
  ...
  const handleEnroll = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to enroll employee";
      setMsg(message);
      notifyError("Failed to enroll employee", message);
    }
  };
      ...
      <div className="ui-stack-6 ui-animate-in">
        ...
        {loadError && (
          <div className="ui-alert ui-alert-danger">
            ...
            Failed to load benefits data — figures below may be stale or
                ...
                schemes.map((s: any) => (
                  <Card key={s.id} padding="sm">
                    ...
                    <div className={styles.schemeCosts}>
                      <span>
                        EE: ${Number(s.employeeCostShare).toFixed(2)}/mo
                      </span>
                      <span>
                        ER: ${Number(s.employerCostShare).toFixed(2)}/mo
                      </span>
                    </div>
                  </Card>
                ))
                ...
                const enrollmentColumns: ListColumn[] = [
                      ...
                      return (
                        <div>
                          <div className="font-semibold">
                            {getEmployeeName(enr.employeeId)}
                          </div>
                          <div className="ui-text-caption">
                            {getEmployeeCode(enr.employeeId)}
                          </div>
                        </div>
                      );
                    },
                  },
                  {
                    ...
                    render: (_v: any, row: any) => {
                      ...
                      return (
                        <div>
                          <div className={styles.enrollmentScheme}>
                            {enr.scheme?.name}
                          </div>
                          <div className="ui-text-caption">
                            {enr.scheme?.provider}
                    ...
                    render: (_v: any, row: any) => {
                      ...
                      return (
                        <div>
                          <div>
                            EE: $
                            {Number(enr.scheme?.employeeCostShare).toFixed(2)}
                            /mo
                          </div>
                          <div>
                            ER: $
                            {Number(enr.scheme?.employerCostShare).toFixed(2)}
                            /mo
                          </div>
                        </div>
                      );
