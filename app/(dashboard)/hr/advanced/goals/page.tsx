...
export default function GoalsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [goalsRes, empRes] = await Promise.all([
        ...
        client.get<Array<{ id: string; firstName: string; lastName: string }>>(
          "/api/v1/hr/employees",
        ),
      ]);
      setGoals(goalsRes);
      ...
      notifyError("Failed to load goals", message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const createGoal = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post("/api/v1/advanced-hr/goals", form);
      {
        setMsg("Goal created");
        setShowForm(false);
        fetchData();
      ...
      setMsg(message);
      notifyError("Failed to create goal", message);
    }
  };

  const updateKR = async (krId: string, current: number) => {
    await client.request(`/api/v1/advanced-hr/key-results/${krId}`, {
      method: "PUT",
  ...
  const submitComment = async (goalId: string) => {
    ...
    try {
      ...
      notifyError("Failed to post comment", message);
    }
  };
  ...
  return (
    <RouteGuard permission="hr.goals.read">
      <div className="ui-stack-6">
        ...
        {showForm && (
          <Card padding="md">
            <form onSubmit={createGoal} className="ui-stack-3">
              <select
                className="ui-input"
              ...
              <Card key={g.id} padding="md">
                ...
                <div className={styles.comments}>
                  <h4 className={styles.commentsTitle}>
                    <MessageSquare size={14} /> Comments & Efforts Log (
                    {g.comments?.length || 0})
                  </h4>
                  ...
                  <div className={styles.commentList}>
                      ...
                      <div key={c.id} className={styles.commentCard}>
                        ...
                        {c.fileUrl && (
                          <div className={styles.attachmentRow}>
                            ...
                            <button
                              onClick={() =>
                                downloadFile(c.fileUrl!, c.fileName!)
                              }
                    ...
                    {(!g.comments || g.comments.length === 0) && (
                      <div className={styles.empty}>
                        No efforts logs or comments posted.
                      </div>
                    )}
                  </div>
                  ...
                  <div className={styles.commentForm}>
                    ...
                    <div className={styles.attachmentControls}>
                      <label className={styles.attachmentLabel}>
                        <Paperclip size={12} />
                        <span>
                          Attach Completion Proof (PDF/Image/Document)
                        </span>
                        <input
                          type="file"
