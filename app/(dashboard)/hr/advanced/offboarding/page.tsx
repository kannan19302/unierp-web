...
export default function OffboardingPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [chkRes, empRes] = await Promise.all([
        client.get<OffboardingChecklist[]>(
          "/api/v1/advanced-hr/offboarding/checklists",
        ),
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Failed to load offboarding checklists";
      setLoadError(message);
      notifyError("Failed to load offboarding checklists", message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const createChecklist = async (e: React.FormEvent) => {
    ...
    try {
      await client.post("/api/v1/advanced-hr/offboarding/checklists", {
        employeeId,
        exitDate,
        exitReason,
        items: parsedItems,
  ...
  const handleUpdateItem = async (
    itemId: string,
    updates: { task?: string; status?: string; comments?: string },
  ) => {
    try {
      await client.request(`/api/v1/advanced-hr/offboarding/items/${itemId}`, {
        method: "PUT",
  ...
  const handleAddItem = async (checklistId: string, task: string) => {
    ...
    try {
      await client.post(
        `/api/v1/advanced-hr/offboarding/checklists/${checklistId}/items`,
        { task },
      );
      {
        ...
        {showForm && (
          <Card padding="md">
            ...
            <form onSubmit={createChecklist} className="ui-stack-3">
              ...
              <div className="ui-grid-2 ui-gap-3">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Contract Termination">
              ...
              <div>
                <label className="ui-text-caption">
                  Exit Clearance Procedures (one per line)
                </label>
                <textarea
                  className="ui-input"
                  value={tasksText}
              ...
              checklists.map((list) => (
                <Card key={list.id} padding="md">
                  ...
                  <div className={styles.s7}>
                    ...
                    {new Date(list.exitDate).toLocaleDateString()}
                  </div>
                          ...
                          {!isEditing && (
                            <div className={styles.s19}>
                              <input
                                ...
                                className={`ui-input ${styles.dyn2}`}
                                style={{
                                  background:
                                    itemStatus === "ONHOLD"
