...
export default function PayrollPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      notifyError("Failed to load payroll data", message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const runPayrollProcess = async (e: React.FormEvent) => {
    ...
    try {
      const res = await client.post(
        "/api/v1/advanced-hr/payroll/run",
        payrollForm,
      );
      if (res) {
  ...
  const saveTaxRule = async (e: React.FormEvent) => {
    ...
    try {
      const res = await client.post("/api/v1/advanced-hr/tax-tables", {
        ...
        incomeBracketMax: taxForm.incomeBracketMax
          ? parseFloat(taxForm.incomeBracketMax)
          : null,
                    ...
                    <div className={styles.s8}>
                      <div>
                        <h4 className="m-0">
                          ...
                          <ArrowRight size={12} className={styles.s9} />{" "}
