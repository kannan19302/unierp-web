...
import { Card, PageHeader, Spinner, ListPageTemplate, useToast, type ListColumn } from "@kannan19302/ui";
import {
  BarChart3,
  Users,
  CreditCard,
...
export default function AnalyticsPage() {
  ...
  const [compensation, setCompensation] =
    useState<CompensationAnalytics | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      const [headRes, compRes, costRes, budgetRes] = await Promise.all([
        client.get<HeadcountAnalytics>(
          "/api/v1/advanced-hr/analytics/headcount",
        ),
        client.get<CompensationAnalytics>(
          "/api/v1/advanced-hr/analytics/compensation",
        ),
        client.get<CostAnalytics>("/api/v1/advanced-hr/analytics/cost"),
        client.get<BudgetVariance[]>(
          "/api/v1/advanced-hr/positions/budget-variance",
        ),
      ]);
      setHeadcount(headRes);
      setCompensation(compRes);
      setCost(costRes);
      setBudgetVariance(budgetRes);
      setLoadError(null);
    } catch (err) {
      const message =
      ...
      notifyError("Failed to load workforce analytics", message);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="ui-stack-6 ui-animate-in">
        ...
        {loadError && (
          <div className="ui-alert ui-alert-danger">
            ...
            Failed to load workforce analytics — figures below may be stale or
          ...
          <>
            ...
            <div className="ui-grid-auto">
              ...
              <Card>
                ...
                <h2 className={styles.kpiValue}>
                  {headcount?.turnoverRate
                    ? `${Number(headcount.turnoverRate).toFixed(1)}%`
                    : "0%"}
                </h2>
              </Card>
              ...
              <Card>
                ...
                <h2 className={styles.kpiValue}>
                  {headcount?.timeToHire
                    ? `${headcount.timeToHire} days`
                    : "15 days"}
                </h2>
              </Card>
            </div>
            ...
            <div className="ui-grid-2 ui-gap-6">
              ...
              <Card padding="md">
                <h3 className={styles.sectionTitle}>
                  <BarChart3 size={18} className="ui-text-primary" /> Department
                  Headcount
                </h3>
              ...
              <Card padding="md">
                <h3 className={styles.sectionTitle}>
                  <CreditCard size={18} className="ui-text-success" /> Salary
                  Statistics
                </h3>
                <div className={styles.metrics}>
                  <div className={styles.metricRow}>
                    ...
                    <span className="font-semibold">
                      ${(compensation?.min || 0).toLocaleString()}/mo
                    </span>
                  </div>
                  <div className={styles.metricRow}>
                    <span className="ui-text-muted">Median Salary</span>
                    <span className="font-semibold">
                      ${(compensation?.median || 0).toLocaleString()}/mo
                    </span>
                  </div>
                  <div className={styles.metricRow}>
                    <span className="ui-text-muted">Maximum Salary</span>
                    <span className="font-semibold">
                      ${(compensation?.max || 0).toLocaleString()}/mo
                    </span>
                  </div>
                  <div className="ui-flex-between">
                    ...
                    <span className={styles.successValue}>
                      ${Math.round(cost?.costPerEmployee || 0).toLocaleString()}
                      /yr
                    </span>
                  </div>
                </div>
              ...
              {(() => {
                ...
                return (
                  <ListPageTemplate
                    ...
                    data={
                      budgetVariance as unknown as Record<string, unknown>[]
                    }
                    loading={false}
