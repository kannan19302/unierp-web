...
export interface UserStatusSchedule {
  id: string;
  presence: string;
...
export interface ChannelAnalytics {
  ...
  totals: { messageCount: number; activeUsers: number; reactions: number };
}
...
export interface ThreadView {
  parent: ConnectMessage | null;
  replies: ConnectMessage[];
}
...
export const uid = (p = "id") =>
  `${p}-${Date.now().toString(36)}-${(idc++).toString(36)}`;
...
export const formatDateSmart = (ts: number) => {
  ...
  if (diff < 7 * 86400000)
    return (
      d.toLocaleDateString([], { weekday: "short" }) + " " + formatTime(ts)
    );
  return (
...
async function req<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  const res = await fetch(`${BASE}${path}`, {
    ...init,
    credentials: "include",
    headers: { ...headers(init?.method), ...(init?.headers || {}) },
  });
  if (!res.ok) {
...
export function uploadAttachment(
  ...
  promise: Promise<{ documentId: string; attachment: Attachment }>;
  xhr: XMLHttpRequest;
} {
...
export const api = {
  ...
  messages: (channelId: string) =>
    req<ConnectMessage[]>(`/channels/${channelId}/messages`),
  markRead: (channelId: string) =>
    req<{ ok: boolean }>(`/channels/${channelId}/read`, { method: "POST" }),
  send: (
    channelId: string,
    body: { content: string; parentId?: string; attachments?: Attachment[] },
  ) =>
    req<ConnectMessage>(`/channels/${channelId}/messages`, {
      method: "POST",
  ...
  edit: (id: string, content: string) =>
    req<ConnectMessage>(`/messages/${id}`, {
      method: "PATCH",
      body: JSON.stringify({ content }),
    }),
  remove: (id: string) =>
    req<ConnectMessage>(`/messages/${id}`, { method: "DELETE" }),
  pin: (id: string) =>
    req<ConnectMessage>(`/messages/${id}/pin`, { method: "POST" }),
  react: (id: string, emoji: string) =>
    req<{ messageId: string; reactions: Reaction[] }>(
      `/messages/${id}/reactions`,
      { method: "POST", body: JSON.stringify({ emoji }) },
    ),
  bookmark: (id: string) =>
    req<{ messageId: string; bookmarked: boolean }>(
      `/messages/${id}/bookmark`,
      { method: "POST" },
    ),
  getBookmarks: () => req<ConnectMessage[]>("/bookmarks"),
  createSpace: (name: string, emoji?: string) =>
    req<Space>("/spaces", {
      method: "POST",
  ...
  createChannel: (body: {
    ...
    req<Conversation>("/channels", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  createDM: (userId: string) =>
    req<Conversation>("/channels/dm", {
      method: "POST",
      body: JSON.stringify({ userId }),
    }),
  createGroup: (name: string, memberIds: string[]) =>
    req<Conversation>("/channels/group", {
      method: "POST",
      body: JSON.stringify({ name, memberIds }),
    }),
  toggleStar: (channelId: string) =>
    req<{ channelId: string; starred: boolean }>(
      `/channels/${channelId}/star`,
      { method: "POST" },
    ),
  toggleMute: (channelId: string) =>
    req<{ channelId: string; muted: boolean }>(`/channels/${channelId}/mute`, {
      method: "POST",
    }),
  setPresence: (
    presence: Presence,
    ...
    req("/presence", {
      method: "PUT",
      body: JSON.stringify({ presence, statusText, statusEmoji }),
    }),
  meetings: () => req<Meeting[]>("/meetings"),
  createMeeting: (body: { title?: string; conversationId?: string }) =>
    req<Meeting>("/meetings", { method: "POST", body: JSON.stringify(body) }),
  endMeeting: (id: string) =>
    req<Meeting>(`/meetings/${id}/end`, { method: "PUT" }),
  ...
  updateChannel: (
    ...
    req<Conversation>(`/channels/${id}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  joinChannel: (id: string) =>
    req<{ id: string }>(`/channels/${id}/join`, { method: "POST" }),
  ...
  channelMembers: (channelId: string) =>
    req<ChannelMemberInfo[]>(`/channels/${channelId}/members`),
  addChannelMember: (channelId: string, userId: string) =>
    req<{ userId: string }>(`/channels/${channelId}/members`, {
      method: "POST",
      body: JSON.stringify({ userId }),
    }),
  removeChannelMember: (channelId: string, userId: string) =>
    req<{ ok: boolean }>(`/channels/${channelId}/members/${userId}`, {
      method: "DELETE",
    }),

  /* ── Notification level (US-B5) ── */
  setNotifyLevel: (channelId: string, notifyLevel: NotifyLevel) =>
    req<{ channelId: string; notifyLevel: NotifyLevel }>(
      `/channels/${channelId}/notify-level`,
      { method: "PUT", body: JSON.stringify({ notifyLevel }) },
    ),
  ...
  getLinkPreview: (url: string) =>
    req<{
      ...
      image?: string;
      siteName?: string;
    }>(`/communication/link-preview?url=${encodeURIComponent(url)}`),

  /* ── Threads ── */
  getThread: (parentId: string) =>
    req<ThreadView>(`/communication/messages/${parentId}/thread`),

  /* ── Forward ── */
  forwardMessage: (messageId: string, toChannelId: string, comment?: string) =>
    req<ConnectMessage>(`/communication/messages/${messageId}/forward`, {
      method: "POST",
      body: JSON.stringify({ toChannelId, comment }),
    }),

  /* ── Presence Scheduling ── */
  getStatusSchedules: () =>
    req<UserStatusSchedule[]>("/communication/status-schedules"),
  ...
  deleteStatusSchedule: (id: string) =>
    req<{ ok: boolean }>(`/communication/status-schedules/${id}`, {
      method: "DELETE",
    }),
  ...
  createTaskFromMessage: (
    ...
    req<any>(`/communication/messages/${messageId}/create-task`, {
      method: "POST",
  ...
  joinMeeting: (meetingId: string) =>
    req<MeetingParticipant>(`/communication/meetings/${meetingId}/join`, {
      method: "POST",
    }),
  leaveMeeting: (meetingId: string) =>
    req<MeetingParticipant>(`/communication/meetings/${meetingId}/leave`, {
      method: "POST",
    }),
  toggleHandRaise: (meetingId: string) =>
    req<MeetingParticipant>(`/communication/meetings/${meetingId}/raise-hand`, {
      method: "POST",
    }),
  toggleMuteMeeting: (meetingId: string) =>
    req<MeetingParticipant>(`/communication/meetings/${meetingId}/mute`, {
      method: "POST",
    }),
  toggleVideoMeeting: (meetingId: string) =>
    req<MeetingParticipant>(`/communication/meetings/${meetingId}/video`, {
      method: "POST",
    }),
  toggleScreenShare: (meetingId: string) =>
    req<MeetingParticipant>(
      `/communication/meetings/${meetingId}/screen-share`,
      { method: "POST" },
    ),
  getMeetingParticipants: (meetingId: string) =>
    req<MeetingParticipant[]>(
      `/communication/meetings/${meetingId}/participants`,
    ),
  getMeetingChat: (meetingId: string) =>
    req<MeetingChatMsg[]>(`/communication/meetings/${meetingId}/chat`),
  ...
  admitToMeeting: (meetingId: string, targetUserId: string) =>
    req<MeetingParticipant>(
      `/communication/meetings/${meetingId}/admit/${targetUserId}`,
      { method: "POST" },
    ),
  startRecording: (meetingId: string) =>
    req<MeetingRecording>(
      `/communication/meetings/${meetingId}/recordings/start`,
      { method: "POST" },
    ),
  stopRecording: (meetingId: string, recordingId: string) =>
    req<MeetingRecording>(
      `/communication/meetings/${meetingId}/recordings/${recordingId}/stop`,
      { method: "POST" },
    ),

  /* ── Channel Tabs ── */
  getTabs: (channelId: string) =>
    req<ChannelTab[]>(`/communication/channels/${channelId}/tabs`),
  createTab: (
    channelId: string,
    body: {
      type: string;
  ...
  updateTab: (tabId: string, body: any) =>
    req<ChannelTab>(`/communication/tabs/${tabId}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  deleteTab: (tabId: string) =>
    req<{ ok: boolean }>(`/communication/tabs/${tabId}`, { method: "DELETE" }),

  /* ── Channel Moderation ── */
  getModeration: (channelId: string) =>
    req<ChannelModeration>(`/communication/channels/${channelId}/moderation`),
  setSlowMode: (channelId: string, slowModeSecs: number) =>
    req<ChannelModeration>(
      `/communication/channels/${channelId}/moderation/slow-mode`,
      { method: "PUT", body: JSON.stringify({ slowModeSecs }) },
    ),
  setWhoCanPost: (channelId: string, whoCanPost: string) =>
    req<ChannelModeration>(
      `/communication/channels/${channelId}/moderation/who-can-post`,
      { method: "PUT", body: JSON.stringify({ whoCanPost }) },
    ),

  /* ── Pinned Messages ── */
  getPinnedMessages: (channelId: string) =>
    req<ConnectMessage[]>(`/communication/channels/${channelId}/pinned`),

  /* ── Channel Analytics ── */
  getChannelAnalytics: (channelId: string, days?: number) =>
    req<ChannelAnalytics>(
      `/communication/channels/${channelId}/analytics${days ? `?days=${days}` : ""}`,
    ),

  /* ── Message Edit History ── */
  getMessageEditHistory: (messageId: string) =>
    req<MessageEditEntry[]>(
      `/communication/messages/${messageId}/edit-history`,
    ),

  /* ── Bots ── */
  getBots: (channelId: string) =>
    req<ConnectBot[]>(`/communication/channels/${channelId}/bots`),
  createBot: (channelId: string, body: { name: string; avatar?: string }) =>
    req<ConnectBot>(`/communication/channels/${channelId}/bots`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  updateBot: (botId: string, body: any) =>
    req<ConnectBot>(`/communication/bots/${botId}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  deleteBot: (botId: string) =>
    req<{ ok: boolean }>(`/communication/bots/${botId}`, { method: "DELETE" }),
  regenerateBotToken: (botId: string) =>
    req<ConnectBot>(`/communication/bots/${botId}/regenerate-token`, {
      method: "POST",
    }),
  setWebhookUrl: (botId: string, webhookUrl: string) =>
    req<ConnectBot>(`/communication/bots/${botId}/webhook`, {
      method: "PUT",
  ...
  createPoll: (
    ...
    req<ConnectPoll>(`/communication/polls`, {
      method: "POST",
      body: JSON.stringify({ channelId, question, options }),
    }),
  getPolls: (channelId: string) =>
    req<ConnectPoll[]>(`/communication/channels/${channelId}/polls`),
  votePoll: (pollId: string, optionId: string) =>
    req<PollVote>(`/communication/polls/${pollId}/vote`, {
      method: "POST",
      body: JSON.stringify({ optionId }),
    }),
  closePoll: (pollId: string) =>
    req<{ ok: boolean }>(`/communication/polls/${pollId}/close`, {
      method: "POST",
    }),

  /* ── Feature 2: Slash Commands ── */
  runCommand: (channelId: string, text: string) =>
    req<SlashCommandResult>(`/communication/commands`, {
      method: "POST",
  ...
  createReminder: (
    ...
    req<Reminder>(`/communication/reminders`, {
      method: "POST",
      body: JSON.stringify({ text, remindAt, channelId, messageId }),
    }),
  getReminders: () => req<Reminder[]>(`/communication/reminders`),
  deleteReminder: (reminderId: string) =>
    req<{ ok: boolean }>(`/communication/reminders/${reminderId}`, {
      method: "DELETE",
    }),
  snoozeReminder: (reminderId: string, minutes?: number) =>
    req<Reminder>(`/communication/reminders/${reminderId}/snooze`, {
      method: "POST",
  ...
  scheduleMessage: (channelId: string, content: string, scheduledAt: Date) =>
    req<ScheduledMessage>(`/communication/channels/${channelId}/schedule`, {
      method: "POST",
      body: JSON.stringify({ content, scheduledAt }),
    }),
  getScheduledMessages: () =>
    req<ScheduledMessage[]>(`/communication/scheduled`),
  deleteScheduledMessage: (messageId: string) =>
    req<{ ok: boolean }>(`/communication/scheduled/${messageId}`, {
      method: "DELETE",
    }),
  ...
  deleteCustomEmoji: (emojiId: string) =>
    req<{ ok: boolean }>(`/communication/emoji/${emojiId}`, {
      method: "DELETE",
    }),

  /* ── Feature 6: Translation ── */
  translateMessage: (messageId: string, targetLang: string) =>
    req<TranslateResult>(`/communication/messages/${messageId}/translate`, {
      method: "POST",
  ...
  generateMeetingSummary: (meetingId: string) =>
    req<MeetingSummary>(`/communication/meetings/${meetingId}/summary`, {
      method: "POST",
    }),
  getMeetingSummary: (meetingId: string) =>
    req<MeetingSummary | null>(`/communication/meetings/${meetingId}/summary`),

  /* ── Feature 8: Channel Templates ── */
  getChannelTemplates: () =>
    req<ChannelTemplate[]>(`/communication/channel-templates`),
  createFromTemplate: (
    templateId: string,
    name: string,
    description?: string,
  ) =>
    req<any>(`/communication/channel-templates/apply`, {
      method: "POST",
      body: JSON.stringify({ templateId, name, description }),
    }),
  createChannelTemplate: (data: {
    name: string;
    description?: string;
    channelType?: string;
    topic?: string;
  ...
  sendEphemeral: (channelId: string, content: string, expiresInSecs?: number) =>
    req<any>(`/communication/channels/${channelId}/ephemeral`, {
      method: "POST",
      body: JSON.stringify({ content, expiresInSecs }),
    }),
  markMessageViewed: (messageId: string) =>
    req<{ ok: boolean }>(`/communication/messages/${messageId}/viewed`, {
      method: "POST",
    }),
