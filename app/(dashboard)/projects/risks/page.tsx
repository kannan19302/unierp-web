...
export default function RisksPage() {
  ...
  const handleAddMitigation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Failed to add mitigation");
    }
  };
      ...
      <div className="ui-flex-between">
        <div>
          <h1 className={styles.p1}>
            <ShieldAlert size={28} className="ui-text-danger" /> Risk Register
          </h1>
          <p className={styles.p2}>
            Track project risks and mitigation actions
          </p>
        </div>
      </div>
                ...
                {(mitigations[risk.id] || []).map((m: any) => (
                  <div key={m.id} className={styles.mitigationItem}>
                    <div className="ui-flex-between">
                      ...
                      <span className={styles.mitigationStatus}>
                        {m.status}
                      </span>
                    </div>
                    {m.dueDate && (
                      <p className={styles.mitigationDue}>
                        Due: {new Date(m.dueDate).toLocaleDateString()}
                      </p>
                    )}
                    {m.notes && (
                      <p className={styles.mitigationNotes}>{m.notes}</p>
                    )}
                  </div>
                ))}
              </div>
