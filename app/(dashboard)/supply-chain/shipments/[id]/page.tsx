...
import { PageHeader, Card, Button, Spinner, Badge, StatusBadge, KPICard, Modal, TextField, FormField, Select } from "@kannan19302/ui";
...
export default function ShipmentDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setShipment(
          await client.get<Shipment>(`/supply-chain/shipments/${id}`),
        );
      } catch {
        /* empty */
      } finally {
  ...
  const handleStatusUpdate = async () => {
    ...
    setUpdating(true);
    try {
      const updated = await client.request<Shipment>(
        `/supply-chain/shipments/${id}/status`,
        ...
        <div className={styles.s1}>
          ...
          <Link href="/supply-chain/shipments">
            <Button variant="secondary">
              <ArrowLeft size={14} className="mr-2" /> Back to Shipments
            </Button>
          </Link>
        </div>
      </RouteGuard>
    ...
    <RouteGuard permission="supply-chain.shipments.read">
      <div className="ui-stack-6">
        ...
        <div className="ui-grid-auto-sm">
          <KPICard
            title="Status"
            value={shipment.status.replace(/_/g, " ")}
            icon={<CheckCircle size={18} />}
            color={getStatusColor(shipment.status)}
          />
          <KPICard
            title="Shipping Cost"
            value={fmtCurrency(shipment.shippingCost)}
          ...
          <KPICard
            title="Est. Delivery"
            value={fmtDate(shipment.estimatedDelivery)}
        ...
        <div className="ui-grid-2">
          ...
          <Card>
            <div className="p-5 ui-hstack-4">
              <div
                style={{
                  background: shipment.actualDelivery
              ...
              <div>
                ...
                <div className={styles.s20}>
                  {fmtDate(shipment.actualDelivery)}
                </div>
              </div>
            </div>
          </Card>
