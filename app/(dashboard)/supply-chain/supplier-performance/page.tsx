...
export default function SupplierPerformancePage() {
    ...
    (async () => {
      try {
        const [kpiData, scoreData] = await Promise.all([
          client.get<KpiDefinition[]>(
            "/supply-chain/supplier-performance/kpis",
          ),
          client.get<Scorecard[]>(
            "/supply-chain/supplier-performance/scorecards",
          ),
        ]);
        setKpis(kpiData ?? []);
  ...
  const handleCreateKpi = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const created = await client.post<KpiDefinition>(
        "/supply-chain/supplier-performance/kpis",
        kpiForm,
      );
  ...
  const handleCalculateScorecard = async () => {
    setSaving(true);
    try {
      const created = await client.post<Scorecard>(
    ...
    },
    {
      ...
      render: (row: any) => (
        <div className="ui-flex ui-gap-1" onClick={(e: any) => e.stopPropagation()}>
          ...
          <button className="ui-btn-icon ui-text-danger" title="Delete">
            <svg
              width="14"
              height="14"
              viewBox="0 0 24 24"
  ...
  return (
    <RouteGuard permission="supply-chain.supplier-performance.read">
      <div className="ui-stack-6">
        <PageHeader
          ...
          actions={
            <div className="ui-flex ui-gap-2">
              ...
              <Button variant="primary" onClick={() => setShowScoreModal(true)}>
                <TrendingUp size={14} /> Calculate Scorecard
              </Button>
            </div>
          }
        />
        ...
        <Card>
          <div
            className="ui-flex ui-gap-3 ui-items-center"
        ...
        <Modal
          ...
          title="Add KPI Definition"
          size="md"
          footer={
            <>
              <Button variant="secondary" onClick={() => setShowKpiForm(false)}>
                Cancel
              </Button>
              <Button
                variant="primary"
                onClick={handleCreateKpi as any}
                disabled={saving}
              >
        ...
        >
          <p className="ui-text-sm-muted">
            This will calculate performance scorecards for all active vendors
            based on current KPI definitions and their latest data.
          </p>
        </Modal>
      </div>
    </RouteGuard>
