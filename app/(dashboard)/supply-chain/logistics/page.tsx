...
import { PageHeader, Card, DataTable, type Column, Button, Modal, TextField, Badge, StatusBadge, Tabs, Spinner } from "@kannan19302/ui";
...
export default function LogisticsPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [l, a, p] = await Promise.all([
        client
          .get<{ data: LoadBuild[] }>("/supply-chain/load-builds")
          .catch(() => ({ data: [] })),
        client
          .get<{ data: Appointment[] }>("/supply-chain/appointments")
          .catch(() => ({ data: [] })),
        client
          .get<{ data: any[] }>("/supply-chain/delivery-confirmations")
          .catch(() => ({ data: [] })),
      ]);
      setLoads(l.data || []);
      setAppts(a.data || []);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      await client.post("/supply-chain/load-builds", {
        ...form,
        stops: [],
        items: [],
      });
  ...
  if (loading)
    return (
      <div className="ui-page">
        <PageHeader title="Logistics Execution" />
        <div
  ...
  return (
    <div className="ui-page">
      <PageHeader
        ...
        description="Load builds, dock appointments, carrier rates, and delivery confirmations"
        actions={
          <Button variant="primary" onClick={() => setCreateOpen(true)}>
            <Plus size={14} /> New Load Build
          </Button>
        }
      />
      <Tabs
        ...
        tabs={[
          { key: "loads", label: `Load Builds (${loads.length})` },
          { key: "appts", label: `Appointments (${appts.length})` },
          { key: "pods", label: `PODs (${pods.length})` },
        ]}
      />
      <Card>
        {activeTab === "loads" && (
        ...
        {activeTab === "pods" && (
          <DataTable
            columns={[
              { key: "confirmationNumber", header: "POD #" },
              { key: "receivedBy", header: "Received By" },
              { key: "carrierName", header: "Carrier" },
              {
                key: "receivedAt",
                header: "Received",
                render: (r: any) => new Date(r.receivedAt).toLocaleString(),
              },
            ]}
            data={pods}
            rowKey={(r: any) => r.id}
