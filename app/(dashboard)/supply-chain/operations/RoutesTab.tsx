...
import { Card, Button, DataTable, type Column, KPICard, DashboardChart } from "@kannan19302/ui";
...
export default function RoutesTab() {
  ...
  const totalSavings = optimizedRoute
    ? Math.round(optimizedRoute.totalDistance * 0.15)
    : 0;
  ...
  return (
    <div className="ui-stack-4">
      ...
      <div className="ui-grid-auto">
        <KPICard
          title="Stops"
          value={String(SAMPLE_STOPS.length)}
          icon={<MapPin size={18} />}
          color="var(--color-primary)"
        />
        <KPICard
          title="Optimized Distance"
          value={optimizedRoute ? `${optimizedRoute.totalDistance} km` : "—"}
        ...
        <KPICard
          title="Est. Duration"
          value={
            optimizedRoute ? `${optimizedRoute.estimatedDuration} min` : "—"
          }
          icon={<Clock size={18} />}
        ...
        <KPICard
          title="Est. Savings"
          value={optimizedRoute ? `$${totalSavings}` : "—"}
          change={optimizedRoute ? -15 : undefined}
          changeLabel="vs unoptimized"
              ...
              i > 0
                ? Math.round(
                    (optimizedRoute.totalDistance /
                      optimizedRoute.orderedStops.length) *
                      10,
                  ) / 10
                : 0,
          }))}
      ...
      <Card title="Optimized Route">
        <DataTable
          ...
          data={routeData}
          rowKey={(r: RouteRow) => String(r.order)}
        />
      </Card>

      {!optimizedRoute && (
        <div className="ui-card ui-p-4 ui-text-center">
          <p className="ui-text-xs-tertiary">
            Click "Run Route Optimization" to optimize the sample delivery route
            using the nearest-neighbor heuristic with priority boosting.
          </p>
        </div>
      )}
    </div>
