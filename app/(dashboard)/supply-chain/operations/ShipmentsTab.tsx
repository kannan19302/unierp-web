...
import { Card, Button, Badge, StatusBadge, DataTable, type Column, Drawer, Modal, TextField, FormField, Select, Spinner } from "@kannan19302/ui";
...
export default function ShipmentsTab() {
  ...
  const fetchDetails = async (shipment: DisplayShipment) => {
    try {
      const [eventsList, exceptionsList] = await Promise.all([
        client.get<TrackingEvent[]>(
          `/supply-chain/shipments/${direction}/${shipment.id}/events`,
        ),
        client.get<ShipmentException[]>(
          `/supply-chain/shipments/${shipment.id}/exceptions`,
        ),
      ]);
  ...
  const handleCreateInbound = async () => {
    ...
    try {
      await client.post("/supply-chain/inbound-shipments", {
        ...
        expectedArrival: inboundForm.expectedArrival
          ? new Date(inboundForm.expectedArrival).toISOString()
          : null,
  ...
  const handleCreateOutbound = async () => {
    ...
    try {
      await client.post("/supply-chain/outbound-shipments", {
        ...
        estimatedDelivery: outboundForm.estimatedDelivery
          ? new Date(outboundForm.estimatedDelivery).toISOString()
          : null,
  ...
  const handleAddEvent = async () => {
    ...
    try {
      await client.post(
        `/supply-chain/shipments/${direction}/${selected.id}/events`,
        eventForm,
      );
  ...
  const handleReportException = async () => {
    if (!selected || !exceptionForm.exceptionCode || !exceptionForm.description)
      return;
    try {
  ...
  const handleResolveException = async () => {
    ...
    try {
      await client.patch(
        `/supply-chain/exceptions/${selectedExceptionId}/resolve`,
        { resolutionNote },
      );
      ...
      render: (row: any) => (
        <div className="ui-hstack-3">
          <div
            className={
              direction === "outbound"
                ? styles.iconWellOutbound
                : styles.iconWellInbound
            }
          >
            <Package size={16} />
          </div>
    ...
    },
    {
      ...
      render: (row: any) =>
        row.trackingNumber ? (
          ...
          <span className={styles.trackingFallback}>—</span>
        ),
    },
    {
      key: "weight",
    ...
    <RouteGuard permission="supply-chain.shipment.read">
      <div className="ui-stack-6">
        ...
        <Modal
          ...
          footer={
            <>
              <Button
                variant="secondary"
                onClick={() => setCreateInboundOpen(false)}
              >
                Cancel
              </Button>
              <Button variant="primary" onClick={handleCreateInbound}>
                Save Shipment
              </Button>
            </>
          }
        ...
        <Modal
          ...
          footer={
            <>
              <Button
                variant="secondary"
                onClick={() => setCreateOutboundOpen(false)}
              >
                Cancel
              </Button>
              <Button variant="primary" onClick={handleCreateOutbound}>
                Save Shipment
              </Button>
            </>
          }
        ...
        >
          {selected && (
            <div className="ui-stack-6">
                ...
                <div className="ui-grid-2 ui-gap-3">
                  ...
                  <div>
                    ...
                    <div className="ui-text-sm font-semibold">
                      {selected.carrierName || "No carrier"}
                    </div>
                  </div>
                  <div>
                    <div className="ui-text-xs-tertiary">Tracking Number</div>
                    <div className="ui-text-sm font-mono font-semibold">
                  ...
                  {direction === "inbound" ? (
                    <>
                      ...
                      <div>
                        ...
                        <div className="ui-text-sm font-semibold">
                          {selected.arrivedAt
                            ? new Date(selected.arrivedAt).toLocaleString()
                            : "—"}
                        </div>
                      </div>
                    </>
              ...
              {exceptions.length === 0 ? (
                <div className="ui-text-sm-muted py-2">
                  No active exceptions reported.
                </div>
              ) : (
                <div className="ui-stack-2">
                  {exceptions.map((ex) => (
                    <Card
                      key={ex.id}
                      padding="md"
                      className="ui-hstack-3 ui-justify-between"
                    >
                      <div className="ui-hstack-2 ui-items-center">
                        ...
                        <div>
                          <div className="ui-heading-sm">
                            ...
                            <Badge
                              variant={
                                ex.status === "RESOLVED" ? "success" : "danger"
                              }
                            >
                              {ex.status}
                            </Badge>
                          ...
                          {ex.resolutionNote && (
                            <div className="ui-text-xs-muted mt-1 font-semibold">
                              Resolution: {ex.resolutionNote}
                            </div>
                          )}
                        </div>
                      </div>
                ...
                <div className="ui-stack-3 border-l-2 border-border pl-4 ml-2">
                  {events.map((ev) => (
                    <div key={ev.id} className="relative py-1">
                      ...
                      <div className="ui-heading-sm font-semibold">
                        {ev.eventCode}
                      </div>
        ...
        <Modal
          ...
          title="Report Shipment Exception"
          size="md"
          footer={
            <>
              <Button
                variant="secondary"
                onClick={() => setReportExceptionOpen(false)}
              >
                Cancel
              </Button>
              <Button variant="danger" onClick={handleReportException}>
                Report Exception
              </Button>
            </>
          }
        >
          <div className="ui-stack-4">
            ...
            <TextField
              ...
              placeholder="Severe storms delaying carrier dispatch."
            />
        ...
        <Modal
          open={resolveExceptionOpen}
          onClose={() => setResolveExceptionOpen(false)}
          ...
          footer={
            <>
              <Button
                variant="secondary"
                onClick={() => setResolveExceptionOpen(false)}
              >
                Cancel
              </Button>
