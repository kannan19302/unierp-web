  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [profiles, hm] = await Promise.all([
        client.get<{ data: RiskProfile[]; total: number }>(
          `/supply-chain/supplier-risk-profiles?page=${page}&limit=${limit}`,
        ),
  ...
  return (
    <div className="ui-page">
      <PageHeader
        ...
        description="Assess and monitor supplier risk profiles, alerts, and diversity"
      />
      {heatmap && (
        <div className="ui-grid-4" style={{ marginBottom: "1.5rem" }}>
          <KPICard
            title="Total Suppliers"
            value={heatmap.summary?.total ?? 0}
            icon={<Shield size={20} />}
            color="var(--primary-600)"
          />
          <KPICard
            title="High Risk"
            value={heatmap.summary?.high ?? 0}
            icon={<AlertTriangle size={20} />}
            color="var(--danger-600)"
          />
          <KPICard
            title="Medium Risk"
            value={heatmap.summary?.medium ?? 0}
            icon={<AlertTriangle size={20} />}
            color="var(--warning-600)"
          />
          <KPICard
            title="Avg Score"
            value={heatmap.summary?.averageScore ?? 0}
            icon={<Shield size={20} />}
            color="var(--info-600)"
          />
        </div>
