...
export default function SupplyChainAnalyticsPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetchJson<DashboardData>("/api/supply-chain/analytics/dashboard")
        .then(setDashboard)
        .catch(() => {}),
      fetchJson<CarrierPerf[]>(
        "/api/supply-chain/analytics/carrier-performance",
      )
        .then(setCarrierPerf)
        .catch(() => {}),
      fetchJson<OnTimeDelivery>("/api/supply-chain/analytics/on-time-delivery")
        .then(setOnTime)
        .catch(() => {}),
      fetchJson<CostAnalysis>("/api/supply-chain/analytics/cost-analysis")
        .then(setCost)
        .catch(() => {}),
      fetchJson<LeadTime>("/api/supply-chain/analytics/lead-time")
        .then(setLeadTime)
        .catch(() => {}),
    ...
    <div className="ui-stack-6">
      ...
      <div className="ui-grid-auto">
        <KPICard
          ...
          value={String(dashboard?.totalShipments ?? 0)}
          icon={<Package size={18} />}
          color="var(--color-primary)"
        />
        <KPICard
          title="In Transit"
          value={String(dashboard?.inTransit ?? 0)}
          icon={<Truck size={18} />}
          color="var(--color-warning)"
        />
        <KPICard
          title="Delivered"
          value={String(dashboard?.delivered ?? 0)}
          icon={<CheckCircle size={18} />}
          color="var(--color-success)"
        />
        <KPICard
          title="On-Time Rate"
          value={onTime ? `${onTime.onTimeRate}%` : "—"}
          icon={<Clock size={18} />}
      ...
      <div className="ui-grid-auto">
        <KPICard
          ...
          value={
            cost
              ? `${cost.currency} ${cost.avgCostPerShipment.toFixed(2)}`
              : "—"
          }
        ...
        <KPICard
          ...
          value={String(dashboard?.activeCarriers ?? 0)}
          icon={<Truck size={18} />}
          color="var(--color-info)"
        />
        <KPICard
        ...
        <KPICard
          ...
          value={String(dashboard?.pendingReturns ?? 0)}
          ...
          {onTime ? (
            <div className="ui-grid-2">
              ...
              <KPICard
                ...
                value={String(onTime.totalDelivered)}
                icon={<Package size={18} />}
                color="var(--color-info)"
              />
              <KPICard
                title="On-Time Rate"
                value={`${onTime.onTimeRate}%`}
        ...
        <Card title="Lead Time Analysis" className="ui-p-4">
          {leadTime ? (
            <div className="ui-grid-2">
              <KPICard
                title="Avg Lead Time"
                value={`${leadTime.avgLeadTimeDays} days`}
                icon={<Clock size={18} />}
                color="var(--color-info)"
              />
              <KPICard
                title="Early"
                value={String(leadTime.early)}
                icon={<TrendingUp size={18} />}
                color="var(--color-success)"
              />
              <KPICard
                title="On Time"
                value={String(leadTime.onTime)}
                icon={<CheckCircle size={18} />}
                color="var(--color-primary)"
              />
              <KPICard
                title="Late"
                value={String(leadTime.late)}
                icon={<AlertTriangle size={18} />}
                color="var(--color-danger)"
              />
            </div>
