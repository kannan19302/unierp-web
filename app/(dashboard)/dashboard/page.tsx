...
function DashboardContent() {
  ...
  useEffect(() => {
    let mounted = true;
    client.get<{ firstName: string; lastName: string; tenant?: { id: string } }>('/auth/me')
      .then((profile) => {
        if (mounted) setUser({ firstName: profile.firstName, lastName: profile.lastName, tenantId: profile.tenant?.id });
      })
      .catch(() => {
  ...
  useEffect(() => {
    if (dashboardId) {
      ...
      client.get<DashboardPayload>(`/builder/dashboards/${dashboardId}`)
      .then(data => {
  ...
  useEffect(() => {
    if (activeTab === 'global' && !customDashboard) {
      ...
      client.get<Record<string, unknown>>('/builder/dashboards/global-stats')
      .then(data => {
        setGlobalStats(data);
      })
      .catch(console.error)
  ...
  const recentLogs = (activityData?.data || []).slice(0, 5).map((log: any, idx: number) => ({
  ...
  return (
    ...
    <div className="ui-stack-6 ui-animate-in">
      ...
      {!customDashboard && (
        <div className={`builder-tabs ${styles.tabs}`}>
          <button
          ...
          <>
                  ...
                  <div className={styles.distribution}>
                    <div className={styles.donut}>
                      <div className={styles.donutCenter}>
                        <span className={styles.donutValue}>{globalMetrics.totalCustomRecords || 0}</span>
                    ...
                    <div className={styles.legend}>
                      {globalStats.charts.submissionsByApp.slice(0, 4).map((app: any, idx: number) => {
                        ...
                        return (
                          <div key={app.appName} className={styles.legendItem}>
                            <div className={`${styles.legendDot} ${styles[`legendDot${idx % 4}`]}`} />
              ...
              <Card padding="lg">
                ...
                <div className={styles.monthlyChart}>
                  ...
                  <div className={styles.monthLabels}>
                    {(globalStats?.charts?.monthlySubmissionsTrend || []).map((d: any, i: number) => (
                      <span key={i} className={styles.monthLabel}>{d.month}</span>
                    ))}
                  </div>
                </div>
              </Card>
            ...
            <Card padding="lg">
              <div className={`ui-flex-between ${styles.titleRow}`}>
                ...
                <span className={`ui-badge ui-badge-primary ${styles.appsBadge}`}>
                  ...
                  {globalStats.customApps.map((app: any) => (
                    ...
                    >
                      <div className="ui-flex-between">
                        ...
                        <span className={`${styles.statusBadge} ${app.status === 'ACTIVE' || app.status === 'Active' ? styles.statusActive : styles.statusPending}`}>
                          {app.status}
                        </span>
                      </div>
                      ...
                      <div className={styles.appMetrics}>
                        <div>
                          <div className={styles.appMetricValue}>{app.pagesCount}</div>
                          <div className={styles.appMetricLabel}>Pages</div>
                        </div>
                        <div>
                          <div className={styles.appMetricValue}>{app.formsCount}</div>
                          <div className={styles.appMetricLabel}>Forms</div>
                        </div>
                        <div>
                          <div className={styles.appMetricValue}>{app.dataModelsCount}</div>
                          <div className={styles.appMetricLabel}>Models</div>
                        </div>
                        <div>
                          <div className={`${styles.appMetricValue} ${styles.appMetricPrimary}`}>{app.submissionsCount}</div>
