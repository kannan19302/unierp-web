...
export default function FeeManagementPage() {
  ...
  const activeTab = (searchParams?.get("subtab") || "structures") as
    | "structures"
    | "ledger";
    ...
    (async () => {
      try {
        const [fRes, sfRes] = await Promise.all([
          client.get<FeeStructure[] | { data?: FeeStructure[] }>(
            "/ext/education/fee-structures",
          ),
          client.get<StudentFee[] | { data?: StudentFee[] }>(
            "/ext/education/student-fees",
          ),
        ]);
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      await client.post("/ext/education/fee-structures", {
        ...form,
        amount: Number(form.amount),
      });
      setCreateOpen(false);
      setForm({ name: "", description: "", amount: 0, dueDate: "" });
      const d = await client.get<FeeStructure[] | { data?: FeeStructure[] }>(
        "/ext/education/fee-structures",
      );
            ...
            [
              {
                ...
                href: "/education/fees?subtab=structures",
                icon: CreditCard,
              },
              {
                id: "ledger",
                label: "Student Ledger",
                href: "/education/fees?subtab=ledger",
                icon: BookOpen,
              },
            ] as SubTab[]
          }
              ...
              columns={
                [
                  ...
                  },
                  {
                    ...
                    render: (row: StudentFee) => (
                      <span className="text-sm">
                        {row.feeStructure?.name || row.feeStructureId}
                      </span>
                    ),
                  },
                  {
