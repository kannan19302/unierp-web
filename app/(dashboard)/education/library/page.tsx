...
export default function LibraryPage() {
    ...
    (async () => {
      try {
        const [bRes, tRes] = await Promise.all([
          ...
          client.get<BookTransaction[] | { data?: BookTransaction[] }>('/ext/education/book-transactions'),
        ]);
  ...
  const handleCreateBook = async () => {
    ...
    setCreating(true);
    try {
      await client.post('/ext/education/books', { ...bookForm, quantity: Number(bookForm.quantity) });
      setCreateOpen(false);
  ...
  const bookColumns: Column<Book>[] = [
    ...
    { key: 'isbn', header: 'ISBN', render: (row) => <code className={styles.s2}>{row.isbn}</code> },
    { key: 'qty', header: 'Total Qty', render: (row) => <span className="text-sm">{row.quantity}</span> },
    ...
    <div className="ui-stack-6">
      <PageHeader title="Library" description="Book register, checkouts, and availability tracking"
        ...
        actions={
          <div className="ui-flex ui-gap-2">
            <Button variant="secondary" onClick={() => setCheckoutOpen(true)}><ArrowLeftRight size={14} className="mr-2" /> Checkout</Button>
            <Button variant="primary" onClick={() => setCreateOpen(true)}><Plus size={14} className="mr-2" /> Add Book</Button>
          </div>
        }
      />
      ...
      <div className={styles.s3}>
        {(['books', 'transactions'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{ borderBottom: activeTab === tab ? '2px solid var(--color-primary)' : '2px solid transparent', color: activeTab === tab ? 'var(--color-primary)' : 'var(--color-text-secondary)' }} className={styles.s4}>
      ...
      {activeTab === 'transactions' && (
        <Card padding="none">
          <DataTable
            columns={[
              { key: 'student', header: 'Student', render: (row: BookTransaction) => <span className="text-sm">{row.student ? `${row.student.firstName} ${row.student.lastName}` : row.studentId}</span> },
              { key: 'book', header: 'Book', render: (row: BookTransaction) => <span className="text-sm">{row.book?.title || row.bookId}</span> },
              { key: 'due', header: 'Due Date', render: (row: BookTransaction) => <span className="text-sm">{new Date(row.dueDate).toLocaleDateString()}</span> },
