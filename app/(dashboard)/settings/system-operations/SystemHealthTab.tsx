...
const statusIcon = (s: string) => {
  if (["connected", "running", "healthy"].includes(s))
    return <CheckCircle size={16} className="ui-text-success" />;
  if (["degraded", "slow"].includes(s))
    return <AlertTriangle size={16} className="ui-text-warning" />;
...
export default function SystemHealthTab() {
  ...
  const fetchHealth = async () => {
    try {
      const data = await client.get<SystemHealthData>(
  ...
  const services = [
    ...
    { name: "Redis Cache", key: "redis", icon: Zap, status: h.services.redis },
    {
      ...
      key: "minio",
      icon: HardDrive,
      status: h.services.minio,
    },
    {
      name: "Queue Processor",
      key: "queueProcessor",
      icon: Activity,
      status: h.services.queueProcessor,
    },
  ];
  ...
  return (
    <div className="ui-stack-6">
      <div className={styles.s1}>
        ...
        <button onClick={fetchHealth} className={styles.s4}>
          <RefreshCw
            size={14}
      ...
      >
        <div className="ui-hstack-3">
          {allHealthy ? (
            <CheckCircle size={24} className="ui-text-success" />
          ) : (
            <AlertTriangle size={24} className="ui-text-warning" />
          )}
          <div>
      ...
      <div className="ui-grid-auto">
        <KPICard
          ...
          icon={<Cpu size={20} />}
          color={
            m.cpuUsage > 80 ? "var(--color-danger)" : "var(--color-primary)"
          }
        />
        <KPICard
          title="Memory Usage"
          ...
          icon={<Server size={20} />}
          color={
            m.memoryUsage > 80 ? "var(--color-warning)" : "var(--color-info)"
          }
        />
        <KPICard
          title="Total Memory"
          value={`${m.totalMemoryGB} GB`}
