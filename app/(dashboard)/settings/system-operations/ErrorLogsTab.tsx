...
export default function ErrorLogsTab() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      const data = await client.get<
        LogEntry[] | { data?: LogEntry[]; totalPages?: number }
      >(`/admin/operations/logs?page=${page}&pageSize=50`);
  ...
  const getLevelIcon = (level: string) => {
    if (level === "ERROR")
      return <AlertCircle size={14} className={styles.s1} />;
    if (level === "WARN")
      return <AlertTriangle size={14} className="ui-text-warning" />;
          ...
          <div className="ui-flex-col">
            {filteredLogs.map((log, idx) => (
              <div
                key={log.id || idx}
                className={styles.s8}
                style={{ opacity: log.resolved ? 0.5 : 1 }}
              >
                <div className="ui-flex-between">
                  <div className="ui-hstack-2">
                    ...
                    <span className="ui-text-muted">
                      ({log.context || log.source})
                    </span>
                    {log.resolved && (
                      <span className={styles.s9}>Resolved</span>
                    )}
                  </div>
                  <div className="ui-hstack-2">
                    {!log.resolved && log.id && (
                ...
                <div className={styles.s12}>{log.message}</div>
              </div>
            ))}
