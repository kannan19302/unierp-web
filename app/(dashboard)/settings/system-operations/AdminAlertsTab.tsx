...
export default function AdminAlertsTab() {
  ...
  useEffect(() => {
    if (subTab === "inbox") fetchAlerts();
  }, [subTab, fetchAlerts]);
  useEffect(() => {
    if (subTab === "thresholds") fetchThresholds();
  }, [subTab, fetchThresholds]);
  ...
  const saveThreshold = async () => {
    try {
      if (editingId) {
        await client.request(`${API}/thresholds/${editingId}`, {
          method: "PUT",
  ...
  const toggleActive = async (t: Threshold) => {
    try {
      await client.request(`${API}/thresholds/${t.id}`, {
        method: "PUT",
  ...
  const openEdit = (t: Threshold) => {
    setEditingId(t.id);
    setForm({
      metric: t.metric as (typeof METRICS)[number],
      operator: t.operator,
      value: t.value,
      severity: t.severity,
      active: t.active,
    ...
    <div>
        ...
        <div className={styles.s2}>
          <div className={styles.s3}>
            ...
            <div className={styles.s12}>
              ...
              <button onClick={saveThreshold} style={btnPrimary}>
                Save
              </button>
            </div>
          </div>
      ...
      <div className={styles.s14}>
        <button
          ...
          style={tabStyle(subTab === "inbox")}
        >
          Alert Inbox
        </button>
        <button
          onClick={() => setSubTab("thresholds")}
          style={tabStyle(subTab === "thresholds")}
        >
      ...
      {subTab === "inbox" && (
        <div>
          ...
          {alertsLoading && <p className={styles.s16}>Loading...</p>}
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className={styles.s21}>
                        {new Date(alert.createdAt).toLocaleString()} &middot;{" "}
                  ...
                  {expanded && (
                    <div className={styles.s23}>
                      <p className={styles.s24}>{alert.message}</p>
                ...
                },
                {
                  ...
                  render: (v, row) => (
                    <div className="ui-flex ui-gap-2">
                      ...
                      <button
                        onClick={() => deleteThreshold(String(v))}
