...
function isAdminStats(value: unknown): value is AdminStats {
  ...
  return typeof candidate.activeUsers === 'number'
...
function normalizeAdminStats(response: unknown): AdminStats | null {
  const candidate = typeof response === 'object'
    && response !== null
    && 'data' in response
  ...
  const auditColumns: Column<AuditEvent>[] = [
    {
      ...
      render: (row) => (
        <div className="ui-hstack-3">
          <div className={styles.s1} style={{background: severityBg[row.severity], color: severityColor[row.severity]}}>
    ...
    },
    {
      ...
      render: (row) => (
        <span className="ui-text-sm-muted">{row.user}</span>
      ),
    },
    {
        ...
        <div className="ui-stack-6">
          ...
          <div>
            ...
            <div className={styles.s10}>
              {quickLinks.map((link) => (
                <Link href={link.href} key={link.title} className={styles.s11}>
                  <div
                    className={styles.s12}
                    style={{ '--quick-link-color': link.color } as React.CSSProperties}
                  >
                    <div className={styles.s13} style={{background: `${link.color}15`, color: link.color}}>
                      <link.icon size={18} />
                    </div>
            ...
            <Card>
              <div className={styles.s23}>
                ...
                <div className="flex-1 overflow-hidden">
                  ...
                  <span className="ui-text-xs-tertiary">Configure kill switch & engine</span>
                </div>
            ...
            <div className={styles.s27}>
              ...
              <div className={styles.s29}>
                {userSegments.map((seg) => (
                  <div key={seg.label} className="ui-flex ui-items-center ui-gap-1">
                    <div className={styles.s30} style={{background: seg.color }} />
  ...
  return (
    <div className={styles.s34}>
      ...
      <div className={styles.s37}>
        <div className={styles.s38} style={{width: `${Math.min(percent, 100)}%`, background: color}} />
      </div>
    </div>
  );
}
