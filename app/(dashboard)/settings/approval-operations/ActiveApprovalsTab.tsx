...
export default function ActiveApprovalsTab() {
  ...
  const handleApprove = async (id: string, status: "APPROVED" | "REJECTED") => {
    ...
    try {
      await client.request(`/workflows/approvals/${id}`, {
        method: "PUT",
    ...
    <div className={styles.s3}>
      ...
      <div className="ui-card p-5">
        ...
        <div className="ui-stack-3">
            ...
            <div key={app.id} className={styles.s6}>
              <div>
                <p className={styles.s7}>
                  Entity: {app.entityType} ({app.entityId})
                </p>
                <p className={styles.s8}>
                  Workflow Step: {app.step?.actionType || "APPROVAL"} assigned
                  to{" "}
                  <strong className={styles.s9}>
                    {app.step?.assigneeRole}
                  </strong>
                </p>
          ...
          {approvals.length === 0 && (
            <div className={styles.s14}>No active approval requests found.</div>
          )}
        </div>
      </div>
    </div>
