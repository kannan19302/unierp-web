...
export default function DataQualityPage() {
  ...
  const fetchDuplicates = useCallback(async () => {
    ...
    try {
      ...
      const res = await client.get<{ data: DuplicateSet[]; stats: Stats }>(
        `/admin/data-quality/duplicates?${params}`,
      );
  ...
  const handleMerge = async () => {
    ...
    try {
      await client.post("/admin/data-quality/merge", {
        setId: mergeModal.id,
    ...
    <RouteGuard permission="settings.data-quality.read">
      <div className={styles.s1}>
        ...
        <div className={styles.s2}>
          ...
          <h1 className={styles.s3}>Data Quality &amp; Duplicate Detection</h1>
        </div>
          ...
          {ENTITY_TYPES.map((type) => (
            <div key={type} className={styles.s8}>
              ...
              <button
                ...
                style={{
                  background: scanning[type]
                    ? "var(--color-bg-tertiary)"
                    : "var(--color-primary)",
                  color: scanning[type]
                    ? "var(--color-text-secondary)"
                    : "#fff",
                  cursor: scanning[type] ? "not-allowed" : "pointer",
                }}
            ...
            {duplicates.map((set) => (
              <div key={set.id} className={styles.s14}>
                ...
                >
                  ...
                  <span
                    style={{ background: badgeColor(set.entityType) }}
                    className={styles.s16}
                  >
                    {set.entityType}
                  </span>
                  <span className={styles.s17}>{set.matchScore}% match</span>
                  ...
                  {set.status === "PENDING" && (
                    ...
                    >
                      <button
                        onClick={() => {
                          setMergeModal(set);
                          setSelectedMaster(set.records[0]?.id || "");
                        }}
          ...
          <div className={styles.s23}>
            <div className={styles.s24}>
                ...
                {mergeModal.records.map((r) => (
                  ...
                  >
                    <input
                      ...
                      name="master"
                      value={r.id}
                      checked={selectedMaster === r.id}
                      onChange={() => setSelectedMaster(r.id)}
                    />
                    <div>
              ...
              <div className={styles.s31}>
                ...
                <button onClick={handleMerge} className={styles.s33}>
                  Merge Records
                </button>
              </div>
            </div>
