...
const MOCK_REPORTS: ScheduledReport[] = [
  {
    ...
    schedule: "weekly",
    recipients: ["cfo@acme.com", "sales@acme.com"],
    filters: {},
    format: "pdf",
    isActive: true,
    lastRunAt: "2026-06-20T06:00:00Z",
...
export default function ScheduledReportsPage() {
  ...
  const openEdit = (r: ScheduledReport) => {
    setFormName(r.name);
    setFormType(r.reportType);
    setFormSchedule(r.schedule);
    setFormFormat(r.format);
    setFormRecipients(
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        await client.request(`/reporting/scheduled/${editingId}`, {
          method: "PATCH",
        ...
        {showModal && (
          <div className={styles.modalBackdrop}>
            <div
              ...
              role="dialog"
              aria-modal="true"
              aria-labelledby="scheduled-report-modal-title"
            >
              <div className="ui-flex-between mb-4">
                <h3
                  id="scheduled-report-modal-title"
                  ...
                  <div>
                    ...
                    {(row.recipients as unknown[]).length > 0 && (
                      <div className="ui-text-micro">
                        {(row.recipients as unknown[]).length} recipient
                        {(row.recipients as unknown[]).length > 1 ? "s" : ""}
                      </div>
                    )}
                  </div>
                ),
              ...
              },
              {
                ...
                render: (v: any, row: any) => (
                  <div className="ui-flex ui-gap-1">
                    ...
                    <button
                      onClick={() =>
                        openEdit(row as unknown as ScheduledReport)
                      }
