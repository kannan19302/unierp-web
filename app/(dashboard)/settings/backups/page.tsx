...
import { PageHeader, Card, Button, Badge, DataTable, type Column, ProtectedComponent, useToast } from "@kannan19302/ui";
...
export default function BackupPage() {
  ...
  const fetchBackups = useCallback(async () => {
    setLoading(true);
    try {
      const data = await client.get<BackupRecord[] | { data?: BackupRecord[] }>(
  ...
  const handleCreateBackup = async () => {
    ...
    try {
      ...
      toast.success(
        "Simulated backup entry recorded",
        "No file was written — this feature is not yet connected to real storage.",
      );
      void fetchBackups();
    } catch {
          ...
          fallback={
            <Card>
              <div className={styles.p2}>
                You do not have permission to view backup records. Contact your
                Platform Admin if you believe this is an error.
              </div>
            </Card>
          }
        >
          <div className={styles.s1}>
            ...
            <p className={styles.p4}>
              <strong className={styles.p5}>Simulated data</strong> — Backup
              &amp; Restore is not yet connected to a real storage backend. No
              production DR coverage exists via this page today. Contact your
              platform team for actual database backup status.
            </p>
          </div>
