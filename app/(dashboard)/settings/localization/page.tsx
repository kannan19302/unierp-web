...
const FALLBACK_OVERRIDES: TranslationOverride[] = [
  ...
  },
  {
    ...
    locale: "ar",
    key: "dashboard.welcome",
    translation: "مرحبًا بكم في نظام UniERP!",
  },
];
...
export default function LocalizationPage() {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      const saved = await client.post<TranslationOverride>(
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const items: { locale: string; key: string; translation: string }[] =
        JSON.parse(text);
      ...
      showToast(
        `Imported ${successCount}/${items.length} overrides`,
        successCount === items.length ? "success" : "error",
      );
    } catch {
  ...
  const filtered = overrides.filter((o) => {
    ...
    const matchSearch =
      ...
      o.translation.toLowerCase().includes(searchQuery.toLowerCase());
        ...
        {languages.map((lang) => {
          ...
          return (
            <Card
              ...
              onClick={() =>
                setFilterLocale(filterLocale === lang.code ? "All" : lang.code)
              }
            >
