...
export default function RolesTab() {
  ...
  const fetchRoles = async () => {
    setLoading(true);
    try {
      const data = await client.get<RoleData[] | { data?: RoleData[] }>(
        "/admin/roles",
      );
    ...
    <div className="ui-stack-6">
      <div className="ui-flex-end ui-gap-2">
        <Button
          variant="outline"
          onClick={() => {
            window.location.href = "/settings/access-control/matrix";
          }}
        >
      ...
      >
        <div className="ui-hstack-3">
          ...
          <div>
            ...
            <p className={styles.s9}>{role.description}</p>
          </div>
        </div>
        <div className="ui-hstack-3">
          <Badge variant="info">{permCount} permissions</Badge>
          {expanded ? (
            <ChevronDown size={16} className="ui-text-tertiary" />
          ) : (
            <ChevronRight size={16} className="ui-text-tertiary" />
          )}
        </div>
      </button>
      ...
      {expanded && (
        <div id={panelId} className={styles.s10}>
          <div className="ui-flex-between mb-4">
            <span className={styles.s11}>MODULE PERMISSIONS</span>
            {!role.isSystem && (
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="ui-stack-4">
        {error && <div className={styles.s26}>{error}</div>}
        ...
        <FormField
          label={`Permissions (${selectedPerms.size} selected)`}
          required
        >
