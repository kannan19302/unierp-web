...
export default function BulkOperationsPage() {
  ...
  const fetchEntityCounts = async () => {
    try {
      setEntityCounts(
        await client.get<EntityCounts>("/admin/bulk-operations/entity-counts"),
      );
    } catch (e) {
      console.error(e);
    }
  ...
  const fetchOperations = async () => {
    ...
    try {
      setOperations(
        await client.get<BulkOperation[]>("/admin/bulk-operations"),
      );
    } catch (e) {
      console.error(e);
    }
  ...
  const previewCriteria = async () => {
    ...
    try {
      JSON.parse(criteria);
      const data = await client.get<EntityCounts>(
        "/admin/bulk-operations/entity-counts",
      );
  ...
  const executeOperation = async () => {
    ...
    try {
      ...
      const iv = setInterval(async () => {
        try {
          const updated = await client.get<BulkOperation>(
            `/admin/bulk-operations/${op.id}`,
          );
          setActiveOp(updated);
  ...
  const renderStep = () => {
    if (step === 1)
      return (
        <div>
          <h3 className={styles.p1}>Step 1: Select Entity Type</h3>
          <div className={styles.p2}>
            {(
              ...
              <div
                ...
                className={styles.s1}
              >
                <div className={styles.p3}>{ENTITY_ICONS[e]}</div>
                <div className={styles.p4}>{e}</div>
                <div className={styles.p5}>
                  {entityCounts
                    ? `${entityCounts[e].toLocaleString()} records`
                    : "..."}
                </div>
              </div>
            ))}
            ...
            {(["MASS_UPDATE", "MASS_DELETE", "MASS_TRANSFER"] as const).map(
              (op) => (
                ...
                >
                  ...
                  <div>
                    <div className={styles.p7}>{op.replace("_", " ")}</div>
    ...
    if (step === 3)
      return (
        <div>
          <h3 className={styles.p8}>Step 3: Define Criteria</h3>
      ...
      return (
        <div>
          ...
          {operationType === "MASS_DELETE" && (
            <div
              style={{ ...card({ background: "var(--color-danger-light)" }) }}
              className={styles.s6}
            >
              <input
                type="checkbox"
              ...
              <label htmlFor="del-confirm" className={styles.p13}>
                <AlertTriangle size={14} className={styles.p14} />I confirm that
                I want to permanently delete all matching {entityType} records.
    ...
    if (step === 5)
      return (
        <div>
          <h3 className={styles.p16}>Step 5: Review & Execute</h3>
          <div style={{ ...card() }} className={styles.s8}>
            <div className="text-sm">
              <strong>Entity:</strong> {entityType}
            </div>
            <div className="text-sm">
              <strong>Operation:</strong> {operationType.replace("_", " ")}
            </div>
            <div className="text-sm">
              <strong>Criteria:</strong> <code>{criteria}</code>
            </div>
            {operationType === "MASS_UPDATE" && (
              <div className="text-sm">
            ...
            {operationType === "MASS_TRANSFER" && (
              <div className="text-sm">
                <strong>New Owner:</strong> {transferOwner}
              </div>
            )}
          </div>
          <button
    ...
    if (step === 6 && activeOp) {
      const pct =
        activeOp.totalRecords > 0
          ? Math.round(
              (activeOp.processedRecords / activeOp.totalRecords) * 100,
            )
          : 0;
      return (
        <div>
          <h3 className={styles.p17}>Operation Progress</h3>
          <div style={card()}>
            <div className={styles.p18}>
              <span>
                Status:{" "}
                <span style={{ ...statusBadge(activeOp.status) }}>
                  {activeOp.status}
                </span>
              </span>
              <span>{pct}%</span>
            ...
            <div className={styles.p20}>
              ...
              <span
                style={{
                  color:
                    activeOp.failedRecords > 0
                      ? "var(--color-danger-text)"
                      : undefined,
                }}
              >
                Failed: {activeOp.failedRecords}
              </span>
            </div>
          </div>
      ...
      <div className="ui-flex-between">
        <div>
          ...
          <p className="ui-text-sm-muted">
            Execute mass updates, deletions, and ownership transfers across
            entity records.
          </p>
        </div>
      </div>
            ...
            {[1, 2, 3, 4, 5].map((s) => (
              <div
                ...
                style={{
                  ...
                  fontWeight:
                    step === s
                      ? ("var(--weight-semibold)" as never)
                      : ("var(--weight-normal)" as never),
                }}
          ...
          {step < 5 && step < 6 && (
            <div className={styles.p24}>
              <button
                ...
                disabled={step === 1}
                style={{ ...btnOutline, opacity: step === 1 ? 0.5 : 1 }}
              >
              ...
              { key: "totalRecords", header: "Records" },
              {
                ...
                render: (v: any) => (
                  <span
                    style={{
                      color:
                        Number(v) > 0 ? "var(--color-danger-text)" : undefined,
                    }}
                  >
                    {String(v)}
                  </span>
