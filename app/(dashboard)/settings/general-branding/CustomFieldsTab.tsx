...
type FieldFormData = Omit<CustomField, "id" | "isActive"> & {
  options: string[];
};
...
export default function CustomFieldsTab() {
  ...
  const fetchFields = useCallback(async () => {
    ...
    try {
      setFields(
        await client.get<CustomField[]>(`${API_BASE}?entityType=${entityType}`),
      );
    } catch {
      setFields([]);
    } finally {
    ...
    <div className="ui-stack-6">
      ...
      >
        <p className="ui-text-sm-muted">
          This will permanently remove this custom field definition and all
