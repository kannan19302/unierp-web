...
export default function EcommerceTaxPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const d = await client.get<any>(`/ecommerce/exp/${storeId}/tax-classes`);
      ...
      notifyError("Failed to load tax classes", message);
    }
    setLoading(false);
  };
  const saveClass = async () => {
    try {
      await client.post(`/ecommerce/exp/${storeId}/tax-classes`, classForm);
  ...
  const saveRate = async () => {
    try {
      await client.post("/ecommerce/exp/tax-rates", rateForm);
  ...
  const delRate = async (id: string) => {
    try {
      await client.delete(`/ecommerce/exp/tax-rates/${id}`);
      load();
    } catch (err) {
      notifyError(
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl ui-hstack-2">
              <Calculator className="ui-text-primary" /> Tax Configuration
            </h1>
            <p className="ui-text-sm-muted">Manage tax classes and rates.</p>
          </div>
          <div className="ui-hstack-2">
            ...
            <button
              ...
              disabled={!storeId}
              onClick={() => {
                setClassForm({ name: "", description: "", isDefault: false });
          ...
          <div key={c.id} className="ui-card p-4">
            <div className="flex justify-between items-center mb-3">
              <h3>
                {c.name}{" "}
                {c.isDefault && (
                  <span className="ui-badge-success ml-2">Default</span>
                )}
              </h3>
              <button
                className="ui-btn"
          ...
          >
            <div className="ui-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="ui-hstack-2 mt-4">
                <button className="ui-btn" onClick={saveClass}>
                  Save
                </button>
                <button
