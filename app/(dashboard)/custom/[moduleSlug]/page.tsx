...
export default function CustomModulePage() {
  ...
  useEffect(() => {
    async function loadCustomApp() {
      ...
      try {
        ...
        const entities =
          typeof foundModule.entities === "string"
            ? JSON.parse(foundModule.entities)
            : foundModule.entities;

        const schemaFields =
          entities && entities.length > 0 ? entities[0].fields || [] : [];
        setFields(schemaFields);

        const schema = await client.get<SchemaRegistry>(
          `/builder/schema-registries/${foundModule.slug}`,
        );
        setSchemaId(schema.id);
        const recordResult = await client.get<{ data?: CustomRecord[] }>(
          `/builder/custom-records/${schema.id}`,
        );
  ...
  const createMockRecord = async () => {
    ...
    fields.forEach((f) => {
      ...
      if (f.type === "date")
        newRecord[f.name] = new Date().toISOString().split("T")[0];
      if (f.type === "boolean") newRecord[f.name] = true;
    });
    try {
      const created = await client.post<CustomRecord>(
        ...
        <div className="builder-header ui-flex-between ui-items-start">
          <div>
            <div className={styles.s2}>
              ...
              <h1 className={styles.s4}>{moduleData.name}</h1>
            </div>
            <p className={styles.s5}>
              {moduleData.description ||
                `Custom application generated by UniERP Zero-Code Builder`}
            </p>
          </div>
