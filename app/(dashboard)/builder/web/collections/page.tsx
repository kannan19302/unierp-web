...
import { PageHeader, ConfirmDialog, ListPageTemplate, type ListColumn } from '@unerp/ui';
...
function EntryField({ field, value, onChange }: { field: any; value: any; onChange: (v: any) => void }) {
  const label = (
    <label className={styles.s1}>
      {field.label}{field.required && <span className="ui-text-danger"> *</span>}
    </label>
  );
  ...
  switch (field.type) {
    ...
    case 'Textarea': case 'RichText':
      return <div>{label}<textarea value={value || ''} onChange={(e) => onChange(e.target.value)} style={{ ...base, minHeight: field.type === 'RichText' ? 140 : 70, fontFamily: field.type === 'RichText' ? 'inherit' : undefined }} className={styles.s3} placeholder={field.type === 'RichText' ? 'Rich content (HTML/markdown supported at render time)…' : ''} /></div>;
    ...
    case 'Color':
      return <div>{label}<div className={styles.s4}><input type="color" value={value || '#000000'} onChange={(e) => onChange(e.target.value)} className={styles.s5} /><input value={value || ''} onChange={(e) => onChange(e.target.value)} style={base} placeholder="#000000" /></div></div>;
    case 'Select':
...
function EntryEditor({ collection, item, onClose, onSaved }: { collection: any; item: any | null; onClose: () => void; onSaved: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (item) {
        await client.patch(`/builder/web-collections/${collection.id}/items/${item.id}`, { data, status, featured });
      } else {
        await client.post(`/builder/web-collections/${collection.id}/items`, { data, status, featured });
      }
      onSaved();
    } catch (err) {
    ...
    <div className={styles.s8}>
      ...
      <div className={styles.s10}>
        <div className={styles.s11}>
          <div>
            <h3 className={styles.s12}>{item ? 'Edit' : 'New'} {collection.singular || 'Entry'}</h3>
            <span className="ui-text-xs-soft">{collection.name}</span>
          </div>
          <button onClick={onClose} className={styles.s13}><X size={20} /></button>
        </div>

        <div className={styles.s14}>
          {error && <div className={styles.s15}>{error}</div>}
          {fields.length === 0 && <div className={styles.s16}>This collection has no fields yet. Add fields in collection settings.</div>}
          {fields.map((f) => (
        ...
        <div className={styles.s17}>
          <div className={styles.s18}>
            {['DRAFT', 'PUBLISHED'].map((s) => (
              <button key={s} onClick={() => setStatus(s)} style={{ background: status === s ? (s === 'PUBLISHED' ? '#16a34a' : 'var(--color-text-secondary)') : 'transparent', color: status === s ? '#fff' : 'var(--color-text-secondary)' }} className={styles.s19}>{s === 'PUBLISHED' ? 'Published' : 'Draft'}</button>
            ))}
          </div>
          <button onClick={() => setFeatured(!featured)} title="Featured" style={{ color: featured ? '#f59e0b' : 'var(--color-text-muted)' }} className={styles.s20}>
...
function CreateCollectionModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  useEffect(() => { client.get<any[]>('/builder/web-collections/presets').then(setPresets).catch(() => setPresets([])); }, [client]);

  const seedPreset = async (preset: string) => {
    setBusy(preset);
    try { await client.post('/builder/web-collections/seed', { preset }); onCreated(); } catch (err) { setError(err instanceof Error ? err.message : 'Failed to create'); }
    setBusy(null);
  };
  ...
  const createCustom = async () => {
    ...
    setBusy('custom');
    try { await client.post('/builder/web-collections', { name, slug, singular: name, fields, settings: { titleField: fields[0]?.name || 'title' } }); onCreated(); } catch (err) { setError(err instanceof Error ? err.message : 'Failed to create'); }
    setBusy(null);
  };
      ...
      <div className={styles.s24}>
        ...
        <div className={styles.s27}>
          {([['preset', 'Start from preset', Sparkles], ['custom', 'Build custom', Wand2]] as const).map(([v, lbl, Icon]) => (
            <button key={v} onClick={() => setTab(v)} style={{ borderBottom: tab === v ? '2px solid var(--color-primary)' : '2px solid transparent', color: tab === v ? 'var(--color-primary)' : 'var(--color-text-secondary)' }} className={styles.s28}><Icon size={15} /> {lbl}</button>
          ))}
        </div>
        ...
        <div className={styles.s29}>
          ...
          {tab === 'preset' ? (
              ...
              {presets.map((p) => (
                <button key={p.preset} onClick={() => seedPreset(p.preset)} disabled={!!busy}
                  ...
                  <div>
                    ...
                    <div className={styles.s35}>{busy === p.preset ? 'Creating…' : `${p.fieldCount} fields · ${p.sampleCount} samples`}</div>
                  </div>
                </button>
              ))}
            </div>
            ...
            <div className="ui-stack-4">
              <div className="ui-grid-2 ui-gap-3">
                <div><label className={styles.s1}>Name</label><input value={name} onChange={(e) => { setName(e.target.value); setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')); }} placeholder="e.g. Case Studies" className={styles.s36} /></div>
                <div><label className={styles.s1}>Slug</label><input value={slug} onChange={(e) => setSlug(e.target.value)} className={styles.s37} /></div>
              </div>
              <div>
                <div className="ui-flex-between mb-2">
                  <label className={styles.s38}>Fields</label>
                  <button onClick={addField} className={styles.s39}><Plus size={14} /> Add Field</button>
                </div>
                {fields.map((f, i) => (
                  <div key={i} className={styles.s40}>
                    <input value={f.label} onChange={(e) => updateField(i, 'label', e.target.value)} placeholder="Label" className={styles.s41} />
                    <input value={f.name} onChange={(e) => updateField(i, 'name', e.target.value)} placeholder="field_name" className={styles.s42} />
                    <select value={f.type} onChange={(e) => updateField(i, 'type', e.target.value)} className={styles.s43}>{FIELD_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}</select>
                    <label className={styles.s44}><input type="checkbox" checked={f.required} onChange={(e) => updateField(i, 'required', e.target.checked)} />Req</label>
                    <button onClick={() => removeField(i)} className={styles.s13}><Trash2 size={14} /></button>
                  </div>
                ))}
              </div>
              <div className="ui-flex-end">
                <button onClick={createCustom} disabled={busy === 'custom'} className={styles.s45}>{busy === 'custom' ? 'Creating…' : 'Create Collection'}</button>
              </div>
            </div>
          )}
        </div>
...
export default function WebCollectionsPage() {
  ...
  const fetchCollections = useCallback(async () => {
    setLoading(true);
    try { setCollections(await client.get<any[]>('/builder/web-collections')); } catch { setCollections([]); }
    setLoading(false);
  }, [client]);
  ...
  const fetchItems = useCallback(async (col: any) => {
    ...
    try { const d = await client.get<{ data?: any[] }>(`/builder/web-collections/${col.id}/items?${qp}`); setItems(d.data || []); } catch { setItems([]); }
    ...
    return (
      <div className="p-6 ui-stack-5">
        <div className={styles.s46}>
          <div>
            <button onClick={() => setActive(null)} className={styles.s47}><ArrowLeft size={14} /> All Collections</button>
            ...
            <p className={styles.s51}>{items.length} entries · {fieldCount} fields · <span className="font-mono">/{active.slug}</span></p>
          </div>
          <button className="ui-btn ui-btn-primary" onClick={() => setEditingItem(null)}><Plus size={15} /> New {active.singular || 'Entry'}</button>
        </div>
        ...
        <div className="ui-stack-3">
          <div className={styles.s52}>
            <div className={styles.s53}>
              ...
              <input value={itemSearch} onChange={(e) => setItemSearch(e.target.value)} placeholder="Search entries…" className={styles.s55} />
            </div>
            {['ALL', 'PUBLISHED', 'DRAFT'].map((s) => (
              <button key={s} onClick={() => setStatusFilter(s)} style={{ border: statusFilter === s ? '1px solid var(--color-primary)' : '1px solid var(--color-border)', background: statusFilter === s ? 'var(--color-primary-bg)' : 'transparent', color: statusFilter === s ? 'var(--color-primary)' : 'var(--color-text-secondary)' }} className={styles.s56}>{s.toLowerCase()}</button>
            ))}
          </div>
          <ListPageTemplate
            title=""
            columns={[
              { key: 'id', header: 'Title', render: (_v, row) => { const it = row as unknown as { id: string; data: Record<string, unknown>; slug: string }; return <span className="font-medium">{titleOf(active, it)}</span>; } },
              { key: 'status', header: 'Status', width: '110px', render: (v) => <span className={`ui-badge ${v === 'PUBLISHED' ? 'ui-badge-success' : 'ui-badge-warning'}`}>{String(v)}</span> },
              { key: 'featured', header: 'Featured', width: '70px', render: (v) => v ? <Star size={14} fill="#f59e0b" className={styles.s57} /> : <span className="ui-text-tertiary">—</span> },
              { key: 'slug', header: 'Slug', width: '160px', render: (v) => <span className={styles.s58}>{String(v)}</span> },
              { key: 'id', header: '', width: '90px', render: (_v, row) => {
                const it = row as unknown as { id: string };
                return (
                  <div className={styles.s59} onClick={(e) => e.stopPropagation()}>
                    <button className={`ui-btn ui-btn-secondary ${styles.s60}`}  onClick={() => setEditingItem(row)}><Edit3 size={13} /></button>
                    <button className={`ui-btn ui-btn-secondary ${styles.s61}`}  onClick={() => setDeleteItemTarget(it.id)}><Trash2 size={13} /></button>
                  </div>
                );
              }},
            ] as ListColumn[]}
      ...
      <PageHeader
        ...
        actions={
          <div className="ui-flex ui-gap-2">
            ...
            <button className="ui-btn ui-btn-primary" onClick={() => setShowCreate(true)}><Plus size={15} /> New Collection</button>
          </div>
        }
      />

      {loading ? (
        <div className={styles.s62}>{[1, 2, 3].map((i) => <div key={i} className={`ui-card ${styles.s63}`}  />)}</div>
        ...
        <div className="ui-card text-center p-12">
          ...
          <p className={styles.s66}>Start from a ready-made preset (Products, Projects, Team…) or build your own.</p>
          <button className={`ui-btn ui-btn-primary ${styles.s67}`}  onClick={() => setShowCreate(true)}><Sparkles size={15} /> Create your first collection</button>
        </div>
      ) : (
        <div className={styles.s62}>
          {collections.map((c) => (
            <div key={c.id} onClick={() => openCollection(c)} className={`ui-card ${styles.s68} ${styles.collectionCard}`} style={{ borderTop: `3px solid ${c.color || 'var(--color-primary)'}` }}>
              <div className="ui-flex-between ui-items-start">
                <div className="ui-hstack-3">
                  <div style={{ background: `${c.color || '#6366f1'}18` }} className={styles.s69}>{c.icon || '📦'}</div>
                  <div>
                    <div className={styles.s70}>{c.name}</div>
                    <div className={styles.s71}>/{c.slug}</div>
                  </div>
                </div>
                <button onClick={(e) => { e.stopPropagation(); setDeleteCollectionTarget(c.id); }} className={styles.s72}><Trash2 size={15} /></button>
              </div>
              <div className={styles.s73}>
                <span><strong className={styles.s74}>{c.itemCount ?? 0}</strong> entries</span>
                <span><strong className={styles.s74}>{Array.isArray(c.fields) ? c.fields.length : 0}</strong> fields</span>
                <span style={{ color: c.color }} className={styles.s75}>{c.kind}</span>
              </div>
            </div>
          ))}
        </div>
