...
function WebAssetsPageContent() {
  ...
  const assetColumns: ListColumn[] = [
    { key: 'name', header: 'Name', render: (_v, row) => {
    ...
    { key: 'url', header: 'URL' },
    { key: 'uploadedBy', header: 'Uploaded By', render: (v) => String(v || 'System') },
    { key: 'createdAt', header: 'Uploaded At', render: (v) => new Date(String(v)).toLocaleDateString() },
    { key: 'id', header: 'Actions', render: (_v, row) => {
      const asset = row as unknown as { id: string; url: string };
      return (
        <div className="ui-flex ui-gap-1">
          <button onClick={() => window.open(asset.url, '_blank')} className={`ui-btn ui-btn-secondary ${styles.s3}`} ><Download size={11} /></button>
          <button onClick={() => { setEditingItem(row); setIsModalOpen(true); }} className={`ui-btn ui-btn-secondary ${styles.s3}`} ><Copy size={11} /></button>
          <button className={`ui-btn ${styles.s4}`}  onClick={() => setDeleteTarget(asset.id)}><Trash2 size={11} /></button>
        </div>
      );
    }},
  ];
      ...
      <PageHeader
        ...
        actions={
          <div className="ui-flex ui-gap-2">
            ...
            <button className="ui-btn ui-btn-primary" onClick={() => { setEditingItem(null); setIsModalOpen(true); }}><Upload size={15} /><span>Upload Files</span></button>
          </div>
        }
      />
      ...
      <div className={styles.s5}>
        ...
        <div className={`ui-card ${styles.s6}`} >
          ...
          <div className={styles.s8}>
            {FOLDERS.map(f => (
              <button key={f} onClick={() => setFolder(f)} style={{ background: folder === f ? 'var(--color-primary-light)' : 'transparent', color: folder === f ? 'var(--color-primary)' : 'var(--color-text-secondary)', fontWeight: folder === f ? 'var(--weight-semibold)' : 'var(--weight-normal)' }} className={styles.s9}>
                {f}
              </button>
            ))}
          </div>
          <div className={styles.s10}>
            <button onClick={() => { /* New folder */ }} className={`ui-btn ui-btn-secondary ${styles.s11}`} >
              <PlusCircle size={12} />
        ...
        <div className="ui-stack-3">
          ...
          <div className="ui-hstack-3">
            <div className={styles.s12}>
              ...
              <input className={`ui-input ${styles.s13}`} type="text" placeholder="Search assets..." value={search} onChange={e => setSearch(e.target.value)}  />
            </div>
            <div className={styles.s14}>
              <button onClick={() => setViewMode('grid')} style={{ background: viewMode === 'grid' ? 'var(--color-primary-light)' : 'transparent', color: viewMode === 'grid' ? 'var(--color-primary)' : 'var(--color-text-secondary)' }} className={styles.s15}><Grid3X3 size={15} /></button>
              <button onClick={() => setViewMode('list')} style={{ background: viewMode === 'list' ? 'var(--color-primary-light)' : 'transparent', color: viewMode === 'list' ? 'var(--color-primary)' : 'var(--color-text-secondary)' }} className={styles.s15}><List size={15} /></button>
            </div>
          </div>
          ...
          <div className={`${styles.s16} ${styles.uploadZone}`}>
            ...
            <p className={styles.s19}>Supports JPG, PNG, SVG, MP4, PDF up to 100MB</p>
          </div>
          ...
          {viewMode === 'grid' ? (
            <div className={styles.s20}>
              {filtered.map((asset: any) => {
                ...
                const color = TYPE_COLORS[asset.type] || 'var(--color-primary)';
                return (
                  <div key={asset.id} className={`ui-card ${styles.s21} ${styles.assetCard}`}>
                    <div style={{ background: `${color}15` }} className={styles.s22}>
                      <Icon size={28} style={{ color }} />
                    </div>
                    <p className={styles.s23}>{asset.name}</p>
                    <p className={styles.s24}>{formatBytes(asset.sizeBytes)}</p>
                    <div className={styles.s25}>
                      <button onClick={() => window.open(asset.url, '_blank')} className={`ui-btn ui-btn-secondary ${styles.s26}`} ><Download size={11} /></button>
                      <button onClick={() => { setEditingItem(asset); setIsModalOpen(true); }} className={`ui-btn ui-btn-secondary ${styles.s26}`} ><Copy size={11} /></button>
                      <button onClick={() => setDeleteTarget(asset.id)} className={`ui-btn ${styles.s27}`} ><Trash2 size={11} /></button>
                    </div>
                  </div>
                );
              })}
...
export default function WebAssetsPage() {
  return (
    <Suspense fallback={<div className={styles.s28}>Loading Asset Manager...</div>}>
