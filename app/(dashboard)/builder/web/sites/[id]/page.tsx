...
export default function SiteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const addPage = async () => {
    ...
    await client.request(`/builder/web-studio/sites/${id}/pages`, { method: 'PUT', body: JSON.stringify(np) });
    setNp({ path: '/', title: '', type: 'PAGE', status: 'DRAFT' });
    flash('Page created'); await load();
  };
  const publishPage = async (p: any) => {
    await client.request(`/builder/web-studio/sites/${id}/pages`, { method: 'PUT', body: JSON.stringify({ ...p, status: p.status === 'PUBLISHED' ? 'DRAFT' : 'PUBLISHED' }) });
    flash('Status updated'); await load();
  };
const delPage = async (pid: string) => { await client.delete(`/builder/web-studio/sites/${id}/pages/${pid}`); await load(); };

  const addDomain = async () => {
    if (!newHost.trim()) return;
    await client.post(`/builder/web-studio/sites/${id}/domains`, { host: newHost.trim(), isPrimary: !site.domains?.length });
    setNewHost(''); flash('Domain added'); await load();
  };
const delDomain = async (did: string) => { await client.delete(`/builder/web-studio/sites/${id}/domains/${did}`); await load(); };
  ...
  const saveTheme = async () => {
    await client.patch(`/builder/web-studio/sites/${id}`, { theme: site.theme, settings: site.settings });
    flash('Theme saved');
  };
    ...
    <div className={styles.s1}>
      ...
      {tab === 'pages' && (
        <section>
          <div className={styles.s7}>
            ...
            <input value={np.title} onChange={(e) => setNp({ ...np, title: e.target.value })} placeholder="Page title" style={{ ...inp }} className={styles.s8} />
              ...
              {pages.map((p) => (
                <div key={p.id} style={row}>
                  <div className="flex-1">
                    ...
                    <span className={styles.s10}>{p.path} · {p.type}</span>
                  </div>
      ...
      {tab === 'domains' && (
        <section>
          <div className={styles.s11}>
            <input value={newHost} onChange={(e) => setNewHost(e.target.value)} placeholder="e.g. www.example.com" style={{ ...inp }} className={styles.s12} onKeyDown={(e) => e.key === 'Enter' && addDomain()} />
            ...
            <div className="ui-stack-2">
              {site.domains.map((d: any) => (
                <div key={d.id} style={row}>
                  ...
                  <span className={styles.s13}>{d.host} {d.isPrimary && <span className={styles.s14}>primary</span>}</span>
      ...
      {tab === 'chatbot' && chatbot && (
        <section className="ui-stack-3">
          ...
          <textarea value={chatbot.config?.persona || ''} onChange={(e) => setChatbot({ ...chatbot, config: { ...chatbot.config, persona: e.target.value } })} rows={3} style={{ ...inp }} className={styles.s15} />
          ...
          <input value={chatbot.config?.accent || '#6366f1'} onChange={(e) => setChatbot({ ...chatbot, config: { ...chatbot.config, accent: e.target.value } })} style={{ ...inp }} className={styles.s16} type="color" />
          <button onClick={saveChatbot} style={{ ...primaryBtn }} className={styles.s17}><Save size={14} /> Save chatbot</button>
        </section>
      )}
      ...
      {tab === 'theme' && (
        <section className="ui-stack-3">
          ...
          <textarea value={JSON.stringify(site.theme || {}, null, 2)} onChange={(e) => { try { setSite({ ...site, theme: JSON.parse(e.target.value) }); } catch {} }} rows={8} style={{ ...inp }} className={styles.s18} />
          <label style={label}>Site settings (JSON)</label>
          <textarea value={JSON.stringify(site.settings || {}, null, 2)} onChange={(e) => { try { setSite({ ...site, settings: JSON.parse(e.target.value) }); } catch {} }} rows={6} style={{ ...inp }} className={styles.s18} />
          <button onClick={saveTheme} style={{ ...primaryBtn }} className={styles.s17}><Save size={14} /> Save theme</button>
        </section>
      )}
    </div>
  );
