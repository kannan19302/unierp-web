...
export default function SitesListPage() {
  ...
  const load = async () => {
    setLoading(true);
try { setSites(await client.get('/builder/web-studio/sites')); } catch { setSites([]); }
  ...
  const create = async () => {
    if (!name.trim()) return;
    setCreating(true);
try { await client.post('/builder/web-studio/sites', { name: name.trim() }); setName(''); await load(); } finally { setCreating(false); }
  };
            ...
            <div key={site.id} className="ui-card p-4">
              <div className="ui-flex-between">
                ...
                <div className="ui-flex ui-gap-2">
                  <Link href={`/builder/web/sites/${site.id}`} className={styles.s7}>
                    <Settings size={14} /> Manage
                  </Link>
                  <button onClick={() => setDeleteTarget(site.id)} className={styles.s8}>
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
