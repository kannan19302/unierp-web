...
export default function WebSubmissionsPage() {
  ...
  const setStatus = async (id: string, status: string) => {
    await client.request(`/builder/web-form-submissions/${id}`, { method: 'PATCH', body: JSON.stringify({ status }) });
    void fetchSubs();
  ...
  return (
    ...
    <div className="p-6 ui-stack-5">
      <PageHeader
        ...
        actions={
          <div className={styles.s1}>
            {newCount > 0 && <span className={styles.s2}>{newCount} new</span>}
      ...
      <div className={styles.s3}>
        <div className={`ui-card ${styles.s4}`} style={{ flex: selected ? '0 0 55%' : 1 }}>
          <div className={styles.s5}>
            <div className={styles.s6}>
              <Search size={14} className={styles.s26} />
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search…" className={styles.s7} />
            </div>
            {['ALL', 'NEW', 'READ', 'ARCHIVED', 'SPAM'].map((s) => (
              <button key={s} onClick={() => setFilter(s)} style={{ border: filter === s ? '1px solid var(--color-primary)' : '1px solid var(--color-border)', background: filter === s ? 'var(--color-primary-bg)' : 'transparent', color: filter === s ? 'var(--color-primary)' : 'var(--color-text-secondary)' }} className={styles.s8}>{s === 'ALL' ? 'All' : STATUS_META[s]?.label}</button>
            ))}
          </div>
          {loading ? <div className={styles.s9}>Loading…</div> : filtered.length === 0 ? (
            <div className={styles.s10}><Inbox size={40} className={styles.s27} /><div>No submissions yet.</div></div>
          ) : (
            <div className={styles.s11}>
              {filtered.map((s) => {
                ...
                return (
                  <div key={s.id} onClick={() => open(s)} style={{ background: selected?.id === s.id ? 'var(--color-primary-bg)' : s.status === 'NEW' ? 'var(--color-bg-subtle)' : 'transparent' }} className={styles.s12}>
                    <meta.icon size={16} style={{ color: meta.color, flexShrink: 0 }} />
                    <div className="flex-1 overflow-hidden">
                      <div className={styles.s13}>
                        <span style={{ fontWeight: s.status === 'NEW' ? 700 : 500 }} className={styles.s14}>{s.formName}</span>
                        <span className={styles.s15}>{new Date(s.createdAt).toLocaleString()}</span>
                      </div>
          ...
          <div className={`ui-card ${styles.s17}`} >
            <div className="ui-flex-between mb-4">
              <div>
                ...
                <span className="ui-text-xs-soft">{new Date(selected.createdAt).toLocaleString()}{selected.pageSlug ? ` · from /${selected.pageSlug}` : ''}</span>
              </div>
              <button onClick={() => setSelected(null)} className={styles.s19}>✕</button>
            </div>
            ...
            <div className={styles.s24}>
              {(['READ', 'ARCHIVED', 'SPAM'] as const).map((s) => (
                <button key={s} onClick={() => setStatus(selected.id, s)} className="ui-btn ui-btn-secondary text-xs">Mark {STATUS_META[s]!.label}</button>
              ))}
              <button onClick={() => setDeleteTarget(selected.id)} className={`ui-btn ui-btn-secondary ${styles.s25}`} ><Trash2 size={13} /> Delete</button>
            </div>
          </div>
        )}
      </div>
