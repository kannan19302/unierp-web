...
function WebBuilderPagesPageContent() {
  ...
  useEffect(() => {
    client.get<unknown>('/builder/web-collections').then((d) => setCollections(Array.isArray(d) ? d : [])).catch(() => {});
    client.get<unknown>('/builder/web-assets').then((d) => setAssets(Array.isArray(d) ? d.filter((a: any) => a.type === 'IMAGE') : [])).catch(() => {});
  }, [client]);
      ...
      <PageHeader
        ...
        actions={
          <div className={styles.s2}>
            {activeTab === 'editor' && (
              <>
                <button className="ui-btn ui-btn-secondary" onClick={() => { setActiveTab('list'); setEditingPageId(null); }}>
                  Back to List
                </button>
                ...
                <button className="ui-btn ui-btn-secondary" onClick={handlePreview}>
                  <Eye size={15} /> <span>Preview</span>
                </button>
                <button className="ui-btn ui-btn-secondary" onClick={handleSaveDraft}>
                  {savingState === 'saving' ? 'Saving…' : savingState === 'saved' ? 'Saved ✓' : 'Save Draft'}
                </button>
              </>
            )}
            {activeTab === 'editor' ? (
              <button className="ui-btn ui-btn-primary" onClick={handlePublishPage}>
                <CheckCircle size={15} />
      ...
      {activeTab === 'list' && (
        <div className={styles.s4}>
          <div className={styles.s5}>
            ...
            <input className={`ui-input ${styles.s6}`} type="text" placeholder="Search pages..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}  />
          </div>
          <DataTable<any>
            columns={[
              {
              ...
              },
              {
                key: 'slug',
                header: 'URL Slug',
                render: (row: any) => <span className={styles.s8}>/{row.slug}</span>,
              },
              {
                key: 'status',
                header: 'Status',
                render: (row: any) => (
                  <span className={`ui-badge ${row.status === 'PUBLISHED' ? 'ui-badge-success' : 'ui-badge-warning'}`}>
                    {row.status || 'DRAFT'}
                  </span>
                ),
              },
              ...
              },
              {
                ...
                render: (row: any) => (
                  <div className="ui-flex ui-gap-2" onClick={(e) => e.stopPropagation()}>
                    <button className={`ui-btn ui-btn-secondary ${styles.s9}`}  onClick={() => { setEditingItem(row); setIsModalOpen(true); }}>
                      <Edit3 size={14} />
                    </button>
                    <button className={`ui-btn ui-btn-secondary ${styles.s10}`}  onClick={() => setDeleteTarget(row.id)}>
                      <Trash2 size={14} />
                    </button>
                  </div>
                ),
                ...
                <div className="ui-stack-2">
                  {sections.map((section, idx) => (
                    <div 
                      ...
                      style={{ background: selectedSectionId === section.id ? 'rgba(59, 130, 246, 0.1)' : 'var(--color-bg-elevated)', border: `1px solid ${selectedSectionId === section.id ? 'var(--color-primary)' : 'var(--color-border)'}` }} className={styles.s21}
                    >
                      <div className="ui-hstack-2">
                        <Move size={14} className="ui-text-tertiary" />
                        <span className={styles.s22}>{section.label || section.type}</span>
                      </div>
                      <button onClick={(e) => removeSection(e, idx)} className="ui-btn-icon">
          ...
          <div className={styles.s23}>
            <div className={styles.s24}>
              <div className={styles.s25}>
                <button className={`ui-btn ui-btn-secondary ${styles.s26}`} ><Monitor size={14}/></button>
              </div>
            </div>
            ...
            <div className={`ui-card ${styles.s29}`} >
              ...
              <div className={styles.s32}>
                {(BLOCK_INSPECTOR[selectedSection.type] || [{ name: 'title', label: 'Title', type: 'text' }]).map((field) => {
                  ...
                  return (
                    <div key={field.name}>
                      {field.type !== 'checkbox' && <label className="ui-label text-xs">{field.label}</label>}
                      {field.type === 'textarea' ? (
                        <textarea className={`ui-input ${styles.s33}`} value={value || ''} onChange={(e) => updateContent({ [field.name]: e.target.value })}  />
                      ) : field.type === 'select' ? (
                        <select className="ui-input" value={value ?? ''} onChange={(e) => updateContent({ [field.name]: e.target.value })}>
                          <option value="">{field.name === 'collectionSlug' ? 'Choose a collection…' : 'Select…'}</option>
                          {options.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                        </select>
                      ) : field.type === 'number' ? (
                        <input className="ui-input" type="number" value={value ?? ''} onChange={(e) => updateContent({ [field.name]: e.target.value === '' ? undefined : Number(e.target.value) })} />
                      ) : (field.type as string) === 'asset' ? (
                        <div className={styles.s34}>
                          <input className="ui-input" type="text" value={value || ''} placeholder="Image URL" onChange={(e) => updateContent({ [field.name]: e.target.value })} />
                          {assets.length > 0 && (
                            <select className="ui-input" value="" onChange={(e) => e.target.value && updateContent({ [field.name]: e.target.value })}>
                              <option value="">📁 Pick from Asset Manager…</option>
                              {assets.map((a) => <option key={a.id} value={a.url}>{a.name}</option>)}
                            </select>
                          )}
                          {value && <img src={value} alt="" className={styles.s35} />}
                        </div>
                      ) : field.type === 'checkbox' ? (
                        ...
                        <input className="ui-input" type="text" value={value || ''} onChange={(e) => updateContent({ [field.name]: e.target.value })} />
                      )}
                    </div>
                  );
                })}
