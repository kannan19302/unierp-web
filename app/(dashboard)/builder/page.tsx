...
function StudioHomeContent() {
  ...
  const startWithAI = async () => {
    ...
    setGenerating(true);
    try {
      const data = await client.post<{ id: string }>('/builder/modules/generate', { prompt: q });
  ...
  return (
    <div className="p-6">
      <PageHeader
        ...
        actions={
          <button className="ui-btn ui-btn-secondary" onClick={openCommandPalette}>
            <Search size={15} />
            <span>Search</span>
        ...
        <div className="ui-card-body">
          <div className={styles.s3}>
            ...
            <div>
              ...
              <p className="ui-text-xs-muted m-0">Describe what you want to build and Studio will scaffold it for you.</p>
            </div>
            <span className={styles.s7}><Badge variant="warning">Preview</Badge></span>
          </div>
          <div className="ui-flex ui-gap-2">
            ...
            />
            <button className={`ui-btn ui-btn-primary ${styles.s8}`} onClick={startWithAI} disabled={generating || !aiPrompt.trim()} >
            ...
            <div className={styles.s11}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={[
                  ...
                  <XAxis dataKey="day" stroke="var(--color-text-secondary)" className={styles.s12} />
                  <YAxis stroke="var(--color-text-secondary)" className={styles.s12} />
            ...
            <div className={styles.s11}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={[
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)" />
                  <XAxis dataKey="name" stroke="var(--color-text-secondary)" className={styles.s12} />
                  <YAxis stroke="var(--color-text-secondary)" className={styles.s12} />
                  <Tooltip contentStyle={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)' }} />
      ...
      <div className={`ui-grid-2 ${styles.s2}`} >
        {PILLARS.map((p) => (
          <div key={p.id} className={`ui-card ${styles.s13}`}  onClick={() => router.push(p.href)}>
            <div className="ui-card-body">
              <div className={styles.s3}>
        ...
        <div className="ui-card-body">
          ...
          <div className="ui-grid-4">
            {QUICK_ACTIONS.map((action) => (
              <button key={action.label} className={`ui-btn ui-btn-secondary ${styles.s19}`} onClick={() => router.push(action.href)} >
                ...
                {recentItems.map((item) => (
                  ...
                  >
                    <div className="ui-hstack-3">
                      <div style={{ background: item.type === 'erp' ? 'var(--color-primary-light)' : 'rgba(124,58,237,0.1)' }} className={styles.s24}>
                        {item.type === 'erp' ? <Cpu size={13} className="ui-text-primary" /> : <Globe size={13} className={styles.s25} />}
                      </div>
                      <div>
                        <p className={styles.s26}>{item.name}</p>
                        <p className="ui-text-xs-muted m-0">{item.type === 'erp' ? 'App Studio' : 'Web Studio'} · {formatTimeAgo(item.updatedAt)}</p>
                      </div>
                    </div>
          ...
          <div className="ui-card-body">
            ...
            <div className="ui-stack-2">
              {MANAGE_SHORTCUTS.map((s) => (
                <button key={s.label} className={`ui-btn ui-btn-secondary ${styles.s19}`} onClick={() => router.push(s.href)} >
