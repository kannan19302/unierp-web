...
export default function QuoteSignaturesPage() {
  ...
  const search = async () => {
    ...
    setSearched(true);
    try {
      const list = await apiGet<QuotationSignature[]>(
        `/crm/quote-signature/quotations/${quotationId.trim()}`,
      );
  ...
  const handleRequest = async (e: React.FormEvent) => {
    ...
    setRequesting(true);
    try {
      await apiPost("/crm/quote-signature/request", {
      ...
      toast.success(
        ...
        `Awaiting signature from ${signerEmail}.`,
      );
      setSignerName("");
      setSignerEmail("");
      search();
  ...
  const viewCertificate = async (sig: QuotationSignature) => {
    ...
    setCertDoc(null);
    try {
      const cert = await apiGet<Certificate>(
        `/crm/quote-signature/certificates/${sig.id}`,
      );
      setCertificate(cert);
      const doc = await apiGet<{ content: string }>(
        `/crm/quote-signature/certificates/${sig.id}/document`,
      );
      ...
      toast.error(
        ...
        err instanceof ApiRequestError
          ? err.message
          : "Quotation may not be signed yet.",
      );
    }
  };
          ...
          <ProtectedComponent permission="crm.opportunity.update">
            <form onSubmit={handleRequest} className={styles.style1}>
              ...
              >
                ...
                {requesting ? "Sending…" : "Request Signature"}
              </Button>
            </form>
          </ProtectedComponent>
        )}
        ...
        {loading ? (
          ...
          <div className="ui-empty-state">
            ...
            <div className="text-sm">
              Request a signature above to start the e-signature flow.
            </div>
          </div>
        ) : signatures.length > 0 ? (
        ...
        <Card>
          <div className={styles.style2}>
            ...
            <h3 className="ui-heading-base">
              Signature Certificate {certificate.certificateNumber}
            </h3>
          </div>
          <div className="p-6">
