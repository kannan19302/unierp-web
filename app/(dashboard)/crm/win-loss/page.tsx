...
import { Card, PageHeader, Spinner, Button, Badge, ProtectedComponent, DataTable } from "@kannan19302/ui";
import {
  Plus,
  TrendingUp,
  TrendingDown,
...
export default function WinLossPage() {
  ...
  const createReason = async () => {
    ...
    setReasonForm({ name: "", category: "WIN" });
    load();
  };

  const deleteReason = async (id: string) => {
    if (confirm("Delete this reason?")) {
      await apiSend(`/crm/win-loss/reasons/${id}`, "DELETE");
      load();
    }
  };
  ...
  const deleteCompetitor = async (id: string) => {
    if (confirm("Delete this competitor?")) {
      await apiSend(`/crm/win-loss/competitors/${id}`, "DELETE");
      load();
    }
  };
  ...
  const inputStyle =
    "w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 mb-2";
        ...
        <Card title="Win/Loss Reasons">
          {showReasonForm && (
            <div className="mb-4 p-3 border rounded bg-gray-50">
              ...
              <div className="flex gap-2">
                <Button variant="primary" size="sm" onClick={createReason}>
                  Save
                </Button>
                <Button
                  variant="secondary"
              ...
              >
                <span>
                  {r.category === "WIN" ? (
                    ...
                    <TrendingDown className="w-4 h-4 inline mr-2 text-red-500" />
                  )}
                  {r.name}{" "}
                  <Badge
        ...
        <Card title="Competitors">
          {showCompForm && (
            <div className="mb-4 p-3 border rounded bg-gray-50">
              ...
              />
              <div className="flex gap-2">
                <Button variant="primary" size="sm" onClick={createCompetitor}>
                  Save
                </Button>
                <Button
                  variant="secondary"
