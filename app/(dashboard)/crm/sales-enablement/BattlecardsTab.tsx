...
export default function BattlecardsTab() {
  ...
  const addObjection = () =>
    setForm({
      ...form,
      objections: [...form.objections, { objection: "", response: "" }],
    });
    ...
    <div className={styles.style1}>
          ...
          <Card key={bc.id} className={styles.style7}>
            ...
            <div className={styles.style16}>
              <span className="ui-flex ui-items-center ui-gap-1">
                <MessageSquare className={styles.style17} />{" "}
                {bc.objections.length} objections
              </span>
            </div>
            ...
            {bc.winStrategy && (
              <div className={styles.style18}>
                ...
                <p className={styles.s2}>
                  {bc.winStrategy.length > 120
                    ? bc.winStrategy.slice(0, 120) + "..."
                    : bc.winStrategy}
                </p>
              </div>
            )}
      ...
      {filtered.length === 0 && (
        <div className={styles.style21}>
          ...
          <p>
            {search
              ? "No battlecards match your search."
              : "No battlecards yet. Create one to get started."}
          </p>
        </div>
      )}
        ...
        footer={
          <>
            ...
            <Button
              ...
              disabled={!form.competitorName.trim() || saving}
            >
              {saving ? "Creating..." : "Create Battlecard"}
            </Button>
          </>
        }
      >
              ...
              {form[field].map((val, i) => (
                <div key={i} className={styles.style25}>
                  ...
                  {form[field].length > 1 && (
                    ...
                    >
                      <X className={styles.style26} />
                    </button>
                  )}
                </div>
              ))}
              <button onClick={() => addRow(field)} className={styles.style27}>
                + Add {field.slice(0, -1)}
              </button>
            </div>
          ))}
              ...
              <div key={i} className={styles.style30}>
                ...
                {form.objections.length > 1 && (
                  <button
                    ...
                    className={styles.style32}
                  >
                    <X className={styles.style33} />
                  </button>
                )}
              </div>
            ))}
            <button onClick={addObjection} className={styles.style34}>
              + Add objection
            </button>
          </div>
