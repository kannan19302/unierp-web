...
export default function CrmReportsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"dashboard" | "builder" | "saved">(
    "dashboard",
  );
    ...
    try {
      ...
      setData({
        funnel: {
          ...
          CLOSED_WON: { count: 2, totalAmount: 35000 },
        },
        ...
        cohort: [
          { month: "2026-04", total: 10, converted: 3, conversionRate: 30 },
          { month: "2026-05", total: 8, converted: 2, conversionRate: 25 },
          { month: "2026-06", total: 7, converted: 3, conversionRate: 43 },
        ],
        repPerformance: [
          {
            userId: "user1",
        ...
        {activeTab === "dashboard" && (
          <>
            ...
            <div className="ui-grid-auto">
              <Card>
                <div className="ui-text-xs-muted">Pipeline Value</div>
                <h3 className={styles.kpiValue}>
                  <DollarSign size={16} /> ${totalPipeline.toLocaleString()}
                </h3>
              </Card>
              <Card>
                <div className="ui-text-xs-muted">Win Rate</div>
                <h3 className={`${styles.kpiValue} ${styles.successText}`}>
                  <TrendingUp size={16} /> {data.winRate.winRate}%
                </h3>
              </Card>
              <Card>
                <div className="ui-text-xs-muted">Lead → Opp Rate</div>
                <h3 className={styles.kpiValue}>
                  <Users size={16} /> {data.conversionFunnel.leadToOppRate}%
                </h3>
              </Card>
              <Card>
                <div className="ui-text-xs-muted">Opp → Win Rate</div>
                <h3 className={styles.kpiValue}>
                  <BarChart3 size={16} /> {data.conversionFunnel.oppToWinRate}%
                </h3>
              </Card>
            </div>
              ...
              <div className={styles.conversionFunnel}>
                {[
                  ...
                  },
                  {
                    ...
                    value: data.conversionFunnel.convertedLeads,
                    color: "var(--color-info)",
                    width: `${Math.max(20, data.conversionFunnel.leadToOppRate)}%`,
                  },
                  {
                    label: "Total Opportunities",
                    value: data.conversionFunnel.totalOpportunities,
                    color: "var(--color-warning)",
                    width: `${Math.max(15, (data.conversionFunnel.totalOpportunities / Math.max(1, data.conversionFunnel.totalLeads)) * 100)}%`,
                  },
                  {
                    label: "Won",
                    value: data.conversionFunnel.wonOpportunities,
                ...
                {Object.entries(data.funnel).map(([stage, info]: any) => (
                  <div key={stage} className={styles.pipelineRow}>
                    ...
                    <span className={styles.pipelineValue}>
                      {info.count} / ${info.totalAmount.toLocaleString()}
                    </span>
                  </div>
                ))}
              </Card>
              ...
              <Card padding="md">
                ...
                {data.repPerformance.length > 0 ? (
                  <ListPageTemplate
                    ...
                    data={
                      data.repPerformance as unknown as Record<
                        string,
                        unknown
                      >[]
                    }
