...
const emptyStage = (): Omit<Stage, "id"> & { id: string } => ({
...
export default function PipelinesPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await apiGet<Stage[]>(
          `/crm/pipelines/${selectedId}/stages`,
        );
  ...
  const saveStages = async () => {
    ...
    try {
      ...
      const fresh = await apiGet<Stage[]>(
        `/crm/pipelines/${selectedId}/stages`,
      );
      setStages(Array.isArray(fresh) ? fresh : []);
    } catch {
      setError("Could not save stages.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const createPipeline = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const p = await apiSend<Pipeline>("/crm/pipelines", "POST", {
        name: newName,
      });
      setCreating(false);
      setNewName("");
