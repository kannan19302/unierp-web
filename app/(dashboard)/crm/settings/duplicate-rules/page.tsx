...
export default function DuplicateRulesSettingsPage() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      if (editing)
        await apiSend(`/crm/duplicate-rules/${editing.id}`, "PATCH", form);
  ...
  const remove = async (id: string) => {
    ...
    try {
      await apiSend(`/crm/duplicate-rules/${id}`, "DELETE");
      await load();
    } catch {
      ...
      <Card padding="none">
        {rules.length === 0 ? (
          <div className={styles.style1}>
            ...
            <p className="text-sm">
              No duplicate rules configured. Add a rule to enable duplicate
              detection.
            </p>
          </div>
        ) : (
              ...
              <div className={styles.style4}>
                {ENTITY_FIELDS[form.entity].map((f: any) => (
                  <label
                    key={f}
                    style={{
                      background: form.matchFields.includes(f)
                        ? "var(--color-bg-sunken)"
                        : "transparent",
                    }}
                    className={styles.s1}
