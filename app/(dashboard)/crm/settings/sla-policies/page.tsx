...
export default function SlaPoliciesSettingsPage() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      if (editing)
        await apiSend(`/crm/sla-policies/${editing.id}`, "PATCH", form);
  ...
  const remove = async (id: string) => {
    ...
    try {
      await apiSend(`/crm/sla-policies/${id}`, "DELETE");
      await load();
    } catch {
      ...
      <Card padding="none">
        {policies.length === 0 ? (
          <div className={styles.style1}>
            ...
            <p className="text-sm">
              No SLA policies. Add a policy so cases get response and resolution
              deadlines.
            </p>
          </div>
        ) : (
