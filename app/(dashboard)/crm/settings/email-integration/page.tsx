...
export default function EmailIntegrationSettingsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiGet<MailboxConnection[]>(
        "/crm/mailbox-connections",
      );
  ...
  const connect = async (provider: Provider) => {
    ...
    try {
      const result = await apiSend<{ authorizationUrl: string }>(
        "/crm/mailbox-connections/connect",
        "POST",
        {
          provider,
          redirectUri: `${window.location.origin}${CALLBACK_PATH}`,
      ...
      setError(
        `Could not start ${provider === "GOOGLE" ? "Google" : "Microsoft"} connection. Check that OAuth is configured on the server.`,
      );
      setConnecting(null);
    }
  };
  ...
  const disconnect = async (id: string) => {
    if (!confirm("Disconnect this mailbox? Synced activity records are kept."))
      return;
    try {
  ...
  const syncNow = async (id: string) => {
    ...
    try {
      ...
      if (result.syncError) {
        ...
        toast.success(
          "Sync complete",
          `Synced ${result.messagesSynced} email(s) and ${result.eventsSynced} calendar event(s).`,
        );
      }
      await load();
    } catch {
    ...
    <div className="ui-stack-6">
          ...
          <ProtectedComponent permission="crm.mailbox.create">
            <div className="ui-flex ui-gap-2">
              ...
              >
                ...
                {connecting === "GOOGLE" ? "Redirecting…" : "Connect Gmail"}
              </Button>
              <Button
                variant="outline"
                size="sm"
      ...
      <Card padding="none">
        {connections.length === 0 ? (
          <div className={styles.style1}>
            ...
            <p className="text-sm">
              No mailbox connected. Connect Gmail or Outlook to sync emails and
              meetings into CRM activity timelines automatically.
            </p>
          </div>
        ) : (
          <ListPageTemplate
                  ...
                  render: (v: any, row: any) => (
                    <div className={styles.style3}>
                      <ProtectedComponent permission="crm.mailbox.update">
                        <button
                          onClick={() => syncNow(String(v))}
                          disabled={
                            syncingId === v || row.status === "DISCONNECTED"
                          }
                        ...
                        >
                          <RefreshCw
                            size={16}
                            className={syncingId === v ? "ui-spin" : undefined}
                          />
                        </button>
                      </ProtectedComponent>
                      <ProtectedComponent permission="crm.mailbox.delete">
                        <button
                          onClick={() => disconnect(String(v))}
      ...
      <Card>
        <div className={styles.style5}>
          ...
          <span>
            Connecting a mailbox authorizes UniERP to read recent emails and
            calendar events via the Gmail API or Microsoft Graph. Use &quot;Sync
            now&quot; to pull the latest messages and meetings — any sender,
            recipient, or attendee whose email address matches an existing
            Contact, Lead, or Customer gets an Activity entry automatically
            added to that record&apos;s timeline.
          </span>
          <span>
            This is a manual/polling sync (no continuous background listener
            yet) — click &quot;Sync now&quot; periodically, or ask an admin to
            schedule it.
          </span>
        </div>
      </Card>
