...
export default function PipelineRiskAlertsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        "Could not load pipeline risk alerts",
  ...
  const recompute = async () => {
    ...
    try {
      const result = await apiPost<{
        ...
        updated: number;
        resolved: number;
      }>("/crm/pipeline-risk/recompute", {});
      toast.success(
        "Pipeline risk recomputed",
        `Scanned ${result.scanned} deals — ${result.created} new, ${result.updated} updated, ${result.resolved} resolved.`,
      );
      await load();
    } catch (err) {
      toast.error(
      ...
      <PageHeader
        ...
        actions={
          <ProtectedComponent permission="crm.opportunity.update">
            ...
            >
              <RefreshCw size={16} />{" "}
              {recomputing ? "Recomputing…" : "Recompute Alerts"}
            </Button>
          </ProtectedComponent>
        }
      />
        ...
        {loading ? (
          ...
          <div className="ui-empty-state">
            ...
            <div className="text-sm">
              Click "Recompute Alerts" to scan the pipeline for stalled or
              at-risk deals.
            </div>
          </div>
        ) : (
