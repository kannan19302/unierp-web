...
export default function RevenueIntelligenceDigestPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiGet<DigestRun[]>(
        "/crm/revenue-intelligence/digest/runs",
      );
  ...
  const generate = async (windowHours: number) => {
    ...
    try {
      const result = await apiPost<GenerateResult>(
        `/crm/revenue-intelligence/digest/generate?windowHours=${windowHours}`,
        {},
      );
      toast.success(
      ...
      render: (r: any) => (
        <div>
          ...
          <div className="ui-text-xs-muted">
            <Badge variant={r.scope === "MANAGER" ? "info" : "default"}>
              {r.scope === "MANAGER" ? "Team rollup" : "Rep digest"}
            </Badge>
          </div>
        </div>
      ),
        ...
        actions={
          <ProtectedComponent permission="crm.opportunity.update">
            <div className="ui-flex ui-gap-2">
              ...
              <Button
                variant="primary"
                onClick={() => generate(168)}
                disabled={generating}
                className="ui-hstack-2"
              >
                <RefreshCw size={16} />{" "}
                {generating ? "Sending…" : "Send Weekly Digest"}
              </Button>
            </div>
          </ProtectedComponent>
        }
        ...
        {loading ? (
          ...
          <div className="ui-empty-state">
            ...
            <div className="text-sm">
              Send a daily or weekly digest to notify reps and managers of
              at-risk deals.
            </div>
          </div>
        ) : (
