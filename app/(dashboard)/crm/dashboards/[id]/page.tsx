...
function MockVisualization({
        ...
        {bars.map((h, i) => (
          <div key={i} className={styles.barColumn}>
            <div
              ...
              style={{ height: h, opacity: 0.7 + i * 0.06 }}
            />
  ...
  if (type === "FUNNEL") {
    const stages = [
      { label: "Prospects", w: 100 },
      { label: "Qualified", w: 75 },
      { label: "Proposal", w: 50 },
      { label: "Won", w: 30 },
    ];
    return (
      <div className={styles.funnel}>
        {stages.map((s, i) => (
          <div
            ...
            style={{ width: `${s.w}%`, opacity: 0.9 - i * 0.15 }}
          >
            {s.label}
          </div>
        ))}
        ...
        {["Sarah Chen|$320K", "James Park|$285K", "Maria Santos|$240K"].map(
          (row, i) => {
            ...
            return (
              <div
                key={i}
                className={`${styles.leaderboardRow} ${i < 2 ? styles.withDivider : ""}`}
              >
                <span>
                  <strong className={styles.rank}>#{i + 1}</strong>
                  {name}
                </span>
...
export default function DashboardCanvasPage() {
  ...
  const [widgetForm, setWidgetForm] = useState({
    ...
    title: "",
    dataSource: "PIPELINE" as DataSource,
  });
      ...
      <div className={styles.page}>
        <PageHeader
          ...
          actions={
            <Button onClick={() => setEditMode(!editMode)}>
              {editMode ? (
                <>
                  <Eye className={styles.buttonIcon} /> View Mode
                </>
              ) : (
                <>
                  <Edit3 className={styles.buttonIcon} /> Edit Mode
                </>
              )}
            </Button>
          }
            ...
            <div className={styles.widgetPalette}>
              ...
              {WIDGET_TYPES.map((wt) => (
                <button
                  key={wt}
                  onClick={() => {
                    setWidgetForm({ ...widgetForm, widgetType: wt });
            ...
            {dashboard.widgets.map((widget) => (
              ...
              >
                <div className={styles.widgetHeader}>
                  ...
                  <span className={styles.widgetSource}>
                    {widget.dataSource}
                  </span>
                </div>
        ...
        {dashboard.widgets.length === 0 && (
          <div className={styles.emptyState}>
            ...
            <p>
              No widgets yet. Switch to Edit Mode and add your first widget.
            </p>
          </div>
        )}
            ...
            <Card className={styles.modal}>
              ...
              <div className={styles.modalActions}>
                ...
                <Button
                  ...
                  disabled={!widgetForm.title.trim() || saving}
                >
