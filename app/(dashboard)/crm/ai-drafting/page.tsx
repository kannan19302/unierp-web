...
export default function AiDraftingPage() {
  ...
  const endpointFor = (draftKind: DraftType) => {
    if (draftKind === "FOLLOWUP_EMAIL")
      return `/crm/ai-drafting/opportunities/${contextId}/followup`;
    if (draftKind === "QUOTE_COVER_NOTE")
      return `/crm/ai-drafting/quotations/${contextId}/cover-note`;
  ...
  const generate = async () => {
    if (!contextId.trim()) {
      toast.error(
        "Enter an opportunity/quotation/lead ID to generate a draft for",
      );
      return;
    }
    setGenerating(true);
    try {
      ...
      toast.success(
        "Draft generated",
        "Review, edit, and send it from your own email client — nothing is auto-sent.",
      );
      setContextId("");
      await load();
    } catch (err) {
  ...
  const regenerate = async (draftId: string, newTone: Tone) => {
    try {
      await apiPost(
        `/crm/ai-drafting/${draftId}/regenerate?tone=${newTone}`,
        {},
      );
  ...
  const saveEdit = async () => {
    if (!editingId) return;
    try {
      await apiPatch(`/crm/ai-drafting/${editingId}`, {
        subject: editSubject,
        body: editBody,
      });
    ...
    },
    {
      ...
      render: (d: any) => (
        <div className="ui-flex ui-gap-2">
          ...
          <Button
            variant="ghost"
            onClick={() => regenerate(d.id, d.tone)}
            title="Regenerate"
          >
            <RefreshCw size={14} />
          </Button>
        ...
        {loading ? (
          ...
          <div className="ui-empty-state">
            ...
            <div className="text-sm">
              Generate a follow-up email, quote cover note, or lead outreach
              draft above.
            </div>
          </div>
        ) : (
