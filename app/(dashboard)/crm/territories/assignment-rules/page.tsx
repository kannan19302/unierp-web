...
export default function TerritoryAssignmentRulesPage() {
  ...
  const [ruleType, setRuleType] =
    useState<TerritoryRule["ruleType"]>("GEOGRAPHY");
  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast.error(
        "Could not load territory rules",
  ...
  const handleToggleActive = async (rule: TerritoryRule) => {
    try {
      await apiPut(`/crm/territory-rules/${rule.id}`, {
  ...
  const handleReassignAll = async () => {
    try {
      const result = await apiPost<{ processed: number; matched: number }>(
        "/crm/territory-rules/reassign-all",
      );
      toast.success(
        "Bulk reassignment complete",
        `${result.matched} of ${result.processed} open leads matched a rule.`,
      );
      loadData();
    } catch (err) {
      toast.error(
      ...
      <Card>
        <div className={styles.style2}>
          <h3 className="ui-heading-base">
            Rules (highest priority evaluated first)
          </h3>
        </div>
        {loading ? (
          ...
          <div className="ui-empty-state">
            ...
            <div className="text-sm">
              Create a rule to start auto-routing leads to territories.
            </div>
          </div>
        ) : (
              ...
              {ruleType === "GEOGRAPHY" && (
                <div>
                  <label style={labelStyle}>
                    Countries (comma-separated, e.g. US,CA)
                  </label>
                  <input
                    value={countries}
