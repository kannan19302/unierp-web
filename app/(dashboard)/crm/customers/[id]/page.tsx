...
export default function CustomerDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "profile" | "orders" | "invoices" | "cases" | "contracts"
  >("profile");
  ...
  const fetchHealth = useCallback(async () => {
    ...
    try {
      ...
      setHealthData({
        ...
        dimensions: {
          paymentTimeliness: { score: 22, maxScore: 25, details: "1 overdue" },
          supportHealth: { score: 20, maxScore: 25, details: "0 open cases" },
          revenueEngagement: { score: 18, maxScore: 20, details: "5 orders" },
          invoiceHealth: { score: 12, maxScore: 15, details: "3 paid" },
        },
      });
    } finally {
  ...
  const { customer, metrics, recentSalesOrders, recentInvoices, recentCases } =
    data;
  ...
  const creditPercent =
    metrics.creditLimit > 0
      ? Math.min(
          100,
          Math.round((metrics.unpaidBalance / metrics.creditLimit) * 100),
        )
      : 0;
      ...
      <Card padding="none">
        <div className={styles.p21}>
          ...
          <Button
            ...
            onClick={() =>
              router.push(
                `/crm/contracts?customerId=${id}&customerName=${encodeURIComponent(customer.name)}`,
              )
            }
          >
            Create Contract
          ...
          <div className={styles.p210}>
            {customer.creditHold ? (
              <Button
                variant="primary"
                size="sm"
                onClick={() => handleToggleCreditHold(false)}
              >
                Release Credit Hold
              </Button>
            ) : (
      ...
      <div className="ui-stack-6 ui-animate-in">
          ...
          actions={
            <div className="ui-flex ui-gap-2">
              ...
              >
                ...
                {loadingHealth ? "Recalculating..." : "Recalculate Health"}
              </Button>
            </div>
          }
        />
          ...
          <Card padding="md">
            <div className={styles.p224}>
              ...
              <div className="flex-1">
                <div className="ui-text-xs-bold-muted">
                ...
                <div className={styles.p225}>
                  ...
                  <span className="ui-text-xs-tertiary">
                    Churn Risk: {healthData.churnProbability || "LOW"}
                  </span>
                </div>
              </div>
                ...
                {Object.entries(healthData.dimensions || {}).map(
                  ([key, val]: [string, any]) => (
                    <div key={key} className={styles.p227}>
                      <div className={styles.p228}>
                        {key.replace(/([A-Z])/g, " $1")}
                      </div>
                      <div className={styles.p229}>
            ...
            <div className="ui-flex-between ui-items-start">
              <div>
                ...
                <h3 className={styles.p232}>
                  $
                  {metrics.ltv.toLocaleString(undefined, {
          ...
          <Card padding="md">
            <div className="ui-flex-between ui-items-start">
              <div>
                ...
                >
                  $
                  {metrics.unpaidBalance.toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}
                </h3>
            ...
            <div className="ui-stack-2">
              <div className="ui-flex-between ui-items-start">
                <div>
                  ...
                  <h3
                    style={{
                      color: metrics.isCreditLimitExceeded
                        ? "var(--color-danger)"
                        : "var(--color-text)",
                    }}
                    className={styles.s5}
                  >
                    $
                    {metrics.availableCredit.toLocaleString(undefined, {
                      minimumFractionDigits: 0,
                    })}{" "}
                    / $
                    {metrics.creditLimit.toLocaleString(undefined, {
                      minimumFractionDigits: 0,
                    })}
                  </h3>
                </div>
                <div
                  style={{
                    backgroundColor: metrics.isCreditLimitExceeded
                      ? "rgba(239, 68, 68, 0.1)"
              ...
              <div className={styles.p235}>
                <div
                  style={{
                    width: `${creditPercent}%`,
                    background: metrics.isCreditLimitExceeded
                      ? "var(--color-danger)"
                      : "var(--color-primary)",
                  }}
                  className={styles.s7}
        ...
        {metrics.isCreditLimitExceeded && (
          <div className={styles.p238}>
            ...
            <span>
              Warning: This customer has exceeded their configured credit limit!
              Immediate actions on new Sales Orders should be placed on hold.
            </span>
          </div>
        )}
