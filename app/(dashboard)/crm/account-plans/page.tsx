...
export default function AccountPlansPage() {
  ...
  const loadData = async () => {
    try {
      ...
      toast.error(
        "Could not load account planning data",
  ...
  const handleLogHealth = async (e: React.FormEvent) => {
    ...
    if (!healthLog.customerId || !healthLog.score) {
      toast.error(
        "Validation Error",
        "Customer and Health Score are required.",
      );
      return;
    }
  ...
  const handleMergeAccounts = async (e: React.FormEvent) => {
    ...
    if (!merge.sourceCustomerId || !merge.targetCustomerId) {
      toast.error(
        "Validation Error",
        "Source and Target Customers are required.",
      );
      return;
    }
    if (merge.sourceCustomerId === merge.targetCustomerId) {
        ...
        <Card>
          <div className="p-6">
            ...
            <form onSubmit={handleMergeAccounts} className="ui-stack-4">
              <div>
                <label className={styles.p215}>
                  Source Customer (To Delete)
                </label>
                <select
