...
export default function DealRoomDetailPage() {
  ...
  const [milestoneForm, setMilestoneForm] = useState({
    title: "",
    ownerType: "SELLER" as "SELLER" | "BUYER" | "MUTUAL",
  });
  ...
  return (
    <div className="ui-stack-6 ui-animate-in">
          ...
          {room.stakeholders.length === 0 ? (
            ...
            room.stakeholders.map((s) => (
              <div key={s.id} className={styles.p29}>
                ...
                <Badge variant={s.side === "BUYER" ? "info" : "default"}>
                  {s.side}
                </Badge>
                {s.sentiment && (
                  <Badge
      ...
      <Card>
        <div className="p-4">
          <h3 className={styles.p210}>
            <FileText size={18} /> Shared Documents
          </h3>
        </div>
            ...
            room.documents.map((d) => (
              <div key={d.id} className={styles.p213}>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className={styles.p214}
                >
                  {d.title}
                </a>
                <Badge variant="default">{d.category}</Badge>
                {d.viewedByBuyerAt && (
                  <Badge variant="success">Viewed by buyer</Badge>
                )}
              </div>
            ))
          )}
      ...
      <Card>
        <div className={styles.p215}>
          Buyer share link:{" "}
          <code>{`/public/deal-rooms/${room.buyerAccessToken}`}</code>
        </div>
      </Card>
          ...
          <Card>
            <div className={styles.p217}>
              ...
              <Button
                ...
                disabled={busy || !milestoneForm.title}
              >
                Add Milestone
              </Button>
            </div>
          ...
          <Card>
            <div className={styles.p219}>
              ...
              <Button
                ...
                disabled={busy || !stakeholderForm.name}
              >
                Add Stakeholder
              </Button>
            </div>
