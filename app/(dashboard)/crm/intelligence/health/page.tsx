...
import { Card, PageHeader, Spinner, StatusBadge } from "@kannan19302/ui";
    ...
    const fetchData = async () => {
      try {
        const res = await client.get<any>(
          "/crm/customers/at-risk?threshold=70",
        );
        ...
        <div className={`ui-grid-2 ${styles.grid}`}>
          <Card>
            <div className="ui-card-header">
              <h3 className="ui-card-title">At-Risk Customers</h3>
            </div>
                ...
                <div className="ui-stack-2">
                  {atRisk.slice(0, 10).map((c: any) => (
                    <button
                      ...
                      onClick={() => fetchCustomerHealth(c.customerId)}
                    >
                      <div>
                        <strong className="text-sm">{c.customerName}</strong>
                        <p className="ui-text-xs-muted m-0">
                          Score: {c.healthScore}/100 - {c.churnProbability} risk
                        </p>
                      </div>
          ...
          <Card>
            <div className="ui-card-header">
              <h3 className="ui-card-title">Health Details</h3>
            </div>
            <div className="ui-card-body">
              {selectedCustomer ? (
                <div>
                  ...
                  <div className={styles.healthSummary}>
                    <StatusBadge
                      status={getHealthColor(selectedCustomer.status)}
                    />
                    <span className="ui-text-sm-muted">
                      Score: {selectedCustomer.healthScore}/100
                    </span>
                  </div>
                  {selectedCustomer.dimensions &&
                    Object.entries(selectedCustomer.dimensions).map(
                      ([key, dim]: [string, any]) => (
                ...
                <p className="ui-text-sm-muted">
                  Click a customer to see health details
                </p>
              )}
            </div>
          </Card>
