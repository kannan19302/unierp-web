  ...
  const leadColumns = [
    ...
    { key: "email", header: "Email" },
    {
      ...
      render: (r: any) => (
        <Badge
          variant={
            r.score >= 80 ? "success" : r.score >= 50 ? "warning" : "default"
          }
        >
          {String(r.score || 0)}
        </Badge>
  ...
  return (
    <RouteGuard permission="crm.read">
      <div className="ui-page">
        ...
        <div className={`ui-grid-3 ${styles.modelsGrid}`}>
          <Card>
            <div className="ui-card-header">
              <h3 className="ui-card-title">ML Models</h3>
            </div>
            <div className="ui-card-body">
              {models.length === 0 ? (
                <p className="ui-text-sm-muted">No models trained yet</p>
              ) : (
                  ...
                  {models.map((m: any) => (
                    <div key={m.id} className={styles.modelRow}>
                      <div>
                        ...
                        <p className="ui-text-xs-muted m-0">{m.model_type}</p>
                      </div>
                      <Badge
          ...
          <Card>
            <div className="ui-card-header">
              <h3 className="ui-card-title">Training Results</h3>
            </div>
            <div className="ui-card-body">
              {trainResult ? (
                <div>
                  <div className="ui-grid-2">
                    <div>
                      <p className="ui-text-xs-muted m-0">Accuracy</p>
                      <p className={styles.metricValue}>
                        {trainResult.accuracy}%
                      </p>
                    </div>
                    <div>
                      <p className="ui-text-xs-muted m-0">Leads Scored</p>
                      <p className={styles.metricValue}>
                        {trainResult.leadsScored}
                      </p>
                    </div>
                    <div>
                      <p className="ui-text-xs-muted m-0">Total Leads</p>
                      <p className={styles.metricValue}>
                        {trainResult.totalLeads}
                      </p>
                    </div>
                    <div>
                      <p className="ui-text-xs-muted m-0">Features</p>
                      <p className={styles.featureCount}>
                        {trainResult.featuresUsed?.length || 0}
                      </p>
                    </div>
                  </div>
                  <div className={styles.featureSection}>
                    ...
                    <div className={styles.featureList}>
                      {trainResult.featuresUsed?.map((f: string) => (
          ...
          <Card>
            <div className="ui-card-header">
              <h3 className="ui-card-title">Scoring Factors</h3>
            </div>
            <div className="ui-card-body">
              <p className={styles.factorDescription}>
                Top factors influencing lead scores:
              </p>
              <div className="ui-stack-2">
                {[
                  { factor: "Email provided", points: 15 },
                  { factor: "Phone provided", points: 15 },
        ...
        <Card>
          <div className="ui-card-header">
            <h3 className="ui-card-title">Leads by Score</h3>
          </div>
          <div className="ui-card-body">
