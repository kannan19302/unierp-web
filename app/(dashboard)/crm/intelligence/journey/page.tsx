...
export default function JourneyTimelinePage() {
  ...
  const [entities, setEntities] = useState<
    Array<{ id: string; name: string; type: "Lead" | "Contact" }>
  >([]);
    ...
    const fetchDetails = async () => {
      ...
      try {
        const [timelineData, attrData] = await Promise.all([
          client.get<any>(
            `/crm/journey/${selectedEntityType}/${selectedEntity}`,
          ),
          client.get<any>(
            `/crm/opportunities/${selectedEntity}/attribution?model=${attributionModel}`,
          ),
        ]);
      ...
      <div className="ui-stack-6 ui-animate-in">
        ...
        <Card padding="md">
          <div className={styles.filterGrid}>
            <div>
              <span className="ui-text-xs-bold-muted">
                ...
                {timeline.map((tp: any) => (
                  <div key={tp.id} className={styles.touchpoint}>
                    ...
                    <div
                      ...
                      style={{ background: getTouchpointColor(tp.type) }}
                    />

                    <div className={styles.touchpointHeader}>
                      <div className={styles.touchpointType}>
                        <span style={{ color: getTouchpointColor(tp.type) }}>
                          {getTouchpointIcon(tp.type)}
                        </span>
                        <strong className={styles.touchpointTitle}>
                          {tp.type.toUpperCase()}
                        </strong>
                      </div>
                      <span className={styles.timestamp}>
                        ...
                        {new Date(tp.timestamp).toLocaleTimeString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </span>
                  ...
                  <div className="ui-stack-3">
                      ...
                      <div key={chan.name}>
                        <div className={styles.channelHeader}>
                          ...
                          <span className="ui-text-muted">
                            {chan.weight}% (${chan.value.toLocaleString()})
                          </span>
                        </div>
                        <div className={styles.progressTrack}>
                          <div
                            className={styles.progressFill}
                            style={{ width: `${chan.weight}%` }}
                          />
                        </div>
                      </div>
                    ))}
                    ...
                    <div className={styles.totalValue}>
                      ...
                      <span className={styles.totalAmount}>
                        $
                        {attributionData.totalOpportunityValue?.toLocaleString() ||
                          "0"}
                      </span>
                    </div>
                  </div>
