    ...
    const fetchAnalytics = async () => {
      ...
      try {
        const [analyticsData, sendTimeData, abData] = await Promise.all([
          client.get<any>(
            `/crm/campaigns-analytics?campaignId=${selectedCampaign}`,
          ),
          client.get<any>("/crm/campaigns-analytics/send-time"),
          client.get<any>(
            `/crm/campaigns-analytics/${selectedCampaign}/ab-test`,
          ),
        ]);
        ...
        setAbTest({
          ...
          variants: [
            {
              variant: "A",
              subjectLine: "Standard Subject Line",
              sentCount: 1000,
              openRate: 22,
              clickRate: 4,
            },
            {
              variant: "B",
              subjectLine: "Personalized Subject Line",
              sentCount: 1000,
              openRate: 31,
              clickRate: 7,
            },
                ...
                <Card padding="md">
                  ...
                  <div className={styles.metricGrid}>
                    ...
                    <div className={styles.metricCard}>
                      ...
                      <div
                        className={`${styles.metricValue} ${styles.primaryValue}`}
                      >
                        {analytics.emailMetrics?.openRate}%
                      </div>
                    </div>
                    <div className={styles.metricCard}>
                      <div className="ui-text-micro">CLICK RATE</div>
                      <div
                        className={`${styles.metricValue} ${styles.infoValue}`}
                      >
                        {analytics.emailMetrics?.clickRate}%
                      </div>
                    </div>
                    <div className={styles.metricCard}>
                      <div className="ui-text-micro">CONVERSION</div>
                      <div
                        className={`${styles.metricValue} ${styles.successValue}`}
                      >
                ...
                <Card padding="md">
                  ...
                  <div className="ui-stack-3">
                    {abTest.variants?.map((v: any) => (
                      <div
                        key={v.variant}
                        className={`${styles.variantCard} ${v.variant === abTest.winner ? styles.winningVariant : ""}`}
                      >
                        ...
                        <div className={styles.variantMetrics}>
                          <div>
                            Sent: <b>{v.sentCount}</b>
                          </div>
                          <div>
                            Open: <b>{v.openRate}%</b>
                          </div>
                          <div>
                            Click: <b>{v.clickRate}%</b>
                          </div>
                        </div>
                      </div>
                    ))}
