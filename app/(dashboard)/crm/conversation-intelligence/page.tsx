...
import { Card, PageHeader, Button, Spinner, Badge, useToast, DataTable, ProtectedComponent, Input, Textarea, type Column } from "@kannan19302/ui";
...
export default function ConversationIntelligencePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [callList, summaryData] = await Promise.all([
        ...
        apiGet<InsightsSummary>(
          "/crm/conversation-intelligence/insights/summary",
        ),
      ]);
      ...
      toast.error(
        "Could not load conversation intelligence",
  ...
  const logCall = async () => {
    ...
    try {
      ...
      toast.success(
        "Call logged",
        "AI summary, sentiment, and action items generated.",
      );
        ...
        <div className={styles.summaryGrid}>
          ...
          <Card>
            <div className="p-4">
              ...
              <div className="text-2xl">
                {summary.totalActionItemsExtracted}
              </div>
            </div>
          </Card>
      ...
      {showForm && (
        <Card>
          <div className="p-4 ui-stack-3">
            ...
            />
            <div>
              <Button variant="primary" onClick={logCall} disabled={saving}>
                {saving ? "Analyzing…" : "Log Call & Generate Summary"}
              </Button>
            </div>
          </div>
        </Card>
        ...
        {loading ? (
          ...
          <div className="ui-empty-state">
            ...
            <div className="text-sm">
              Log a call to get an AI-generated summary, sentiment, and action
              items.
            </div>
          </div>
        ) : (
