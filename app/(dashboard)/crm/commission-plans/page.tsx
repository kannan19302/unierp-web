...
export default function CommissionPlansPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        "Could not load commission plans",
  ...
  const addTier = async () => {
    ...
    try {
      await apiPost(`/crm/commission-plans/${tierModalPlanId}/tiers`, {
        ...
        maxAttainmentPct:
          tierForm.maxAttainmentPct === ""
            ? undefined
            : Number(tierForm.maxAttainmentPct),
  ...
  const calculatePayouts = async () => {
    setBusy(true);
    try {
      const results = await apiPost<Payout[]>(
        "/crm/commission-plans/calculate-payouts",
        calcForm,
      );
      toast.success(
        "Payouts calculated",
        `${results.length} rep payout(s) computed for ${calcForm.period}`,
      );
        ...
        {(["plans", "payouts"] as const).map((t) => (
          <button
            ...
            style={{
              ...
              fontWeight:
                tab === t ? "var(--weight-semibold)" : "var(--weight-normal)",
              color:
                tab === t
                  ? "var(--color-primary)"
          ...
          plans.length === 0 ? (
            <div className="ui-empty-state">
              ...
              <div className="text-sm">
                Create a plan and add attainment-based accelerator tiers.
              </div>
            </div>
          ) : (
            <DataTable<Plan>
          ...
          <Card>
            <div className={styles.p26}>
              <div className="ui-flex-between">
                ...
                <button
                  onClick={() => setTierModalPlanId(null)}
                  className="ui-btn-icon"
                >
                  <X size={18} />
                </button>
              ...
              <Button variant="primary" onClick={addTier} disabled={busy}>
                Add Tier
              </Button>
            </div>
          </Card>
          ...
          <Card>
            <div className={styles.p211}>
              ...
              <Button
                ...
                disabled={busy || !calcForm.planId || !calcForm.period}
              >
                Calculate
              </Button>
            </div>
