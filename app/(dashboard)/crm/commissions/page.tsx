...
export default function CommissionsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const list = <T,>(value: unknown): T[] => Array.isArray(value) ? value as T[] : (typeof value === 'object' && value !== null && 'data' in value && Array.isArray(value.data) ? value.data as T[] : []);
      setRules(list<CommissionRule>(rulesRes));
      setEntries(list<CommissionEntry>(entriesRes));
      setLeaderboard(list(leaderboardRes));
      setRevopsMetrics(typeof metricsRes === 'object' && metricsRes !== null && 'data' in metricsRes ? metricsRes.data : metricsRes);
    } catch (err) {
  ...
  const handleCalculate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await client.post('/api/v1/crm/commissions/calculate', { periodStart: calcStart, periodEnd: calcEnd });
      setModalSuccess(true);
  ...
  const ruleColumns: Column<CommissionRule>[] = [
    ...
    { key: 'entries', header: 'Entries', align: 'right', render: (r) => r._count?.entries || 0 },
    {
      ...
      render: (r) => <button onClick={(e) => { e.stopPropagation(); handleDeleteRule(r); }} className={styles.p20} title="Delete"><Trash2 size={16} /></button>,
    },
  ];

  const entryColumns: Column<CommissionEntry>[] = [
    { key: 'userName', header: 'User', render: (e) => <span className="font-semibold">{e.userName || e.userId}</span> },
    ...
    { key: 'amount', header: 'Amount', align: 'right', sortable: true, render: (e) => <span className={styles.p21}>${e.amount.toLocaleString()}</span> },
    { key: 'status', header: 'Status', align: 'center', render: (e) => getStatusBadge(e.status) },
    { key: 'period', header: 'Period', render: (e) => <span className="text-xs">{e.periodStart} - {e.periodEnd}</span> },
  ];
      ...
      <PageHeader
        ...
        actions={
          <div className="ui-flex ui-gap-3">
            <Button onClick={() => setIsCalcModalOpen(true)} variant="secondary" className="ui-hstack-2">
              <Calculator size={16} />
              <span>Calculate</span>
            </Button>
            ...
            <div className={styles.p24}>
              <Card>
                <div className="p-5">
                  ...
                  <div className={styles.p26}>${revopsMetrics.totalPipelineValue?.toLocaleString() || '0'}</div>
                </div>
              </Card>
              <Card>
                <div className="p-5">
                  <div className={styles.p27}>Closed Won Amount</div>
                  <div className={styles.p28}>${revopsMetrics.closedWonValue?.toLocaleString() || '0'}</div>
                </div>
              </Card>
              <Card>
                <div className="p-5">
                  <div className={styles.p29}>Win Rate</div>
                  <div className={styles.p210}>{revopsMetrics.winRatePct?.toFixed(1) || '0'}%</div>
                </div>
              </Card>
            </div>
          )}
          ...
          <div className={styles.p216}>
            ...
            {modalSuccess ? (
              <div className={styles.p218}>
                ...
                <div className="ui-heading-base">Rule Created Successfully</div>
              </div>
            ) : (
              <form onSubmit={handleCreateRule} className="p-6 ui-stack-4">
                <div>
                  <label style={labelStyle}>Rule Name</label>
                  <input type="text" required placeholder="e.g. Standard Sales Commission" value={ruleName} onChange={e => setRuleName(e.target.value)} className="ui-input" style={inputStyle} />
                </div>
                <div className="ui-grid-2">
                  <div>
                    <label style={labelStyle}>Type</label>
                    <select value={ruleType} onChange={e => setRuleType(e.target.value as 'PERCENTAGE' | 'FLAT' | 'TIERED')} className="ui-input" style={inputStyle}>
                  ...
                  <div>
                    ...
                    <input type="number" required min={0} step={ruleType === 'PERCENTAGE' ? 0.5 : 1} value={ruleRate} onChange={e => setRuleRate(Number(e.target.value))} className="ui-input" style={inputStyle} />
                  </div>
                </div>
                <div>
                  <label style={labelStyle}>Applies To</label>
                  <select value={ruleAppliesTo} onChange={e => setRuleAppliesTo(e.target.value)} className="ui-input" style={inputStyle}>
        ...
        <div className={styles.p221}>
          <div className={styles.p222}>
            <div className={styles.p223}>
              ...
              <button onClick={() => setIsCalcModalOpen(false)} className="ui-btn-icon ui-text-muted"><X size={18} /></button>
            </div>
            {modalSuccess ? (
              <div className={styles.p224}>
                ...
                <div className="ui-text-sm-muted">Check the Earned Commissions tab for results.</div>
              </div>
            ) : (
              <form onSubmit={handleCalculate} className="p-6 ui-stack-4">
                <div>
                  <label style={labelStyle}>Period Start</label>
                  <input type="date" required value={calcStart} onChange={e => setCalcStart(e.target.value)} className="ui-input" style={inputStyle} />
                </div>
                <div>
                  <label style={labelStyle}>Period End</label>
                  <input type="date" required value={calcEnd} onChange={e => setCalcEnd(e.target.value)} className="ui-input" style={inputStyle} />
                </div>
