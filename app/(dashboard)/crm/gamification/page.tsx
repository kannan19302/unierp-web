...
export default function GamificationPage() {
  const [tab, setTab] = useState<"leaderboard" | "streaks" | "badges">(
    "leaderboard",
  );
    ...
    try {
      const [lb, st, bd] = await Promise.all([
        apiGet<LeaderboardRow[]>(
          `/crm/gamification/leaderboard?period=${encodeURIComponent(period)}`,
        ),
  ...
  const recomputeLeaderboard = async () => {
    ...
    try {
      await apiPost(
        `/crm/gamification/leaderboard/recompute?period=${encodeURIComponent(period)}`,
        {},
      );
  ...
  const recomputeStreaks = async () => {
    setBusy(true);
    try {
      const r = await apiPost<{ usersProcessed: number }>(
        "/crm/gamification/streaks/recompute",
        {},
      );
  ...
  const evaluateBadges = async () => {
    setBusy(true);
    try {
      const r = await apiPost<{ evaluated: number; awarded: number }>(
        "/crm/gamification/badges/evaluate",
        {},
      );
      toast.success(
        "Badge evaluation complete",
        `${r.awarded} new badge(s) awarded across ${r.evaluated} reps`,
      );
      await load();
    } catch (err) {
      toast.error(
          ...
          {loading ? (
            ...
            <div className="ui-empty-state">
              ...
              <div className="text-sm">
                Click "Recompute" to rank reps for this period.
              </div>
            </div>
          ) : (
        ...
        <Card>
          ...
          {loading ? (
            ...
            <div className="ui-empty-state">
              ...
              <div className="text-sm">
                Create a badge to start recognizing top performers.
              </div>
            </div>
          ) : (
            <DataTable<BadgeDef>
