...
const emptyForm = (): Omit<Segment, "id"> => ({
  ...
  criteria: {
    logic: "AND",
    rules: [{ field: "status", op: "EQUALS", value: "ACTIVE" }],
  },
});
...
export default function SegmentsPage() {
  ...
  const remove = async (id: string) => {
    ...
    try {
      await apiSend(`/crm/segments/${id}`, "DELETE");
      await load();
    } catch {
      setError("Could not delete segment.");
    }
  };
      ...
      {segments.length === 0 ? (
        <Card>
          <div className={styles.emptyState}>
            ...
            <p className="text-sm">
              No segments yet. Create a segment to group records for targeted
              campaigns.
            </p>
          </div>
        </Card>
        ...
        <div className={styles.segmentGrid}>
          {segments.map((s: any) => (
            <Card key={s.id} padding="md">
              ...
              {s.description && (
                <p className="ui-text-xs-muted m-0">{s.description}</p>
              )}
              <div className={styles.ruleSummary}>
                {s.criteria.rules.length} rule
                {s.criteria.rules.length === 1 ? "" : "s"} joined by{" "}
                {s.criteria.logic}
              </div>
            </Card>
          ))}
