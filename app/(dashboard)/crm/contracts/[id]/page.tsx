...
export default function ContractDetailPage() {
  ...
  const handleCreateSalesOrder = async () => {
    ...
    try {
      const order: any = await client.post(
        `/crm/contracts/${id}/sales-order`,
        {},
      );
      success(
        `Sales Order ${order.orderNumber} generated successfully from contract!`,
      );
  ...
  const handleRenew = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        renewalTermMonths: renewForm.renewalTermMonths
          ? Number(renewForm.renewalTermMonths)
          : undefined,
        newValue: renewForm.newValue ? Number(renewForm.newValue) : undefined,
      };
      const renewed: any = await client.post(
        `/crm/contracts/${id}/renew`,
        payload,
      );
      ...
      return (
        <div className="ui-stack-4">
          <div className="ui-alert ui-alert-danger">
            <AlertCircle size={16} />
            Failed to load contract: {fetchError}
          </div>
        </div>
      );
    }
            ...
            <div className="ui-flex ui-gap-2">
              ...
              {(contract.status === "ACTIVE" ||
                ...
                >
                  ...
                  {revising ? "Revising..." : "Revise/Amend"}
                </Button>
              )}
            ...
            <div className={styles.p20}>
              ...
              {[
                { label: "Draft", active: true },
                {
                  ...
                  active:
                    ["PENDING_APPROVAL", "APPROVED", "REJECTED"].includes(
                ...
                },
                {
                  label: "Approved",
                  active:
                    ["APPROVED"].includes(contract.approvalStatus) ||
                    contract.signatureStatus === "SIGNED" ||
                    contract.status === "ACTIVE",
                },
                {
              ...
              {activeTab === "items" && (
                <Card padding="none">
                  ...
                  <ListPageTemplate
                    columns={
                      [
                        ...
                        { key: "quantity", header: "Quantity" },
                        {
                          ...
                          header: "Unit Price",
                          render: (v: any) =>
                            `${contract.currency} ${Number(v).toLocaleString()}`,
                        },
                        {
                          key: "discount",
                          header: "Discount",
                    ...
                    data={
                      (contract.lineItems || []) as unknown as Record<
                        string,
                        unknown
                      >[]
                    }
                ...
                <Card padding="md">
                  ...
                  {contract.renewedFrom && (
                    <div className={styles.p210}>
                      <span className="ui-text-xs-tertiary">
                        Renewed From Contract
                      </span>
                      <div>
                        <a
                          onClick={() =>
                            router.push(
                              `/crm/contracts/${contract.renewedFrom!.id}`,
                            )
                          }
                          className={styles.p211}
                        >
                          {contract.renewedFrom.contractNumber} —{" "}
                          {contract.renewedFrom.title}
                        </a>
                      </div>
                    </div>
                  )}
                  {contract.revisedFrom && (
                    <div className={styles.p212}>
                      <span className="ui-text-xs-tertiary">
                        Revised From (Historical Edit)
                      </span>
                      <div>
                        <a
                          onClick={() =>
                            router.push(
                              `/crm/contracts/${contract.revisedFrom!.id}`,
                            )
                          }
                          className={styles.p213}
                        >
                          {contract.revisedFrom.contractNumber} —{" "}
                          {contract.revisedFrom.title}
                        </a>
                      </div>
                    </div>
                  )}
                  {!(contract.renewedFrom || contract.revisedFrom) && (
                    <span className={styles.p214}>
                      No revisions or renewal lineage available.
                    </span>
                  )}
                </Card>
              )}
        ...
        >
          <form onSubmit={handleRenew} className="ui-stack-4">
            <p className={styles.p219}>
              Renewing creates a new follow-on contract starting where this one
              ends, and marks this contract as RENEWED.
            </p>
              ...
              <input
                ...
                placeholder={
                  contract.renewalTermMonths
                    ? String(contract.renewalTermMonths)
                    : "12"
                }
