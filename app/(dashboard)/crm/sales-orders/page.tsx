...
import { PageHeader, Card, Button, Spinner, StatusBadge, DataTable, type Column, type SortOrder, KPICard, Badge, Modal } from "@kannan19302/ui";
import {
  ClipboardList,
  Search,
  DollarSign,
...
export default function CrmSalesOrdersPage() {
  ...
  const sortedOrders = [...filteredOrders].sort((a: any, b: any) => {
    ...
    else if (sortBy === "orderDate" || sortBy === "createdAt")
  ...
  const recordPayment = async (id: string, amount: number) => {
    try {
      await client.post(`/sales/orders/${id}/payment`, {
        amount,
        method: "CASH",
      });
      setDetailOpen(false);
    ...
    try {
      ...
      alert(
        err instanceof Error
          ? err.message
          : "Failed to convert to purchase order",
      );
    } finally {
      ...
      <div className="ui-stack-6">
          ...
          actions={
            <div className="ui-flex ui-gap-2">
              ...
              <Button variant="primary" onClick={() => {}}>
                <Plus size={14} className="mr-2" /> New Order
              </Button>
            </div>
          }
        />
        ...
        <Card padding="none">
          <DataTable
            ...
            emptyMessage="Create your first sales order."
            ...
            selected ? (
              <>
                {selected.status === "CONFIRMED" && (
                  <Button
                    variant="primary"
                    onClick={() => convertToPurchaseOrder(selected.id)}
                ...
                {selected.status === "CREDIT_HOLD" && (
                  <Button
                    variant="primary"
                    onClick={() => approveCredit(selected.id)}
                  >
                    Approve Credit Hold
                  </Button>
                )}
                {selected.paymentStatus !== "PAID" && (
                  <Button
                    variant="primary"
                    onClick={() =>
                      recordPayment(selected.id, selected.totalAmount)
                    }
                  >
                    Record Payment
                  </Button>
          ...
          {selected && (
            <div className="ui-stack-3">
              <div className="ui-grid-2 ui-gap-3">
                <div>
                  <div className="ui-text-xs-tertiary">Customer</div>
                ...
                <div>
                  <div className="ui-text-xs-tertiary">Order Date</div>
                  <div>{new Date(selected.orderDate).toLocaleDateString()}</div>
                </div>
                <div>
                  <div className="ui-text-xs-tertiary">Items</div>
                  <div>{selected.items || "—"}</div>
                </div>
              </div>
              <div className={styles.style8}>
                <div className="text-2xl">
                  {fmtCurrency(selected.totalAmount)}
                </div>
              </div>
            </div>
          )}
