...
export default function SalesCadencesPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const t = await apiGet<StepTask[]>(
        "/crm/cadences/step-tasks/mine?status=PENDING",
      );
      setTasks(Array.isArray(t) ? t : []);
    } catch (err) {
      toast.error(
        "Could not load cadence tasks",
  ...
  const handleProcessDue = async () => {
    try {
      const result = await apiPost<{
        evaluated: number;
        emailStepsAdvanced: number;
        tasksCreated: number;
      }>("/crm/cadences/process-due-steps");
      toast.success(
        "Cadence steps processed",
        `${result.emailStepsAdvanced} email steps advanced, ${result.tasksCreated} call/task touchpoints created.`,
      );
      loadData();
    } catch (err) {
      toast.error(
        "Could not process due steps",
        err instanceof ApiRequestError ? err.message : undefined,
      );
    }
  };
  ...
  const handleCompleteTask = async (
    ...
    try {
      ...
      toast.success(
        status === "COMPLETED" ? "Touchpoint completed" : "Touchpoint skipped",
      );
      loadData();
    } catch (err) {
      toast.error(
    ...
    },
    {
      ...
      render: (t: any) => (
        <div className="ui-flex-end ui-gap-2">
          <Button
            variant="secondary"
            onClick={() => handleCompleteTask(t, "SKIPPED")}
          >
            Skip
          </Button>
          <Button
            variant="primary"
            onClick={() => handleCompleteTask(t, "COMPLETED")}
      ...
      <Card>
        <div className={styles.style0}>
          ...
          <p className="ui-text-sm-muted">
            Call, task, and LinkedIn steps that need manual completion (email
            steps auto-advance).
          </p>
        </div>
        {loading ? (
          ...
          <div className="ui-empty-state">
            ...
            <div className="text-sm">
              Run "Process Due Steps" to generate new call/task/LinkedIn
              touchpoints from active enrollments.
            </div>
          </div>
        ) : (
          <DataTable<StepTask>
