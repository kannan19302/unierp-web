...
function CustomAppPageContent() {
    ...
    async function loadPage() {
      try {
        const data = await client.get(
          `/builder/page-registries/${moduleName}/${slug}`,
        );
  ...
  // ── Derived fields ──
  const parsedLayout = mapping
    ? typeof mapping.layout === "string"
      ? JSON.parse(mapping.layout)
      : mapping.layout
    : null;
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const data = await client.get<{ data?: unknown[]; total?: number }>(
        `/builder/custom-records/${schemaId}?${qp}`,
      );
  ...
  if (mapping.type === "CUSTOM") {
    return (
      <CustomLayoutRuntimeRenderer mapping={mapping} moduleName={moduleName} />
    );
  }
        ...
        <div className={styles.s6}>
          <div>
            ...
            <p className="ui-text-muted mt-1">
              {mapping.schemaRegistry?.description ||
                `Custom form for ${mapping.module} module`}
            </p>
          </div>
          <button
  ...
  return (
    <div className="p-6">
      ...
      <div className={styles.s7}>
        <div>
          <h1 className="text-2xl">{mapping.title}</h1>
          <p className="ui-text-muted mt-1">
            {mapping.schemaRegistry?.description ||
              `Custom form for ${mapping.module} module`}
          </p>
      ...
      <div className="ui-card">
        ...
        {/* Table */}
        {loadingRecords ? (
          <div className={styles.s12}>Loading records...</div>
        ) : records.length === 0 ? (
          <div className={styles.s1}>
            ...
            <button
              className="ui-btn ui-btn-primary mx-auto"
              onClick={() => {
                setEditingRecord(null);
                setView("form");
              }}
...
function CustomLayoutRuntimeRenderer({
  mapping,
  moduleName,
}: CustomLayoutRuntimeRendererProps) {
  const client = useApiClient();
      ...
      try {
        ...
        if (isMounted) {
          const filtered = data.filter(
            (s: any) => s.module.toLowerCase() === moduleName.toLowerCase(),
          );
          setSchemas(filtered);
        }
      } catch {
    ...
    <div className="p-6">
      <div className={styles.s26}>
        {layout.map((w: any) => {
              ...
              >
                <div className={styles.s28}>
                  <div>
                    ...
                    {w.config?.subtitle && (
                      <p className={styles.s30}>{w.config.subtitle}</p>
                    )}
                  </div>
                  {w.config?.badge && (
                    <span className={`builder-pill active ${styles.s31}`}>
                      {w.config.badge}
                    </span>
                  )}
                </div>
              ...
              >
                ...
                <div className="ui-grid-3">
                  {items.map((item: any, i: number) => (
                    ...
                    >
                      <div style={{ color: item.color }} className={styles.s34}>
                        {item.value || "0"}
                      </div>
          ...
          if (w.type === "form") {
            return <RuntimeFormWidget key={w.id} widget={w} />;
          }
          ...
          if (w.type === "kpi") {
            return (
              <RuntimeKpiWidget key={w.id} widget={w} moduleName={moduleName} />
            );
          }
...
function RuntimeFormWidget({ widget }: { widget: any }) {
  ...
  const handleSubmit = async (formData: Record<string, any>) => {
    ...
    try {
      ...
      showToast(
        `Submission failed: ${error instanceof Error ? error.message : "Server error"}`,
        "error",
      );
    }
  };
  ...
  if (!formId) {
    return (
      <div
        style={{ gridColumn: `span ${widget.gridSpan || 12}` }}
        className={styles.s27}
      >
...
function RuntimeTableWidget({
  ...
  useEffect(() => {
    ...
    const schema = schemas.find((s) => {
      ...
      return (
        s.id === dataModelId ||
        s.slug === dataModelId ||
        cleanSlug === dataModelSlug ||
        s.slug === `${moduleName.toLowerCase()}_${dataModelSlug}`
      );
    });
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const data = await client.get<{ data?: unknown[]; total?: number }>(
        `/builder/custom-records/${matchingSchema.id}?${qp}`,
      );
      setRecords(data.data ?? []);
      setTotal(data.total ?? 0);
    } catch {
  ...
  const fields: any[] = Array.isArray(matchingSchema.fields)
    ? matchingSchema.fields
    : [];
  const columns = fields
    .filter((f: any) => f.type !== "Section Break" && f.type !== "Column Break")
    .slice(0, 4);
...
function RuntimeKpiWidget({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await client.get<{ metrics?: Record<string, number> }>(
          `/admin/marketplace/installed/${moduleName}/metrics`,
        );
        if (mounted) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`ui-grid-3 ${styles.s53}`}
        ...
        <div className={styles.s57}>
          ...
          <div className={styles.s61}>
            <div className={styles.s62}>
              <div /> <span>Active (40%)</span>
            </div>
            <div className={styles.s63}>
              <div /> <span>Completed (30%)</span>
            </div>
            <div className={styles.s64}>
              <div /> <span>Pending (20%)</span>
            </div>
            <div className={styles.s62}>
              <div /> <span>Draft (10%)</span>
            </div>
          </div>
        </div>
      ) : chartType === "line" ? (
...
function RemoteAppPageRenderer({
  mapping,
  moduleName,
}: {
  mapping: any;
  ...
  const columns: { key: string; label?: string }[] = Array.isArray(
    layout.columns,
  )
    ? layout.columns
    : [];
    ...
    (async () => {
      try {
        const data = await client.get<unknown>(
          `/ext/${moduleName}/${endpoint.replace(/^\//, "")}`,
        );
        if (!mounted) return;
        const response = data as { data?: unknown[]; items?: unknown[] };
        setRows(
          Array.isArray(data) ? data : (response.data ?? response.items ?? []),
        );
        setState("ok");
      } catch {
        if (mounted) {
          setState("error");
          setMessage("Network error contacting the app service.");
        }
      }
    })();
    return () => {
  ...
  const cols: { key: string; label?: string }[] = columns.length
    ? columns
    : rows[0]
      ? Object.keys(rows[0])
