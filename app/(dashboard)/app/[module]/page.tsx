...
export default function AppShellPage() {
  ...
  const view: "home" | "admin" =
    searchParams.get("view") === "admin" ? "admin" : "home";
  ...
  const load = useCallback(async () => {
    try {
      setShell(
        await client.get<AppShell>(
          `/admin/marketplace/installed/${moduleSlug}/modules`,
        ),
      );
    } catch {
      setNotFound(true);
  ...
  const toggle = async (slug: string, enabled: boolean) => {
    ...
    try {
      await client.request(
        `/admin/marketplace/installed/${moduleSlug}/modules/${slug}`,
  ...
  if (loading)
    return (
      <div className={styles.s1}>
        <Loader2 size={32} className={styles.s28} />
      </div>
    );
      ...
      <div className={styles.s4}>
        ...
        <div className={styles.s5}>
          <div className={styles.s6}>
            <Link href="/apps" className={styles.s30}>
              <ArrowLeft size={18} />
            </Link>
            ...
            <div>
              ...
              <p className={styles.s9}>
                {shell.app.description || "Industry application"}{" "}
                {shell.app.version && (
          ...
          <div className={styles.s10}>
            {shell.app.slug === "healthcare" && (
              <Link
                href={`/app/${moduleSlug}/clinical`}
                style={{ ...tabBtn(false) }}
                className={styles.s31}
              >
                <Stethoscope size={15} /> Clinical Tools
              </Link>
            )}
            {view === "admin" ? (
              <Link
              ...
              <Link
                href={`/app/${moduleSlug}?view=admin`}
                style={{ ...tabBtn(false) }}
                className={styles.s31}
              >
          ...
          <>
            {enabledModules.length === 0 ? (
              <div className={styles.s11}>
                <p className="ui-text-muted">
                  All modules are disabled. Enable one in the{" "}
                  <Link
                    href={`/app/${moduleSlug}?view=admin`}
              ...
              <>
                <div className={styles.s12}>
                  ...
                  Use the sidebar to navigate this app’s modules and pages.{" "}
                  ...
                  {enabledModules.map((m) => {
                    ...
                    const inner = (
                      <>
                        ...
                        {first && (
                          <ChevronRight size={14} className={styles.s29} />
                        )}
                      </>
                    );
                    ...
                    return first ? (
                      <Link
                        key={m.slug}
                        href={`/app/${moduleSlug}/${first.slug}`}
                        style={cardStyle}
                      >
                        {inner}
                      </Link>
                    ) : (
                      <div key={m.slug} style={cardStyle}>
                        {inner}
                      </div>
                    );
                  })}
          ...
          <div className={styles.s17}>
            <div className={styles.s18}>
              <div className={styles.s19}>
                <Settings size={16} /> Module Management
              </div>
              <p className={styles.s20}>
                Enable or disable functionality for this app. Disabled modules
                are hidden from the sidebar; data is preserved and returns when
                re-enabled.
              </p>
            </div>
            {shell.modules.map((m) => (
              <div key={m.slug} className={styles.s21}>
                ...
                <div className="flex-1">
                  ...
                  {m.description && (
                    <div className={styles.s23}>{m.description}</div>
                  )}
