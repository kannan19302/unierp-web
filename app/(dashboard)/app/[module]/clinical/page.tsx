...
const SIDE_INFO: Record<TabId, { title: string; body: string }> = {
  eligibility: {
    ...
    body: "Checks the patient’s active coverage on file and returns payer, member ID, copay and deductible — the same data an X12 271 response would carry.",
  },
  scrubber: {
    title: "Claim Scrubbing",
    body: "Rule-based edits validate diagnosis (ICD-10), procedure (CPT), payer and charge amount before submission, flagging errors that would cause denials.",
  },
  interactions: {
    title: "Drug Interactions",
    body: "Cross-checks a medication list (plus the patient’s active meds) against the interaction reference set, surfacing major/moderate conflicts.",
  },
  cds: {
    title: "Clinical Decision Support",
    body: "Evaluates allergies, active problems and critical labs against a proposed order to raise real-time safety and care-gap alerts.",
  },
  quality: {
    title: "Quality Measures",
    body: "HEDIS/MIPS-style measures (HbA1c testing & control, immunization rates) computed live from the tenant’s clinical records.",
  },
  fhir: {
    title: "FHIR R4 Interop",
    body: "Read-only FHIR projections expose Patient and Observation resources as searchset Bundles for downstream interoperability.",
  },
};
    ...
    return (
      <div className={styles.s1}>
        ...
        <p className="ui-text-muted">
          This workspace is only available for the Healthcare app.
        </p>
        <Link href={`/app/${moduleSlug}`} className="ui-text-primary">
          ← Back to app
        </Link>
      </div>
    );
      ...
      <div className={styles.s3}>
        ...
        <div className={styles.s4}>
          <div className={styles.s5}>
            <Link href={`/app/${moduleSlug}`} className={styles.s34}>
              <ArrowLeft size={18} />
            </Link>
            <div>
              ...
              <p className={styles.s7}>
                Smart clinical &amp; revenue-cycle services — eligibility, claim
                scrubbing, drug-interaction &amp; decision support, quality
                measures and FHIR.
              </p>
            </div>
          </div>
...
function RunButton({
  ...
  return (
    ...
    >
      {loading ? (
        <Loader2 size={16} className={styles.s33} />
      ) : (
        <Search size={16} />
      )}{" "}
      {children}
...
function severityColor(sev: string): string {
  ...
  if (s === "error" || s === "high" || s === "major" || s === "critical")
    return "var(--color-danger)";
  if (s === "warning" || s === "moderate" || s === "warn")
...
function useApi() {
  ...
  const call = useCallback(
    async (method: "GET" | "POST", path: string, body?: any) => {
      ...
      setResult(null);
      try {
        const data = await client.request<unknown>(`/ext/healthcare${path}`, {
          method,
...
function EligibilityTool() {
  ...
  return (
    <div className="ui-stack-4">
      ...
      <RunButton
        loading={loading}
        onClick={() => call("POST", "/eligibility/check", { patient_mrn: mrn })}
      >
        Check Eligibility
      </RunButton>
        ...
        <div className="ui-stack-3">
          ...
          {result.eligible && (
            <div className={styles.s17}>
              ...
              <Stat
                label="Deductible"
                value={`$${result.plan?.deductible ?? 0}`}
              />
            </div>
          )}
        </div>
...
function ScrubberTool() {
  ...
  return (
    <div className="ui-stack-4">
      ...
      <RunButton
        loading={loading}
        onClick={() => call("POST", "/claims/scrub", form)}
      >
        Scrub Claim
      </RunButton>
      <ErrorNote error={error} />
        ...
        <div className="ui-stack-3">
          ...
          >
            ...
            {result.clean
              ...
              : `${result.edits.length} edit(s) found`}
          </div>
...
function InteractionsTool() {
  ...
  return (
    <div className="ui-stack-4">
      <Field label="Patient MRN (optional — pulls active meds)">
        <input
          style={inputStyle}
          value={mrn}
          onChange={(e: any) => setMrn(e.target.value)}
      ...
      {result && (
        <div className="ui-stack-3">
          <div className="ui-text-xs-muted">
            Evaluated: {(result.meds || []).join(", ") || "—"}
          </div>
          {result.interactions.length === 0 ? (
            <div className={styles.s22}>
              <CheckCircle2 size={18} /> No interactions detected
            </div>
          ) : (
  ...
  return (
    <div className="ui-stack-4">
      ...
      <RunButton
        loading={loading}
        onClick={() =>
          call("POST", "/cds/evaluate", {
            patient_mrn: mrn,
            order_drug: drug || undefined,
          })
        }
      >
        Evaluate
      ...
      {result && (
        <div className="ui-stack-3">
          {result.alerts.length === 0 ? (
            <div className={styles.s22}>
              <CheckCircle2 size={18} /> No alerts
            </div>
          ) : (
...
function QualityTool() {
  ...
  return (
    <div className="ui-stack-4">
      <RunButton
        loading={loading}
        onClick={() => call("GET", "/quality/measures")}
      >
        Compute Measures
      </RunButton>
      <ErrorNote error={error} />
...
function FhirTool() {
  const [mrn, setMrn] = useState("MRN1002");
  const [resource, setResource] = useState<"Patient" | "Observation">(
    "Patient",
  );
  const { loading, result, error, call } = useApi();
  return (
    <div className="ui-stack-4">
      ...
      <RunButton
        ...
        onClick={() =>
          call(
            "GET",
            `/fhir/${resource}${mrn ? `?mrn=${encodeURIComponent(mrn)}` : ""}`,
          )
        }
      >
        Fetch Bundle
      ...
      {result && (
        <>
          <div className="ui-text-xs-muted">
            {result.resourceType} · {result.type} · {result.total} entr
            {result.total === 1 ? "y" : "ies"}
          </div>
          <pre className={styles.s32}>{JSON.stringify(result, null, 2)}</pre>
        </>
      )}
    </div>
  );
