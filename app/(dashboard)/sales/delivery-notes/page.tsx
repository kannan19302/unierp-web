...
export default function DeliveryNotesPage() {
  ...
  const [noteDetails, setNoteDetails] = useState<DeliveryNoteDetail | null>(
    null,
  );
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const data = await client.get<DeliveryNote[] | { data?: DeliveryNote[] }>(
  ...
  const loadNoteDetails = async (note: DeliveryNote) => {
    ...
    try {
      setNoteDetails(
        await client.get<DeliveryNoteDetail>(
          `/sales/delivery-notes/${note.id}`,
        ),
      );
    } catch {
      setNoteDetails({
  ...
  const filteredNotes = deliveryNotes.filter((dn: any) => {
    const matchesSearch =
      ...
      (dn.carrierName &&
        dn.carrierName.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <RouteGuard permission="sales.delivery-note.read">
      <div className="ui-stack-6 ui-animate-in">
        ...
        <div
          style={{ gridTemplateColumns: selectedNote ? "1.5fr 1fr" : "1fr" }}
        >
          <Card padding="none">
            <ListPageTemplate
              title=""
            ...
            <Card padding="none" className={styles.p10}>
              <div className={styles.p11}>
                <h4 className={styles.p12}>
                  Delivery Note {selectedNote.deliveryNumber}
                </h4>
                <button
                  ...
                  <div className="p-5 ui-stack-4">
                    <div className="ui-flex-between">
                      ...
                      <span className="ui-heading-sm">
                        {noteDetails.salesOrder.orderNumber}
                      </span>
                    </div>
                    ...
                    <div className="ui-flex-between">
                      ...
                      <span className={styles.p13}>
                        <Truck size={14} />{" "}
                        {noteDetails.carrierName || "Self-Pickup"}
                      </span>
                    </div>
                    ...
                    <div className="ui-flex-between">
                      ...
                      <Badge
                        variant={
                          noteDetails.status === "DELIVERED"
                            ? "success"
                            : noteDetails.status === "IN_TRANSIT"
                              ? "info"
                              : "default"
                        }
                      >
                    ...
                    <div className={styles.p16}>
                      ...
                      <div className={styles.p18}>
                        {noteDetails.lineItems.map(
                          (li: DeliveryNoteDetailItem) => (
                            <div key={li.id} className={styles.p19}>
                              <span>{li.description}</span>
                              <span className="font-bold">
                                Qty {Number(li.deliveredQty)}
                              </span>
                            </div>
                          ),
                        )}
