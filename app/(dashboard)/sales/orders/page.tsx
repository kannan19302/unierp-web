...
export default function SalesOrdersHub() {
  ...
  const [activeChannel, setActiveChannel] = useState<
    "ALL" | "B2B" | "B2C" | "D2C" | "ONLINE"
  >("ALL");
  const [statusFilter, setStatusFilter] = useState("ALL");

  // Detail View Drawer
  const [selectedOrder, setSelectedOrder] = useState<SalesOrder | null>(null);
  const [orderDetails, setOrderDetails] = useState<SalesOrderDetail | null>(
    null,
  );
  ...
  const [salesChannel, setSalesChannel] = useState<"B2B" | "B2C" | "D2C">(
    "B2B",
  );
  ...
  const loadOrderDetails = async (order: SalesOrder) => {
    ...
    try {
      setOrderDetails(
        await client.get<SalesOrderDetail>(`/sales/orders/${order.id}`),
      );
    } catch {
      setOrderDetails({
        id: order.id,
        ...
        lineItems: [
          {
            id: "li-1",
            description: "Heavy Office Furniture Supply",
            quantity: 1,
            unitPrice: order.totalAmount / 1.1,
            taxRate: 10,
            totalAmount: order.totalAmount,
          },
        ],
  ...
  const handleRecordPayment = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await client.post(`/sales/orders/${selectedOrder.id}/payment`, {
  ...
  const filteredOrders = orders.filter((o) => {
    ...
    const matchesChannel =
      activeChannel === "ALL" || o.salesChannel === activeChannel;
  ...
  return (
    <RouteGuard permission="sales.order.read">
      <div className="ui-stack-6 ui-animate-in">
        ...
        {/* Grid listing */}
        <div
          style={{ gridTemplateColumns: selectedOrder ? "1.5fr 1fr" : "1fr" }}
        >
          <ListPageTemplate
            title=""
            columns={
                  ...
                  <div className="p-5 ui-stack-4">
                          ...
                          (li: SalesOrderDetailItem) => (
                            <div key={li.id} className={styles.p18}>
                              ...
                              <span className="font-bold">
                                ${Number(li.totalAmount).toLocaleString()}
                              </span>
                            </div>
                          ),
                        )}
                    ...
                    <div className={styles.p20}>
                      ...
                      <div className={styles.p22}>
                        ...
                        <span>
                          ${Number(orderDetails.taxAmount).toLocaleString()}
                        </span>
                      </div>
                    ...
                    <div className={styles.p24}>
                      {orderDetails.status === "CREDIT_HOLD" && (
                        <Button
                          variant="primary"
                          onClick={() => approveCreditHold(orderDetails.id)}
                      ...
                      {orderDetails.paymentStatus !== "PAID" && (
                        <Button
                          variant="outline"
                          onClick={() => {
                            setPaymentAmount(Number(orderDetails.totalAmount));
                      ...
                      {orderDetails.status === "CONFIRMED" && (
                        <Button
                          ...
                          onClick={() => {
                            setDeliveryNumber(
                              `DEL-${orderDetails.orderNumber.replace("SO-", "")}`,
                            );
                ...
                {modalSuccess ? (
                  <div className={styles.p33}>
                    ...
                    <p className="ui-text-sm-muted">
                      The order registry has been synced.
                    </p>
                  </div>
                ) : (
                  <>
          ...
          <div className={styles.p44}>
            <div className={styles.p45}>
              <div className={styles.p46}>
                <h3 className={styles.p47}>
                  Record Payment: {selectedOrder.orderNumber}
                </h3>
                <button
                  ...
                  <>
                    ...
                    <div className="ui-flex-end ui-gap-2 mt-2">
                      ...
                      <Button
                        ...
                        disabled={submitting}
                      >
                        {submitting ? <Spinner size="sm" /> : "Log Payment"}
                      </Button>
                    </div>
                  </>
                )}
                  ...
                  <>
                    ...
                    <div className="ui-flex-end ui-gap-2 mt-2">
                      ...
                      <Button
                        ...
                        disabled={submitting}
                      >
                        {submitting ? <Spinner size="sm" /> : "Log Ship Out"}
                      </Button>
                    </div>
                  </>
                )}
