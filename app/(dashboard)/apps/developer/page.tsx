...
interface PendingBundle extends Bundle {
  package: { name: string; slug: string; vendor: { name: string } };
}
...
export default function DeveloperPortalPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const message =
        e instanceof Error ? e.message : "Failed to load developer portal data";
  ...
  const createApp = async () => {
    ...
    try {
      ...
      setShowCreate(false);
      setNewApp({ name: "", category: "Healthcare", description: "" });
      await load();
    } catch {
  ...
  const createBundle = async () => {
    ...
    try {
      ...
      showToast("Manifest is not valid JSON", "error");
      return;
    }
    setBusy(true);
    try {
      const data = await client.post<Bundle>(
        `/developer/apps/${bundleFor.id}/bundles`,
        { manifest, changelog },
      );
      ...
      showToast(
        typeof e?.message === "string" ? e.message : "Failed to create version",
        "error",
      );
    } finally {
      setBusy(false);
  ...
  const submitBundle = async (bundleId: string) => {
    setBusy(true);
    try {
      await client.request(`/developer/bundles/${bundleId}/submit`, {
        method: "PUT",
      });
  ...
  const review = async (bundleId: string, action: "approve" | "reject") => {
    ...
    try {
      const body =
        action === "reject"
          ? JSON.stringify({ reviewNotes: "Needs changes" })
          : undefined;
      await client.request(`/developer/review/${bundleId}/${action}`, {
        method: "PUT",
        body,
      });
  ...
  return (
    <RouteGuard permission="apps.developer.read">
      <div className="ui-stack-5">
        {toast && (
          <div
            className={`ui-alert ${styles.toast} ${toast.type === "success" ? "ui-alert-success" : "ui-alert-danger"}`}
          >
            {toast.msg}
          </div>
        )}
        ...
        {vendor && (
          <p className="ui-text-sm-muted">
            Publishing as <strong>{vendor.name}</strong>{" "}
            {vendor.verified && (
              <Shield size={12} className="ui-text-success" />
            )}{" "}
            · vendor slug <code>{vendor.slug}</code>
          </p>
        )}
        {loadError && (
                  ...
                  <div className={styles.bundleList}>
                    {app.bundles.map((b) => (
                      <div key={b.id} className={styles.bundleRow}>
                        ...
                        {b.reviewNotes && (
                          <span className={styles.reviewNotes}>
                            — {b.reviewNotes}
                          </span>
                        )}
                    ...
                    {app.bundles.length === 0 && (
                      <span className={styles.appMeta}>No versions yet.</span>
                    )}
                  </div>
                </div>
              ))}
              ...
              {pending.map((b) => (
                <div key={b.id} className={styles.reviewCard}>
                  <div>
                    <div className={styles.appName}>
                      {b.package.name}{" "}
                      <span className="ui-text-tertiary font-normal">
                        v{b.version}
                      </span>
                    </div>
                    <div className={styles.appMeta}>
                      by {b.package.vendor.name} · /{b.package.slug}
                    </div>
                    {b.changelog && (
                      <div className={styles.reviewChangelog}>
                        {b.changelog}
                      </div>
                    )}
                  </div>
          ...
          <Modal
            ...
            footer={
              <>
                ...
                <Button variant="primary" onClick={createApp} disabled={busy}>
        ...
        {bundleFor && (
          <Modal
            ...
            title={`New Version — ${bundleFor.name}`}
            size="lg"
            footer={
              <>
                <Button variant="secondary" onClick={() => setBundleFor(null)}>
                  Cancel
                </Button>
                <Button
                  variant="primary"
                  ...
                  disabled={busy}
                >
                  {busy ? "Saving…" : "Create Draft Version"}
                </Button>
              </>
            }
          >
            <p className="ui-text-sm-muted">
              Define the app manifest (schemas + pages). The <code>slug</code>{" "}
              and <code>vendor</code> are pinned to your app automatically.
            </p>
            <textarea
              value={manifestText}
