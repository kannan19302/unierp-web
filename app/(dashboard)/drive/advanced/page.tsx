...
export default function DocumentsAdvancedPage() {
  ...
  const handleSearch = async () => {
    ...
    setIsSearching(true);
    try {
      const data = await client.get<{ documents?: Array<Record<string, unknown>> }>(`/drive/search?q=${encodeURIComponent(searchQuery)}`);
  ...
  return (
    ...
    <div className="ui-stack-6">
      ...
      <div className={styles.p1}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ borderBottom: activeTab === t.id ? '2px solid var(--color-primary)' : '2px solid transparent', color: activeTab === t.id ? 'var(--color-primary)' : 'var(--color-text-secondary)' }} className={styles.s1}>
            {t.icon} {t.label}
          </button>
        ))}
      </div>
        ...
        <div className="ui-stack-4">
          <div className="ui-flex ui-gap-2">
            <input
              ...
              className={styles.p2}
            />
            <button onClick={handleSearch} disabled={isSearching} className={styles.p3}>
          ...
          {searchResults.length > 0 && (
            <div className="ui-stack-3">
              <span className="ui-text-xs-muted">{searchResults.length} results found</span>
              {searchResults.map(r => (
                <div key={r.id} className={styles.p4}>
                  <div className="ui-flex-between">
                    <div className="ui-hstack-2">
                      <FileText size={18} className="ui-text-primary" />
                      <span className="ui-heading-sm font-bold">{r.name}</span>
                  ...
                  <div className={styles.p8}>
                    ...
                    {r.tags.map(tag => (
                      <span key={tag} className={styles.p10}>#{tag}</span>
                    ))}
                  </div>
                </div>
              ))}
            ...
            {(['PENDING_REVIEW', 'APPROVED', 'REJECTED', 'PUBLISHED'] as const).map(status => {
              ...
              return (
                <div key={status} className={styles.p14}>
                  ...
                  <div className="ui-text-xs-muted">{status.replace('_', ' ')}</div>
                </div>
              );
            })}
          </div>
                  ...
                  <tr key={a.id} className="border-b">
                    <td className="py-3 px-4">
                      <div className="ui-hstack-2">
                        ...
                        <div>
                          <div className="font-semibold">{a.documentName}</div>
                          <div className="ui-text-micro">Submitted {a.submittedAt}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <span className={styles.p26}>{a.status.replace('_', ' ')}</span>
                    </td>
                    <td className="py-3 px-4">
              ...
              <div key={p.id} className={styles.p33}>
                ...
                <div className={styles.p35}>
                  <div>
                    ...
                    <div className="ui-heading-sm">{Math.round(p.retentionDays / 365)} years</div>
                  </div>
                  <div>
                ...
                <div className="ui-flex-between">
                  <span style={{ background: p.autoDispose ? 'var(--color-warning-light)' : 'var(--color-bg)', color: p.autoDispose ? 'var(--color-warning)' : 'var(--color-text-secondary)' }} className={styles.s2}>
                    {p.autoDispose ? 'Auto-Dispose' : 'Manual Review'}
                  </span>
                  <div className="ui-flex ui-gap-1">
                    <button className="ui-btn-icon ui-text-muted"><Filter size={14} /></button>
                    <button className={styles.p36}><Trash2 size={14} /></button>
                  </div>
                </div>
              </div>
            ))}
            ...
            {[
              ...
              <div key={cat.label} className={styles.p37}>
                ...
                <div>
                  ...
                  <div className="ui-text-micro ui-text-muted">{cat.count} documents</div>
                </div>
              </div>
            ))}
          </div>
              ...
              {[
                ...
                <div key={i} className={styles.p40}>
                    ...
                    <div>
                      ...
                      <div className={styles.p41}>
                        {doc.autoTagged.map(t => (
                          <span key={t} className={styles.p42}>#{t}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                  <div className="ui-hstack-3">
                    ...
                    <span style={{ color: doc.confidence >= 90 ? 'var(--color-success)' : 'var(--color-warning)' }} className={styles.s3}>{doc.confidence}%</span>
                  </div>
                </div>
              ))}
            </div>
