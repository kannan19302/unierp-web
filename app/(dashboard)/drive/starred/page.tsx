...
interface StarredItem { id: string; name: string; type: string; updatedAt: string; isStarred: boolean; }
...
export default function StarredPage() {
  ...
  const columns: Column<StarredItem>[] = [
    ...
    { key: 'actions', header: 'Actions', render: () => (
      <div className="ui-hstack-2">
        ...
        <Button variant="ghost" size="sm" icon={Star} title="Unstar" className="ui-text-warning" />
      </div>
    )},
  ];
  ...
  return (
    <div className="ui-stack-6">
      <PageHeader title="Starred" description="Your starred files and folders" breadcrumbs={[{ label: 'Apps', href: '/apps' }, { label: 'Drive', href: '/drive' }, { label: 'Starred' }]} icon={Star} />
      <Card padding="none">
        <DataTable columns={columns} data={items} rowKey={(r) => r.id} emptyTitle="No starred items" emptyMessage="Star files and folders to quickly find them here." emptyIcon={<Star size={48} />} />
      </Card>
    </div>
  );
}
