    ...
    const loadQuotas = async () => {
      try {
        const data = await client.get<
          StorageTenant[] | { data?: StorageTenant[] }
        >("/storage/quotas");
      ...
      <div className="ui-stack-6">
        <div>
          ...
          <p className="ui-text-sm-muted">
            Monitor per-tenant storage usage, quotas, and file counts across the
            platform.
          </p>
        </div>
        ...
        <div className="ui-grid-auto">
          {[
            {
              label: "Total Used",
              value: `${(totalUsed / 1000).toFixed(1)} GB`,
              color: "var(--color-primary)",
            },
            {
              label: "Total Quota",
              value: `${(totalQuota / 1000).toFixed(0)} GB`,
            ...
            return (
              <div key={t.id} className={styles.p5}>
                ...
                <div className={styles.p8}>
                  <div>
                    <span className="ui-text-tertiary">Used:</span>{" "}
                    <strong>{(t.usedMB / 1000).toFixed(1)} GB</strong>
                  </div>
                  <div>
                    <span className="ui-text-tertiary">Quota:</span>{" "}
                    <strong>{(t.quotaMB / 1000).toFixed(0)} GB</strong>
                  </div>
                  <div>
                    <span className="ui-text-tertiary">Files:</span>{" "}
                    <strong>{t.filesCount.toLocaleString()}</strong>
                  </div>
                  <div>
                    <span className="ui-text-tertiary">Available:</span>{" "}
                    <strong>
                      {((t.quotaMB - t.usedMB) / 1000).toFixed(1)} GB
                    </strong>
                  </div>
                </div>
              </div>
