  ...
  return (
    <div className={styles.s1}>
      ...
      <div className={styles.s2}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ borderBottom: activeTab === t.id ? '2px solid var(--color-primary)' : '2px solid transparent', color: activeTab === t.id ? 'var(--color-primary)' : 'var(--color-text-secondary)' }} className={styles.s3}>
            {t.icon} {t.label}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'threads' && (
        <div className={styles.s4}>
          {messages.map(msg => (
            <div key={msg.id} style={{ borderLeft: msg.isPinned ? '3px solid var(--color-primary)' : 'none' }} className={styles.s5}>
              <div className={styles.s6}>
                <div className="ui-hstack-2">
                  ...
                  <div>
                    <span className="ui-heading-sm font-bold">{msg.author}</span>
                    <span className={styles.s8}>{msg.timestamp}</span>
                  </div>
                </div>
                <div className="ui-flex ui-gap-1">
                  <button onClick={() => togglePin(msg.id)} style={{ color: msg.isPinned ? 'var(--color-primary)' : 'var(--color-text-tertiary)' }} className={styles.s9} title="Pin">
                    <Pin size={14} />
                  </button>
                  <button onClick={() => convertToTask(msg.content)} className={styles.s10} title="Convert to Task">
                    <CheckSquare size={14} />
                  </button>
                  <button onClick={() => setReplyingTo(replyingTo === msg.id ? null : msg.id)} className={styles.s10} title="Reply">
                    <Reply size={14} />
                  </button>
                </div>
              </div>
              ...
              {/* Reactions */}
              <div style={{ marginBottom: msg.replies.length > 0 ? 'var(--space-3)' : 0 }} className={styles.s12}>
                {msg.reactions.map(r => (
                  <button key={r.emoji} onClick={() => addReaction(msg.id, r.emoji)} style={{ background: r.reacted ? 'var(--color-primary-light)' : 'var(--color-bg)', border: r.reacted ? '1px solid var(--color-primary)' : '1px solid var(--color-border)' }} className={styles.s13}>
                    {r.emoji} <span className="ui-text-micro ui-text-muted">{r.count}</span>
                  </button>
                ))}
                <button onClick={() => addReaction(msg.id, '👍')} className={styles.s14}>
                  <Smile size={12} />
                </button>
              </div>
              ...
              {msg.replies.length > 0 && (
                <div className={styles.s15}>
                  <span className={styles.s16}>{msg.replies.length} {msg.replies.length === 1 ? 'reply' : 'replies'}</span>
                  {msg.replies.map(r => (
              ...
              {replyingTo === msg.id && (
                <div className={styles.s23}>
                  <input value={replyText} onChange={e => setReplyText(e.target.value)} onKeyDown={e => e.key === 'Enter' && addReply(msg.id)} placeholder="Write a reply..." className={styles.s24} />
                  <button onClick={() => addReply(msg.id)} className={styles.s25}>
                    <Send size={14} />
                  </button>
                </div>
              )}
            ...
            {(['OPEN', 'ASSIGNED', 'RESOLVED'] as const).map(status => {
              ...
              return (
                <div key={status} className={styles.s28}>
                  <div style={{ color: colors[status] }} className={styles.s29}>{count}</div>
                  ...
                  <tr key={item.id} className="border-b">
                    <td className="py-3 px-4">
                      <span className={styles.s34}>
                        <AtSign size={10} className={styles.s35} />{item.channel}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className={styles.s36}>
                      {item.assignedTo || <span className={styles.s37}>Unassigned</span>}
                    </td>
                    <td className="py-3 px-4">
                      <div style={{ color: slaColor(item.slaDeadline) }} className={styles.s38}>
                        <Clock size={12} />
                        {new Date(item.slaDeadline).toLocaleDateString()}
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <span style={{ color: item.status === 'OPEN' ? 'var(--color-error)' : item.status === 'ASSIGNED' ? 'var(--color-warning)' : 'var(--color-success)', background: item.status === 'OPEN' ? 'var(--color-error-light)' : item.status === 'ASSIGNED' ? 'var(--color-warning-light)' : 'var(--color-success-light)' }} className={styles.s39}>{item.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {activeTab === 'outbound' && (
        <div className={styles.s26}>
          <div className="ui-grid-2">
              ...
              {[
                ...
                <div key={i} className={styles.s40}>
                  ...
                  <div className={styles.s43}>
                    {t.vars.map(v => (
                      <span key={v} className={styles.s44}>{v}</span>
                    ))}
                  </div>
                </div>
              ))}
            ...
            {/* WhatsApp Business */}
            <div className="ui-card p-4">
              <h3 className="ui-section-header">💬 WhatsApp Business Templates</h3>
              {[
                ...
                <div key={i} className={styles.s45}>
                  ...
                  <span style={{ color: t.status === 'APPROVED' ? 'var(--color-success)' : t.status === 'PENDING' ? 'var(--color-warning)' : 'var(--color-error)', background: t.status === 'APPROVED' ? 'var(--color-success-light)' : t.status === 'PENDING' ? 'var(--color-warning-light)' : 'var(--color-error-light)' }} className={styles.s46}>{t.status}</span>
                </div>
              ))}
            </div>
          </div>
