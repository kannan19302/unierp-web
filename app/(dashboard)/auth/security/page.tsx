...
import { PageHeader, Button, DataTable, Card, toast } from "@unerp/ui";
...
export default function SecurityPage() {
  ...
  const updatePolicy = async (key: string, value: any) => {
    ...
    if (res.ok) { setPolicy(await res.json()); toast.success("Policy updated"); }
  ...
  const addIpEntry = async () => {
    ...
    if (res.ok) { setNewIp(""); await fetchAll(); toast.success("IP range added"); }
    else { const d = await res.json(); toast.error(d.message ?? "Failed to add IP range"); }
  };

  const deleteIpEntry = async (id: string) => {
    const res = await fetch(`/api/v1/auth/ip-allowlist/${id}`, { method: "DELETE" });
    if (res.ok) { await fetchAll(); toast.success("Entry removed"); }
  };

  const columns: Column<IpAllowlistEntry>[] = [
    { id: "ipRange", header: "IP Range", render: (r) => <code>{r.ipRange}</code> },
    { id: "description", header: "Description", render: (r) => r.description ?? "-" },
    { id: "isActive", header: "Active", render: (r) => r.isActive ? <span className="ui-badge ui-badge-success">Yes</span> : <span className="ui-badge ui-badge-muted">No</span> },
    { id: "actions", header: "", render: (r) => <Button size="sm" variant="ghost" leftIcon={<Trash2 size={14} />} onClick={(e) => { e.stopPropagation(); deleteIpEntry(r.id); }}>Remove</Button> },
  ];

  return (
    <RouteGuard permission="auth.password-policy.read">
      <div className="ui-stack-6">
        <PageHeader title="Security Settings" description="Password policies and IP allowlisting." icon={Shield} breadcrumbs={[{ label: "Apps", href: "/apps" }, { label: "Auth", href: "/auth" }, { label: "Security" }]} />
