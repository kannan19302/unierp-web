  ...
  const loadData = async () => {
    ...
    try {
      const [receiptRes, poRes, whRes] = await Promise.all([
        client.get<PurchaseReceipt[] | { data?: PurchaseReceipt[] }>(
          "/procurement/purchase-receipts",
        ),
        client.get<PurchaseOrder[]>("/procurement/purchase-orders"),
        client.get<Warehouse[] | { data?: Warehouse[] }>(
          "/inventory/warehouses",
        ),
      ]);

      setReceipts(
        Array.isArray(receiptRes) ? receiptRes : receiptRes.data || [],
      );
      setPos(
        poRes.filter(
      ...
      render: (_v: any, row: any) => {
        ...
        return (
          <span className="font-mono">
            {r.purchaseOrder?.poNumber || "Direct GRN"}
          </span>
        );
      },
    },
