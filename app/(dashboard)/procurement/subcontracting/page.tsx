...
import { PageHeader, Button, Badge, Spinner, Modal, DataTable } from "@kannan19302/ui";
...
export default function SubcontractingPage() {
  ...
  const load = () => {
    setLoading(true);
    client
      .get("/procurement/expansion/subcontracting")
      .then((res: any) => {
        setOrders(res.data || []);
        setLoading(false);
      })
  ...
  const columns = [
    ...
    { key: "quantity", header: "Qty" },
    {
      key: "totalCost",
      header: "Total Cost",
      render: (row: any) => `$${Number(row.totalCost).toLocaleString()}`,
    },
    {
      key: "deliveryDate",
      header: "Delivery",
  ...
  return (
    <RouteGuard permission="procurement.purchase-order.read">
      <PageHeader
        ...
        description="Manage subcontracting orders and material shipments to vendors"
        actions={
