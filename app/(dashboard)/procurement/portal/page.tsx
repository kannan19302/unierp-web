...
export default function SupplierPortalPage() {
  ...
  const loadPortalData = useCallback(
    async (vendor: Vendor, t: Tab) => {
      setLoading(true);
      setError(null);
      try {
        if (t === "users") {
          const d = await client.get<PortalUser[]>(
            `/procurement/vendors/${vendor.id}/portal-users`,
          );
          setPortalUsers(Array.isArray(d) ? d : []);
        } else if (t === "pos") {
          const d = await client.get<PurchaseOrder[]>(
            `/procurement/vendors/${vendor.id}/portal/purchase-orders`,
          );
          setPurchaseOrders(Array.isArray(d) ? d : []);
        } else {
          const d = await client.get<Rfq[]>(
            `/procurement/vendors/${vendor.id}/portal/rfqs`,
          );
  ...
  const invite = async () => {
    ...
    setError(null);
    try {
      const res = await client.post<{ email: string; tempPassword: string }>(
        `/procurement/vendors/${selectedVendor.id}/portal-users`,
      ...
      <div className="p-8 ui-stack-6">
        ...
        <div>
          ...
          <p className="ui-text-muted mt-1">
            Manage vendor self-service access — invite portal users, view their
            POs and open RFQs.
          </p>
        </div>
                  ...
                  {vendors.length === 0 ? (
                    ...
                    vendors.map((v) => (
                      ...
                      >
                        {v.name}
                        {v.email && (
                          <span className={styles.p7}>({v.email})</span>
                        )}
                      </button>
                    ))
                  )}
        ...
        {selectedVendor && (
          <>
            ...
            <div className={styles.p8}>
              ...
              <button
                onClick={() =>
                  selectedVendor && loadPortalData(selectedVendor, tab)
                }
            ...
            {error && <div className={styles.p11}>{error}</div>}

            {inviteResult && (
              <div className={styles.p12}>
                <strong>Portal user invited:</strong> {inviteResult.email}
                <br />
                Temporary password:{" "}
                <code className={styles.p13}>{inviteResult.tempPassword}</code>
                <br />
                <span className={styles.p14}>
                  Share this with the vendor. They should change it after first
                  login.
                </span>
              </div>
            )}
                ...
                <div className="p-5">
                  ...
                  {loading ? (
                    ...
                    <div className={styles.p19}>
                      No portal users yet for {selectedVendor.name}. Invite one
                      above.
                    </div>
                  ) : (
