...
export default function IntelligencePage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      setData(await client.get("/procurement/procurement-intelligence"));
    } catch {
      /* empty */
    } finally {
      setLoading(false);
  ...
  const generateSpendAnalysis = async () => {
    setGenerating(true);
    try {
      await client.post("/procurement/procurement-intelligence/spend-analysis");
      fetchData();
    } catch {
      /* empty */
    } finally {
  ...
  const generateConcentration = async () => {
    ...
    try {
      await client.post(
        "/procurement/procurement-intelligence/supplier-concentration",
      );
      fetchData();
    } catch {
      /* empty */
  ...
  return (
    <div className="ui-page">
      <PageHeader
        ...
        description="Spend analysis, supplier concentration, and savings tracking"
        actions={
          <div className="ui-flex ui-gap-2">
            <Button
              variant="secondary"
              onClick={generateSpendAnalysis}
              disabled={generating}
            >
              <TrendingUp size={14} /> Generate Spend Analysis
            </Button>
            <Button
              variant="secondary"
              onClick={generateConcentration}
              disabled={generating}
            >
              <Activity size={14} /> Supplier Concentration
            </Button>
          </div>
        }
      />
