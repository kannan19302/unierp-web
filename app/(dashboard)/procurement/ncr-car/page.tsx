...
import { PageHeader, Button, Badge, Spinner, Tabs, Modal, DataTable } from "@kannan19302/ui";
...
export default function NcrCarPage() {
  ...
  const severityVariant = (
    ...
    (
      ({ MINOR: "success", MAJOR: "warning", CRITICAL: "danger" }) as Record<
        string,
        "default" | "success" | "warning" | "danger" | "info" | "primary"
      >
    )[s] || "default";
  ...
  const ncrColumns = [
    ...
    },
    {
      ...
      render: (row: any) =>
        row.status === "OPEN" ? (
          <Button size="sm" onClick={() => setShowCarForm(row.id)}>
            Raise CAR
          </Button>
        ) : null,
    },
  ...
  return (
    <RouteGuard permission="procurement.purchase-order.read">
      <PageHeader
        title="Supplier NCR & CAR"
        description="Non-conformance reports and corrective action requests"
        actions={
      ...
      <Tabs
        ...
        onChange={setTab}
        tabs={[
          { key: "ncr", label: "Non-Conformance Reports" },
          { key: "car", label: "Corrective Action Requests" },
        ]}
      />
      ...
      >
        ...
        <div className="ui-grid-3">
          <div className="ui-form-group">
            <label className="ui-label">Defect Type</label>
            <select
              className="ui-input"
          ...
          <div className="ui-form-group">
            <label className="ui-label">Defect Qty</label>
            <input
              className="ui-input"
              type="number"
        ...
        >
          <Button variant="ghost" onClick={() => setShowNcrForm(false)}>
            Cancel
          </Button>
      ...
      <Modal
        open={!!showCarForm}
        onClose={() => setShowCarForm(null)}
        title="Raise Corrective Action Request"
      >
        <div className="ui-form-group">
          <label className="ui-label">Root Cause</label>
          <textarea
            className="ui-input"
        ...
        <div className="ui-form-group">
          <label className="ui-label">Corrective Action</label>
          <textarea
            className="ui-input"
        ...
        >
          <Button variant="ghost" onClick={() => setShowCarForm(null)}>
            Cancel
          </Button>
          <Button onClick={() => showCarForm && createCar(showCarForm)}>
            Raise CAR
          </Button>
        </div>
      </Modal>
