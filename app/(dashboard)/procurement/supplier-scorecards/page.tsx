...
import { PageHeader, Button, Badge, Spinner, Modal, DataTable, StatCardRow } from "@kannan19302/ui";
...
export default function SupplierScorecardsPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Quality",
      render: (row: any) =>
        row.qualityScore ? `${Number(row.qualityScore).toFixed(1)}` : "—",
    },
    {
      key: "deliveryScore",
      header: "Delivery",
      render: (row: any) =>
        row.deliveryScore ? `${Number(row.deliveryScore).toFixed(1)}` : "—",
    },
    {
      key: "fillRateScore",
      header: "Fill Rate",
      render: (row: any) =>
        row.fillRateScore ? `${Number(row.fillRateScore).toFixed(1)}` : "—",
    },
    {
      key: "overallScore",
      header: "Overall",
      render: (row: any) =>
        row.overallScore ? (
          <strong>{Number(row.overallScore).toFixed(1)}</strong>
        ) : (
          "—"
        ),
    },
  ...
  return (
    <RouteGuard permission="procurement.purchase-order.read">
      <PageHeader
        ...
        description="Track and evaluate supplier performance with automated scorecards"
        actions={
          <Button onClick={() => setShowCompute(true)}>
            <Plus size={16} /> Compute Scorecard
          </Button>
        }
      />
          ...
          stats={[
            {
              ...
              value: stats.totalScorecards,
              icon: <Award size={20} />,
              color: "var(--color-blue)",
            },
            {
              label: "Vendors Scored",
              value: stats.vendorsScored,
              icon: <BarChart3 size={20} />,
              color: "var(--color-green)",
            },
            {
              label: "Avg Quality",
              value: stats.qualityAvg ? `${stats.qualityAvg}/100` : "—",
      ...
      {loading ? (
        ...
        <DataTable columns={columns} data={scorecards} />
      )}

      <Modal
        open={showCompute}
        onClose={() => setShowCompute(false)}
        title="Compute Supplier Scorecard"
      >
        ...
        >
          <Button variant="ghost" onClick={() => setShowCompute(false)}>
            Cancel
          </Button>
