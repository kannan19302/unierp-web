      ...
      <div className="ui-flex-between">
        <div>
          <h1 className={styles.p1}>
            <LayoutDashboard size={28} className="ui-text-primary" /> Dashboards
          </h1>
        ...
        <div key={d.id} className={styles.dashboardCard}>
          <div className="ui-flex-between">
            ...
            <div className="ui-flex ui-gap-1">
              <button onClick={() => openEdit(d)} className={styles.iconBtn}>
                <Edit3 size={14} />
              </button>
              <button
          ...
          {d.widgets.length === 0 && (
            <p className="ui-text-muted" style={{ marginTop: "0.75rem" }}>
              No widgets yet. Add via the dashboard builder.
            </p>
          )}
        </div>
      ))}
      {dashboards.length === 0 && !loading && (
        <div className="ui-text-muted">No dashboards created yet.</div>
      )}
      {isModalOpen && (
        <div className={styles.overlay}>
          <div className={styles.modal}>
            <div className="ui-flex-between">
              <h3>{editDashboard ? "Edit" : "New"} Dashboard</h3>
              <button
