...
export default function StockEntriesPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const items = (value: unknown) => value && typeof value === 'object' && 'data' in value ? (value as { data?: unknown }).data : value;
      const plist = items(pData);
      setProducts((Array.isArray(plist) ? plist : []).map((p: any) => ({ ...p, costPrice: Number(p.costPrice) })));
      {
      ...
      setAllBins((Array.isArray(items(bins)) ? items(bins) : []) as BinLocation[]);
      {
        const entries = items(seData);
        const entryList = Array.isArray(entries) ? entries : [];
        setStockEntries(entryList.map((se: any) => ({
          ...se,
  ...
  const handleCancelStockEntry = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post(`/inventory/stock-entries/${cancelTargetId}/cancel`, { reason: cancelReason });
  ...
  return (
    ...
    <div className="ui-stack-6 ui-animate-in">
      <PageHeader
        ...
        actions={
          <Button variant="primary" onClick={() => setIsEntryModalOpen(true)} className="ui-hstack-2">
            <Plus size={14} />
            Create Stock Entry
          </Button>
        }
      ...
      {loading ? (
        ...
        <ListPageTemplate
          columns={[
            { key: 'entryNumber', header: 'Voucher ID', render: (v) => <span className="font-semibold">{String(v)}</span> },
            { key: 'purpose', header: 'Type', render: (v) => <Badge variant={String(v) === 'MATERIAL_RECEIPT' ? 'success' : String(v) === 'MATERIAL_ISSUE' ? 'danger' : 'info'}>{String(v).replace('_', ' ')}</Badge> },
            { key: 'postingDate', header: 'Posting Date', render: (v) => new Date(String(v)).toLocaleString() },
            { key: 'status', header: 'Status', render: (v) => <Badge variant={String(v) === 'SUBMITTED' ? 'success' : String(v) === 'CANCELLED' ? 'danger' : 'warning'}>{String(v)}</Badge> },
            { key: 'totalValue', header: 'Total Value', render: (v) => <span className="font-semibold">${Number(v).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span> },
            { key: 'remarks', header: 'Remarks', render: (v) => <span className="ui-text-muted">{String(v || 'No remarks')}</span> },
            { key: 'id', header: 'Actions', render: (v, row) => (
              <div className={styles.s2}>
                {row.status === 'DRAFT' && <button onClick={() => handleSubmitStockEntry(String(v))} className={`ui-btn ui-btn-primary ${styles.s3}`} >Submit Slip</button>}
                {row.status === 'SUBMITTED' && <button onClick={() => triggerCancel(String(v))} className={`ui-btn ${styles.s4}`} >Cancel</button>}
              </div>
            ) },
          ] as ListColumn[]}
      ...
      {isEntryModalOpen && (
        <div className={styles.s5}>
          <div className={`ui-card ${styles.s6}`} >
            <div className={`ui-card-header flex items-center justify-between ${styles.s7}`} >
              <span className={styles.s8}>Create Warehouse Material Slip</span>
              <button onClick={() => setIsEntryModalOpen(false)} className="ui-btn-icon ui-text-muted">Close</button>
            </div>
              ...
              <form onSubmit={handleSaveStockEntry} className="ui-stack-4">
                <div className="ui-grid-2">
                  <div className="ui-form-group ui-stack-1">
                    <label className="ui-label ui-text-xs-label">Transaction Purpose *</label>
                    <select
                      className="ui-input"
                      value={entryPurpose}
                  ...
                  <div className="ui-form-group ui-stack-1">
                    <label className="ui-label ui-text-xs-label">Internal Remarks</label>
                    <input
                      type="text"
                      className="ui-input"
                      value={entryRemarks}
                ...
                <div className="ui-grid-2 ui-grid-2">
                  {entryPurpose !== 'MATERIAL_RECEIPT' && (
                    <div className="ui-form-group ui-stack-1">
                      <label className="ui-label ui-text-xs-label">Source Warehouse *</label>
                      <select
                        className="ui-input"
                        value={fromWarehouse}
                  ...
                  {entryPurpose !== 'MATERIAL_ISSUE' && (
                    <div className="ui-form-group ui-stack-1">
                      <label className="ui-label ui-text-xs-label">Destination Warehouse *</label>
                      <select
                        className="ui-input"
                        value={toWarehouse}
                            ...
                            <div className={styles.s19}>
                              ...
                              {selectedProduct.hasBatchTracking && (
                                <div className={styles.s20}>
                                  <label className={styles.s21}>Batch Number</label>
                                  <input
                                    type="text"
                                    className="ui-input"
                              ...
                              {selectedProduct.hasSerialTracking && (
                                <div className={styles.s20}>
                                  <label className={styles.s21}>Serial Number</label>
                                  <input
                                    type="text"
                                    className="ui-input"
                                    placeholder="SN-..."
                              ...
                              {entryPurpose !== 'MATERIAL_RECEIPT' && sourceBins.length > 0 && (
                                <div className={styles.s20}>
                                  <label className={styles.s21}>Source Bin</label>
                                  <select
                                    className="ui-input"
                              ...
                              {entryPurpose !== 'MATERIAL_ISSUE' && targetBins.length > 0 && (
                                <div className={styles.s20}>
                                  <label className={styles.s21}>Destination Bin</label>
                                  <select
                                    className="ui-input"
                                    value={item.toBinId || ''}
                                    onChange={(e) => {
      ...
      {cancelModalOpen && (
        <div className={styles.s5}>
          <div className={`ui-card ${styles.s23}`} >
            ...
            <form onSubmit={handleCancelStockEntry} className="p-5 ui-stack-4">
