...
const columns: ListColumn[] = [
  { key: 'productName', header: 'Product' },
  {
    ...
    render: (v, row) => {
      ...
      return <span className="font-mono">{r.currentBinCode} ({r.currentZone})</span>;
    },
  },
...
export default function SlottingPage() {
  ...
  const loadWarehouses = useCallback(async () => {
    try {
      ...
      const whs = (Array.isArray(data) ? data : []) as Array<{ id: string; name: string }>;
      setWarehouses(whs);
  ...
  const loadRecommendations = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await client.get<{ recommendations?: Recommendation[] }>(`/inventory/slotting/recommendations?warehouseId=${warehouseId}`);
    ...
    <div className="ui-stack-6 ui-animate-in">
      ...
      <div className={styles.s2}>
        <FormField label="Warehouse" htmlFor="slotting-warehouse">
          <Select id="slotting-warehouse" value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)}>
            {warehouses.map((w) => (
