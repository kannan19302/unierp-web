...
export default function ConsignmentPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [sRes, cRes] = await Promise.all([
        ...
        client.get<Consumption[]>('/inventory/consignment-stocks/consumptions/unbilled'),
      ]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post('/inventory/consignment-stocks', { supplierName, productId, warehouseId, unitCost, quantityOnHand: 0 });
  ...
  const handleMarkBilled = async (id: string) => {
    try {
      await client.post(`/inventory/consignment-stocks/consumptions/${id}/mark-billed`, {});
      loadData();
    } catch {
  ...
  const stocksColumns: ListColumn[] = [
    ...
    { key: 'product', header: 'Product', render: (v) => (v as ConsignmentStock['product'])?.name ?? '—' },
    { key: 'warehouse', header: 'Warehouse', render: (v) => (v as ConsignmentStock['warehouse'])?.name ?? '—' },
  ...
  const unbilledColumns: ListColumn[] = [
    { key: 'consignmentStock', header: 'Supplier', render: (v) => (v as Consumption['consignmentStock'])?.supplierName ?? '—' },
    ...
    { key: 'totalCost', header: 'Total Cost', render: (v) => `$${v}` },
    {
      ...
      render: (v) => (
        <div className="text-right">
          <button onClick={() => handleMarkBilled(String(v))} className={`ui-btn ui-btn-primary ${styles.s1}`} >Mark Billed</button>
        </div>
      ),
    },
  ];
            ...
            <div className="ui-card-body p-5">
              <form onSubmit={handleCreate} className="ui-stack-4">
                <div className="ui-form-group">
                  ...
                  <input type="text" className="ui-input" value={supplierName} onChange={(e) => setSupplierName(e.target.value)} required />
                </div>
                <div className="ui-form-group">
                ...
                <div className="ui-form-group">
                  ...
                  <input type="number" className="ui-input" value={unitCost} onChange={(e) => setUnitCost(Number(e.target.value))} required min={0} />
                </div>
