    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard
          ...
          value={data.activeCarriers}
          sub={`${data.totalCarriers} total`}
        />
        <StatCard
          label="Pending ASNs"
          value={data.pendingAsns}
          sub={`${data.inTransitAsns} in transit`}
        />
        <StatCard
          label="Expected Inbound"
          value={data.expectedInbound}
          sub={`${data.inTransitInbound} in transit`}
        />
...
function AsnsTab() {
  ...
  useEffect(() => {
    apiFetch<Asn[]>("/inventory/logistics/asns")
      .then(setAsns)
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);
...
function InboundTab() {
  ...
  useEffect(() => {
    apiFetch<InboundShipment[]>("/inventory/logistics/inbound")
      .then(setShipments)
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);
...
function OutboundTab() {
  ...
  useEffect(() => {
    apiFetch<OutboundShipment[]>("/inventory/logistics/outbound")
      .then(setShipments)
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);
...
function CarriersTab() {
  ...
  useEffect(() => {
    apiFetch<Carrier[]>("/inventory/logistics/carriers")
      .then(setCarriers)
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);
