...
export default function CatchWeightRecallPage() {
  ...
  const [tab, setTab] = useState<"dashboard" | "catch-weight" | "recalls">(
    "dashboard",
  );
        ...
        {tab === "dashboard" && dashboard && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                {
                  label: "Catch-Weight Configs",
                  value: dashboard.catchWeightConfigs,
                },
                {
                  label: "Out-of-Tolerance Readings",
                  value: dashboard.outOfToleranceReadings,
                  warn: dashboard.outOfToleranceReadings > 0,
                },
                {
                  label: "Open Recalls",
                  value:
                    dashboard.recalls.issued + dashboard.recalls.inProgress,
                  warn:
                    dashboard.recalls.issued + dashboard.recalls.inProgress > 0,
                },
                {
                ...
                >
                  <div
                    className={`text-2xl font-bold ${c.warn ? "text-red-700" : "text-gray-900"}`}
                  >
                    {c.value}
                  </div>
              ...
              <div className="bg-white rounded-lg border p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h2 className="text-lg font-semibold">
                      {selectedRecall.recallNumber} — {selectedRecall.title}
                    </h2>
                    <div className="flex gap-2 mt-1">
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-medium ${STATUS_COLORS[selectedRecall.status] ?? ""}`}
                      >
                        {selectedRecall.status}
                      </span>
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-medium ${STATUS_COLORS[selectedRecall.recallClass] ?? ""}`}
                      >
                        {selectedRecall.recallClass}
                      </span>
                    </div>
                  </div>
                  <button
                ...
                <div className="flex gap-2">
                  {selectedRecall.status === "DRAFT" && (
                    <button
                      onClick={() =>
                        recallAction(selectedRecall.id, "issue").then(() =>
                  ...
                  {selectedRecall.status === "IN_PROGRESS" && (
                    <>
                      <button
                        onClick={() =>
                          recallAction(selectedRecall.id, "send-notices").then(
                            () => setSelectedRecall(null),
                          )
                        }
                      ...
                      <button
                        onClick={() =>
                          recallAction(selectedRecall.id, "complete").then(() =>
                            setSelectedRecall(null),
                          )
                        }
                  ...
                  {["DRAFT", "ISSUED", "IN_PROGRESS"].includes(
                    selectedRecall.status,
                  ) && (
                    <button
                      onClick={() =>
                        recallAction(selectedRecall.id, "cancel").then(() =>
                          setSelectedRecall(null),
                        )
                      }
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-medium ${STATUS_COLORS[r.status] ?? ""}`}
                        >
                          {r.status}
                        </span>
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-medium ${STATUS_COLORS[r.recallClass] ?? ""}`}
                        >
                          {r.recallClass}
                        </span>
                      </div>
                      ...
                      <div className="text-xs text-gray-500 mt-0.5">
                        Action: {r.actionRequired} · {r.totalUnitsAffected}{" "}
                        units affected · {r.totalUnitsRecovered} recovered
                      </div>
                    </div>
