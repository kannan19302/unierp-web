...
function CapasTab() {
  ...
  return (
    <ListPageTemplate
      columns={[
        { key: 'capaNumber', header: 'CAPA #', render: (v) => <span className="font-mono text-blue-600 font-medium">{String(v)}</span> },
        { key: 'title', header: 'Title' },
        { key: 'type', header: 'Type' },
        { key: 'priority', header: 'Priority', render: (v) => <span className={`font-semibold text-xs ${priorityColor[String(v)] ?? 'text-gray-500'}`}>{String(v)}</span> },
        { key: 'status', header: 'Status', render: (v) => statusBadge(String(v)) },
        { key: 'actions', header: 'Actions Done', render: (v) => { const arr = (v as any[]) ?? []; return `${arr.filter((a: any) => a.status === 'COMPLETE').length}/${arr.length}`; } },
        { key: 'dueDate', header: 'Due Date', render: (v) => v ? new Date(String(v)).toLocaleDateString() : '—' },
      ] as ListColumn[]}
  ...
  return (
    <ListPageTemplate
      columns={[
        ...
        { key: 'serialNumber', header: 'Serial #', render: (v) => String(v ?? '—') },
        { key: 'calibrationType', header: 'Type' },
        { key: 'status', header: 'Status', render: (v) => statusBadge(String(v)) },
        { key: 'scheduledDate', header: 'Scheduled', render: (v) => new Date(String(v)).toLocaleDateString() },
        { key: 'nextDueDate', header: 'Next Due', render: (v) => v ? new Date(String(v)).toLocaleDateString() : '—' },
...
function DeviationsTab() {
  ...
  return (
    <ListPageTemplate
      columns={[
        { key: 'deviationNumber', header: 'Deviation #', render: (v) => <span className="font-mono text-blue-600 font-medium">{String(v)}</span> },
        { key: 'title', header: 'Title' },
        { key: 'type', header: 'Type' },
        { key: 'severity', header: 'Severity', render: (v) => severityBadge(String(v)) },
        { key: 'status', header: 'Status', render: (v) => statusBadge(String(v)) },
        { key: 'detectedAt', header: 'Detected', render: (v) => new Date(String(v)).toLocaleDateString() },
...
function SopsTab() {
  ...
  return (
    <ListPageTemplate
      columns={[
        { key: 'docNumber', header: 'Doc #', render: (v) => <span className="font-mono text-blue-600 font-medium">{String(v)}</span> },
        { key: 'title', header: 'Title' },
        { key: 'category', header: 'Category' },
        { key: 'department', header: 'Dept', render: (v) => String(v ?? '—') },
        { key: 'version', header: 'Version' },
        { key: 'status', header: 'Status', render: (v) => statusBadge(String(v)) },
        { key: 'effectiveDate', header: 'Effective', render: (v) => v ? new Date(String(v)).toLocaleDateString() : '—' },
        { key: 'reviewDate', header: 'Review Due', render: (v) => v ? new Date(String(v)).toLocaleDateString() : '—' },
      ] as ListColumn[]}
