...
export default function LicensePlatesPage() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const [platesResponse, tasksResponse, warehousesResponse] = await Promise.all([
      ...
      {loading ? (
        ...
        <>
          <Card padding="none" className="builder-table-wrapper">
            ...
            <ListPageTemplate
              columns={[
                ...
                { key: 'items', header: 'Items', render: (v) => String((v as any)?.length ?? 0) },
                { key: 'status', header: 'Status', render: (v) => <Badge variant={v === 'OPEN' ? 'success' : v === 'CLOSED' ? 'info' : 'default'}>{String(v)}</Badge> },
                { key: 'id', header: 'Actions', render: (v, row) => row.status === 'OPEN' ? (
                  <button onClick={() => handleCloseLicensePlate(String(v))} className={`ui-btn ui-btn-primary ${styles.s3}`} >Close Plate</button>
                ) : null },
              ] as ListColumn[]}
          ...
          <Card padding="none" className="builder-table-wrapper">
            ...
            <ListPageTemplate
              columns={[
                { key: 'stockEntryId', header: 'Stock Entry', render: (v) => <span className="font-mono">{String(v)}</span> },
                { key: 'quantity', header: 'Quantity' },
                { key: 'suggestedBin', header: 'Suggested Bin', render: (v) => String((v as any)?.code || '—') },
                { key: 'id', header: 'Actions', render: (v) => (
                  <button onClick={() => handleCompleteTask(String(v))} className={`ui-btn ui-btn-primary ${styles.s4}`} >Scan &amp; Complete</button>
                ) },
              ] as ListColumn[]}
            ...
            <div className="ui-card-body p-5">
              <form onSubmit={handleCreatePlate} className="ui-stack-4">
                <div className="ui-form-group">
                  ...
                  <input type="text" className="ui-input" value={code} onChange={(e) => setCode(e.target.value)} placeholder="e.g. LP-000123 (scan barcode)" required />
                </div>
                <div className="ui-form-group">
