...
function DashboardTab() {
  ...
  useEffect(() => {
    apiFetch<Record<string, any>>(`${BASE}/dashboard`).then(setData).catch(() => {}).finally(() => setLoading(false));
  }, []);
...
function LevelsTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const d = await apiFetch<{ data: any[]; total: number }>(`${BASE}/levels`); setLevels(d.data); setTotal(d.total); }
    ...
    <div className="space-y-4">
      ...
      <ListPageTemplate
        columns={[
          ...
          { key: 'minQty', header: 'Min', render: (v) => Number(v).toFixed(2) },
          { key: 'maxQty', header: 'Max', render: (v) => Number(v).toFixed(2) },
          { key: 'reorderQty', header: 'Reorder Qty', render: (v) => v != null ? Number(v).toFixed(2) : '—' },
          { key: 'method', header: 'Method' },
          { key: 'leadTimeDays', header: 'Lead Days' },
          { key: 'active', header: 'Active', render: (v) => v ? '✅' : '❌' },
          { key: 'id', header: 'Actions', render: (v, row) => (row.active ? <button onClick={() => deactivate(String(v))} className={styles.s1}>Deactivate</button> : null) },
        ] as ListColumn[]}
...
function RunTab() {
  ...
  const loadLogs = useCallback(async () => {
    setLoading(true);
    try { const d = await apiFetch<{ data: any[] }>(`${BASE}/run-logs`); setLogs(d.data); }
    catch {} finally { setLoading(false); }
  }, []);
      ...
      <div>
        ...
        <ListPageTemplate
          columns={[
            ...
            { key: 'completedAt', header: 'Completed At', render: (v) => v ? new Date(String(v)).toLocaleString() : '—' },
          ] as ListColumn[]}
...
function SuggestionsTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const d = await apiFetch<{ data: any[]; total: number }>(`${BASE}/suggestions?${params}`);
  ...
  return (
    <div className="space-y-4">
      ...
      <ListPageTemplate
        columns={[
          { key: 'suggestionNumber', header: 'Suggestion #', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'productId', header: 'Product', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'warehouseId', header: 'Warehouse' },
          { key: 'currentStock', header: 'Current Stock', render: (v) => Number(v).toFixed(2) },
          { key: 'suggestedQty', header: 'Suggested Qty', render: (v) => <strong>{Number(v).toFixed(2)}</strong> },
          { key: 'method', header: 'Method' },
          { key: 'neededByDate', header: 'Needed By', render: (v) => v ? new Date(String(v)).toLocaleDateString() : '—' },
          { key: 'status', header: 'Status', render: (v) => <Badge label={String(v)} colorMap={SUGG_COLORS} /> },
          { key: 'id', header: 'Actions', render: (v, row) => {
            const id = String(v);
            const status = String(row.status);
            return (
              <div className={styles.s2}>
                {status === 'OPEN' && <button onClick={() => action(id, 'approve')} className={styles.s3}>Approve</button>}
                {status === 'APPROVED' && <button onClick={() => action(id, 'order')} className={styles.s4}>Mark Ordered</button>}
                {status === 'ORDERED' && <button onClick={() => action(id, 'receive')} className={styles.s5}>Mark Received</button>}
                {!['RECEIVED', 'CANCELLED'].includes(status) && <button onClick={() => action(id, 'cancel', { reason: 'Manual cancel' })} className={styles.s6}>Cancel</button>}
              </div>
            );
          }},
        ] as ListColumn[]}
