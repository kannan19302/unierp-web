...
export default function ColdChainWriteoffPage() {
  ...
  const [tab, setTab] = useState<
    "dashboard" | "requirements" | "excursions" | "write-downs" | "write-offs"
  >("dashboard");
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (tab === "dashboard")
        ...
        setRequirements(await apiFetch<ColdChainReq[]>(`${BASE}/requirements`));
      else if (tab === "excursions")
        setExcursions(await apiFetch<Excursion[]>(`${BASE}/excursions`));
      else if (tab === "write-downs")
        setWriteDowns(await apiFetch<WriteDown[]>(`${BASE}/write-downs`));
      else if (tab === "write-offs")
        setWriteOffs(await apiFetch<WriteOff[]>(`${BASE}/write-offs`));
    } catch (e: unknown) {
      ...
      <div className="ui-page-shell">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            Cold Chain & Write-Off Management
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            Cold-chain storage requirements, temperature excursion logging,
            stock write-downs and write-offs
          </p>
        </div>
        ...
        {tab === "dashboard" && dashboard && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {[
                {
                  label: "Cold Chain Products",
                  value: dashboard.coldChainProducts,
                },
                {
                  label: "Open Excursions",
                  value: dashboard.openExcursions,
                  red: dashboard.openExcursions > 0,
                },
                {
                  label: "Critical Excursions",
                  value: dashboard.criticalExcursions,
                  red: dashboard.criticalExcursions > 0,
                },
                {
                  label: "Pending Write-Downs",
                  value: dashboard.pendingWriteDowns,
                },
                {
                  label: "Pending Write-Offs",
                  value: dashboard.pendingWriteOffs,
                },
                {
                ...
                },
                {
                  ...
                  render: (v, row) =>
                    row.status === "OPEN" ? (
                      <button
                        onClick={() =>
                          action(`/excursions/${v}/review`, "PATCH", {
                            status: "UNDER_REVIEW",
                          })
                        }
                  ...
                  render: (v, row) => (
                    <div className={styles.s2}>
                      {row.status === "PENDING_APPROVAL" && (
                        <>
                          <button
                            onClick={() => action(`/write-downs/${v}/approve`)}
                          ...
                          <button
                            onClick={() =>
                              action(`/write-downs/${v}/reject`, "PATCH", {
                                rejectionNotes: "Rejected",
                              })
                            }
                      ...
                      {row.status === "APPROVED" && (
                        <button
                          onClick={() => action(`/write-downs/${v}/apply`)}
                  ...
                  render: (v, row) => (
                    <div className={styles.s2}>
                      {row.status === "PENDING_APPROVAL" && (
                        <>
                          <button
                            onClick={() => action(`/write-offs/${v}/approve`)}
                            className="text-xs px-2 py-1 bg-green-50 text-green-700 rounded hover:bg-green-100"
                          >
                            Approve
                          </button>
                          <button
                            onClick={() =>
                              action(`/write-offs/${v}/reject`, "PATCH", {
                                rejectionNotes: "Rejected",
                              })
                            }
                            className="text-xs px-2 py-1 bg-red-50 text-red-700 rounded hover:bg-red-100"
                      ...
                      {row.status === "APPROVED" && (
                        <button
                          onClick={() => action(`/write-offs/${v}/complete`)}
                          className="text-xs px-2 py-1 bg-blue-50 text-blue-700 rounded hover:bg-blue-100"
                        >
                          Complete
                        </button>
