...
function useFrameworkFetch() {
  ...
  return useCallback(
    <T,>(path: string, opts?: RequestInit) =>
      client.request<T>(`/inventory/freight-claims${path}`, {
        method: opts?.method,
...
function DashboardTab() {
  ...
  const cards = [
    ...
    ["Draft Reports", stats.draftReports],
    ["Submitted Reports", stats.submittedReports],
    ["Total Claims", stats.totalClaims],
    ["Draft Claims", stats.draftClaims],
    ["Filed Claims", stats.filedClaims],
    ["Under Investigation", stats.underInvestigation],
    ["Settlement Offered", stats.settlementOffered],
  ];
  return (
    <StatCardRow
...
function DamageReportsTab() {
  ...
  const doSubmit = async (id: string) => {
    try {
      await apiFetch(`/damage-reports/${id}/submit`, { method: "PATCH" });
      load();
    } catch (err: any) {
      setMsg(err.message);
    }
...
function FileClaimTab() {
  ...
  return (
    <form
      onSubmit={submit}
      className="bg-white border rounded-lg p-4 max-w-lg space-y-3"
    >
      <h2 className="font-semibold text-sm">File Freight Claim</h2>
      {[
        {
          label: "Damage Report ID *",
          key: "damageReportId" as const,
          required: true,
        },
        { label: "Carrier ID *", key: "carrierId" as const, required: true },
        {
          label: "Claimed Amount *",
          key: "claimedAmount" as const,
          required: true,
          type: "number",
        },
...
function ClaimsTab() {
  ...
  const updateStatus = async (id: string) => {
    ...
    if (!status) return;
    try {
      await apiFetch(`/claims/${id}/status`, {
        method: "PATCH",
...
function EventsTab() {
  ...
  const load = useCallback(() => {
    if (!claimId) return;
    apiFetch<any[]>(`/claims/${claimId}/events`)
      .then(setEvents)
      .catch(console.error);
  }, [claimId]);
  useEffect(load, [load]);
  ...
  const addEvent = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await apiFetch(`/claims/${claimId}/events`, {
        method: "POST",
