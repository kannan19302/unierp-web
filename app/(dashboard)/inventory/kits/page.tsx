...
export default function KitsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      {
        const whs = items(warehousesResponse) as Array<{ id: string; name: string }>;
        setWarehouses(whs);
  ...
  const viewKit = async (kit: Kit) => {
    ...
    setVersions([]);
    try {
      const [availabilityResponse, costResponse, versionsResponse] = await Promise.all([
        client.get(`/inventory/kits/${kit.id}/availability?warehouseId=${warehouseId}`),
  ...
  const handleAssemble = async () => {
    ...
    try {
      await client.post(`/inventory/kits/${selectedKit.id}/assemble`, { warehouseId, quantity: assembleQty });
      viewKit(selectedKit);
    } catch {
  ...
  const handleActivateVersion = async (versionId: string) => {
    if (!selectedKit) return;
    try {
      await client.post(`/inventory/kits/${selectedKit.id}/versions/${versionId}/activate`, {});
      viewKit(selectedKit);
    } catch {
  ...
  const handleDisassemble = async () => {
    if (!selectedKit) return;
    try {
      await client.post(`/inventory/kits/${selectedKit.id}/disassemble`, { warehouseId, quantity: assembleQty });
      viewKit(selectedKit);
    } catch {
  ...
  const kitColumns: ListColumn[] = [
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return <Badge variant={k.isActive ? 'success' : 'default'}>{k.isActive ? 'Active' : 'Inactive'}</Badge>;
      },
    },
  ];
  ...
  return (
    <RouteGuard permission="inventory.kits.read">
      ...
      <div className={`ui-form-group ${styles.s2}`} >
        ...
        <select className="ui-input" value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)}>
          {warehouses.map((w) => (
      ...
      {loading ? (
        ...
        <div className="ui-grid-2 ui-gap-6">
          ...
          <Card className="p-5">
            {!selectedKit ? (
              <div className={styles.s4}>Select a kit to view availability and margin.</div>
            ) : (
              <div className="ui-stack-4">
                <div className="font-semibold">{selectedKit.name}</div>
                {availability && (
                  <div className="text-sm">Max buildable now: <strong>{availability.maxBuildable}</strong></div>
                )}
                {costRollup && (
                ...
                <div className={styles.s6}>
                  <input type="number" className={`ui-input ${styles.s7}`}  value={assembleQty} min={1} onChange={(e) => setAssembleQty(Number(e.target.value))} />
                ...
                <div className={styles.s8}>
                  <div className="ui-flex-between mb-2">
                    ...
                    <Button variant="outline" onClick={handleSnapshotVersion} className={styles.s10}>Snapshot Version</Button>
                  </div>
                  {versions.map((v) => (
                    <div key={v.id} className={styles.s11}>
                      <span>v{v.versionNo} {v.notes ? `— ${v.notes}` : ''} {v.isActive && <Badge variant="success">Active</Badge>}</span>
                      {!v.isActive && (
                        <button onClick={() => handleActivateVersion(v.id)} className={`ui-btn ui-btn-primary ${styles.s12}`} >Activate</button>
                      )}
                    </div>
                  ))}
                </div>
