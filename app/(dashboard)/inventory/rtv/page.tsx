...
const makeRmaColumns = (advance: (url: string, body?: object) => void): ListColumn[] => [
  {
    key: 'rmaNumber',
    header: 'RMA #',
  ...
  },
  {
    key: 'status',
    header: 'Status',
    render: (v) => <Badge variant={STATUS_VARIANT[String(v)] ?? 'default'}>{String(v)}</Badge>,
  },
  {
    key: 'id',
    header: '',
    render: (v, row) => {
      ...
      return (
        <div className={styles.s2}>
          {r.status === 'PENDING' && (
            <button onClick={() => advance(`/api/v1/inventory/rtv/rma-requests/${r.id}/submit`)} className={`ui-btn ui-btn-primary ${styles.s3}`} >Submit</button>
          )}
          {r.status === 'SUBMITTED' && (
            <>
              <button onClick={() => {
              ...
              }} className={`ui-btn ui-btn-primary ${styles.s3}`} >Authorize</button>
              <button onClick={() => {
                const reason = prompt('Rejection reason:');
                if (reason) advance(`/api/v1/inventory/rtv/rma-requests/${r.id}/reject`, { rejectionReason: reason });
              }} className={`ui-btn ${styles.s3}`} >Reject</button>
            </>
          )}
          {r.status === 'AUTHORIZED' && (
            <button onClick={() => advance(`/api/v1/inventory/rtv/rma-requests/${r.id}/complete`)} className={`ui-btn ${styles.s3}`} >Complete</button>
          )}
        </div>
      );
    },
...
const makeShipmentColumns = (advance: (url: string, body?: object) => void): ListColumn[] => [
  {
    key: 'shipmentNumber',
    header: 'Shipment #',
    render: (v) => <span className={styles.s1}>{String(v)}</span>,
  ...
  },
  {
    ...
    render: (v, row) => {
      const s = row as unknown as ReturnShipment;
      return s.creditMemoRef ? `${s.creditMemoRef} ($${Number(s.creditAmount).toFixed(2)})` : '—';
    },
  },
  {
    key: 'status',
  ...
  },
  {
    ...
    render: (v, row) => {
      ...
      return (
        <div className={styles.s2}>
          {s.status === 'PENDING' && <button onClick={() => advance(`/api/v1/inventory/rtv/shipments/${s.id}/pack`)} className={`ui-btn ${styles.s3}`} >Pack</button>}
          {s.status === 'PACKED' && <button onClick={() => advance(`/api/v1/inventory/rtv/shipments/${s.id}/ship`)} className={`ui-btn ui-btn-primary ${styles.s3}`} >Ship</button>}
          {s.status === 'SHIPPED' && <button onClick={() => advance(`/api/v1/inventory/rtv/shipments/${s.id}/deliver`)} className={`ui-btn ui-btn-primary ${styles.s3}`} >Mark Delivered</button>}
          {s.status === 'DELIVERED' && !s.creditMemoRef && (
            ...
            }} className={`ui-btn ${styles.s3}`} >Record Credit</button>
          )}
        </div>
      );
    },
...
export default function RtvPage() {
  ...
  useEffect(() => {
    loadData();
    client.get<unknown>('/procurement/vendors')
      .then((d) => setVendors(Array.isArray(d) ? d as Array<{ id: string; name: string }> : []))
  ...
  const handleCreateRma = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post('/inventory/rtv/rma-requests', { purchaseReturnId, vendorId, notes: notes || undefined });
  ...
  return (
    ...
    <div className="ui-stack-6 ui-animate-in">
      <PageHeader
        ...
        actions={
          <Button variant="primary" onClick={() => setIsCreateRmaOpen(true)} className="ui-hstack-2">
      ...
      {dashboard && (
        <StatCardRow stats={[
          ...
          { label: 'Authorized', value: dashboard.byStatus.AUTHORIZED ?? 0, icon: <Package size={16} />, color: 'success' },
          { label: 'Pending Shipments', value: dashboard.pendingShipments, icon: <Package size={16} />, color: 'warning' },
          { label: 'Credit Received', value: `$${Number(dashboard.totalCreditReceived).toFixed(2)}`, icon: <TrendingDown size={16} />, color: 'info' },
        ]} />
      )}
        ...
        <div className={styles.s6}>
          <div className={`ui-card modal-card ${styles.s7}`} >
            <div className={styles.s8}>
              ...
              <button onClick={() => setIsCreateRmaOpen(false)} className="ui-btn-icon ui-text-muted">Close</button>
            </div>
            <div className="ui-card-body p-5">
              <form onSubmit={handleCreateRma} className="ui-stack-4">
                <div className="ui-form-group">
                  ...
                  <input type="text" className="ui-input" value={purchaseReturnId} onChange={(e) => setPurchaseReturnId(e.target.value)} required placeholder="PR-xxx" />
                </div>
                <div className="ui-form-group">
                  <label className="ui-label">Vendor *</label>
                  <select className="ui-input" value={vendorId} onChange={(e) => setVendorId(e.target.value)} required>
                ...
                <div className="ui-form-group">
                  ...
                  <textarea className={`ui-input ${styles.s9}`} value={notes} onChange={(e) => setNotes(e.target.value)} rows={3}  />
                </div>
