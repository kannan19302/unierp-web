...
export default function CrossDockPage() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const data = await client.get<{ opportunities?: Opportunity[] }>('/inventory/cross-dock/opportunities');
  ...
  const handleExecute = async (putawayTaskId: string, pickWaveItemId: string) => {
    try {
      await client.post(`/inventory/cross-dock/execute?putawayTaskId=${putawayTaskId}&pickWaveItemId=${pickWaveItemId}`, {});
      loadData();
    } catch {
  ...
  const columns: ListColumn[] = [
    ...
    { key: 'matchedQty', header: 'Matched Qty', render: (v) => <span className="font-bold">{String(v)}</span> },
    {
      key: 'putawayTaskId',
      header: 'Actions',
      render: (_, row) => {
