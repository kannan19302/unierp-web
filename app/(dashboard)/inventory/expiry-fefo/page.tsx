...
export default function ExpiryFefoPage() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const data = await client.get<{ batches?: ExpiringBatch[] }>(
        "/inventory/batches/reports/expiring?withinDays=30",
      );
  ...
  const handleRecallNotice = async () => {
    try {
      setRecallNotice(
        await client.get(`/inventory/batches/${recallBatchId}/recall-notice`),
      );
    } catch {
      setRecallNotice({
        batch: { batchNo: recallBatchId },
        affectedSalesOrders: [],
        untracedConsumptions: 0,
        recommendedAction:
