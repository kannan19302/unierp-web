...
function Dashboard() {
  ...
  useEffect(() => {
    apiFetch<Record<string, unknown>>('/inventory/hazmat/dashboard').then(setData).catch(console.error).finally(() => setLoading(false));
  }, []);
...
function Classifications() {
  ...
  const load = useCallback(() => {
    apiFetch<Record<string, unknown>[]>('/inventory/hazmat/classifications').then(setItems).catch(console.error);
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const columns: ListColumn[] = [
    ...
    {
      key: 'hazardClass', header: 'Hazard Class',
      render: (row) => classLabel(String((row as unknown as { hazardClass: string }).hazardClass)),
    },
    {
      key: 'packingGroup', header: 'Packing Grp',
      render: (row) => String((row as unknown as { packingGroup?: string }).packingGroup ?? '—'),
    },
    { key: 'regulation', header: 'Regulation' },
    {
      key: 'sds', header: 'SDS',
      render: (row) => String(((row as unknown as { sdsRecords?: unknown[] }).sdsRecords ?? []).length),
    },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <ListPageTemplate columns={columns} data={items as unknown as Record<string, unknown>[]} loading={false} searchable />
    </div>
  );
}
...
function Manifests() {
  ...
  const load = useCallback(() => {
    ...
    apiFetch<Record<string, unknown>[]>(`/inventory/hazmat/manifests${qs}`).then(setItems).catch(console.error);
  }, [filterStatus]);
  useEffect(() => { load(); }, [load]);
  ...
  const columns: ListColumn[] = [
    ...
    { key: 'destAddress', header: 'Destination' },
    {
      ...
      render: (row) => String((row as unknown as { carrierName?: string }).carrierName ?? '—'),
    },
    {
      key: 'status', header: 'Status',
      render: (row) => statusBadge(String((row as unknown as { status: string }).status)),
    },
    {
        ...
        return (
          <div className="flex gap-1 flex-wrap">
            ...
            {!['DELIVERED', 'CANCELLED'].includes(m.status) && (
              <button onClick={() => transition(m.id, 'cancel')}
...
function Incidents() {
  ...
  const load = useCallback(() => {
    apiFetch<Record<string, unknown>[]>('/inventory/hazmat/incidents').then(setItems).catch(console.error);
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const columns: ListColumn[] = [
    ...
    { key: 'productId', header: 'Product' },
    {
      ...
      render: (row) => new Date(String((row as unknown as { incidentDate: string }).incidentDate)).toLocaleDateString(),
    },
    {
      ...
      render: (row) => {
        ...
        return (
          <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${colorMap[inc.severity] ?? 'bg-blue-100 text-blue-800'}`}>
            {inc.severity}
          </span>
        );
      },
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return !inc.closedAt
          ? <button onClick={() => closeInc(inc.id)} className="px-2 py-0.5 text-xs bg-green-600 text-white rounded hover:bg-green-700">Close</button>
          : null;
      },
    },
  ];
...
function ComplianceReport() {
  ...
  useEffect(() => {
    apiFetch<Record<string, unknown>[]>('/inventory/hazmat/compliance-report').then(setRows).catch(console.error).finally(() => setLoading(false));
  }, []);
  ...
  const columns: ListColumn[] = [
    ...
    {
      key: 'currentSds', header: 'Current',
      render: (row) => <span className="text-green-700">{String((row as unknown as { currentSds: number }).currentSds)}</span>,
    },
    {
      key: 'expiredSds', header: 'Expired',
      render: (row) => <span className="text-red-600">{String((row as unknown as { expiredSds: number }).expiredSds)}</span>,
    },
    {
      key: 'unacknowledgedSds', header: 'Unacknowledged',
      render: (row) => <span className="text-orange-600">{String((row as unknown as { unacknowledgedSds: number }).unacknowledgedSds)}</span>,
    },
    {
      key: 'compliant', header: 'Compliant',
      render: (row) => (row as unknown as { compliant: boolean }).compliant
  ...
  return (
    <div className="space-y-4">
      ...
      <ListPageTemplate columns={columns} data={rows as unknown as Record<string, unknown>[]} loading={false} searchable />
    </div>
  );
}
