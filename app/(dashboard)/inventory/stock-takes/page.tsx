...
export default function StockTakesPage() {
  ...
  const loadDashboard = useCallback(async () => {
    try { setDashboard(await client.get<Dashboard>('/inventory/stock-takes/dashboard')); } catch { /* ignore */ }
  }, [client]);

  const loadStockTakes = useCallback(async () => {
    setLoading(true);
    try { setStockTakes(await client.get<StockTake[]>('/inventory/stock-takes')); }
    catch (e: unknown) { setError(String(e)); } finally { setLoading(false); }
  }, [client]);

  const loadAccuracy = useCallback(async () => {
    setLoading(true);
    try { setAccuracy(await client.get<AccuracyRow[]>('/inventory/stock-takes/accuracy-report')); }
    catch (e: unknown) { setError(String(e)); } finally { setLoading(false); }
  }, [client]);

  const loadVariances = useCallback(async (id: string) => {
    try {
      const result = await client.get<StockTakeVariance[]>(`/inventory/stock-takes/${id}/variances`);
      setVariances(result);
      setSelectedSt(id);
  ...
  const varianceAction = async (varianceId: string, act: string, body?: unknown) => {
    try {
      await client.patch(`/inventory/stock-takes/variances/${varianceId}/${act}`, body);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {tab === 'stock-takes' && (
        <div>
          ...
          <ListPageTemplate
            columns={[
              { key: 'stockTakeNumber', header: 'Stock Take #', render: (v) => <span className="font-mono">{String(v)}</span> },
              { key: 'warehouseId', header: 'Warehouse', render: (v) => String(v).slice(0, 8) },
              { key: 'countType', header: 'Type' },
              { key: 'countDate', header: 'Count Date', render: (v) => new Date(String(v)).toLocaleDateString() },
              { key: 'status', header: 'Status', render: (v) => <span className={`px-2 py-0.5 rounded text-xs ${STATUS_COLORS[String(v)] ?? ''}`}>{String(v)}</span> },
              { key: '_count', header: 'Sheets', render: (v) => String((v as any)?.sheets ?? 0) },
              { key: 'id', header: 'Variances', render: (v, row) => <button onClick={() => { setTab('variances'); loadVariances(String(v)); }} className="text-blue-600 underline text-xs">{(row._count as any)?.variances ?? 0}</button> },
              { key: 'id', header: 'Actions', render: (v, row) => (
                <div className={styles.s1}>
                  {row.status === 'DRAFT' && <button onClick={() => action(String(v), 'start')} className="text-blue-600 underline text-xs">Start</button>}
                  {['COUNTING', 'IN_PROGRESS'].includes(String(row.status)) && <button onClick={() => action(String(v), 'generate-variances')} className="text-orange-600 underline text-xs">Gen Variances</button>}
                  {row.status === 'VARIANCE_REVIEW' && <button onClick={() => action(String(v), 'approve')} className="text-green-600 underline text-xs">Approve</button>}
                  {row.status === 'APPROVED' && <button onClick={() => action(String(v), 'post')} className="text-emerald-600 underline text-xs">Post</button>}
                  {!['POSTED', 'CANCELLED'].includes(String(row.status)) && <button onClick={() => action(String(v), 'cancel')} className="text-red-600 underline text-xs">Cancel</button>}
                </div>
              )},
            ] as ListColumn[]}
      ...
      {tab === 'variances' && (
        <div>
          ...
          {!selectedSt ? (
            ...
            <ListPageTemplate
              columns={[
                { key: 'productId', header: 'Product', render: (v) => <span className="font-mono text-xs">{String(v).slice(0, 10)}</span> },
                { key: 'systemQty', header: 'System Qty' },
                { key: 'countedQty', header: 'Counted Qty' },
                { key: 'varianceQty', header: 'Variance', render: (v) => <span className={Number(v) < 0 ? 'text-red-600 font-medium' : 'text-green-600 font-medium'}>{Number(v) > 0 ? '+' : ''}{String(v)}</span> },
                { key: 'variancePct', header: 'Variance %', render: (v) => `${Number(v).toFixed(1)}%` },
                { key: 'varianceValue', header: 'Value', render: (v) => v ? `$${Number(v).toFixed(2)}` : '-' },
                { key: 'status', header: 'Status', render: (v) => <span className={`px-2 py-0.5 rounded text-xs ${STATUS_COLORS[String(v)] ?? ''}`}>{String(v)}</span> },
                { key: 'id', header: 'Actions', render: (v, row) => row.status === 'PENDING' ? (
                  <div className={styles.s2}>
                    <button onClick={() => varianceAction(String(v), 'approve')} className="text-green-600 underline text-xs">Approve</button>
                    <button onClick={() => varianceAction(String(v), 'reject', { rejectionReason: 'Rejected' })} className="text-red-600 underline text-xs">Reject</button>
                  </div>
                ) : null },
              ] as ListColumn[]}
        ...
        <div>
          ...
          <ListPageTemplate
            columns={[
              { key: 'stockTakeNumber', header: 'Stock Take #', render: (v) => <span className="font-mono">{String(v)}</span> },
              { key: 'warehouseId', header: 'Warehouse', render: (v) => String(v).slice(0, 8) },
              { key: 'postedAt', header: 'Posted At', render: (v) => v ? new Date(String(v)).toLocaleDateString() : '-' },
              { key: 'totalLines', header: 'Total Lines' },
              { key: 'varianceLines', header: 'Variance Lines', render: (v) => <span className="text-red-600">{String(v)}</span> },
              { key: 'accuracyRate', header: 'Accuracy Rate', render: (v) => (
                <div className={styles.s3}>
                  <div className={styles.s4}>
                    <div style={{ width: `${v}%` }} className={styles.s5} />
                  </div>
                  <span className={Number(v) >= 95 ? 'text-green-600 font-medium' : 'text-orange-600 font-medium'}>{String(v)}%</span>
                </div>
              ) },
            ] as ListColumn[]}
