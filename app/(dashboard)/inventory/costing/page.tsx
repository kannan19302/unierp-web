...
function DashboardTab() {
  ...
  const cards = [
    ...
    ["FIFO Profiles", stats.fifoProfiles],
    ["WAC Profiles", stats.wacProfiles],
    ["LIFO Profiles", stats.lifoProfiles],
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      await apiFetch("/profiles", {
        ...
        body: JSON.stringify({
          ...form,
          standardCost: form.standardCost ? +form.standardCost : undefined,
        }),
      });
...
function CostLayersTab() {
  ...
  const load = useCallback(() => {
    if (!profileId) return;
    apiFetch<any[]>(`/profiles/${profileId}/layers`)
      .then(setLayers)
      .catch(console.error);
  }, [profileId]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMsg("Layer added");
      setForm({ unitCost: "", qty: "", receiptRef: "" });
      load();
    } catch (err: any) {
      setMsg(err.message);
    }
    ...
    <div className="space-y-4">
      ...
      {result && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-sm space-y-1">
          <p>
            <strong>Qty Consumed:</strong> {result.qtyConsumed}
          </p>
          <p>
            <strong>Total Cost:</strong> {result.totalCost.toFixed(4)}
          </p>
          <p>
            <strong>Avg Cost/Unit:</strong> {result.avgCost.toFixed(4)}
          </p>
        </div>
      )}
    </div>
