...
export default function PackagingGs1Page() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      else if (tab === 'specs') setSpecs(await apiFetch<PackagingSpec[]>(`${BASE}/specs?productId=`));
      else if (tab === 'barcodes') setBarcodes(await apiFetch<typeof barcodes>(`${BASE}/barcodes`));
      else if (tab === 'labels') setTemplates(await apiFetch<LabelTemplate[]>(`${BASE}/label-templates`));
      else if (tab === 'sscc') setSsccRecords(await apiFetch<SsccRecord[]>(`${BASE}/sscc`));
  ...
  return (
    ...
    <div className="ui-page-shell">
      ...
      {tab === 'specs' && (
        <ListPageTemplate
          columns={[
            ...
            { key: 'level', header: 'Level', render: (v) => <span className={`px-2 py-0.5 rounded text-xs font-medium ${LEVEL_COLORS[String(v)] ?? 'bg-gray-100 text-gray-700'}`}>{String(v)}</span> },
            { key: 'unitsPerLevel', header: 'Units/Level', render: (v) => <strong>{String(v)}</strong> },
            { key: 'lengthMm', header: 'Dimensions', render: (v, row) => v ? `${v}×${row.widthMm}×${row.heightMm}` : '—' },
            { key: 'grossWeightKg', header: 'Weight', render: (v) => v ? `${Number(v).toFixed(3)} kg` : '—' },
            { key: 'barcodes', header: 'Barcodes', render: (v) => String((v as any)?.length ?? 0) },
          ] as ListColumn[]}
      ...
      {tab === 'barcodes' && (
        <ListPageTemplate
          columns={[
            { key: 'barcodeValue', header: 'Barcode Value', render: (v) => <span className="font-mono">{String(v)}</span> },
            { key: 'symbology', header: 'Symbology', render: (v) => <span className="px-2 py-0.5 rounded text-xs bg-blue-50 text-blue-700">{String(v)}</span> },
            { key: 'isPrimary', header: 'Primary', render: (v) => v ? <span className="text-green-600 font-medium">✓</span> : '—' },
            { key: 'packagingSpecId', header: 'Spec', render: (v) => <span className={styles.s1}>{String(v ?? '').slice(-8)}</span> },
          ] as ListColumn[]}
      ...
      {tab === 'sscc' && (
        <ListPageTemplate
          columns={[
            { key: 'sscc', header: 'SSCC', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
            { key: 'gs1CompanyPrefix', header: 'Company Prefix' },
            { key: 'allocatedAt', header: 'Allocated', render: (v) => new Date(String(v)).toLocaleDateString() },
            { key: 'usedAt', header: 'Used', render: (v) => v ? <span className="text-green-600 text-xs">{new Date(String(v)).toLocaleDateString()}</span> : <span className="text-gray-400 text-xs">Available</span> },
            { key: 'shipmentRef', header: 'Shipment Ref', render: (v) => String(v ?? '—') },
          ] as ListColumn[]}
