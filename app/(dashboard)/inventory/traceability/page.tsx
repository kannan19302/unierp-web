...
export default function TraceabilityPage() {
  ...
  const handleReceiveWithTraceability = async (e: React.FormEvent) => {
    ...
    try {
      setReceiveResult(
        await client.post("/inventory/receive-with-traceability", {
      ...
      setReceiveResult({
        ...
        serialNumbers: receiveSerials.split(",").filter(Boolean),
        batch: receiveBatchNo ? { batchNo: receiveBatchNo } : null,
      });
    }
  };
  ...
  const handleQuarantine = async () => {
    ...
    try {
      await client.post(`/inventory/batches/${batchId}/quarantine`, {
  ...
  return (
    <RouteGuard permission="inventory.traceability.read">
      <InventoryTabLayout
        ...
        moduleDescription="Genealogy trace for batches/lots and where-used trace for serial numbers, plus the batch quarantine workflow."
      >
        <div className="ui-stack-6 ui-animate-in">
          ...
          <Card className="p-5">
            ...
            {batchTrace && (
              <div className={styles.s4}>
                <div>
                  <strong>{batchTrace.batch.product?.name}</strong> —{" "}
                  {batchTrace.batch.batchNo}{" "}
                  <Badge
                    variant={
                  ...
                  >
                    {batchTrace.batch.status}
                  </Badge>
                </div>
                <div>
                  Origin stock entry: {batchTrace.origin?.entryNumber || "—"}
                </div>
                <div>
                  Consumed in:{" "}
                  {batchTrace.consumedIn
                    ?.map((e: any) => e.entryNumber)
                    .join(", ") || "—"}
                </div>
                <div>
                  License plates:{" "}
                  {batchTrace.licensePlates
                    ?.map((p: any) => p.code)
                    .join(", ") || "—"}
                </div>
                <div className={styles.s5}>
                  ...
                  {batchTrace.batch.status === "QUARANTINE" ? (
                    <button
                      ...
                      className={`ui-btn ui-btn-primary ${styles.s6}`}
                    >
                    ...
                    <button
                      ...
                      className={`ui-btn ui-btn-primary ${styles.s7}`}
                    >
          ...
          <Card className="p-5">
            ...
            {serialTrace && (
              <div className={styles.s4}>
                <div>
                  <strong>{serialTrace.serial.product?.name}</strong> —{" "}
                  {serialTrace.serial.serialNo}{" "}
                  <Badge variant="info">{serialTrace.serial.status}</Badge>
                </div>
                <div>
                  History:{" "}
                  {serialTrace.history?.map((h: any, i: number) => (
                    <span key={i}>
                      {h.action} → {h.toStatus};{" "}
                    </span>
                  ))}
          ...
          <Card className="p-5">
            ...
            <form
              onSubmit={handleReceiveWithTraceability}
            ...
            {receiveResult && (
              <div className={styles.s9}>
                Receipt {receiveResult.stockEntry?.status} —{" "}
                {receiveResult.serialNumbers?.length || 0} serial(s) captured
                {receiveResult.batch
                  ? `, batch ${receiveResult.batch.batchNo}`
                  : ""}
                .
              </div>
            )}
