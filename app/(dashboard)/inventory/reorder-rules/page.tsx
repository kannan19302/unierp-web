...
const makeColumns = (handleCreateRequisition: (id: string) => void): ListColumn[] => [
  ...
  },
  {
    ...
    header: 'Status',
    render: (v) => (
      <Badge variant={v ? 'warning' : 'success'}>{v ? 'Reorder Needed' : 'OK'}</Badge>
    ),
  },
  {
    key: 'ruleId',
...
export default function ReorderRulesPage() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const data = await client.get<{ rules?: ReorderDashboardRow[] }>('/inventory/reorder-rules/dashboard');
      setRows(data.rules || []);
    } catch {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post('/inventory/reorder-rules', { productId, minQty, reorderQty, leadTimeDays, autoCreatePO: false });
  ...
  const handleCreateRequisition = async (ruleId: string) => {
    try {
      await client.post(`/inventory/reorder-rules/${ruleId}/create-requisition`, {});
            ...
            <div className="ui-card-body p-5">
              <form onSubmit={handleCreate} className="ui-stack-4">
                ...
                <div className="ui-form-group">
                  ...
                  <input type="number" className="ui-input" value={minQty} onChange={(e) => setMinQty(Number(e.target.value))} required min={0} />
                </div>
                <div className="ui-form-group">
                  <label className="ui-label">Reorder Qty *</label>
                  <input type="number" className="ui-input" value={reorderQty} onChange={(e) => setReorderQty(Number(e.target.value))} required min={1} />
                </div>
                <div className="ui-form-group">
                  <label className="ui-label">Lead Time (days) *</label>
                  <input type="number" className="ui-input" value={leadTimeDays} onChange={(e) => setLeadTimeDays(Number(e.target.value))} required min={0} />
                </div>
