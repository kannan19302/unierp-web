...
export default function TransferOrdersPage() {
  ...
  const loadDashboard = useCallback(async () => {
      try { setDashboard(await apiFetch<Dashboard>('/inventory/transfer-orders/dashboard')); } catch { /* ignore */ }
  }, []);
  const loadOrders = useCallback(async () => {
    setLoading(true);
      try { setOrders(await apiFetch<TransferOrder[]>('/inventory/transfer-orders')); } catch (e: unknown) { setError(String(e)); } finally { setLoading(false); }
  }, []);
  const loadInTransit = useCallback(async () => {
    setLoading(true);
      try { setInTransit(await apiFetch<InTransitItem[]>('/inventory/transfer-orders/in-transit')); } catch (e: unknown) { setError(String(e)); } finally { setLoading(false); }
  }, []);
    ...
    <div className="ui-page-shell">
      ...
      {tab === 'orders' && (
        <div>
          ...
          <ListPageTemplate
            columns={[
              { key: 'transferNumber', header: 'Order #', render: (v) => <span className="font-mono">{String(v)}</span> },
              { key: 'fromWarehouseId', header: 'From', render: (v) => String(v).slice(0, 8) },
              { key: 'toWarehouseId', header: 'To', render: (v) => String(v).slice(0, 8) },
              { key: 'priority', header: 'Priority', render: (v) => <span className="px-2 py-0.5 bg-gray-100 rounded text-xs">{String(v)}</span> },
              { key: 'status', header: 'Status', render: (v) => <span className={`px-2 py-1 rounded text-xs ${STATUS_COLORS[String(v)] ?? ''}`}>{String(v)}</span> },
              { key: 'expectedDate', header: 'Expected', render: (v) => v ? new Date(String(v)).toLocaleDateString() : '-' },
              { key: '_count', header: 'Lines', render: (v) => String((v as any)?.lines ?? 0) },
              { key: 'id', header: 'Actions', render: (v, row) => (
                <div className={styles.s1}>
                  {row.status === 'DRAFT' && <button onClick={() => action(String(v), 'submit')} className="text-blue-600 underline text-xs">Submit</button>}
                  {row.status === 'PENDING_APPROVAL' && <button onClick={() => action(String(v), 'approve')} className="text-green-600 underline text-xs">Approve</button>}
                  {row.status === 'APPROVED' && <button onClick={() => action(String(v), 'ship', { shippedLines: [] })} className="text-purple-600 underline text-xs">Ship</button>}
                  {row.status === 'PARTIALLY_RECEIVED' && <button onClick={() => action(String(v), 'close-out')} className="text-orange-600 underline text-xs">Close Out</button>}
                  {!['COMPLETED', 'CANCELLED'].includes(String(row.status)) && <button onClick={() => action(String(v), 'cancel')} className="text-red-600 underline text-xs">Cancel</button>}
                </div>
              ) },
            ] as ListColumn[]}
