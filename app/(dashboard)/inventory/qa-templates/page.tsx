...
export default function QaTemplatesPage() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const data = await client.get<Template[] | { data?: Template[] }>(
        "/inventory/qa-inspection-templates",
      );
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post("/inventory/qa-inspection-templates", {
        name,
        checklist: [],
        isActive: true,
      });
  ...
  const handleRouteDisposition = async () => {
    try {
      setRouteResult(
        await client.post(
          `/inventory/qa-inspections/${inspectionId}/route-disposition`,
        ),
      );
    } catch {
  ...
  return (
    <InventoryTabLayout
      ...
      moduleDescription="Reusable inspection checklists, plus routing a resolved inspection's disposition to a real downstream action."
    >
      <div className="ui-stack-6 ui-animate-in">
        ...
        <Card className="p-5">
          ...
          {routeResult && (
            <div className={styles.s3}>Action: {routeResult.action}</div>
          )}
        </Card>
