...
function TasksTab() {
  ...
  return (
    <div>
      ...
      <ListPageTemplate
        columns={[
          { key: 'taskNumber', header: 'Task #', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'taskType', header: 'Type', render: (v) => <Badge label={String(v)} color={taskTypeColor(String(v))} /> },
          { key: 'priority', header: 'Priority' },
          { key: 'status', header: 'Status', render: (v) => <Badge label={String(v)} color={taskStatusColor(String(v))} /> },
          { key: 'sourceLocation', header: 'From → To', render: (v, row) => `${v ?? '—'} → ${row.destLocation ?? '—'}` },
          { key: 'productId', header: 'Product', render: (v) => String(v ?? '—') },
          { key: 'assignedTo', header: 'Assigned', render: (v) => String(v ?? 'Unassigned') },
        ] as ListColumn[]}
...
function BinTransfersTab() {
  ...
  return (
    <div>
      ...
      <ListPageTemplate
        columns={[
          { key: 'transferNumber', header: 'Transfer #', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'productId', header: 'Product' },
          { key: 'fromBin', header: 'From Bin', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'toBin', header: 'To Bin', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'qty', header: 'Qty', render: (v, row) => `${v} ${row.uom ?? ''}` },
          { key: 'status', header: 'Status', render: (v) => <Badge label={String(v)} color={transferStatusColor(String(v))} /> },
...
function GrnTab() {
  ...
  return (
    <div>
      ...
      <ListPageTemplate
        columns={[
          { key: 'grnNumber', header: 'GRN #', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'warehouseId', header: 'Warehouse' },
          { key: 'receivedDate', header: 'Received Date', render: (v) => new Date(String(v)).toLocaleDateString() },
          { key: 'supplierId', header: 'Supplier', render: (v) => String(v ?? '—') },
          { key: 'totalCartons', header: 'Cartons', render: (v) => String(v ?? '—') },
          { key: 'status', header: 'Status', render: (v) => <Badge label={String(v)} color={grnStatusColor(String(v))} /> },
          { key: 'vehicleNumber', header: 'Vehicle', render: (v) => String(v ?? '—') },
        ] as ListColumn[]}
...
function PackingTab() {
  ...
  return (
    <div>
      ...
      <ListPageTemplate
        columns={[
          { key: 'sessionNumber', header: 'Session #', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'workerId', header: 'Worker', render: (v) => String(v ?? '—') },
          { key: 'totalCartons', header: 'Cartons' },
          { key: 'totalWeight', header: 'Total Weight', render: (v) => v != null ? `${v} kg` : '—' },
          { key: 'status', header: 'Status', render: (v) => <Badge label={String(v)} color={packingStatusColor(String(v))} /> },
