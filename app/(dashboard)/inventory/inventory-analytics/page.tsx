...
export default function InventoryAnalyticsPage() {
  ...
  useEffect(() => {
    client.get<AnalyticsDashboard>('/inventory/analytics/dashboard').then(setOverview).catch(() => undefined);
  }, [client]);

  useEffect(() => {
    if (tab === 'health') client.get<HealthScore>('/inventory/analytics/health').then(setHealth).catch(() => undefined);
    if (tab === 'slow') client.get<SlowMovers>('/inventory/analytics/slow-moving').then(setSlow).catch(() => undefined);
    if (tab === 'dio') client.get<DIO>('/inventory/analytics/dio').then(setDio).catch(() => undefined);
    if (tab === 'fillrate') client.get<FillRate>('/inventory/analytics/fill-rate').then(setFillRate).catch(() => undefined);
    if (tab === 'trends') client.get<VolumeTrends>('/inventory/analytics/volume-trends').then(setTrends).catch(() => undefined);
    if (tab === 'shrinkage') client.get<Shrinkage>('/inventory/analytics/shrinkage').then(setShrinkage).catch(() => undefined);
    if (tab === 'capacity') client.get<CapacityRow[]>('/inventory/analytics/capacity').then(setCapacity).catch(() => undefined);
    if (tab === 'multiwarehouse') client.get<MultiWarehouse[]>('/inventory/analytics/multi-warehouse').then(setMulti).catch(() => undefined);
  }, [tab, client]);
    ...
    <div className="ui-page-shell">
        ...
        <Card className="p-4">
          ...
          <ListPageTemplate
            columns={[
              ...
              { key: 'warehouseId', header: 'Warehouse', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
              { key: 'onHandQty', header: 'On-Hand Qty', render: (v) => Number(v).toFixed(0) },
            ] as ListColumn[]}
            data={(slow.items.slice(0, 100) as unknown as Record<string, unknown>[])}
            loading={false}
        ...
        <Card className="p-4">
          ...
          <ListPageTemplate
            columns={[
              ...
              { key: 'onHand', header: 'On-Hand', render: (v) => Number(v).toFixed(0) },
              { key: 'avgDailySales', header: 'Avg Daily Sales', render: (v) => Number(v).toFixed(2) },
              { key: 'daysInventoryOutstanding', header: 'DIO', render: (v) => v != null ? Number(v).toFixed(1) : '—' },
            ] as ListColumn[]}
            data={(dio.products.filter(p => p.daysInventoryOutstanding !== null).sort((a, b) => (b.daysInventoryOutstanding ?? 0) - (a.daysInventoryOutstanding ?? 0)).slice(0, 50) as unknown as Record<string, unknown>[])}
            loading={false}
        ...
        <Card className="p-4">
          ...
          <ListPageTemplate
            columns={[
              ...
              { key: 'inbound', header: 'Inbound', render: (v) => <span className="text-green-600">{Number(v).toFixed(0)}</span> },
              { key: 'outbound', header: 'Outbound', render: (v) => <span className="text-red-600">{Number(v).toFixed(0)}</span> },
              { key: 'inbound', header: 'Net', render: (v, row) => { const net = Number(v) - Number(row.outbound); return <span className={net >= 0 ? 'text-green-600 font-medium' : 'text-red-600 font-medium'}>{net.toFixed(0)}</span>; } },
            ] as ListColumn[]}
            data={(trends.daily.slice(-30).reverse() as unknown as Record<string, unknown>[])}
            loading={false}
      ...
      {tab === 'shrinkage' && shrinkage && (
        <Card className="p-4">
          ...
          <ListPageTemplate
            columns={[
              { key: 'productId', header: 'Product ID', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
              { key: 'shrinkage', header: 'Shrinkage', render: (v) => <span className="text-red-600 font-medium">{Number(v).toFixed(2)}</span> },
            ] as ListColumn[]}
      ...
      {tab === 'capacity' && (
        <Card className="p-4">
          ...
          <ListPageTemplate
            columns={[
              { key: 'warehouse', header: 'Warehouse', render: (v, row) => String((v as any)?.name ?? row.warehouseId) },
              { key: 'warehouse', header: 'Code', render: (v) => <span className="font-mono text-xs">{String((v as any)?.code ?? '—')}</span> },
              { key: 'occupiedBins', header: 'Occupied Bins' },
              { key: 'totalQuantity', header: 'Total Qty', render: (v) => Number(v).toLocaleString() },
            ] as ListColumn[]}
      ...
      {tab === 'multiwarehouse' && (
        <Card className="p-4">
          ...
          <ListPageTemplate
            columns={[
              { key: 'warehouse', header: 'Warehouse', render: (v, row) => <span className="font-medium">{String((v as any)?.name ?? row.warehouseId)}</span> },
              { key: 'onHandQty', header: 'On-Hand', render: (v) => Number(v).toLocaleString() },
              { key: 'occupiedBins', header: 'Bins' },
              { key: 'inbound30d', header: 'Inbound', render: (v) => <span className="text-green-600">{Math.round(Number(v)).toLocaleString()}</span> },
              { key: 'outbound30d', header: 'Outbound', render: (v) => <span className="text-red-600">{Math.round(Number(v)).toLocaleString()}</span> },
