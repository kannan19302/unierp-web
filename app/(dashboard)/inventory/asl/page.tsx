...
function useFrameworkFetch() {
  ...
  return useCallback(<T,>(path: string, opts?: RequestInit) => client.request<T>(path, {
    method: opts?.method,
    body: opts?.body ? String(opts.body) : undefined,
  }), [client]);
}

const ASL_STATUSES = ['APPROVED', 'CONDITIONAL', 'DISQUALIFIED', 'PENDING_APPROVAL'];
...
function Dashboard() {
  ...
  useEffect(() => {
    apiFetch<Record<string, number>>('/inventory/asl/dashboard').then(setData).catch(console.error).finally(() => setLoading(false));
  }, []);
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
      {cards.map((c) => (
        <div key={c.label} className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
          ...
          <p className={`text-2xl font-bold mt-1 ${(c as Record<string, unknown>).danger && (c.value ?? 0) > 0 ? 'text-red-600' : (c as Record<string, unknown>).warn && (c.value ?? 0) > 0 ? 'text-orange-500' : ''}`}>{c.value}</p>
        </div>
      ))}
    </div>
  );
...
function ApprovedSuppliers() {
  ...
  const load = useCallback(() => {
    ...
    apiFetch<Record<string, unknown>[]>(`/inventory/asl${qs}`).then(setItems).catch(console.error);
  }, [filterStatus]);
  useEffect(() => { load(); }, [load]);
  ...
  async function save() {
    ...
    try {
      ...
      setMsg('Added to ASL'); setShowForm(false); setForm({}); load();
    } catch (e: unknown) { setMsg((e as Error).message); }
    finally { setSaving(false); }
  }

  async function transition(id: string, action: string, body: Record<string, unknown> = {}) {
    try {
      await apiFetch(`/inventory/asl/${id}/${action}`, { method: 'PATCH', body: JSON.stringify(body) });
      load();
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
          ...
          {ASL_STATUSES.map(s => <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>)}
        </select>
      ...
      <ListPageTemplate
        columns={[
          ...
          { key: 'vendorId', header: 'Vendor', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'vendorProductRef', header: "Vendor's SKU", render: (v) => String(v ?? '—') },
          { key: 'unitPrice', header: 'Unit Price', render: (v, row) => v ? `${row.currency} ${Number(v).toFixed(4)}` : '—' },
          { key: 'moq', header: 'MOQ', render: (v) => v ? String(v) : '—' },
          { key: 'leadTimeDays', header: 'Lead Time', render: (v) => v ? `${v}d` : '—' },
          { key: 'isPreferred', header: 'Preferred', render: (v) => v ? '★' : '' },
          { key: 'status', header: 'Status', render: (v) => statusBadge(String(v)) },
          { key: 'expiryDate', header: 'Expires', render: (v) => v ? new Date(String(v)).toLocaleDateString() : '—' },
          { key: 'id', header: 'Actions', render: (v, row) => (
            <div className={styles.s1}>
              {row.status === 'PENDING_APPROVAL' && <button onClick={() => transition(String(v), 'approve', {})} className="px-2 py-0.5 text-xs bg-green-600 text-white rounded">Approve</button>}
              {row.status === 'APPROVED' && !row.isPreferred && <button onClick={() => transition(String(v), 'set-preferred', { rank: 1 })} className="px-2 py-0.5 text-xs bg-indigo-600 text-white rounded">Set Preferred</button>}
              {row.status !== 'DISQUALIFIED' && <button onClick={() => { const r = prompt('Disqualify reason?'); if (r) transition(String(v), 'disqualify', { reason: r }); }} className="px-2 py-0.5 text-xs bg-red-600 text-white rounded">Disqualify</button>}
            </div>
          )},
        ] as ListColumn[]}
...
function Compliance() {
  ...
  const loadRules = useCallback(() => {
    apiFetch<Record<string, unknown>[]>('/inventory/asl/compliance/rules').then(setRules).catch(console.error);
  }, []);
    ...
    try {
      await apiFetch('/inventory/asl/compliance/rules', {
        ...
        body: JSON.stringify({
          ...
          minApprovedVendors: form.minApprovedVendors ? Number(form.minApprovedVendors) : 1,
          requiresQualification: form.requiresQualification === 'true',
          requiresPreferred: form.requiresPreferred === 'true',
          notes: form.notes || undefined,
        }),
      });
      setMsg('Rule saved'); setForm({}); loadRules();
    } catch (e: unknown) { setMsg((e as Error).message); }
    finally { setSaving(false); }
  }
  ...
  async function runCheck() {
    if (!checkId) return;
    try {
      const result = await apiFetch<Record<string, unknown>>(`/inventory/asl/compliance/check/${checkId}`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 space-y-3">
        ...
        {checkResult && (
          <div className={`rounded p-3 text-sm ${checkResult.compliant ? 'bg-green-50 dark:bg-green-900/20 text-green-800 dark:text-green-300' : 'bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-300'}`}>
            <p className="font-medium">{checkResult.compliant ? '✓ Compliant' : '✗ Non-Compliant'}</p>
            <p>Approved: {String(checkResult.approvedCount)} | Preferred: {String(checkResult.preferredCount)} | Expired: {String(checkResult.expiredCount)}</p>
            {(checkResult.issues as string[]).length > 0 && (
              <ul className="mt-1 list-disc list-inside">
                {(checkResult.issues as string[]).map((issue, i) => <li key={i}>{issue}</li>)}
              </ul>
            )}
          </div>
        )}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { key: 'productCategory', label: 'Product Category (blank = default)' },
          ...
          {[
            ...
            <div key={key} className="flex items-center gap-2">
              <input type="checkbox" id={key} checked={form[key] === 'true'}
                onChange={e => setForm(f => ({ ...f, [key]: String(e.target.checked) }))}
                className="rounded" />
        ...
        <ListPageTemplate
          columns={[
            { key: 'productCategory', header: 'Category', render: (v) => String(v ?? 'Default') },
            { key: 'minApprovedVendors', header: 'Min Vendors' },
            { key: 'requiresQualification', header: 'Req. Qualification', render: (v) => v ? 'Yes' : 'No' },
            { key: 'requiresPreferred', header: 'Req. Preferred', render: (v) => v ? 'Yes' : 'No' },
...
function SourcingReport() {
  ...
  async function load() {
    ...
    setLoading(true);
    try {
      const result = await apiFetch<Record<string, unknown>>(`/inventory/asl/sourcing-report/${productId}`);
      setReport(result);
    } catch (e: unknown) { alert((e as Error).message); }
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Has Preferred', value: report.hasPreferred ? 'Yes' : 'No' },
              { label: 'Lowest Price', value: report.lowestPrice !== Infinity ? `$${Number(report.lowestPrice).toFixed(4)}` : 'N/A' },
              { label: 'Shortest Lead', value: report.shortestLeadTime !== Infinity ? `${report.shortestLeadTime}d` : 'N/A' },
            ].map(c => (
  ...
  return (
    <RouteGuard permission="inventory.asl.read">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Vendor item catalog, supplier qualification, price tiers, and compliance management</p>
      </div>
