...
export default function DemandForecastingPage() {
  ...
  const [replenishments, setReplenishments] = useState<Record<string, unknown>[]>([]);
    ...
    try {
      if (tab === 'dashboard') {
        const [dash, alerts] = await Promise.all([
          ...
          client.get<Record<string, unknown>[]>('/inventory/demand-forecasting/reorder-alerts'),
        ]);
        setDashboard(dash); setReorderAlerts(alerts);
      } else if (tab === 'forecasts') {
        setForecasts(await client.get<Record<string, unknown>[]>('/inventory/demand-forecasting/forecasts'));
      } else if (tab === 'reorder-points') {
        setReorderPoints(await client.get<Record<string, unknown>[]>('/inventory/demand-forecasting/reorder-points'));
      } else if (tab === 'replenishment') {
        setReplenishments(await client.get<Record<string, unknown>[]>('/inventory/demand-forecasting/replenishment-orders'));
      } else if (tab === 'stockout') {
        setStockouts(await client.get<Record<string, unknown>[]>('/inventory/demand-forecasting/stockout-predictions'));
      } else if (tab === 'safety-stock') {
        setSafetyConfigs(await client.get<Record<string, unknown>[]>('/inventory/demand-forecasting/safety-stock'));
      }
              ...
              <div className="space-y-2">
                {reorderAlerts.map((a, i) => (
                  <div key={i} className="flex justify-between text-sm text-yellow-700 dark:text-yellow-400">
                    ...
                    <span>Stock: {String(a.currentQty ?? '')} / ROP: {String(a.reorderPoint ?? '')} {a.belowSafetyStock ? '🔴 BELOW SAFETY' : ''}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="space-y-4">
          <div className="flex gap-2">
            <button
              onClick={async () => {
                try {
                  await client.post('/inventory/demand-forecasting/forecasts/run-engine', { horizon: 30, method: 'MOVING_AVG', lookbackDays: 90 });
                  load();
                } catch (e: any) { setError(e.message); }
              }}
          ...
          <ListPageTemplate
            columns={[
              { key: 'productId', header: 'Product', render: (v) => <span className="font-mono text-xs">{String(v).slice(0, 12)}…</span> },
              { key: 'forecastDate', header: 'Date', render: (v) => new Date(String(v)).toLocaleDateString() },
              { key: 'horizon', header: 'Horizon', render: (v) => `${v}d` },
              { key: 'method', header: 'Method' },
              { key: 'forecastedQty', header: 'Forecasted Qty', render: (v) => Number(v).toFixed(2) },
              { key: 'actualQty', header: 'Actual Qty', render: (v) => v ? Number(v).toFixed(2) : '—' },
              { key: 'mape', header: 'MAPE', render: (v) => v ? `${(Number(v) * 100).toFixed(1)}%` : '—' },
              { key: 'status', header: 'Status', render: (v) => (
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                  v === 'ACTIVE' ? 'bg-green-100 text-green-700' : v === 'SUPERSEDED' ? 'bg-gray-100 text-gray-600' : 'bg-red-100 text-red-700'
                }`}>{String(v)}</span>
              ) },
            ] as ListColumn[]}
      ...
      {tab === 'reorder-points' && (
        <ListPageTemplate
          columns={[
            ...
            { key: 'reorderPoint', header: 'Reorder Point', render: (v) => Number(v).toFixed(2) },
            { key: 'reorderQty', header: 'Reorder Qty', render: (v) => Number(v).toFixed(2) },
            { key: 'safetyStock', header: 'Safety Stock', render: (v) => Number(v).toFixed(2) },
            { key: 'leadTimeDays', header: 'Lead Time', render: (v) => `${v}d` },
            { key: 'avgDailyDemand', header: 'Avg Daily Demand', render: (v) => Number(v).toFixed(2) },
            { key: 'serviceLevel', header: 'Service Level', render: (v) => `${(Number(v) * 100).toFixed(0)}%` },
            { key: 'isActive', header: 'Active', render: (v) => (
              <span className={`px-2 py-0.5 rounded-full text-xs ${v ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{v ? 'Active' : 'Inactive'}</span>
            ) },
          ] as ListColumn[]}
        ...
        <div className="space-y-4">
          <div className="flex gap-2">
            <button
              onClick={async () => {
                try {
                  const r = await client.post<{ generated: number }>('/inventory/demand-forecasting/replenishment-orders/auto-generate');
          ...
          <ListPageTemplate
            columns={[
              ...
              { key: 'suggestedQty', header: 'Suggested Qty', render: (v, row) => `${Number(v).toFixed(2)} ${row.uom ?? ''}` },
              { key: 'triggerType', header: 'Trigger' },
              { key: 'priority', header: 'Priority', render: (v) => (
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                  v === 'URGENT' ? 'bg-red-100 text-red-700' : v === 'HIGH' ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-600'
                }`}>{String(v)}</span>
              ) },
              { key: 'status', header: 'Status', render: (v) => (
                <span className={`px-2 py-0.5 rounded-full text-xs ${
                  v === 'PENDING' ? 'bg-yellow-100 text-yellow-700' : v === 'APPROVED' ? 'bg-blue-100 text-blue-700'
                  : v === 'ORDERED' ? 'bg-purple-100 text-purple-700' : v === 'RECEIVED' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'
                }`}>{String(v)}</span>
              ) },
        ...
        <div className="space-y-4">
          <div className="flex gap-2">
            <button
              onClick={async () => {
                try {
                  const r = await client.post<{ generated: number }>('/inventory/demand-forecasting/stockout-predictions/generate', { riskThresholdDays: 30 });
          ...
          <ListPageTemplate
            columns={[
              { key: 'productId', header: 'Product', render: (v) => <span className="font-mono text-xs">{String(v).slice(0, 12)}…</span> },
              { key: 'warehouseId', header: 'Warehouse' },
              { key: 'currentStock', header: 'Current Stock', render: (v) => Number(v).toFixed(2) },
              { key: 'avgDailyDemand', header: 'Avg Daily Demand', render: (v) => Number(v).toFixed(2) },
              { key: 'daysOfStock', header: 'Days of Stock', render: (v) => Number(v).toFixed(1) },
              { key: 'predictedStockoutDate', header: 'Predicted Stockout', render: (v) => v ? new Date(String(v)).toLocaleDateString() : '—' },
              { key: 'riskLevel', header: 'Risk', render: (v) => (
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                  v === 'CRITICAL' ? 'bg-red-100 text-red-700' : v === 'HIGH' ? 'bg-orange-100 text-orange-700'
                  : v === 'MEDIUM' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'
                }`}>{String(v)}</span>
              ) },
              { key: 'acknowledged', header: 'Acknowledged', render: (v) => (
                <span className={`px-2 py-0.5 rounded-full text-xs ${v ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{v ? 'Yes' : 'No'}</span>
              ) },
            ] as ListColumn[]}
      ...
      {tab === 'safety-stock' && (
        <ListPageTemplate
          columns={[
            ...
            { key: 'warehouseId', header: 'Warehouse', render: (v) => String(v ?? '—') },
            { key: 'method', header: 'Method' },
            { key: 'fixedQty', header: 'Fixed Qty', render: (v) => v ? Number(v).toFixed(2) : '—' },
            { key: 'coverageDays', header: 'Coverage Days', render: (v) => String(v ?? '—') },
            { key: 'serviceLevel', header: 'Service Level', render: (v) => v ? `${(Number(v) * 100).toFixed(0)}%` : '—' },
            { key: 'calculatedSafety', header: 'Calculated Safety', render: (v) => v ? Number(v).toFixed(2) : '—' },
            { key: 'lastRecalcAt', header: 'Last Recalc', render: (v) => v ? new Date(String(v)).toLocaleDateString() : '—' },
          ] as ListColumn[]}
