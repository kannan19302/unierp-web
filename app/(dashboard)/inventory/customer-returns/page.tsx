...
function RmasTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const d = await apiFetch<{ data: Record<string, unknown>[]; total: number }>(`${BASE}/rmas?${params}`);
  ...
  return (
    <div className="space-y-4">
      ...
      <ListPageTemplate
        columns={[
          { key: 'rmaNumber', header: 'RMA #', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'customerId', header: 'Customer', render: (v) => <span className="text-xs">{String(v)}</span> },
          { key: 'returnReason', header: 'Reason', render: (v) => <span className="text-xs truncate block max-w-xs">{String(v)}</span> },
          { key: 'status', header: 'Status', render: (v) => <Badge label={String(v)} colorMap={STATUS_COLORS} /> },
          { key: 'lines', header: 'Lines', render: (v) => <span className="text-xs text-center block">{Array.isArray(v) ? v.length : 0}</span> },
          { key: 'id', header: 'Actions', render: (v, row) => (
            <div className="flex flex-wrap gap-1">
              {row.status === 'REQUESTED' && (<><button onClick={() => action(String(v), `rmas/${v}/approve`)} className="px-2 py-1 text-xs bg-green-600 text-white rounded">Approve</button><button onClick={() => action(String(v), `rmas/${v}/reject`, 'PATCH', { reason: 'Not eligible' })} className="px-2 py-1 text-xs bg-red-600 text-white rounded">Reject</button></>)}
              {row.status === 'APPROVED' && <button onClick={() => action(String(v), `rmas/${v}/receive`, 'PATCH', { lines: [] })} className="px-2 py-1 text-xs bg-yellow-600 text-white rounded">Mark Received</button>}
              {row.status === 'INSPECTED' && <button onClick={() => action(String(v), `rmas/${v}/close`)} className="px-2 py-1 text-xs bg-gray-600 text-white rounded">Close</button>}
            </div>
          ) },
        ] as ListColumn[]}
        ...
        <div className="space-y-3">
          ...
          <ListPageTemplate
            columns={[
              ...
              { key: 'lotNumber', header: 'Lot', render: (v) => String(v ?? '—') },
              { key: 'quantityRequested', header: 'Qty Req', render: (v) => Number(v).toFixed(2) },
              { key: 'quantityReceived', header: 'Qty Recv', render: (v) => Number(v).toFixed(2) },
              { key: 'disposition', header: 'Disposition', render: (v) => v ? <Badge label={String(v)} colorMap={DISP_COLORS} /> : '—' },
              { key: 'id', header: 'Set Disposition', render: (v) => <select className="border rounded p-1 text-xs" defaultValue="" onChange={e => { if (e.target.value) inspect(String(v), e.target.value); }}><option value="">Set…</option>{dispositions.map(d => <option key={d} value={d}>{d}</option>)}</select> },
            ] as ListColumn[]}
...
function CreditsTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const d = await apiFetch<{ data: Record<string, unknown>[]; total: number }>(`${BASE}/credits`); setCredits(d.data); setTotal(d.total); }
    catch {} finally { setLoading(false); }
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  return (
    <div className="space-y-4">
      ...
      <ListPageTemplate
        columns={[
          { key: 'creditNumber', header: 'Credit #', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'rmaId', header: 'RMA', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'customerId', header: 'Customer' },
          { key: 'creditAmount', header: 'Amount', render: (v) => <span className="font-semibold">{Number(v).toLocaleString()}</span> },
          { key: 'currency', header: 'Currency' },
          { key: 'status', header: 'Status', render: (v) => <Badge label={String(v)} colorMap={CREDIT_COLORS} /> },
          { key: 'issuedAt', header: 'Issued At', render: (v) => v ? new Date(String(v)).toLocaleDateString() : '—' },
          { key: 'id', header: 'Actions', render: (v, row) => row.status === 'ISSUED' ? <button onClick={() => voidCredit(String(v))} className="px-2 py-1 text-xs border border-red-300 text-red-600 rounded hover:bg-red-50">Void</button> : null },
        ] as ListColumn[]}
...
function RestocksTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      <ListPageTemplate
        columns={[
          { key: 'rmaLineId', header: 'RMA Line', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'productId', header: 'Product', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'warehouseId', header: 'Warehouse' },
          { key: 'binLocationId', header: 'Bin', render: (v) => String(v ?? '—') },
          { key: 'quantityRestocked', header: 'Qty Restocked', render: (v) => Number(v).toFixed(2) },
          { key: 'restockedAt', header: 'Restocked At', render: (v) => new Date(String(v)).toLocaleString() },
        ] as ListColumn[]}
