...
import {
  PageHeader, Button, Spinner, StatusBadge, DashboardChart, ViewSwitcher, type ViewMode,
  Modal, StatCardRow, ListPageTemplate, DetailPageTemplate, type ListColumn,
...
import { Package, Warehouse, AlertTriangle, TrendingDown, CheckCircle } from 'lucide-react';
...
export default function InventoryPage() {
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setProductName(''); setSku(''); setSellPrice(0); setCategory('');
  ...
  const closeModal = () => { setIsCreateModalOpen(false); setModalSuccess(false); setModalError(null); };
  ...
  const productColumns: ListColumn[] = [
    ...
    { key: 'sku', header: 'SKU' },
    { key: 'sellPrice', header: 'Price', render: (v) => `$${Number(v || 0).toFixed(2)}` },
    { key: 'status', header: 'Status', render: (v) => <StatusBadge status={String(v || 'ACTIVE')} /> },
  ];
  ...
  const stockLevelColumns: ListColumn[] = [
    ...
    { key: 'currentQty', header: 'Qty' },
    {
      ...
      render: (v, row) => (
        <span className={Number((row as Record<string, unknown>).currentQty || 0) <= Number(v || 0) ? styles.dangerText : 'ui-text-muted'}>
          {String(v || 0)}
        </span>
      ),
    },
  ...
  const flatStockLevels = stockLevels.map((sl: Record<string, unknown>) => ({
    ...sl,
    productName: String((sl.product as Record<string, unknown>)?.name || sl.productId || ''),
    warehouseName: String((sl.warehouse as Record<string, unknown>)?.name || sl.warehouseId || ''),
  }));
  ...
  const tabs = [
    {
      ...
      content: (
        <ListPageTemplate
          title="" columns={productColumns}
          data={products as unknown as Record<string, unknown>[]}
          loading={loading} searchable searchPlaceholder="Search products…"
          above={<div />}
        />
      ),
    },
    {
      ...
      content: (
        <ListPageTemplate
          title="" columns={warehouseColumns}
          data={warehouses as unknown as Record<string, unknown>[]}
          loading={loading} searchable searchPlaceholder="Search warehouses…"
          above={<div />}
        />
      ),
    },
    {
      key: 'stock-levels', label: 'Stock Levels', count: stockLevels.length,
      content: (
        <ListPageTemplate
          title="" columns={stockLevelColumns}
          data={flatStockLevels as unknown as Record<string, unknown>[]}
          loading={loading} searchable searchPlaceholder="Search stock levels…"
          above={<div />}
        />
      ),
    },
  ...
  return (
    <RouteGuard permission="inventory.stock.read">
      ...
      <StatCardRow stats={[
        { label: 'Total Products', value: totalProducts, icon: <Package size={16} />, color: 'var(--chart-1)', loading },
        { label: 'Warehouses', value: totalWarehouses, icon: <Warehouse size={16} />, color: 'var(--chart-2)', loading },
        { label: 'Low Stock Items', value: lowStockCount, icon: <AlertTriangle size={16} />, color: 'var(--chart-4)', loading },
        { label: 'Total Stock Value', value: `$${totalStockValue.toLocaleString()}`, icon: <TrendingDown size={16} />, color: 'var(--chart-5)', loading },
      ]} />

      {activeView === 'chart' && (
        <div className={styles.chartGrid}>
          <DashboardChart
            title="Stock by Warehouse" subtitle="Total quantity across warehouses"
            data={stockByWarehouseData}
            config={{ xAxisKey: 'name', series: [{ dataKey: 'quantity', name: 'Quantity', color: 'var(--chart-1)' }] }}
            defaultChartType="bar" allowedChartTypes={['bar', 'pie', 'donut']} height={280} loading={loading}
          />
          <DashboardChart
            title="Low Stock Alerts" subtitle="Items at or below reorder point"
            data={lowStockData}
            config={{ xAxisKey: 'name', series: [{ dataKey: 'current', name: 'Current Qty', color: 'var(--chart-4)' }, { dataKey: 'reorderPoint', name: 'Reorder Point', color: 'var(--chart-3)' }] }}
            defaultChartType="bar" allowedChartTypes={['bar', 'composed', 'line']} height={280} loading={loading}
          />
          <DashboardChart
            title="Stock Entry Types" subtitle="Breakdown of movement types"
            data={entryTypeData}
      ...
      {activeView === 'list' && (
        <DetailPageTemplate
          title="" tabs={tabs} above={<div />}
        />
      )}

      <Modal
        open={isCreateModalOpen} onClose={closeModal}
        title="Add Product"
        footer={
          !modalSuccess ? (
            <>
              <Button variant="secondary" type="button" onClick={closeModal}>Cancel</Button>
              <Button variant="primary" type="submit" form="create-product-form" disabled={submitting}>
      ...
      >
        {modalSuccess ? (
          ...
          <form id="create-product-form" onSubmit={handleCreateProduct} className="ui-stack-4">
            {modalError && (
              <div className={styles.errorBox}>
                {modalError}
              </div>
            )}
            <TextField label="Product Name" required value={productName} onChange={(e) => setProductName(e.target.value)} />
            <div className="ui-grid-2 ui-gap-3">
              <TextField label="SKU" required value={sku} onChange={(e) => setSku(e.target.value)} />
              <FormField label="Sell Price">
                <Input type="number" step="0.01" value={sellPrice} onChange={(e) => setSellPrice(parseFloat(e.target.value) || 0)} />
              </FormField>
            </div>
