...
function LotsTab() {
  ...
  const load = useCallback(() => {
    apiFetch<{ items: any[]; total: number }>('/lots').then(r => { setLots(r.items); setTotal(r.total); }).catch(console.error);
  }, []);
  useEffect(load, [load]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <ListPageTemplate
          columns={[
            { key: 'lotNumber', header: 'Lot #', render: (v) => <span className="font-mono">{String(v)}</span> },
            { key: 'productId', header: 'Product' },
            { key: 'warehouseId', header: 'Warehouse' },
            { key: 'remainingQty', header: 'Qty Rem.' },
            { key: 'expiryDate', header: 'Expiry', render: (v) => new Date(String(v)).toLocaleDateString() },
            { key: 'status', header: 'Status', render: (v) => <span className={`px-2 py-0.5 rounded text-xs ${statusBadge[String(v)] ?? ''}`}>{String(v)}</span> },
            { key: 'id', header: 'Actions', render: (v, row) => (
              <div className={styles.s1}>
                {row.status === 'ACTIVE' && <button onClick={() => quarantine(String(v))} className="text-xs text-yellow-600 hover:underline">Quarantine</button>}
                {row.status === 'QUARANTINE' && <button onClick={() => release(String(v))} className="text-xs text-green-600 hover:underline">Release</button>}
              </div>
            )},
          ] as ListColumn[]}
...
function FEFOTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {result && (
        <div>
          <p className="text-sm font-medium mb-2">Pick Plan — Total: {result.totalQty}</p>
          <ListPageTemplate
            columns={[
              { key: 'lotNumber', header: 'Lot #', render: (v) => <span className="font-mono">{String(v)}</span> },
              { key: 'expiryDate', header: 'Expiry Date', render: (v) => new Date(String(v)).toLocaleDateString() },
              { key: 'qty', header: 'Pick Qty', render: (v) => <strong>{String(v)}</strong> },
            ] as ListColumn[]}
...
function AlertsTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      <ListPageTemplate
        columns={[
          { key: 'alertLevel', header: 'Level', render: (v) => <span className={`px-2 py-0.5 rounded text-xs ${levelColor[String(v)] ?? ''}`}>{String(v)}</span> },
          { key: 'daysToExpiry', header: 'Days to Expiry' },
          { key: 'alertedAt', header: 'Alerted At', render: (v) => new Date(String(v)).toLocaleString() },
          { key: 'id', header: 'Action', render: (v) => <button onClick={() => dismiss(String(v))} className="text-xs text-gray-500 hover:underline">Dismiss</button> },
        ] as ListColumn[]}
...
function DisposalsTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <ListPageTemplate
        columns={[
          { key: 'disposalNumber', header: 'Disposal #', render: (v) => <span className="font-mono">{String(v)}</span> },
          { key: 'disposalMethod', header: 'Method' },
          { key: 'qtyDisposed', header: 'Qty' },
          { key: 'reason', header: 'Reason' },
          { key: 'disposedAt', header: 'Date', render: (v) => new Date(String(v)).toLocaleDateString() },
        ] as ListColumn[]}
