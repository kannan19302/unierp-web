...
const inspectionColumns: ListColumn[] = [
    ...
    render: (v, row) => {
      ...
      return (
        <div className="ui-flex-col">
          ...
          <span className="ui-text-micro ui-text-muted">{insp.product?.sku}</span>
        </div>
      );
    },
  },
  ...
  },
  {
    ...
    render: (v) => (
      <Badge variant={v === 'PASS' ? 'success' : v === 'PENDING' ? 'warning' : 'danger'}>{String(v)}</Badge>
    ),
  },
];
...
export default function QaInspectionsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [pRes, qaRes] = await Promise.all([
        ...
        client.get<QualityInspection[] | { data?: QualityInspection[] }>('/inventory/qa-inspections')
      ]);
  ...
  const handleSubmitPerform = async (e: React.FormEvent) => {
    ...
    try {
      await client.post(`/inventory/qa-inspections/${activeInspection.id}/submit`, {
          status: submitStatus,
        ...
        <div className={styles.s3}>
          <div className={`ui-card modal-card ${styles.s4}`} >
            <div className={styles.s5}>
              <span className="ui-heading-base">Create Inspection Checkpoints Record</span>
              <button onClick={() => setIsCreateModalOpen(false)} className="ui-btn-icon ui-text-muted">Close</button>
            </div>
            <div className="ui-card-body p-5">
              <form onSubmit={handleCreateQAInspection} className="ui-stack-4">
                <div className="ui-grid-2">
                  <div className="ui-form-group">
                    <label className="ui-label">Reference Document Type</label>
                    <select className="ui-input" value={qaRefType} onChange={e => setQaRefType(e.target.value as any)}>
                      <option value="STOCK_ENTRY">Stock Entry</option>
                      <option value="PURCHASE_RECEIPT">Purchase Receipt</option>
                    </select>
                  </div>
                  <div className="ui-form-group">
                    <label className="ui-label">Reference ID (UUID/Code)</label>
                    <input type="text" className="ui-input" value={qaRefId} onChange={e => setQaRefId(e.target.value)} required />
                  </div>
                </div>
                ...
                <div className="ui-grid-2">
                  <div className="ui-form-group">
                    ...
                    <select className="ui-input" value={qaProduct} onChange={e => setQaProduct(e.target.value)} required>
                      {products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                    </select>
                  </div>
                  <div className="ui-form-group">
                    <label className="ui-label">Inspected Lot Qty</label>
                    <input type="number" className="ui-input" value={qaInsQty} onChange={e => setQaInsQty(Number(e.target.value))} required />
                  </div>
                </div>

                <div className={styles.s6}>
                  <div className="ui-flex-between mb-2">
                    <span className={styles.s7}>QA Inspection Criteria checklist</span>
                    <Button variant="outline" type="button" onClick={() => setQaCheckpoints([...qaCheckpoints, { parameter: '', criteria: '', sortOrder: qaCheckpoints.length }])} className={styles.s2}>Add Row</Button>
                  </div>
                  {qaCheckpoints.map((cp, idx) => (
                    <div key={idx} className={styles.s8}>
                      <input type="text" className="flex-1" placeholder="Param (e.g. Dimensions)" value={cp.parameter} onChange={e => {
                      ...
                      <input type="text" className="flex-1" placeholder="Criteria (e.g. +/- 0.5mm)" value={cp.criteria} onChange={e => {
                        const updated = [...qaCheckpoints];
                        if (updated[idx]) {
        ...
        <div className={styles.s3}>
          <div className={`ui-card modal-card ${styles.s10}`} >
            <div className={styles.s11}>
              <span className="ui-heading-base">Perform Quality Check: {activeInspection.inspectionNumber}</span>
              <button onClick={() => setActiveInspection(null)} className="ui-btn-icon ui-text-muted">Close</button>
            </div>
            <div className="ui-card-body p-5">
              <form onSubmit={handleSubmitPerform} className="ui-stack-4">
                  ...
                  {activeInspection.checkpoints.map((cp) => (
                    <div key={cp.id} className={styles.s15}>
                      <div className="ui-flex-between">
                        ...
                        <select className={`ui-input ${styles.s16}`}  value={checkpointResults[cp.id]?.result} onChange={e => {
                      ...
                      <div className={styles.s17}>
                        <input type="text" className={styles.s18} placeholder="Observed Value" value={checkpointResults[cp.id]?.observedValue} onChange={e => {
                          setCheckpointResults({
                            ...checkpointResults,
                            [cp.id]: { ...checkpointResults[cp.id]!, observedValue: e.target.value }
                          });
                        }} />
                        <input type="text" className={styles.s18} placeholder="Remarks/Deviation" value={checkpointResults[cp.id]?.remarks} onChange={e => {
                          setCheckpointResults({
                            ...checkpointResults,
                ...
                <div className="ui-grid-3">
                  <div className="ui-form-group">
                    ...
                    <select className="ui-input" value={submitStatus} onChange={e => setSubmitStatus(e.target.value as any)}>
                      <option value="PASS">PASS</option>
                      <option value="FAIL">FAIL</option>
                  ...
                  <div className="ui-form-group">
                    ...
                    <input type="number" className="ui-input" value={submitAcceptedQty} onChange={e => setSubmitAcceptedQty(Number(e.target.value))} required />
                  </div>
                  <div className="ui-form-group">
                    <label className="ui-label">Rejected Qty</label>
                    <input type="number" className="ui-input" value={submitRejectedQty} onChange={e => setSubmitRejectedQty(Number(e.target.value))} required />
                  </div>
                </div>

                <div className="ui-form-group">
                  <label className="ui-label">Disposition Verdict / Action</label>
                  <input type="text" className="ui-input" value={submitDisposition} onChange={e => setSubmitDisposition(e.target.value)} placeholder="e.g. Scrapped, Restocked, Quarantine" />
                </div>
