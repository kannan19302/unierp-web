...
function DashboardTab() {
  ...
  const cards = [
    ...
    ["Draft Agreements", stats.draftAgreements],
...
function AgreementsTab() {
  ...
  const doAction = async (id: string, action: string) => {
    try {
      await apiFetch(`/agreements/${id}/${action}`, { method: "PATCH" });
      load();
    } catch (err: any) {
      setMsg(err.message);
    }
        ...
        <div className="grid grid-cols-3 gap-3">
          {(
            ...
            <div key={key}>
              ...
              <input
                type={
                  ["minQty", "maxQty", "targetQty"].includes(key)
                    ? "number"
                    : "text"
                }
              ...
              },
              {
                ...
                render: (v, row) => (
                  <div className={styles.s1}>
                    {row.status === "DRAFT" && (
                      <button
                        onClick={() => doAction(String(v), "activate")}
                    ...
                    {row.status === "ACTIVE" && (
                      <button
                        onClick={() => doAction(String(v), "suspend")}
                    ...
                    {row.status !== "TERMINATED" && (
                      <button
                        onClick={() => doAction(String(v), "terminate")}
...
function SnapshotsTab() {
  ...
  const load = useCallback(() => {
    ...
    apiFetch<any[]>(`/snapshots?agreementId=${agreementId}`)
      .then(setSnapshots)
      .catch(console.error);
  }, [agreementId]);
  useEffect(load, [load]);
    ...
    try {
      await apiFetch("/snapshots", {
        ...
        body: JSON.stringify({
          ...
          onOrderQty: form.onOrderQty ? +form.onOrderQty : 0,
          notes: form.notes,
        }),
      });
      setMsg("Snapshot recorded");
      setForm({ onHandQty: "", onOrderQty: "", notes: "" });
      load();
    } catch (err: any) {
      setMsg(err.message);
    }
