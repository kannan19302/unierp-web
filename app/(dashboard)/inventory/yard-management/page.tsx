...
export default function YardManagementPage() {
  ...
  const loadDashboard = useCallback(async () => {
    try {
      setDashboard(await apiFetch("/inventory/yard-management/dashboard"));
    } catch {
      /* ignore */
    }
  }, []);
  ...
  const loadDoors = useCallback(async () => {
    setLoading(true);
    try {
      setDoors(await apiFetch("/inventory/yard-management/dock-doors"));
    } catch (e: unknown) {
      setError(String(e));
    } finally {
      setLoading(false);
  ...
  const loadAppointments = useCallback(async () => {
    ...
    try {
      setAppointments(
        await apiFetch("/inventory/yard-management/appointments"),
      );
    } catch (e: unknown) {
      setError(String(e));
    } finally {
  ...
  const loadMoves = useCallback(async () => {
    setLoading(true);
    try {
      setMoves(await apiFetch("/inventory/yard-management/yard-moves"));
    } catch (e: unknown) {
      setError(String(e));
    } finally {
      setLoading(false);
  ...
  const loadYardInventory = useCallback(async () => {
    ...
    try {
      setYardInventory(
        await apiFetch("/inventory/yard-management/yard-inventory"),
      );
    } catch (e: unknown) {
      setError(String(e));
    } finally {
  ...
  const createDoor = async () => {
    try {
      await apiFetch("/inventory/yard-management/dock-doors", {
        method: "POST",
        body: JSON.stringify(doorForm),
      });
  ...
  const createMove = async () => {
    try {
      await apiFetch("/inventory/yard-management/yard-moves", {
        method: "POST",
  ...
  const apptAction = async (id: string, action: string, body?: unknown) => {
    try {
      await apiFetch(
        `/inventory/yard-management/appointments/${id}/${action}`,
        {
          method: "PATCH",
  ...
  const moveAction = async (id: string, action: string) => {
    try {
      await apiFetch(`/inventory/yard-management/yard-moves/${id}/${action}`, {
        method: "PATCH",
      });
      loadMoves();
      loadDashboard();
                ...
                {[
                  {
                    ...
                    value: dashboard.doors.available,
                    sub: `${dashboard.doors.total} total`,
                  },
                  {
                    label: "Occupied Doors",
                    value: dashboard.doors.occupied,
                    sub: "loading/unloading",
                  },
                  {
                    label: "Active Appointments",
                    value:
                      dashboard.appointments.checkedIn +
                      dashboard.appointments.loading,
                    sub: `${dashboard.appointments.scheduled} scheduled`,
                  },
                  {
                    label: "Yard Trailers",
              ...
              <div className="bg-white border rounded-lg p-4">
                ...
                <div className="grid grid-cols-5 gap-3">
                  {[
                    {
                      label: "Scheduled",
                      value: dashboard.appointments.scheduled,
                      color: "bg-blue-50 text-blue-700",
                    },
                    {
                      label: "Checked In",
                      value: dashboard.appointments.checkedIn,
                      color: "bg-purple-50 text-purple-700",
                    },
                    {
                      label: "Loading",
                      value: dashboard.appointments.loading,
                      color: "bg-orange-50 text-orange-700",
                    },
                    {
                      label: "Today Completed",
                      value: dashboard.appointments.todayComplete,
                  ...
                  {doors.map((d) => (
                    <div key={d.id} className="border rounded-lg p-4">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <div className="text-sm text-gray-500">
                            {d.doorType} · {d.warehouseId.slice(0, 8)}
                          </div>
                        </div>
                        <span
                          className={`px-2 py-1 rounded text-xs ${STATUS_COLORS[d.status] ?? "bg-gray-100"}`}
                        >
                          {d.status}
                        </span>
                      </div>
                      ...
                      <div className="mt-2 text-xs text-gray-400">
                        {d._count?.appointments ?? 0} appointments
                      </div>
                    </div>
                  ))}
                  {doors.length === 0 && (
                    <div className="col-span-3 text-center text-gray-400 py-8">
                      No dock doors configured
                    </div>
                  )}
                </div>
              )}
                        ...
                        <div className={styles.s1}>
                          {row.status === "SCHEDULED" && (
                            <>
                              ...
                              <button
                                onClick={() => apptAction(String(v), "no-show")}
                                className="text-gray-500 underline text-xs"
                              >
                                No Show
                              </button>
                              <button
                                onClick={() => apptAction(String(v), "cancel")}
                          ...
                          {row.status === "CHECKED_IN" && (
                            <>
                              <button
                                onClick={() =>
                                  apptAction(String(v), "start-loading")
                                }
                  ...
                  {moves.map((m) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        <span
                          className={`px-2 py-1 rounded text-xs ${STATUS_COLORS[m.status] ?? ""}`}
                        >
                          {m.status}
                        </span>
                        {m.status === "PENDING" && (
                          <button
                            onClick={() => moveAction(m.id, "start")}
                        ...
                        {m.status === "IN_PROGRESS" && (
                          <button
                            onClick={() => moveAction(m.id, "complete")}
                            className="text-green-600 underline text-xs"
                          >
                            Complete
                          </button>
                        ...
                        {!["COMPLETE", "CANCELLED"].includes(m.status) && (
                          <button
                            onClick={() => moveAction(m.id, "cancel")}
                            className="text-red-600 underline text-xs"
                          >
                            Cancel
                          </button>
          ...
          {tab === "yard-inventory" && (
            <div>
              <h2 className="text-lg font-medium mb-4">
                Yard Inventory (Trailers on Site)
              </h2>
              <ListPageTemplate
                columns={
                  [
                      ...
                      render: (v) => (
                        <button
                          onClick={async () => {
                            try {
                              await apiFetch(
                                `/inventory/yard-management/yard-inventory/${v}/depart`,
                                { method: "PATCH" },
                              );
                              loadYardInventory();
                            } catch (e: unknown) {
