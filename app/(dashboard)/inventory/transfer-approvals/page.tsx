...
const makeColumns = (
...
): ListColumn[] => [
  {
    ...
    render: (v, row) => {
      ...
      return <span className="font-mono">{a.stockEntry?.entryNumber || a.stockEntryId}</span>;
    },
  },
...
export default function TransferApprovalsPage() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const data = await client.get<unknown>('/inventory/transfer-approvals/pending');
      const items = Array.isArray(data) ? data : data && typeof data === 'object' && 'data' in data ? (data as { data?: unknown }).data : [];
  ...
  const handleReject = async (id: string) => {
    ...
    if (!reason) return;
    try {
      await client.post(`/inventory/transfer-approvals/${id}/reject`, { reason });
      loadData();
    } catch {
  ...
  const handleCreateRule = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post('/inventory/transfer-approval-rules', { warehouseId: warehouseId || null, thresholdValue, isActive: true });
  ...
  return (
    ...
    <div className="ui-stack-6 ui-animate-in">
      <PageHeader
        ...
        actions={
          <Button variant="primary" onClick={() => setIsRuleModalOpen(true)} className="ui-hstack-2">
        ...
        <div className={styles.s5}>
          <div className={`ui-card modal-card ${styles.s6}`} >
            <div className={styles.s7}>
              ...
              <button onClick={() => setIsRuleModalOpen(false)} className="ui-btn-icon ui-text-muted">Close</button>
            </div>
            <div className="ui-card-body p-5">
              <form onSubmit={handleCreateRule} className="ui-stack-4">
                <div className="ui-form-group">
                  <label className="ui-label">Warehouse ID (blank = tenant-wide)</label>
                  <input type="text" className="ui-input" value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)} />
                </div>
                <div className="ui-form-group">
                  <label className="ui-label">Threshold Value *</label>
                  <input type="number" className="ui-input" value={thresholdValue} onChange={(e) => setThresholdValue(Number(e.target.value))} required min={0} />
                </div>
