...
export default function DockSchedulingPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [aRes, wRes] = await Promise.all([
        ...
        client.get<Array<{ id: string; name: string }>>('/inventory/warehouses'),
      ]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post('/inventory/dock-appointments', { warehouseId, dockDoor, type, carrierName, scheduledAt, durationMinutes: 60 });
  ...
  const columns: ListColumn[] = [
    { key: 'dockDoor', header: 'Dock Door', render: (v) => <span className="font-mono">{String(v)}</span> },
    ...
    { key: 'scheduledAt', header: 'Scheduled', render: (v) => new Date(String(v)).toLocaleString() },
    {
      ...
      render: (v) => {
        const s = String(v);
        return <Badge variant={s === 'COMPLETED' ? 'success' : s === 'CHECKED_IN' ? 'info' : 'warning'}>{s}</Badge>;
      },
    },
    {
      key: 'id',
      ...
      render: (_, row) => {
        ...
        return (
          <div className={styles.s1}>
            {a.status === 'SCHEDULED' && (
              <button onClick={() => handleCheckIn(a.id)} className={`ui-btn ui-btn-primary ${styles.s2}`} >Check In</button>
            )}
            {a.status === 'CHECKED_IN' && (
              <button onClick={() => handleComplete(a.id)} className={`ui-btn ui-btn-primary ${styles.s3}`} >Complete</button>
            )}
          </div>
        );
      },
            ...
            <div className="ui-card-body p-5">
              <form onSubmit={handleCreate} className="ui-stack-4">
                <div className="ui-form-group">
                  ...
                  <select className="ui-input" value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)} required>
                    {warehouses.map((w) => (
                ...
                <div className="ui-form-group">
                  ...
                  <input type="text" className="ui-input" value={dockDoor} onChange={(e) => setDockDoor(e.target.value)} required />
                </div>
                <div className="ui-form-group">
                  <label className="ui-label">Type *</label>
                  <select className="ui-input" value={type} onChange={(e) => setType(e.target.value as typeof type)}>
                ...
                <div className="ui-form-group">
                  ...
                  <input type="text" className="ui-input" value={carrierName} onChange={(e) => setCarrierName(e.target.value)} required />
                </div>
                <div className="ui-form-group">
                  <label className="ui-label">Scheduled Time *</label>
                  <input type="datetime-local" className="ui-input" value={scheduledAt} onChange={(e) => setScheduledAt(e.target.value)} required />
                </div>
