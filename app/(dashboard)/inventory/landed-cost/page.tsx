...
export default function LandedCostPage() {
  ...
  const loadDashboard = useCallback(async () => {
    try {
      setDashboard(await apiFetch("/inventory/landed-cost/dashboard"));
    } catch {
      /* ignore */
    }
  }, []);
  ...
  const loadVouchers = useCallback(async () => {
    setLoading(true);
    try {
      setVouchers(await apiFetch("/inventory/landed-cost/vouchers"));
    } catch (e: unknown) {
      setError(String(e));
    } finally {
      setLoading(false);
  ...
  const loadChargeLines = useCallback(async (vid: string) => {
    try {
      setChargeLines(
        await apiFetch(`/inventory/landed-cost/vouchers/${vid}/charge-lines`),
      );
    } catch {
      /* ignore */
    }
  ...
  const loadAllocations = useCallback(async () => {
    try {
      const r = await apiFetch<{ allocations?: Allocation[] }>(
        "/inventory/landed-cost/allocation-report",
      );
  ...
  useEffect(() => {
    if (tab === "charge-lines" && selectedVoucher)
      loadChargeLines(selectedVoucher.id);
  }, [tab, selectedVoucher, loadChargeLines]);
  useEffect(() => {
  ...
  const createVoucher = async () => {
    try {
      await apiFetch("/inventory/landed-cost/vouchers", {
        method: "POST",
  ...
  const submitVoucher = async (id: string) => {
    try {
      await apiFetch(`/inventory/landed-cost/vouchers/${id}/submit`, {
        method: "PATCH",
      });
      loadVouchers();
    } catch (e: unknown) {
  ...
  const allocate = async (id: string) => {
    try {
      await apiFetch(`/inventory/landed-cost/vouchers/${id}/allocate`, {
        method: "PATCH",
      });
      loadVouchers();
      loadDashboard();
  ...
  const cancelVoucher = async (id: string) => {
    try {
      await apiFetch(`/inventory/landed-cost/vouchers/${id}/cancel`, {
        method: "PATCH",
      });
      loadVouchers();
    } catch (e: unknown) {
  ...
  const addChargeLine = async () => {
    ...
    try {
      await apiFetch(
        `/inventory/landed-cost/vouchers/${selectedVoucher.id}/charge-lines`,
        {
          method: "POST",
          body: JSON.stringify({
            ...chargeForm,
  ...
  const removeChargeLine = async (lineId: string) => {
    ...
    try {
      await apiFetch(
        `/inventory/landed-cost/vouchers/${selectedVoucher.id}/charge-lines/${lineId}`,
        { method: "DELETE" },
      );
      loadChargeLines(selectedVoucher.id);
    } catch (e: unknown) {
  ...
  const voucherColumns: ListColumn[] = [
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return (
          <span
            className={`px-2 py-1 rounded text-xs ${STATUS_COLORS[v.status] ?? ""}`}
          >
            {v.status}
          </span>
        );
      ...
      render: (row) => {
        ...
        return (
          <div className="flex gap-2 flex-wrap">
            ...
            {v.status !== "ALLOCATED" && v.status !== "CANCELLED" && (
              <button
