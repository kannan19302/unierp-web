...
export default function StockValuationPage() {
  ...
  const loadDashboard = useCallback(async () => {
    try { setDashboard(await apiFetch<Dashboard>('/inventory/stock-valuation/dashboard')); } catch { /* ignore */ }
  }, []);
  const loadPolicies = useCallback(async () => {
    setLoading(true);
    try { setPolicies(await apiFetch<Policy[]>('/inventory/stock-valuation/policies')); } catch (e: unknown) { setError(String(e)); } finally { setLoading(false); }
  }, []);
  const loadAdjustments = useCallback(async () => {
    setLoading(true);
    try { setAdjustments(await apiFetch<Adjustment[]>('/inventory/stock-valuation/adjustments')); } catch (e: unknown) { setError(String(e)); } finally { setLoading(false); }
  }, []);
  const loadRevaluations = useCallback(async () => {
    setLoading(true);
    try { setRevaluations(await apiFetch<Revaluation[]>('/inventory/stock-valuation/revaluations')); } catch (e: unknown) { setError(String(e)); } finally { setLoading(false); }
  }, []);
  const loadLedger = useCallback(async () => {
    setLoading(true);
    try { setLedger(await apiFetch<unknown[]>('/inventory/stock-valuation/ledger?limit=50')); } catch (e: unknown) { setError(String(e)); } finally { setLoading(false); }
  }, []);
  const loadSummary = useCallback(async () => {
    setLoading(true);
    try { setSummary(await apiFetch<{ products: unknown[]; totalValue: number }>('/inventory/stock-valuation/valuation-summary')); } catch (e: unknown) { setError(String(e)); } finally { setLoading(false); }
  }, []);
    ...
    <div className="ui-page-shell">
      ...
      {tab === 'policies' && (
        <div>
          ...
          <ListPageTemplate
            columns={[
              { key: 'productId', header: 'Product', render: (v) => String(v ?? 'All') },
              { key: 'warehouseId', header: 'Warehouse', render: (v) => String(v ?? 'All') },
              { key: 'method', header: 'Method', render: (v) => <span className="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs">{String(v)}</span> },
              { key: 'standardCost', header: 'Standard Cost', render: (v) => v ? Number(v).toFixed(4) : '-' },
              { key: 'currency', header: 'Currency' },
              { key: 'isActive', header: 'Active', render: (v) => v ? <span className="text-green-600">✓</span> : <span className="text-red-500">✗</span> },
              { key: 'effectiveFrom', header: 'Effective From', render: (v) => new Date(String(v)).toLocaleDateString() },
            ] as ListColumn[]}
        ...
        <div>
          ...
          <ListPageTemplate
            columns={[
              ...
              { key: 'oldUnitCost', header: 'Old Cost', render: (v) => Number(v).toFixed(4) },
              { key: 'newUnitCost', header: 'New Cost', render: (v) => Number(v).toFixed(4) },
              { key: 'qty', header: 'Qty', render: (v) => Number(v).toFixed(2) },
              { key: 'impactAmount', header: 'Impact', render: (v) => <span className={Number(v) >= 0 ? 'text-green-600' : 'text-red-600'}>{Number(v) >= 0 ? '+' : ''}{Number(v).toFixed(2)}</span> },
              { key: 'status', header: 'Status', render: (v) => <span className={`px-2 py-1 rounded text-xs ${STATUS_COLORS[String(v)] ?? ''}`}>{String(v)}</span> },
              { key: 'id', header: 'Actions', render: (v, row) => (
                <div className={styles.s1}>
                  {row.status === 'PENDING' && <button onClick={() => adjAction(String(v), 'approve')} className="text-blue-600 underline text-xs">Approve</button>}
                  {row.status === 'APPROVED' && <button onClick={() => adjAction(String(v), 'post')} className="text-green-600 underline text-xs">Post</button>}
                  {row.status === 'PENDING' && <button onClick={() => adjAction(String(v), 'reject')} className="text-red-600 underline text-xs">Reject</button>}
                </div>
              ) },
            ] as ListColumn[]}
                ...
                <div key={r.id} className="border rounded-lg p-4">
                  ...
                  {r.lines.length > 0 && (
                    <ListPageTemplate
                      columns={[
                        { key: 'productId', header: 'Product' },
                        { key: 'currentUnitCost', header: 'Current Cost', render: (v) => Number(v).toFixed(4) },
                        { key: 'newUnitCost', header: 'New Cost', render: (v) => Number(v).toFixed(4) },
                        { key: 'impactAmount', header: 'Impact', render: (v) => <span className={Number(v) >= 0 ? 'text-green-600' : 'text-red-600'}>{Number(v) >= 0 ? '+' : ''}{Number(v).toFixed(2)}</span> },
                      ] as ListColumn[]}
        ...
        <div>
          ...
          <ListPageTemplate
            columns={[
              ...
              { key: 'transactionRef', header: 'Ref', render: (v) => <span className="font-mono text-xs">{String(v ?? '')}</span> },
              { key: 'qty', header: 'Qty', render: (v) => Number(v).toFixed(2) },
              { key: 'unitCost', header: 'Unit Cost', render: (v) => Number(v).toFixed(4) },
              { key: 'totalCost', header: 'Total Cost', render: (v) => Number(v).toFixed(2) },
              { key: 'runningQty', header: 'Running Qty', render: (v) => Number(v).toFixed(2) },
              { key: 'runningValue', header: 'Running Value', render: (v) => Number(v).toFixed(2) },
              { key: 'runningAvgCost', header: 'Avg Cost', render: (v) => Number(v ?? 0).toFixed(4) },
            ] as ListColumn[]}
        ...
        <div>
          ...
          <ListPageTemplate
            columns={[
              ...
              { key: 'method', header: 'Method', render: (v) => <span className="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs">{String(v ?? '')}</span> },
              { key: 'qty', header: 'Qty', render: (v) => Number(v).toFixed(2) },
              { key: 'value', header: 'Value', render: (v) => `$${Number(v).toLocaleString(undefined, { minimumFractionDigits: 2 })}` },
              { key: 'avgCost', header: 'Avg Cost', render: (v) => Number(v).toFixed(4) },
            ] as ListColumn[]}
