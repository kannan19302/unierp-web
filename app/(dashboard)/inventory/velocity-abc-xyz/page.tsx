...
function useFrameworkFetch() {
  ...
  return useCallback(<T,>(path: string, opts?: RequestInit) => client.request<T>(path.replace('/api', ''), {
    method: opts?.method,
  ...
  return (
    <div className="space-y-6">
      <StatCardRow stats={[
        ...
        { label: 'Active Run Products', value: data.activeRun?.totalProducts ?? '—' },
...
function RunsTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const d = await apiFetch<{ data: any[]; total: number }>(`${BASE}/runs`); setRuns(d.data); setTotal(d.total); } catch {}
    finally { setLoading(false); }
  }, []);
  useEffect(() => { load(); }, [load]);
    ...
    <div className="space-y-4">
      ...
      {(() => {
        const runsColumns: ListColumn[] = [
          { key: 'runNumber', header: 'Run #', render: (v) => <span className="font-mono text-xs">{String(v)}</span> },
          { key: 'periodStart', header: 'Period', render: (v, row) => <span className="text-xs">{String(v).slice(0, 10)} – {String(row.periodEnd).slice(0, 10)}</span> },
          { key: 'warehouseId', header: 'Warehouse', render: (v) => <span className="text-xs">{String(v ?? 'All')}</span> },
          { key: 'status', header: 'Status', render: (v) => { const s = String(v); return <span className={`px-2 py-0.5 rounded text-xs font-semibold ${s === 'ACTIVE' ? 'bg-green-100 text-green-800' : s === 'DRAFT' ? 'bg-gray-100 text-gray-700' : 'bg-yellow-100 text-yellow-800'}`}>{s}</span>; } },
          { key: 'totalProducts', header: 'Products' },
          { key: 'classACount', header: 'A/B/C', render: (v, row) => <span className="text-xs">{String(v)}/{String(row.classBCount)}/{String(row.classCCount)}</span> },
          { key: 'classXCount', header: 'X/Y/Z', render: (v, row) => <span className="text-xs">{String(v)}/{String(row.classYCount)}/{String(row.classZCount)}</span> },
          { key: 'id', header: 'Actions', render: (v, row) => (
            <div className="flex gap-1">
              {row.status === 'DRAFT' && Number(row.totalProducts) > 0 && (
                <button onClick={() => activate(String(v))} className="px-2 py-1 text-xs bg-green-600 text-white rounded hover:bg-green-700">Activate</button>
              )}
              {row.status !== 'ACTIVE' && (
                <button onClick={() => deleteRun(String(v))} className="px-2 py-1 text-xs border border-red-300 text-red-600 rounded hover:bg-red-50">Delete</button>
              )}
            </div>
          ) },
        ];
        return <ListPageTemplate columns={runsColumns} data={runs as unknown as Record<string, unknown>[]} loading={loading} searchable />;
      })()}
    </div>
  );
}
...
function ItemsTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const d = await apiFetch<{ data: any[]; total: number }>(`${BASE}/runs/${runId}/items?${params}`);
    ...
    <div className="space-y-4">
      ...
      {!runId ? (
        ...
        const itemsColumns: ListColumn[] = [
          ...
          { key: 'warehouseId', header: 'Warehouse', render: (v) => <span className="text-xs">{String(v ?? 'All')}</span> },
          { key: 'totalRevenue', header: 'Revenue', render: (v) => <span className="text-xs">{Number(v).toLocaleString()}</span> },
          { key: 'totalQuantitySold', header: 'Qty Sold', render: (v) => <span className="text-xs">{Number(v).toLocaleString()}</span> },
          { key: 'revenueShare', header: 'Rev Share', render: (v) => <span className="text-xs">{(Number(v) * 100).toFixed(2)}%</span> },
          { key: 'cumulativeShare', header: 'Cum Share', render: (v) => <span className="text-xs">{(Number(v) * 100).toFixed(2)}%</span> },
          { key: 'abcClass', header: 'ABC', render: (v) => <Badge label={String(v)} colorMap={ABC_COLORS} /> },
          { key: 'xyzClass', header: 'XYZ', render: (v) => <Badge label={String(v)} colorMap={XYZ_COLORS} /> },
          { key: 'combinedClass', header: 'Class', render: (v) => <span className="font-mono font-bold text-sm">{String(v)}</span> },
          { key: 'demandCv', header: 'CV', render: (v) => <span className="text-xs">{v != null ? Number(v).toFixed(3) : '—'}</span> },
        ];
        return (
          <>
            <p className="text-sm text-gray-500 mb-2">{total} item(s)</p>
            <ListPageTemplate columns={itemsColumns} data={items as unknown as Record<string, unknown>[]} loading={loading} searchable />
          </>
        );
      })()}
    </div>
...
function PoliciesTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      <ListPageTemplate
        columns={[
          { key: 'combinedClass', header: 'Class', render: (v) => <span className="font-mono font-bold">{String(v)}</span> },
          { key: 'reviewFrequency', header: 'Review Frequency' },
          { key: 'reorderMethod', header: 'Reorder Method' },
          { key: 'safetyStockMultiplier', header: 'Safety Stock ×', render: (v) => `${Number(v).toFixed(2)}×` },
          { key: 'preferredZone', header: 'Zone', render: (v) => String(v ?? '—') },
          { key: 'active', header: 'Active', render: (v) => v ? '✅' : '❌' },
          { key: 'id', header: 'Actions', render: (v) => <button onClick={() => deleteP(String(v))} className="px-2 py-1 text-xs border border-red-300 text-red-600 rounded hover:bg-red-50">Delete</button> },
        ] as ListColumn[]}
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? <p className="text-sm text-gray-500">Loading…</p> : snapshots.length === 0 && productId ? (
        ...
        <ListPageTemplate
          columns={[
            { key: 'snapshotMonth', header: 'Month', render: (v) => <span className="font-mono text-xs">{String(v).slice(0, 7)}</span> },
            { key: 'quantitySold', header: 'Qty Sold', render: (v) => Number(v).toLocaleString() },
            { key: 'revenue', header: 'Revenue', render: (v) => Number(v).toLocaleString() },
            { key: 'transactionCount', header: 'Transactions' },
            { key: 'avgSellingPrice', header: 'Avg Price', render: (v) => v != null ? Number(v).toFixed(2) : '—' },
            { key: 'abcClass', header: 'ABC', render: (v) => v ? <Badge label={String(v)} colorMap={ABC_COLORS} /> : '—' },
            { key: 'xyzClass', header: 'XYZ', render: (v) => v ? <Badge label={String(v)} colorMap={XYZ_COLORS} /> : '—' },
          ] as ListColumn[]}
