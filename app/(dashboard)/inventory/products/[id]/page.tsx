...
export default function ProductDetailPage({
  ...
  const { data: product } = useResourceDoc<FieldValues>(
    productResource,
    productId,
  );
  ...
  const refreshProduct = () =>
    queryClient.invalidateQueries({
      queryKey: resourceKeys(apiClient.tenantId, productResource.name).all,
    });
  const ledger = useResourceList(stockLedgerResource, {
    pageSize: 50,
    filters: { productId },
  });
        ...
        <Card padding="md" className="ui-stack-4">
          <Tabs
            ...
            tabs={[
              {
                key: "stock",
                label: `Stock by Warehouse (${stockRows.length})`,
              },
