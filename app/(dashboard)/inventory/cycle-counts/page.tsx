...
import { Card, PageHeader, Button, Badge, ListPageTemplate, FormField, Input, Select, type ListColumn } from '@unerp/ui';
import {
  Plus,
  AlertCircle,
} from 'lucide-react';
...
export default function CycleCountsPage() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const [cycleCountsResponse, productsResponse, warehousesResponse] = await Promise.all([
      ...
      setCycleCounts((Array.isArray(items(cycleCountsResponse)) ? items(cycleCountsResponse) : []) as CycleCount[]);
      setProducts((Array.isArray(items(productsResponse)) ? items(productsResponse) : []) as Product[]);
      {
        const whs = (Array.isArray(items(warehousesResponse)) ? items(warehousesResponse) : []) as Warehouse[];
        setWarehouses(whs);
  ...
  const handleSubmitWorksheet = async (e: React.FormEvent) => {
    ...
    try {
      await client.post(`/inventory/cycle-counts/${activeCountSession.id}/submit`, {
  ...
  const columns: ListColumn[] = [
    { key: 'createdAt', header: 'Audit Date', render: (v) => <span className="ui-text-muted">{new Date(String(v)).toLocaleString()}</span> },
    {
      key: 'status',
      header: 'Status',
      render: (v) => {
        const s = String(v);
        return <Badge variant={s === 'APPROVED' ? 'success' : s === 'COMPLETED' ? 'info' : 'warning'}>{s}</Badge>;
      },
    },
    {
      key: 'items',
    ...
    },
    {
      ...
      render: (v) => {
        ...
        return <span style={{ color: varQty !== 0 ? 'var(--color-warning-text)' : 'inherit' }} className={styles.s1}>{varQty}</span>;
      },
    },
    { key: 'notes', header: 'Notes', render: (v) => <span className="ui-text-muted">{String(v || 'Routine Audit')}</span> },
    {
      key: 'id',
      header: 'Actions',
      render: (v, row) => {
        ...
        <div className={styles.s4}>
          <Card padding="none" className={styles.s5}>
            <div className={styles.s6}>
              <span className="ui-heading-base">Setup Physical Count Session</span>
            ...
            <div className="p-5">
              <form onSubmit={handleCreateCycleCount} className="ui-stack-4">
                <FormField label="Warehouse Depot" htmlFor="cc-warehouse" required>
                  <Select id="cc-warehouse" value={selectedWarehouse} onChange={e => setSelectedWarehouse(e.target.value)} required>
                    {warehouses.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
                  </Select>
                </FormField>
                <FormField label="Session Notes" htmlFor="cc-notes">
                  <Input id="cc-notes" type="text" value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g. Month-end audits" />
                </FormField>

                <div className={styles.s7}>
                  <div className="ui-flex-between mb-2">
                    <span className={styles.s8}>Products Worksheet List</span>
                    <Button variant="outline" type="button" onClick={() => setNewCountItems([...newCountItems, { productId: '', expectedQty: 0 }])} className={styles.s9}>Add Product</Button>
                  </div>
                  {newCountItems.map((item, idx) => (
                    <div key={idx} className={styles.s10}>
                      <Select className={styles.s11} value={item.productId} onChange={e => {
                      ...
                      <Input type="number" className={styles.s12} placeholder="Book Qty" value={item.expectedQty} onChange={e => {
                        const updated = [...newCountItems];
                        if (updated[idx]) {
      ...
      {activeCountSession && (
        <div className={styles.s4}>
          <Card padding="none" className={styles.s14}>
            <div className={styles.s6}>
              <span className="ui-heading-base">Enter Physical Count Measurements</span>
              <button onClick={() => setActiveCountSession(null)} className="ui-btn-icon ui-text-muted">Close</button>
            </div>
            <div className="p-5">
              <form onSubmit={handleSubmitWorksheet} className="ui-stack-4">
                      ...
                      <tr key={item.id} className="border-b">
                        <td className={styles.s20}>
                          <div className="ui-flex-col">
                            ...
                            <span className="ui-text-micro ui-text-muted">{item.product?.sku}</span>
                          </div>
                        </td>
                ...
                <FormField label="Final Remarks" htmlFor="cc-final-remarks">
                  <Input id="cc-final-remarks" type="text" value={worksheetRemarks} onChange={e => setWorksheetRemarks(e.target.value)} placeholder="All counts checked by inventory manager" />
                </FormField>
