...
const makeWaveColumns = (
    ...
    render: (v, row) => {
      ...
      return (
        <span
          ...
          style={{ fontWeight: selectedWave?.id === w.id ? 'var(--weight-bold)' : undefined }} className={styles.s1}
        >
          {String(v)}
        </span>
      );
  ...
  },
  {
    key: 'status',
    header: 'Status',
    render: (v) => <Badge variant={v === 'COMPLETED' ? 'success' : 'warning'}>{String(v)}</Badge>,
  },
  {
    key: 'id',
    header: '',
    render: (v, row) => {
      ...
      return (
        <div className={styles.s2}>
          <Button variant="secondary" size="sm" onClick={() => setSelectedWave(w)}>View</Button>
          {w.status !== 'COMPLETED' && (
            <Button variant="primary" size="sm" onClick={() => handleComplete(String(v))}>Complete</Button>
          )}
        </div>
      );
    },
...
export default function PickWavesPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      else if (data && typeof data === 'object' && 'data' in data) setWaves(((data as { data?: unknown }).data ?? []) as PickWave[]);
    } catch {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post('/inventory/pick-waves', { warehouseId, salesOrderIds: orderIds.split(',').map((s) => s.trim()).filter(Boolean) });
  ...
  const handleRecordPick = async (itemId: string, quantity: number, scannedSerials: string[] = []) => {
    try {
      await client.post(`/inventory/pick-waves/items/${itemId}/record-pick`, { pickedQty: quantity, scannedSerials });
      if (selectedWave) {
        setSelectedWave(await client.get<PickWave>(`/inventory/pick-waves/${selectedWave.id}`));
      }
    } catch {
  ...
  const waveColumns = makeWaveColumns(selectedWave, setSelectedWave, handleComplete);
  ...
  return (
    ...
    <div className="ui-stack-6 ui-animate-in">
      ...
      <div className={styles.s4}>
        ...
        <Card className="p-5">
          {!selectedWave ? (
            <div className={styles.s6}>Select a wave to see its pick list.</div>
          ) : (
          ...
          <Card padding="none" className={styles.s10}>
            ...
            <div className="p-5">
              <form onSubmit={handleCreate} className="ui-stack-4">
                <FormField label="Warehouse ID" required htmlFor="wave-warehouse-id">
                  <Input id="wave-warehouse-id" type="text" value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)} required />
                </FormField>
                <FormField label="Sales Order IDs (comma-separated)" required htmlFor="wave-order-ids">
                  <Input id="wave-order-ids" type="text" value={orderIds} onChange={(e) => setOrderIds(e.target.value)} required />
                </FormField>
