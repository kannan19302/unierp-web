...
function StatCard({
  ...
  return (
    <div
      className={`rounded-lg border p-4 ${warn ? "border-yellow-400 bg-yellow-50 dark:bg-yellow-900/20" : "border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800"}`}
    >
      <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{label}</p>
      <p
        className={`text-2xl font-bold ${warn ? "text-yellow-700 dark:text-yellow-300" : "text-gray-900 dark:text-white"}`}
      >
        {value}
      </p>
    </div>
...
function pickStatusColor(s: string) {
  return (
    { PENDING: "yellow", CONFIRMED: "green", CANCELLED: "gray" }[s] ?? "gray"
  );
}
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <StatCard
            label="Quarantine"
            value={data.batches?.quarantine ?? 0}
            warn={(data.batches?.quarantine ?? 0) > 0}
          />
          <StatCard
            label="Expired"
            value={data.batches?.expired ?? 0}
            warn={(data.batches?.expired ?? 0) > 0}
          />
        </div>
      </div>
      <div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <StatCard
            label="Pending"
            value={data.picks?.pending ?? 0}
            warn={(data.picks?.pending ?? 0) > 5}
          />
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard
            ...
            warn={(expiry?.expired ?? 0) > 0}
          />
          <StatCard
            label="Expiring ≤7 days"
            value={expiry?.expiringIn7 ?? 0}
            warn={(expiry?.expiringIn7 ?? 0) > 0}
          />
          <StatCard
            label="Expiring ≤30 days"
            value={expiry?.expiringIn30 ?? 0}
            warn={(expiry?.expiringIn30 ?? 0) > 0}
          />
          <StatCard
            label="Expiring ≤90 days"
            value={expiry?.expiringIn90 ?? 0}
          />
        </div>
      </div>
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <StatCard
            ...
            warn={(data.alerts?.unacknowledged ?? 0) > 0}
          />
          <StatCard
            label="Critical Alerts"
            value={data.alerts?.critical ?? 0}
            warn={(data.alerts?.critical ?? 0) > 0}
          />
          <StatCard
            label="Active Quarantine Orders"
            value={data.quarantine?.active ?? 0}
            warn={(data.quarantine?.active ?? 0) > 0}
          />
        </div>
      </div>
    </div>
...
function BatchesTab() {
  ...
  const load = useCallback(() => {
    ...
    apiFetch(`/inventory/lot-serial/batches${qs}`)
      .then((d: any) => setBatches(Array.isArray(d) ? d : []))
      .catch(() => {});
  }, [status]);
...
function SerialsTab() {
  ...
  const load = useCallback(() => {
    const qs = status ? `?status=${status}` : "";
    apiFetch(`/inventory/lot-serial/serials${qs}`)
      .then((d: any) => setSerials(Array.isArray(d) ? d : []))
      .catch(() => {});
  }, [status]);
...
function PickSuggestionsTab() {
  ...
  const load = useCallback(() => {
    const qs = status ? `?status=${status}` : "";
    apiFetch(`/inventory/lot-serial/pick-suggestions${qs}`)
      .then((d: any) => setPicks(Array.isArray(d) ? d : []))
      .catch(() => {});
  }, [status]);
...
function QuarantineTab() {
  ...
  const load = useCallback(() => {
    const qs = status ? `?status=${status}` : "";
    apiFetch(`/inventory/lot-serial/quarantine${qs}`)
