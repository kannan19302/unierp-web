...
export default function LeasesPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setLeases(Array.isArray(d) ? d : d.data || []);
        setLoadError(null);
      } catch (err) {
        const message =
          err instanceof Error ? err.message : "Failed to load leases";
        setLoadError(message);
        notifyError("Failed to load leases", message);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleCreate = async () => {
    if (!form.propertyId || !form.tenantName) return;
    setCreating(true);
    try {
      await client.post("/ext/real-estate/leases", {
        ...form,
      ...
      <div className="ui-stack-6">
        ...
        {loadError && (
          <div className="ui-alert ui-alert-danger">
            <AlertTriangle size={16} /> Failed to load leases — list below may
            be stale. {loadError}
          </div>
        )}
