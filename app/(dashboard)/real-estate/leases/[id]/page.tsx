...
import { PageHeader, Card, Spinner, Badge, KPICard, useToast } from "@unerp/ui";
...
export default function LeaseDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const d = await client.get<Lease[] | { data?: Lease[] }>(
        ...
        notifyError("Failed to load lease", message);
      } finally {
        setLoading(false);
      }
    })();
  ...
  if (loadError && !lease)
    return (
      <RouteGuard permission="real-estate.leases.read">
        <div className={styles.s1}>
          <AlertTriangle size={64} className="ui-text-tertiary" />
          <h2>Failed to Load Lease</h2>
          <p className="ui-text-sm-muted">{loadError}</p>
          <Link href="/real-estate/leases">
  ...
  if (!lease)
    return (
      <RouteGuard permission="real-estate.leases.read">
        <div className={styles.s1}>
          <Key size={64} className="ui-text-tertiary" />
    ...
    <RouteGuard permission="real-estate.leases.read">
      <div className="ui-stack-6">
        ...
        <div className="ui-grid-auto">
          <KPICard
            title="Monthly Rent"
            value={fmtCurrency(lease.rentAmount)}
            icon={<DollarSign size={18} />}
            color="var(--color-primary)"
          />
          <KPICard
            title="Security Deposit"
            value={fmtCurrency(lease.securityDeposit || 0)}
            icon={<DollarSign size={18} />}
            color="var(--color-info)"
          />
          <KPICard
            title="Billing"
            value={lease.billingFrequency || "Monthly"}
        ...
        <Card>
          <div className="p-5">
            <h3 className="ui-heading-base mb-4">Lease Details</h3>
            {[
