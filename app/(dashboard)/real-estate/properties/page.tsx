...
export default function PropertiesPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        notifyError("Failed to load properties", message);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      await client.post("/ext/real-estate/properties", form);
      setCreateOpen(false);
      window.location.reload();
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Failed to create property";
      notifyError("Failed to create property", message);
    } finally {
      setCreating(false);
    }
  };
  ...
  const columns: Column<Property>[] = [
    {
      ...
      render: (row: any) => (
        <div className="ui-hstack-3">
          ...
          <div>
            <Link
              href={`/real-estate/properties/${row.id}`}
              className={styles.s2}
            >
              {row.name}
            </Link>
            <div className="ui-text-xs-tertiary">{row.portfolio}</div>
          </div>
        </div>
      ),
    },
      ...
      <div className="ui-stack-6">
        ...
        {loadError && (
          <div className="ui-alert ui-alert-danger">
            <AlertTriangle size={16} /> Failed to load properties — list below
