...
import { PageHeader, Card, Button, Spinner, Badge, DataTable, type Column, KPICard } from "@kannan19302/ui";
...
export default function LabResultsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const d = await client.get<Drug[] | { data?: Drug[] }>(
          "/ext/healthcare/drugs",
        );
  ...
  const columns: Column<Drug>[] = [
    {
      ...
      render: (row: any) => (
        <div className="ui-hstack-3">
          <div
            style={{
              background: row.isControlled
                ? "var(--color-danger-light)"
                : "var(--color-info-light)",
              color: row.isControlled
                ? "var(--color-danger)"
                : "var(--color-info)",
            }}
            className={styles.s1}
          ...
          <div>
            ...
            {row.isControlled && <Badge variant="danger">Controlled</Badge>}
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (row: any) => {
        ...
        return (
          <Badge
            variant={
              expired ? "danger" : row.quantity <= 10 ? "warning" : "success"
            }
          >
            {expired
              ? "Expired"
