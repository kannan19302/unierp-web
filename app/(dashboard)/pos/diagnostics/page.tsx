...
export default function POSDiagnosticsPage() {
  ...
  const loadDiagnostics = async (termId: string) => {
    try {
      const data = await client.get<{ diagnosticData?: DiagnosticData }>(
        `/pos/terminals/${termId}/diagnostics`,
      );
  ...
  const handleRunDiagnostics = async () => {
    ...
    try {
      ...
      await client.request(`/pos/terminals/${selectedTerminal}/diagnostics`, {
        method: "PUT",
      ...
      });
      {
        ...
        addLog(
          `Printer status: ${nextDiag.printerOnline ? "CONNECTED" : "DISCONNECTED"}`,
        );
        addLog(
          `Barcode Scanner status: ${nextDiag.scannerOnline ? "CONNECTED" : "DISCONNECTED"}`,
        );
        addLog(
          `Cash Drawer status: ${nextDiag.cashDrawerClosed ? "CLOSED" : "OPEN (DRAWER ALERT)"}`,
        );
        addLog(
          `Diagnostics check completed successfully for terminal ${selectedTerminal}.`,
        );
      }
    } catch {
        ...
        <div className="ui-flex-between">
          <div>
            ...
            <p className="ui-text-sm-muted">
              Monitor thermal receipt printers, barcode scanners, and drawer
              connections on the retail floor.
            </p>
          </div>
          ...
          >
            <RefreshCw
              size={14}
              className={isRunning ? "spin-animation" : ""}
            />
            ...
            <div className={["ui-card", styles.p4].join(" ")}>
              ...
              <div className="ui-grid-2">
                ...
                <div className={styles.p7}>
                  <div
                    style={{
                      background: diagnosticData.printerOnline
                        ? "var(--color-success-light)"
                        : "var(--color-danger-light)",
                      color: diagnosticData.printerOnline
                        ? "var(--color-success)"
                        : "var(--color-danger)",
                    }}
                  >
                ...
                <div className={styles.p10}>
                  <div
                    style={{
                      background: diagnosticData.scannerOnline
                        ? "var(--color-success-light)"
                        : "var(--color-danger-light)",
                      color: diagnosticData.scannerOnline
                        ? "var(--color-success)"
                        : "var(--color-danger)",
                    }}
                  >
                ...
                <div className={styles.p13}>
                  <div
                    style={{
                      background: diagnosticData.cashDrawerClosed
                        ? "var(--color-success-light)"
                        : "var(--color-warning-light)",
                      color: diagnosticData.cashDrawerClosed
                        ? "var(--color-success)"
                        : "var(--color-warning)",
                    }}
                  >
                ...
                <div className={styles.p16}>
                  ...
                  <div>
                    ...
                    <p className={styles.p18}>{diagnosticData.status}</p>
                  </div>
                </div>
              </div>
              ...
              <div className={styles.p19}>
                ...
                {new Date(diagnosticData.lastDiagnosticsCheck).toLocaleString()}
              </div>
            </div>
          </div>
