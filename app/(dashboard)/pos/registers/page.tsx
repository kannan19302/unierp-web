...
export default function POSRegistersPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to load registers";
      setLoadError(message);
      notifyError("Failed to load registers", message);
    }
    setLoading(false);
  };
  ...
  const openRegister = async () => {
    try {
      await client.post("/pos/exp/registers", modalData);
      setShowModal(false);
      load();
    } catch (err) {
      notifyError(
  ...
  const closeRegister = async (id: string) => {
    try {
      await client.put(`/pos/exp/registers/${id}/close`, {
        endingCash: 0,
        actualCash: 0,
      });
      load();
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl ui-hstack-2">
              <Monitor className="ui-text-primary" /> POS Registers
            </h1>
            <p className="ui-text-sm-muted">
              Manage register sessions across terminals.
            </p>
          </div>
          <div className="ui-hstack-2">
            ...
            <button className="ui-btn" onClick={() => setShowModal(true)}>
              <Plus size={14} /> Open Register
            </button>
          </div>
        </div>
        {loadError && (
          <div className="ui-alert ui-alert-danger">
            Failed to load registers — {loadError}
          </div>
        )}
          ...
          <div className="ui-modal-overlay" onClick={() => setShowModal(false)}>
            <div className="ui-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="ui-hstack-2 mt-4">
                <button className="ui-btn" onClick={openRegister}>
                  Open
                </button>
                <button
