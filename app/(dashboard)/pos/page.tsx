...
export default function POSPage() {
  ...
  const [activeRegister, setActiveRegister] = useState<POSRegister | null>(
    null,
  );
  ...
  const [posViewMode, setPosViewMode] = useState<"checkout" | "analytics">(
    "checkout",
  );

  // Completed transactions tracking (simulated for charts)
  const [salesHistory, setSalesHistory] = useState<
    Array<{ id: string; total: number; itemsCount: number; date: string }>
  >([
    { id: "1", total: 45.9, itemsCount: 3, date: "10:00 AM" },
    { id: "2", total: 120.5, itemsCount: 5, date: "10:30 AM" },
    { id: "3", total: 15.0, itemsCount: 1, date: "11:15 AM" },
  ]);
  ...
  const handleOpenRegister = async () => {
    ...
    try {
      const data = await client.post<POSRegister>("/pos/registers/open", {
  ...
  const handleCloseRegister = async () => {
    ...
    try {
      await client.request(`/pos/registers/${activeRegister.id}/close`, {
        method: "PUT",
        body: JSON.stringify({
  ...
  const handleStartShift = async () => {
    if (!activeRegister) return;
    try {
      const data = await client.post<POSShift>(
        `/pos/registers/${activeRegister.id}/shifts/start`,
        { employeeId },
      );
  ...
  const handleEndShift = async () => {
    ...
    try {
      await client.request(`/pos/shifts/${currentShift.id}/end`, {
        method: "PUT",
      });
      setShiftOpen(false);
          ...
          <div className={styles.p2}>
            <div className={styles.p3}>
              ...
              <p className="ui-text-sm-muted">
                Configure a terminal counter and set starting drawer cash to log
                transactions.
              </p>
              ...
              <div className="ui-flex ui-gap-3">
                ...
                {!shiftOpen ? (
                  <button
                    ...
                    className={["ui-btn ui-btn-primary", styles.p19].join(" ")}
                  >
                  ...
                  <button
                    ...
                    className={["ui-btn", styles.p20].join(" ")}
                  >
                ...
                <button
                  ...
                  className={["ui-btn", styles.p21].join(" ")}
                >
                  Close Drawer
                </button>
              </div>
                ...
                <div className={styles.p28}>
                  <StatCardRow
                    ...
                    stats={[
                      ...
                      {
                        label: "Drawer Cash Balance",
                        value: `$${drawerBalance.toFixed(2)}`,
                ...
                {cart.map((item) => (
                  <div key={item.id} className={styles.p34}>
                    <div className={styles.p35}>
                      ...
                      <p className={styles.p37}>
                        ${parseFloat(item.sellPrice).toFixed(2)} each
                      </p>
                    </div>
