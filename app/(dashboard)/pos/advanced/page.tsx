  ...
  return (
    <div className="ui-stack-6">
      ...
      <div className={styles.p1}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ borderBottom: activeTab === t.id ? '2px solid var(--color-primary)' : '2px solid transparent', color: activeTab === t.id ? 'var(--color-primary)' : 'var(--color-text-secondary)' }}>
            {t.icon} {t.label}
          </button>
        ))}
      </div>
              ...
              <thead>
                <tr className={styles.p7}>
                  {['Order #', 'Product', 'Qty', 'Refund', 'Reason', 'Status', 'Actions'].map(h => (
                    <th key={h} className={styles.p8}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
              ...
              <thead>
                <tr className={styles.p22}>
                  {['Member', 'Tier', 'Points', 'Total Spent', 'Last Visit'].map(h => (
                    <th key={h} className={styles.p23}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {loyaltyMembers.map(m => (
                  <tr key={m.id} className="border-b">
                    ...
                    <td className={styles.p25}>{m.points.toLocaleString()} pts</td>
                    <td className="py-3 px-4">${m.totalSpent.toLocaleString()}</td>
              ...
              <div key={p.id} className={styles.p30}>
                <div className="ui-flex-between ui-items-start">
                  <div>
                    ...
                    <span className="ui-text-micro">{p.type.replace('_', ' ')}</span>
                  </div>
                  <span style={{ ...statusStyles(p.status) }}>{p.status}</span>
                </div>
                <div className={styles.p32}>
                  <div><div className="ui-text-micro">Value</div><div className="ui-heading-sm">{p.type === 'PERCENTAGE' ? `${p.value}%` : p.type === 'FIXED' ? `$${p.value}` : p.type === 'BOGO' ? 'Buy 2 Get 1' : `${p.value}% bundle`}</div></div>
                  <div><div className="ui-text-micro">Used</div><div className="ui-heading-sm">{p.appliedCount} times</div></div>
                </div>
                <div className="ui-text-micro ui-text-muted">
                  <Clock size={10} className={styles.p33} />{p.startDate} → {p.endDate}
                </div>
              </div>
            ))}
          </div>
                ...
                <div className="ui-flex-between">
                  <div className="ui-hstack-2">
                    ...
                    <div>
                      ...
                      <div className="ui-text-micro">{t.code} • {t.store}</div>
                    </div>
                  </div>
                  <span style={{ ...statusStyles(t.status) }}>{t.status}</span>
                </div>
                <div className={styles.p37}>
                  <div><div className="ui-text-micro">Cashier</div><div className="text-sm">{t.currentCashier || '—'}</div></div>
                  <div><div className="ui-text-micro">Register</div><div className="text-sm">{t.registerStatus}</div></div>
                </div>
        ...
        <div className="ui-stack-4">
            ...
            <div className="ui-stack-3">
              {[
                ...
                <div key={i} className={styles.p48}>
                  ...
                  <div style={{ background: opt.enabled ? 'var(--color-primary)' : 'var(--color-border)' }}>
          ...
          <div className="ui-card p-5">
            ...
            {[
              ...
              <div key={i} style={{ borderBottom: i < 3 ? '1px solid var(--color-border)' : 'none' }}>
                <div className="text-sm">
                  <span className="font-semibold">{sync.time}</span>
                  <span className={styles.p52}>{sync.direction === 'UP' ? '↑ Upload' : '↓ Download'} ({sync.count} records)</span>
                </div>
                <span style={{ color: sync.status === 'SUCCESS' ? 'var(--color-success)' : 'var(--color-warning)', background: sync.status === 'SUCCESS' ? 'var(--color-success-light)' : 'var(--color-warning-light)' }}>{sync.status}</span>
              </div>
            ))}
          </div>
        </div>
