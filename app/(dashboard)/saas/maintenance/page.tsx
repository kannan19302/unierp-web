...
export default function SaasMaintenancePage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const d = await client.get<any>("/saas/exp/maintenance-windows");
      ...
      setError(message);
      notifyError("Failed to load maintenance windows", message);
    }
    setLoading(false);
  };
  const save = async () => {
    try {
      await client.post("/saas/exp/maintenance-windows", form);
      setShowModal(false);
      load();
    } catch (err) {
      const message =
      ...
      notifyError("Failed to schedule maintenance window", message);
    }
  };
  const remove = async (id: string) => {
    try {
      await client.patch(`/saas/exp/maintenance-windows/${id}/status`, {
        status: "CANCELLED",
      });
      load();
    } catch (err) {
      ...
      notifyError("Failed to cancel maintenance window", message);
    }
  };
  const complete = async (id: string) => {
    try {
      ...
      notifyError("Failed to complete maintenance window", message);
    }
  };
  return (
      ...
      <div className="ui-stack-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl ui-hstack-2">
              <Wrench className="ui-text-primary" /> Maintenance Windows
            </h1>
        ...
        {error && (
          <div className="ui-alert ui-alert-danger">
            ...
            Failed to load maintenance windows — list below may be stale.{" "}
            {error}
          </div>
        )}
        <div className="card-grid-3">
          {loading ? (
            ...
            <p className="ui-text-muted col-span-3">
            ...
            items.map((i) => (
              <div key={i.id} className="ui-card p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span
                    className={`ui-badge-${i.status === "SCHEDULED" || i.status === "IN_PROGRESS" ? "info" : i.status === "COMPLETED" ? "success" : ""}`}
                  >
                    {i.status || (i.isActive ? "Active" : "Completed")}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 text-sm mb-3">
                  ...
                  -{" "}
                  {i.scheduledEnd
                    ? new Date(i.scheduledEnd).toLocaleString()
                    : "TBD"}
                </div>
                <div className="flex items-center gap-1 text-xs mb-3">
                  {(i.affectedServices || []).map((s: string) => (
                    <span key={s} className="ui-badge">
                      {s}
                    </span>
                  ))}
                </div>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      const d = await client.get<any>(
                        `/saas/exp/maintenance-windows/${i.id}`,
                      );
