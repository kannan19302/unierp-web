...
export default function SaasBillingPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [invRes, pmRes, txRes] = await Promise.all([
        client.get<any>("/saas/billing/invoices"),
        client.get<any>("/saas/billing/payment-methods"),
        client.get<any>("/saas/billing/transactions"),
      ]);
      ...
      notifyError("Failed to load billing data", message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSetDefault = async (id: string) => {
    try {
      ...
      notifyError("Failed to set default payment method", message);
    }
  };
  ...
  const handleRemoveCard = async (id: string) => {
    try {
      ...
      notifyError("Failed to remove payment method", message);
    }
  };
  ...
  const handleDownloadInvoice = async (invoiceId: string) => {
    try {
      const blob = await client.request(
        `/saas/billing/invoices/${invoiceId}/download`,
        {},
        "blob",
      );
      ...
      notifyError("Failed to download invoice", message);
    }
  };
      ...
      <div className="ui-stack-6 ui-animate-in">
        ...
        {loadError && (
          <div className="ui-alert ui-alert-danger">
            ...
            Failed to load billing data — figures below may be stale.{" "}
            {loadError}
          </div>
        )}
        ...
        <Card padding="lg">
          ...
          <DataTable
            ...
            onRowClick={(row) =>
              setSelectedInvoice(
                invoices.find((i) => i.id === (row as any).id) || null,
              )
            }
                ...
                >
                  <div className="ui-hstack-3">
                    ...
                    <div>
                      ...
                      {pm.isDefault && (
                        <span className="ui-badge ui-badge-success ui-ml-2">
                          Default
                        </span>
                      )}
                    </div>
