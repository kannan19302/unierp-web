  ...
  return (
    <div>
      <div
        ...
        style={{
          marginLeft: `calc(${depth} * var(--space-6))`,
...
export default function OrgHierarchyPage() {
  ...
  const fetchHierarchy = useCallback(async () => {
    setError(null);
    try {
      const data = await client.get<OrgTreeResponse>(
        "/saas-portal/org-hierarchy/tree",
      );
  ...
  const openDeptAdd = () => {
    setDeptForm({ name: "", code: "", parentId: "", managerId: "" });
  ...
  const saveDept = async () => {
    try {
      if (deptModal?.mode === "add") {
        ...
        await client.patch(
          `/saas-portal/org-hierarchy/departments/${deptModal.dept.id}`,
          {
            name: deptForm.name,
            code: deptForm.code,
  ...
  const openCcAdd = () => {
    setCcForm({ code: "", name: "", parentId: "", budget: "", isActive: true });
    setCcModal({ mode: "add" });
  };
  const openCcEdit = (cc: CostCenter) => {
    setCcForm({
      code: cc.code,
      name: cc.name,
      parentId: cc.parentId || "",
      budget: String(cc.budget ?? ""),
      isActive: cc.isActive,
    });
    setCcModal({ mode: "edit", cc });
  };
    ...
    try {
      if (ccModal?.mode === "add") {
        await client.post("/saas-portal/org-hierarchy/cost-centers", {
          ...
          code: ccForm.code,
          name: ccForm.name,
          parentId: ccForm.parentId || undefined,
          budget: parseFloat(ccForm.budget) || undefined,
        });
        showToast("Cost center created");
      } else if (ccModal?.cc) {
        await client.patch(
          `/saas-portal/org-hierarchy/cost-centers/${ccModal.cc.id}`,
          {
            name: ccForm.name,
            code: ccForm.code,
            parentId: ccForm.parentId || null,
            budget: parseFloat(ccForm.budget) || null,
    ...
    <RouteGuard permission="admin.org-hierarchy.read">
      <div className="ui-stack-6 ui-animate-in">
              ...
              {loading ? (
                ...
                <div className="ui-empty-state">
                  ...
                  <p className="ui-text-xs-muted">
                    Create your first department to build the org tree.
                  </p>
                </div>
              ) : (
                rootDepts.map((d) => (
              ...
              <DataTable
                ...
                data={
                  costCenters.map((c) => ({
                    ...
                    status: (
                      <span
                        className={`ui-badge ${c.isActive ? "ui-badge-success" : "ui-badge-neutral"}`}
                      >
        ...
        <Modal
          ...
          title={
            deptModal?.mode === "add" ? "Add Department" : "Edit Department"
          }
          footer={
            <>
              <button
          ...
          <div className="ui-stack-4">
            ...
            <FormField label="Parent Department">
              <Select
                ...
                onChange={(e) =>
                  setDeptForm((f) => ({ ...f, parentId: e.target.value }))
                }
              >
                <option value="">None (Root)</option>
                {departments
                  .filter((d) => d.id !== deptModal?.dept?.id)
                  .map((d) => (
            ...
            <TextField
              ...
              onChange={(e) =>
                setDeptForm((f) => ({ ...f, managerId: e.target.value }))
              }
        ...
        <Modal
          ...
          title={
            ccModal?.mode === "add" ? "Add Cost Center" : "Edit Cost Center"
          }
          footer={
            <>
              <button
              ...
              <button
                className="ui-btn ui-btn-primary"
                disabled={!ccForm.name.trim() || !ccForm.code.trim()}
                onClick={saveCc}
              >
                Save
              </button>
          ...
          <div className="ui-stack-4">
            ...
            <FormField label="Parent Cost Center">
              <Select
                ...
                onChange={(e) =>
                  setCcForm((f) => ({ ...f, parentId: e.target.value }))
                }
              >
                <option value="">None (Root)</option>
                {costCenters
                  .filter((c) => c.id !== ccModal?.cc?.id)
                  .map((c) => (
            ...
            <TextField
              ...
              value={ccForm.budget}
              onChange={(e) =>
                setCcForm((f) => ({ ...f, budget: e.target.value }))
              }
              placeholder="0"
            />
            {ccModal?.mode === "edit" && (
              <label className="ui-hstack-2">
                <input
                  ...
                  onChange={(e) =>
                    setCcForm((f) => ({ ...f, isActive: e.target.checked }))
                  }
                />
          ...
          footer={
            <>
              ...
              <button
                ...
                onClick={() => deleteConfirm && deleteDept(deleteConfirm.id)}
              >
                Delete Anyway
              </button>
            </>
