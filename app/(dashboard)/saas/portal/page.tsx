...
export default function SaasPortalPage() {
  ...
  const handleUpgrade = async () => {
    ...
    setUpgrading(true);
    try {
      await client.post('/saas/subscribe', { planId: upgradeTarget.id });
      await loadData();
    } catch {
        ...
        <Card className={styles.s1}>
          {loading ? (
            <span className={styles.s2}>
              <RefreshCw size={16} className={styles.s3} /> Loading subscription data…
            </span>
          ) : (
  ...
  return (
    ...
    <div className="ui-stack-6 ui-animate-in">
      ...
      <div className={styles.s21}>
        {plans.map((plan) => (
          <Card key={plan.id} padding="lg" style={{ border: plan.isCurrent ? '2px solid var(--color-primary)' : '1px solid var(--color-border)' }} className={styles.s22}>
            {plan.isCurrent && (
            ...
            <button
              ...
              style={{ background: plan.isCurrent ? 'var(--color-bg-sunken)' : 'var(--color-primary)', color: plan.isCurrent ? 'var(--color-text-tertiary)' : 'var(--color-bg-elevated)', cursor: plan.isCurrent ? 'default' : 'pointer' }} className={styles.s27}
            >
        ...
        <div className={styles.s28}>
          <div className={styles.s29}>
            <div className={styles.s30}>
              ...
              <button onClick={() => setUpgradeTarget(null)} className="ui-btn-icon ui-text-muted"><X size={18} /></button>
            </div>
            <div className="p-5 ui-stack-4">
              ...
              <div className="ui-flex-end ui-gap-2">
                <button onClick={() => setUpgradeTarget(null)} className={styles.s33}>Cancel</button>
                <button onClick={handleUpgrade} disabled={upgrading} className={styles.s34}>
              ...
              {FEATURE_MATRIX.map((row, idx) => (
                <tr key={idx} className="border-b">
                  ...
                  <td className={styles.s40}>
                    {typeof row.starter === 'boolean' ? (row.starter ? <CheckCircle size={16} className="ui-text-success" /> : '-') : row.starter}
                  </td>
                  <td className={styles.s40}>
                    {typeof row.growth === 'boolean' ? (row.growth ? <CheckCircle size={16} className="ui-text-success" /> : '-') : row.growth}
                  </td>
                  <td className={styles.s40}>
                    {typeof row.enterprise === 'boolean' ? (row.enterprise ? <CheckCircle size={16} className="ui-text-success" /> : '-') : row.enterprise}
                  </td>
                </tr>
              ))}
            </tbody>
