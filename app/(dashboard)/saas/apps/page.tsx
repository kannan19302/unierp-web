...
export default function SaasAppsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const d = await client.get<any>(`/saas/exp/apps?${params}`);
      if (d?.data) {
        setItems(d.data);
      ...
      setError(message);
      notifyError("Failed to load marketplace apps", message);
    }
    setLoading(false);
  };
  const save = async () => {
    try {
      if (editId) {
        await client.put(`/saas/exp/apps/${editId}`, form);
      } else {
        await client.post("/saas/exp/apps", form);
      }
      setShowModal(false);
      setEditId(null);
      load();
      ...
      notifyError("Failed to save app", message);
    }
  };
  const remove = async (id: string) => {
    try {
      await client.delete(`/saas/exp/apps/${id}`);
      load();
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Failed to delete app";
      notifyError("Failed to delete app", message);
    }
  };
  return (
      ...
      <div className="ui-stack-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl ui-hstack-2">
              <Puzzle className="ui-text-primary" /> Marketplace Apps
            </h1>
            <p className="ui-text-sm-muted">Manage the app marketplace.</p>
          </div>
          <button
            className="ui-btn"
            onClick={() => {
        ...
        {error && (
          <div className="ui-alert ui-alert-danger">
            ...
            Failed to load marketplace apps — list below may be stale. {error}
          </div>
        )}
              ...
              {loading ? (
                ...
                items.map((i) => (
                  <tr key={i.id}>
                    ...
                    <td>{i.category || "-"}</td>
                    <td>
                      {i.isFree ? "Free" : `$${Number(i.price).toFixed(2)}`}
                    </td>
                    <td>
                      {i.isPublished ? (
                    ...
                    <td>{i._count?.installations || i.installCount || 0}</td>
