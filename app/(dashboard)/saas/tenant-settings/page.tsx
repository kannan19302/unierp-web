...
export default function SaasTenantSettingsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const d = await client.get<any>("/saas/exp/tenant-settings");
      ...
      setError(message);
      notifyError("Failed to load tenant settings", message);
    }
    setLoading(false);
  };
  const saveSetting = async () => {
    try {
      await client.post("/saas/exp/tenant-settings", settingForm);
      setSettingForm({ category: "general", key: "", value: "" });
      load();
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Failed to save setting";
      notifyError("Failed to save setting", message);
    }
  };
  return (
      ...
      <div className="ui-stack-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl ui-hstack-2">
              <Settings className="ui-text-primary" /> Tenant Settings
            </h1>
            <p className="ui-text-sm-muted">Manage configuration settings.</p>
          </div>
        </div>
        {error && (
          <div className="ui-alert ui-alert-danger">
            <AlertTriangle size={16} />
            Failed to load tenant settings — list below may be stale. {error}
          </div>
        )}
        ...
        <div className="ui-card p-4">
          ...
          <div className="ui-hstack-2">
            ...
            />
            <button className="ui-btn" onClick={saveSetting}>
              Save
            </button>
          </div>
        </div>
