...
// Maps the sidebar/AI_TABS `?tab=` deep-link ids (see AiTabLayout.tsx) to this
// page's internal Mode ids, so links like `/ai?tab=invoice-scanner` actually
// land on the right mode instead of always opening on the default "chat" tab.
const TAB_PARAM_TO_MODE: Record<string, Mode> = {
  "ask-data": "chat",
  "invoice-scanner": "invoice",
  "email-drafter": "email",
  "form-generator": "form",
...
const MODES: {
...
}[] = [
  {
    ...
    description: "Ask questions about your ERP data in plain English",
  },
  {
    id: "invoice",
    label: "Invoice Scan",
    icon: <FileText size={16} />,
    description: "Paste invoice text to extract structured fields",
  },
  {
    id: "email",
    label: "Draft Email",
    icon: <Mail size={16} />,
    description: "Generate a professional email for any business context",
  },
  {
    id: "form",
    label: "Build Form",
    icon: <Wand2 size={16} />,
    description:
      "Describe a data-collection form and get a Builder Studio definition",
  },
  {
    id: "workflow",
    ...
    description:
      "Describe an approval workflow and get a configurable definition",
  },
];
...
export default function AiCopilotPage() {
  ...
  const [mode, setMode] = useState<Mode>(
    (initialTab && TAB_PARAM_TO_MODE[initialTab]) || "chat",
  );
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages((prev: any) => [
        ...
        {
          role: "assistant",
          content: "Network error. Is the API server running?",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  const placeholders: Record<Mode, string> = {
    ...
    invoice:
      "Paste vendor invoice text here (OCR output, email body, or raw text)…",
    email:
      'e.g. "Follow up with Acme Corp about their overdue invoice #INV-0042"',
    form: 'e.g. "A contact form with name, company, phone, and reason for inquiry"',
    workflow:
      'e.g. "Purchase order approval: manager approves under $5k, finance director above that"',
  };
      ...
      <div className={`p-8 ui-stack-6 ${styles.s1}`}>
        ...
        <div>
          ...
          <p className="ui-text-muted mt-1">
            Your intelligent assistant for data queries, document processing,
          ...
          <div className={`p-6 ui-stack-4 flex-1 ${styles.s5}`}>
            ...
            {messages.map((msg: any, i: any) => (
              ...
              >
                ...
                <div
                  className={`ui-stack-1 ${styles.s7}`}
                  style={{
