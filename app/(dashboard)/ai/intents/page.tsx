...
export default function IntentsPage() {
  ...
  const fetchExamples = async () => {
    try {
      setLoading(true);
      const data = await client.get<{ data: IntentExample[]; meta: unknown }>(
        "/ai/intent/training-data",
      );
  ...
  const classify = async () => {
    ...
    try {
      const result = await client.post<{
        ...
        entities: { entity: string; value: string }[];
        allScores: Record<string, number>;
      }>("/ai/intent/classify", { text: classifyText });
  ...
  const addEntity = () => {
    if (newExample.entityName && newExample.entityValue) {
      setEntities([
        ...entities,
        { entity: newExample.entityName, value: newExample.entityValue },
      ]);
  ...
  const createExample = async () => {
    ...
    try {
      await client.post("/ai/intent/training-data", {
  ...
  const deleteExample = async (id: string) => {
    try {
      await client.delete(`/ai/intent/training-data/${id}`);
      fetchExamples();
    } catch {
      /* ignore */
    }
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl ui-hstack-3">
              <Tag size={28} className="ui-text-primary" /> Intent
              Classification
            </h1>
            <p className="ui-text-muted mt-1">
              Classify text intent and manage training data
            </p>
          </div>
          <Button
        ...
        <Card className="p-6">
          ...
          {classification && (
            <div className="mt-4 p-4 ui-bg-subtle rounded-lg">
              <p>
                <strong>Intent:</strong> {classification.intent}
              </p>
              <p>
                    ...
                    ([intent, score]) => (
                      <div key={intent} className="ui-flex ui-gap-2 text-sm">
                        ...
                        <div className="flex-1 ui-bg-muted rounded h-4 mt-1">
                          <div
                            className="ui-bg-primary rounded h-4"
        ...
        {isModalOpen && (
          <div className="fixed inset-0 ui-bg-overlay flex items-center justify-center z-50">
            <Card className="w-full max-w-lg p-6 ui-stack-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold">Add Training Example</h3>
