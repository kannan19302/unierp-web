...
interface ServiceDispatch {
  ...
  status: string;
  ticket?: { title: string; customerName: string };
}
export default function DispatchBoardPage() {
    ...
    (async () => {
      try {
        const d = await client.get<
          ServiceDispatch[] | { data?: ServiceDispatch[] }
        >("/ext/field-service/dispatches");
        ...
        notifyError("Failed to load dispatches", message);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleCreate = async () => {
    if (!form.ticketId || !form.technicianId) return;
    setCreating(true);
    try {
      await client.post("/ext/field-service/dispatches", form);
      setCreateOpen(false);
      const d = await client.get<
        ServiceDispatch[] | { data?: ServiceDispatch[] }
      >("/ext/field-service/dispatches");
      ...
      notifyError("Failed to create dispatch", message);
    } finally {
      setCreating(false);
    }
  };
  ...
  const columns: Column<ServiceDispatch>[] = [
    {
      ...
      render: (row: any) => (
        <div>
          ...
          <div className="ui-text-xs-tertiary">{row.ticket?.customerName}</div>
        </div>
      ),
    },
    {
  ...
  return (
    <RouteGuard permission="field-service.dispatch.read">
      <div className="ui-stack-6">
        ...
        {loadError && (
          <div className="ui-alert ui-alert-danger">
            <AlertTriangle size={16} /> Failed to load dispatches — list below
