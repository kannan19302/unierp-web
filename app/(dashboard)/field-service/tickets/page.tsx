...
export default function ServiceTicketsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        notifyError("Failed to load tickets", message);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      await client.post("/ext/field-service/tickets", form);
      setCreateOpen(false);
      const d = await client.get<ServiceTicket[] | { data?: ServiceTicket[] }>(
        "/ext/field-service/tickets",
      );
      setTickets(Array.isArray(d) ? d : d.data || []);
    } catch (err) {
      ...
      <div className="ui-stack-6">
        ...
        {loadError && (
          <div className="ui-alert ui-alert-danger">
            <AlertTriangle size={16} /> Failed to load tickets — list below may
            be stale. {loadError}
          </div>
        )}
