  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const list = (value: unknown): Array<{ status?: string }> =>
          Array.isArray(value)
            ? (value as Array<{ status?: string }>)
            : (value as { data?: Array<{ status?: string }> })?.data || [];
        const ticketList = list(t);
        const dispatchList = list(d);

        setStats({
          openTickets: ticketList.filter(
            (x: any) => x.status !== "COMPLETED" && x.status !== "RESOLVED",
          ).length,
      ...
      <div className="ui-stack-6">
        ...
        <div className="ui-grid-2">
          <DashboardChart
            ...
            allowedChartTypes={["bar", "area", "composed"]}
            height={280}
          />
          <DashboardChart
            title="Tickets by Priority"
            subtitle="Active ticket priority distribution"
