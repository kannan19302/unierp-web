...
export default function PreventiveMaintenancePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const d = await client.get<PMRule[] | { data?: PMRule[] }>(
          "/ext/field-service/preventive-maintenances",
        );
        ...
        const message =
          err instanceof Error
            ? err.message
            : "Failed to load maintenance rules";
        setLoadError(message);
        notifyError("Failed to load maintenance rules", message);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      await client.post("/ext/field-service/preventive-maintenances", {
        ...form,
      ...
      notifyError("Failed to create maintenance rule", message);
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <RouteGuard permission="field-service.preventive.read">
      <div className="ui-stack-6">
        ...
        {loadError && (
          <div className="ui-alert ui-alert-danger">
            <AlertTriangle size={16} /> Failed to load maintenance rules — list
            below may be stale. {loadError}
          </div>
        )}
        <KPICard
