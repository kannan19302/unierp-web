...
export default function SchedulingPage() {
  ...
  const runScheduling = async () => {
    ...
    try {
      setResult(
        await client.post<ScheduleResult>(
          "/manufacturing/scheduling/schedule",
          { algorithm },
        ),
      );
    } finally {
  ...
  const lookupBomCost = async () => {
    ...
    try {
      setBomCost(
        await client.get<BomCostResult>(
          `/manufacturing/scheduling/bom-cost/${bomId.trim()}`,
        ),
      );
    } finally {
      setBomLoading(false);
      ...
      <div className="ui-stack-6">
        ...
        <Card padding="md">
          <div className={styles.p1}>
            ...
            >
              ...
              {running ? "Scheduling…" : "Run Scheduling"}
            </Button>
          </div>
        </Card>
          ...
          <>
            <div className="ui-grid-auto">
              <KPICard
                ...
                value={result.scheduledOrders}
                icon={<CheckCircle2 size={20} />}
                color="var(--color-success)"
              />
              <KPICard
                title="Unscheduled Orders"
                value={result.unscheduledOrders}
        ...
        <Card padding="md">
          ...
          <div className={styles.p4}>
            ...
            <Button
              variant="outline"
              onClick={lookupBomCost}
              disabled={bomLoading}
            >
              <Search size={14} className="mr-2" />{" "}
              {bomLoading ? "Looking up…" : "Look Up"}
            </Button>
          </div>
          {bomCost && (
              ...
              <div className={styles.p6}>
                <div>
                  ...
                  <div className="font-bold">
                    {fmtCurrency(bomCost.materialCost)}
                  </div>
                </div>
                <div>
                  <div className="ui-text-xs-tertiary">Labor Cost</div>
                  <div className="font-bold">
                    {fmtCurrency(bomCost.laborCost)}
                  </div>
                </div>
              </div>
          ...
          {!bomCost && !bomLoading && (
            <Badge variant="default">
              Enter a BOM ID to view its material cost breakdown.
            </Badge>
          )}
        </Card>
      </div>
