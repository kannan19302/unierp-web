...
export default function RoutingPage() {
  ...
  const fetchRoutes = async () => {
    try {
      setLoading(true);
      const data = await client.get<Route[] | { data?: Route[] }>(
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await client.post("/manufacturing/routes", {
        name: newRoute.name,
        code: newRoute.code,
        description: newRoute.description || undefined,
        operations,
      });
      ...
      alert("Failed to create route - check operations JSON");
    }
  };
  ...
  return (
    <div className="ui-stack-6">
      <div className="ui-flex-between">
        <div>
          <h1 className={styles.p1}>
            <GitBranch size={28} className="ui-text-primary" /> Manufacturing
            Routes
          </h1>
          <p className={styles.p2}>
            Define production routings with sequenced operations
          </p>
        </div>
          ...
          >
            <div>
              <h3 className={styles.cardTitle}>
                {r.name} <span className={styles.code}>{r.code}</span>
              </h3>
              <p className={styles.cardDesc}>
                {r.description || "No description"}
              </p>
              <div className={styles.meta}>
                <span>{r.operations.length} operations</span>
                <span>Lead: {r.totalLeadTimeMin || 0} min</span>
                <span className={r.isActive ? styles.active : styles.inactive}>
                ...
                <div key={op.id} className={styles.opItem}>
                  ...
                  <div className={styles.opBody}>
                    ...
                    <div className={styles.opMeta}>
                      {op.workstationCode || "Any WS"} · {op.durationMinutes}min{" "}
                      {op.setupMinutes > 0 ? `(+${op.setupMinutes} setup)` : ""}
                    </div>
                  </div>
                </div>
              ))}
      ...
      {routes.length === 0 && !loading && (
        <div className="ui-text-muted">No routes defined yet.</div>
      )}
      {isModalOpen && (
        <div className={styles.overlay}>
          <div className={styles.modal}>
            ...
            <form onSubmit={handleCreate} className="ui-stack-3">
              ...
              <div className="ui-form-group">
                <label className="ui-label">Operations (JSON array)</label>
                <textarea
