...
export default function MESDiagnosticsPage() {
  ...
  const [simLog, setSimLog] = useState<string[]>([
    "System diagnostics monitoring active.",
  ...
  const fetchOeeData = async () => {
    try {
      setLoading(true);
      setOeeData(
        await client.get<OeeDetails>("/manufacturing/diagnostics/oee"),
      );
    } catch {
      // Ignored
    } finally {
  ...
  const handleSearchGenealogy = async () => {
    ...
    try {
      setGenLoading(true);
      setGenealogy(
        await client.get<GenealogyResult>(
          `/manufacturing/diagnostics/genealogy/${searchLot}`,
        ),
      );
    } catch {
      // Ignored
    ...
    const interval = setInterval(() => {
      setSensors((prev) =>
        prev.map((s) => {
          ...
          const nextVibStatus =
            nextVib > 2.5 ? "CRITICAL" : nextVib > 1.8 ? "WARNING" : "NORMAL";
          return {
            ...s,
        ...
        <div className="ui-flex-between">
          <div>
            ...
            <p className={styles.p2}>
              Real-time OEE breakdowns, equipment cycle telemetry, and
              upstream/downstream material genealogy lot tracking.
            </p>
          </div>
          <button onClick={fetchOeeData} className={styles.p3}>
              ...
              <div className={styles.p14}>
                ...
                {genLoading ? (
                  ...
                  <div className={styles.p20}>
                    ...
                    <div>
                      <h4 className={styles.p21}>
                        Upstream Genealogy (Component Ingredients)
                      </h4>
                      {genealogy.upstream ? (
                        ...
                        <p className={styles.p25}>
                          No upstream history. This lot is a purchased raw
                          material component.
                        </p>
                      )}
                    </div>
                    ...
                    <div className={styles.p26}>
                      <h4 className={styles.p27}>
                        Downstream Genealogy (Finished Assemblies Consumed In)
                      </h4>
                      ...
                      genealogy.downstream.length > 0 ? (
                            ...
                            <div key={i} className={styles.p29}>
                              <div className={styles.p30}>
                                ...
                                <span className="ui-text-success">
                                  Resulting Lot: {d.finishedProductLot}
                                </span>
                              </div>
                              <div className="ui-text-micro ui-text-muted">
                                Consumed in WO: {d.workOrderNumber} | Qty
                                Consumed: {d.quantityConsumed} units
                              </div>
                            </div>
                          ))}
                        </div>
                        ...
                        <p className={styles.p31}>
                          This lot has not been consumed in any downstream
                          manufacturing WOs.
                        </p>
                      )}
                    </div>
              ...
              <div className={styles.p32}>
                <div className="ui-flex-between">
                  <h3 className={styles.p33}>
                    <Wrench size={18} className="ui-text-primary" /> IoT Sensor
                    Monitoring
                  </h3>
                  <button
