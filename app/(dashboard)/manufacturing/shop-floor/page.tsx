...
export default function ShopFloorTerminal() {
  ...
  const fetchInitialData = async () => {
    try {
      const [workstationData, productData] = await Promise.all([
        client.get<Workstation[] | { data?: Workstation[] }>(
          "/manufacturing/workstations",
        ),
        client.get<Product[] | { data?: Product[] }>("/inventory/products"),
      ]);
      const workstations = Array.isArray(workstationData)
        ? workstationData
        : (workstationData.data ?? []);
      setWorkstations(workstations);
      if (workstations.length > 0 && workstations[0])
  ...
  const fetchOperations = async (woId: string) => {
    try {
      setOpLoading(true);
      setOperations(
        await client.get<Operation[]>(
          `/manufacturing/work-orders/${woId}/operations`,
        ),
      );
    } catch {
      // Ignored
  ...
  const handleStartOperation = async (opId: string) => {
    ...
    try {
      await client.post(
        `/manufacturing/work-orders/${activeWO.id}/operations/${opId}/start`,
      );
  ...
  const handleCompleteOperation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await client.post(
        `/manufacturing/work-orders/${activeWO.id}/operations/${selectedOp.id}/complete`,
        {
      ...
      const ops = await client.get<Operation[]>(
        `/manufacturing/work-orders/${activeWO.id}/operations`,
      );
      ...
      if (allDone) {
        ...
        await client.request(
          `/manufacturing/work-orders/${activeWO.id}/status`,
          { method: "PATCH", body: JSON.stringify({ status: "COMPLETED" }) },
        );
        alert(
          "All routing operations complete. Work Order has been marked COMPLETED!",
        );
        setActiveWO(null);
        void fetchWorkOrders();
      }
  ...
  return (
    <RouteGuard permission="manufacturing.shop-floor.read">
      <div
        style={{ gridTemplateColumns: activeWO ? "1.2fr 1fr" : "1fr" }}
        className={styles.s1}
      >
        {/* Main Terminal Panel */}
          ...
          <div className="ui-flex-between">
            <div>
              ...
              <p className={styles.p3}>
                Clock-in/out workstation operations, verify routing queues, and
                log lot consumptions.
              </p>
            </div>
        ...
        {activeWO && (
          <div className={styles.p15}>
            <div className="ui-flex-between">
              ...
              <button onClick={() => setActiveWO(null)} className={styles.p17}>
                &times;
              </button>
            </div>

            <p className="ui-text-sm-muted">
              Work Order: <strong>{activeWO.workOrderNumber}</strong> (
              {Number(activeWO.quantity)} units)
            </p>
                ...
                {operations.map((op) => (
                  <div key={op.id} className={styles.p19}>
                    ...
                    <div className={styles.p22}>
                      ...
                      {op.status === "COMPLETED" && (
                        <span className={styles.p26}>Closed</span>
                      )}
                    </div>
                  </div>
                ))}
          ...
          <div className={styles.p27}>
            <form onSubmit={handleCompleteOperation} className={styles.p28}>
              <h3 className="ui-heading-lg">
                Complete Operation: {selectedOp.name}
              </h3>

              <div>
                <label className="ui-text-xs-label">
                  Log Scrap Quantity during this Step
                </label>
                <input
                  required
                  type="number"
              ...
              <div className={styles.p30}>
                <span className={styles.p31}>
                  Genealogy Lot Traceability (Optional)
                </span>

                <div className={styles.p32}>
                  <div>
                    <label className="ui-text-micro ui-text-muted">
                      Component Consumed
                    </label>
                    <select
              ...
              <div className={styles.p35}>
                <label className={styles.p36}>
                  Log Downtime Failure Code (If Any)
                </label>
                <select
                  value={downtimeCode}
