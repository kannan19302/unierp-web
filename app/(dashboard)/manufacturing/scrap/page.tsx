...
export default function ScrapPage() {
  ...
  const fetchRecords = async () => {
    try {
      setLoading(true);
      const data = await client.get<ScrapRecord[] | { data?: ScrapRecord[] }>(
        "/manufacturing/scrap",
      );
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post("/manufacturing/scrap", {
        ...newRecord,
        scrappedQty: parseFloat(newRecord.scrappedQty),
        costImpact: newRecord.costImpact
          ? parseFloat(newRecord.costImpact)
          : undefined,
      });
      ...
      <div className="ui-flex-between">
        <div>
          <h1 className={styles.p1}>
            <AlertTriangle size={28} className="ui-text-warning" /> Scrap &
            Waste Tracking
          </h1>
          <p className={styles.p2}>
            Record production scrap and calculate cost impact
          </p>
        </div>
        <button onClick={() => setIsModalOpen(true)} className={styles.addBtn}>
          <Plus size={18} /> Record Scrap
        </button>
      </div>
      {records.length > 0 && (
        <div className={styles.summary}>
          <div className={styles.summaryItem}>
            <span className={styles.summaryLabel}>Total Scrapped</span>
            <span className={styles.summaryValue}>{totalScrapped} units</span>
          </div>
          <div className={styles.summaryItem}>
            <span className={styles.summaryLabel}>Cost Impact</span>
          ...
          <div className={styles.modal}>
            ...
            <form onSubmit={handleCreate} className="ui-stack-3">
              <div className="ui-form-group">
                <label className="ui-label">Work Order ID</label>
                <input
                  className="ui-input"
              ...
              <div className="ui-grid-2">
                <div className="ui-form-group">
                  <label className="ui-label">Scrapped Qty</label>
                  <input
                    className="ui-input"
                    type="number"
                ...
                <div className="ui-form-group">
                  <label className="ui-label">Cost Impact ($)</label>
                  <input
                    className="ui-input"
                    type="number"
              ...
              <div className="ui-form-group">
                <label className="ui-label">Detail</label>
                <textarea
                  className="ui-input"
