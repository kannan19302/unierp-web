...
export default function ManufacturingDashboard() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const list = <T,>(data: T[] | { data?: T[] }) => Array.isArray(data) ? data : (data.data ?? []);

      if (activeTab === 'dashboard') {
        const [boms, orders, workstations, loadBalancing, maintenance, subcontracting, tools] = await Promise.all([
          client.get<BOM[] | { data?: BOM[] }>('/manufacturing/boms'), client.get<WorkOrder[] | { data?: WorkOrder[] }>('/manufacturing/work-orders'), client.get<WorkstationModel[] | { data?: WorkstationModel[] }>('/manufacturing/workstations'), client.get<LoadBalance[] | { data?: LoadBalance[] }>('/manufacturing/workstations/load-balancing'), client.get<MaintenanceRequest[] | { data?: MaintenanceRequest[] }>('/manufacturing/maintenance'), client.get<SubcontractingOrder[] | { data?: SubcontractingOrder[] }>('/manufacturing/subcontracting'), client.get<EquipmentTool[] | { data?: EquipmentTool[] }>('/manufacturing/tools'),
        ]);
        setBoms(list(boms)); setWorkOrders(list(orders)); setWorkstations(list(workstations)); setLoadBalancing(list(loadBalancing)); setMaintenance(list(maintenance)); setSubcontracting(list(subcontracting)); setTools(list(tools));
      } else if (activeTab === 'work-orders') {
        const [boms, orders, workstations] = await Promise.all([
          client.get<BOM[] | { data?: BOM[] }>('/manufacturing/boms'), client.get<WorkOrder[] | { data?: WorkOrder[] }>('/manufacturing/work-orders'), client.get<WorkstationModel[] | { data?: WorkstationModel[] }>('/manufacturing/workstations'),
        ]);
        setBoms(list(boms)); setWorkOrders(list(orders)); setWorkstations(list(workstations));
      } else if (activeTab === 'capacity') {
        const [loadBalancing, shifts, workstations] = await Promise.all([
          client.get<LoadBalance[] | { data?: LoadBalance[] }>('/manufacturing/workstations/load-balancing'), client.get<WorkstationShift[] | { data?: WorkstationShift[] }>('/manufacturing/shifts'), client.get<WorkstationModel[] | { data?: WorkstationModel[] }>('/manufacturing/workstations'),
        ]);
        setLoadBalancing(list(loadBalancing)); setShifts(list(shifts)); setWorkstations(list(workstations));
      } else if (activeTab === 'cmms') {
        const [maintenance, workstations] = await Promise.all([
          client.get<MaintenanceRequest[] | { data?: MaintenanceRequest[] }>('/manufacturing/maintenance'), client.get<WorkstationModel[] | { data?: WorkstationModel[] }>('/manufacturing/workstations'),
        ]);
        setMaintenance(list(maintenance)); setWorkstations(list(workstations));
      } else if (activeTab === 'subcontracting') {
        const [subcontracting, vendors, products] = await Promise.all([
          client.get<SubcontractingOrder[] | { data?: SubcontractingOrder[] }>('/manufacturing/subcontracting'), client.get<VendorModel[] | { data?: VendorModel[] }>('/crm/vendors'), client.get<ProductModel[] | { data?: ProductModel[] }>('/inventory/products'),
        ]);
        setSubcontracting(list(subcontracting)); setVendors(list(vendors)); setProducts(list(products));
  ...
  const handleIssueMaterials = async (e: React.FormEvent) => {
    ...
    try {
      await client.post(`/manufacturing/subcontracting/${issueSubOrder.id}/issue`, {
          materials: [
            {
  ...
  return (
    ...
    <div className="ui-stack-6 ui-animate-in">
      ...
      <div className={styles.p1}>
        {['dashboard', 'work-orders', 'capacity', 'cmms', 'subcontracting', 'equipment'].map((tab) => (
          <button
            ...
            style={{ borderBottom: activeTab === tab ? '2px solid var(--color-primary)' : 'none', color: activeTab === tab ? 'var(--color-primary)' : 'var(--color-text-secondary)' }} className={styles.s1}
          >
            {tab.replace('-', ' ')}
          </button>
        ))}
      ...
      {/* Tab Contents */}
      {loading ? (
        <div className="text-center p-12"><Spinner size="lg" /></div>
      ) : (
        <div>
          {/* TAB 0: DASHBOARD */}
          {activeTab === 'dashboard' && (
            <div className="ui-stack-6">
              <StatCardRow stats={[
                { label: 'Average Machine OEE', value: `${averageOee}%`, icon: <ShieldCheck size={16} />, color: 'var(--chart-2)' },
                { label: 'Active Work Orders', value: workOrders.filter(w => w.status === 'IN_PROGRESS').length, icon: <Play size={16} />, color: 'var(--chart-1)' },
                { label: 'Production Scrap Qty', value: totalScrap, icon: <AlertTriangle size={16} />, color: 'var(--chart-4)' },
                { label: 'Cost Variance', value: `$${totalCostVariance.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, icon: <TrendingUp size={16} />, color: totalCostVariance > 0 ? 'var(--chart-4)' : 'var(--chart-2)' },
              ]} />
              ...
              <div className={styles.p2}>
                ...
                <DashboardChart
                  ...
                  data={oeeTrendData}
                  config={{ xAxisKey: 'name', series: [{ dataKey: 'OEE', name: 'OEE %', color: 'var(--color-success)' }] }}
            ...
            <div className="ui-stack-4">
              ...
              <div className="ui-stack-3">
                {workOrders.map((wo) => (
                  ...
                  >
                    <div>
                      <p className="ui-heading-sm font-bold">{wo.workOrderNumber}</p>
                      <p className={styles.p5}>BOM: {wo.bom.name}</p>
                      {wo.workstation && (
                        <p className={styles.p6}>Machine: {wo.workstation.name}</p>
                      )}
                    </div>

                    <div>
                      <span style={{ background: wo.status === 'COMPLETED' ? 'var(--color-success-light)' :
                          wo.status === 'IN_PROGRESS' ? 'var(--color-primary-light)' : 'var(--color-bg-hover)', color: wo.status === 'COMPLETED' ? 'var(--color-success)' :
                          wo.status === 'IN_PROGRESS' ? 'var(--color-primary)' : 'var(--color-text-secondary)' }} className={styles.s2}>
                        {wo.status}
                      </span>
                    </div>
                    ...
                    <div>
                      ...
                      <p style={{ color: wo.costVariance && Number(wo.costVariance) > 0 ? 'var(--color-danger)' : 'var(--color-text)' }} className={styles.s3}>
          ...
          {activeTab === 'capacity' && (
            <div className={styles.p10}>
              ...
              <div className={styles.p11}>
                <h3 className="ui-heading-lg">Finite Capacity Load Utilization</h3>
                  ...
                  {loadBalancing.map((load, idx) => (
                    <div key={idx} className={styles.p13}>
                      <div className="ui-flex-between">
                        ...
                        <span style={{ background: load.status === 'OVERLOADED' ? 'var(--color-danger-light)' : 'var(--color-success-light)', color: load.status === 'OVERLOADED' ? 'var(--color-danger)' : 'var(--color-success)' }} className={styles.s4}>
                          {load.status}
                        </span>
                      </div>
                      ...
                      <div className={styles.p15}>
                        <div style={{ width: `${Math.min(load.utilizationRate, 100)}%`, background: load.utilizationRate > 90 ? 'var(--color-danger)' : 'var(--color-primary)' }} className={styles.s5} />
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {shifts.map((s) => (
                    <div key={s.id} className={styles.p19}>
                      <div>
                        ...
                        <p className="ui-text-micro ui-text-muted">Station: {s.workstation.name}</p>
                      </div>
                ...
                {maintenance.map((req) => (
                  ...
                  >
                    <div>
                      ...
                      <p className={styles.p26}>Machine: {req.workstation.name}</p>
                    </div>
                    <div>
                      <span className={styles.p27}>{req.type}</span>
                    </div>
                    <div>
                      <span style={{ background: req.priority === 'HIGH' ? 'var(--color-danger-light)' : 'var(--color-bg-hover)', color: req.priority === 'HIGH' ? 'var(--color-danger)' : 'var(--color-text-secondary)' }} className={styles.s4}>
                        {req.priority}
                      </span>
                    </div>
                    <div>
                      <span style={{ background: req.status === 'COMPLETED' ? 'var(--color-success-light)' : 'var(--color-primary-light)', color: req.status === 'COMPLETED' ? 'var(--color-success)' : 'var(--color-primary)' }} className={styles.s4}>
                        {req.status}
                      </span>
                    </div>
                ...
                {subcontracting.map((sub) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className={styles.p33}>${Number(sub.totalCost).toFixed(2)}</p>
                    </div>

                    <div>
                      <span style={{ background: sub.status === 'MATERIALS_SHIPPED' ? 'var(--color-success-light)' : 'var(--color-primary-light)', color: sub.status === 'MATERIALS_SHIPPED' ? 'var(--color-success)' : 'var(--color-primary)' }} className={styles.s4}>
                        {sub.status}
                      </span>
                    </div>
                ...
                {tools.map((t) => (
                  <div key={t.id} className={styles.p37}>
                    <div className="ui-flex-between">
                      ...
                      <span style={{ background: t.status === 'OK' ? 'var(--color-success-light)' : 'var(--color-danger-light)', color: t.status === 'OK' ? 'var(--color-success)' : 'var(--color-danger)' }} className={styles.s6}>
                        {t.status}
                      </span>
                    </div>
                    <p className="ui-text-micro ui-text-muted">Code: {t.code} | Station: {t.workstation.name}</p>
                    <div className={styles.p39}>
                      ...
                      <div className={styles.p41}>
                        <div style={{ width: `${Math.min((t.currentCycles / t.maxCycles) * 100, 100)}%`, background: t.status !== 'OK' ? 'var(--color-danger)' : 'var(--color-primary)' }} className={styles.s7} />
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className={styles.p42}>
          <form onSubmit={handleCreateWorkOrder} className={styles.p43}>
            ...
            <div>
              ...
              <select required value={newWO.bomId} onChange={(e) => setNewWO({ ...newWO, bomId: e.target.value })} className={styles.p44}>
            ...
            <div>
              ...
              <select required value={newWO.workstationId} onChange={(e) => setNewWO({ ...newWO, workstationId: e.target.value })} className={styles.p45}>
            ...
            <div>
              ...
              <input required type="text" placeholder="e.g. WO-2026-101" value={newWO.workOrderNumber} onChange={(e) => setNewWO({ ...newWO, workOrderNumber: e.target.value })} className={styles.p46} />
            </div>

            <div className="ui-grid-2 ui-gap-3">
              <div>
                <label className="ui-text-xs-label">Quantity</label>
                <input required type="number" min="1" value={newWO.quantity} onChange={(e) => setNewWO({ ...newWO, quantity: e.target.value })} className={styles.p47} />
              </div>
              <div>
                <label className="ui-text-xs-label">Start Date</label>
                <input type="date" value={newWO.startDate} onChange={(e) => setNewWO({ ...newWO, startDate: e.target.value })} className={styles.p48} />
              </div>
            </div>

            <div className="ui-flex-end ui-gap-2 mt-2">
              <button type="button" onClick={() => setIsWOModalOpen(false)} className={styles.p49}>Cancel</button>
              <button type="submit" className={styles.p50}>Save & Dispatch</button>
            </div>
          </form>
        </div>
      )}
        ...
        <div className={styles.p51}>
          <form onSubmit={handleCreateCmmsRequest} className={styles.p52}>
            ...
            <div>
              ...
              <select required value={newCmms.workstationId} onChange={(e) => setNewCmms({ ...newCmms, workstationId: e.target.value })} className={styles.p53}>
            ...
            <div className="ui-grid-2 ui-gap-3">
              <div>
                ...
                <select value={newCmms.type} onChange={(e) => setNewCmms({ ...newCmms, type: e.target.value })} className={styles.p54}>
              ...
              <div>
                ...
                <select value={newCmms.priority} onChange={(e) => setNewCmms({ ...newCmms, priority: e.target.value })} className={styles.p55}>
            ...
            <div>
              ...
              <input required type="text" placeholder="e.g. Clean main calibration dial" value={newCmms.title} onChange={(e) => setNewCmms({ ...newCmms, title: e.target.value })} className={styles.p56} />
            </div>

            <div>
              <label className="ui-text-xs-label">Detailed Description</label>
              <textarea value={newCmms.description} onChange={(e) => setNewCmms({ ...newCmms, description: e.target.value })} className={styles.p57} />
            </div>

            <div className="ui-flex-end ui-gap-2 mt-2">
              <button type="button" onClick={() => setIsCmmsModalOpen(false)} className={styles.p58}>Cancel</button>
              <button type="submit" className={styles.p59}>Log Request</button>
            </div>
          </form>
        </div>
      )}
      ...
      {isSubModalOpen && (
        <div className={styles.p60}>
          <form onSubmit={handleCreateSubcontracting} className={styles.p61}>
            ...
            <div>
              <label className="ui-text-xs-label">Select Subcontractor Vendor</label>
              <select required value={newSub.vendorId} onChange={(e) => setNewSub({ ...newSub, vendorId: e.target.value })} className={styles.p62}>
            ...
            <div>
              <label className="ui-text-xs-label">Select Material Product</label>
              <select required value={newSub.productId} onChange={(e) => setNewSub({ ...newSub, productId: e.target.value })} className={styles.p63}>
            ...
            <div className="ui-grid-2 ui-gap-3">
              <div>
                <label className="ui-text-xs-label">Quantity</label>
                <input required type="number" min="1" value={newSub.quantity} onChange={(e) => setNewSub({ ...newSub, quantity: e.target.value })} className={styles.p64} />
              </div>
              <div>
                <label className="ui-text-xs-label">Unit Cost</label>
                <input required type="number" min="0.1" step="0.1" value={newSub.unitCost} onChange={(e) => setNewSub({ ...newSub, unitCost: e.target.value })} className={styles.p65} />
              </div>
            </div>

            <div>
              <label className="ui-text-xs-label">Target Delivery Date</label>
              <input type="date" value={newSub.deliveryDate} onChange={(e) => setNewSub({ ...newSub, deliveryDate: e.target.value })} className={styles.p66} />
            </div>

            <div className="ui-flex-end ui-gap-2 mt-2">
              <button type="button" onClick={() => setIsSubModalOpen(false)} className={styles.p67}>Cancel</button>
              <button type="submit" className={styles.p68}>Issue Subcontract PO</button>
            </div>
          </form>
        </div>
      )}
      ...
      {isIssueModalOpen && issueSubOrder && (
        <div className={styles.p69}>
          <form onSubmit={handleIssueMaterials} className={styles.p70}>
            <h3 className="ui-heading-lg">Issue Materials to Subcontractor</h3>
            <p className="ui-text-xs-muted">
            ...
            <div>
              <label className="ui-text-xs-label">Source Stock Warehouse</label>
              <select value={issueForm.warehouseId} onChange={(e) => setIssueForm({ ...issueForm, warehouseId: e.target.value })} className={styles.p71}>
            ...
            <div>
              ...
              <input required type="number" value={issueForm.quantity} onChange={(e) => setIssueForm({ ...issueForm, quantity: e.target.value })} className={styles.p72} />
            </div>

            <div className="ui-flex-end ui-gap-2 mt-2">
              <button type="button" onClick={() => setIsIssueModalOpen(false)} className={styles.p73}>Cancel</button>
              <button type="submit" className={styles.p74}>Issue materials</button>
            </div>
          </form>
        </div>
      )}
        ...
        <div className={styles.p75}>
          <form onSubmit={handleCreateShift} className={styles.p76}>
            ...
            <div>
              ...
              <select required value={newShift.workstationId} onChange={(e) => setNewShift({ ...newShift, workstationId: e.target.value })} className={styles.p77}>
                <option value="">Select workstation...</option>
                {workstations.map(w => (
            ...
            <div>
              ...
              <input required type="text" placeholder="e.g. Morning Shift" value={newShift.name} onChange={(e) => setNewShift({ ...newShift, name: e.target.value })} className={styles.p78} />
            </div>

            <div className="ui-grid-2 ui-gap-3">
              <div>
                <label className="ui-text-xs-label">Start Time</label>
                <input type="text" placeholder="e.g. 08:00" value={newShift.startTime} onChange={(e) => setNewShift({ ...newShift, startTime: e.target.value })} className={styles.p79} />
              </div>
              <div>
                <label className="ui-text-xs-label">End Time</label>
                <input type="text" placeholder="e.g. 16:00" value={newShift.endTime} onChange={(e) => setNewShift({ ...newShift, endTime: e.target.value })} className={styles.p80} />
              </div>
            </div>

            <div className="ui-flex-end ui-gap-2 mt-2">
              <button type="button" onClick={() => setIsShiftModalOpen(false)} className={styles.p81}>Cancel</button>
              <button type="submit" className={styles.p82}>Save Shift</button>
            </div>
          </form>
        </div>
      )}
