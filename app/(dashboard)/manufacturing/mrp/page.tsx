...
export default function MRPPage() {
  ...
  const handleProcessItem = async (id: string) => {
    try {
      ...
      const result = await client.post<{ reference: string }>(
        `/manufacturing/mrp/planned-items/${id}/process`,
      );
  ...
  const getWOsOnDate = (wsId: string, dateStr: string): WorkOrder[] => {
    return workOrders.filter((wo) => {
      ...
      const end = wo.endDate
        ? wo.endDate.slice(0, 10)
        : new Date(new Date(wo.startDate).getTime() + 48 * 3600000)
            .toISOString()
            .slice(0, 10);
        ...
        <div className="ui-flex-between">
          <div>
            ...
            <p className={styles.p2}>
              Trigger demand-driven replenishment loops, calculate net safety
              stock requirements, and dispatch POs/WOs.
            </p>
          </div>
          <button
            onClick={handleRunMRP}
            disabled={runningMRP}
            style={{ opacity: runningMRP ? 0.7 : 1 }}
            className={styles.s1}
          >
                ...
                {schedulerDays.map((day) => {
                  ...
                  return (
                    <div key={day} className={styles.p18}>
                      ...
                      <div className={styles.p19}>{dateObj.getDate()}</div>
                    </div>
                  );
                })}
              </div>
                  ...
                  {schedulerDays.map((day) => {
                    ...
                    return (
                      <div key={day} className={styles.p22}>
                        ...
                        {isConflict && (
                          <span title="Workstation Overload Conflict!">
                            <AlertTriangle
                              size={10}
            ...
            <div className="ui-stack-3">
                ...
                >
                  ...
                  <div>
                    <span
                      style={{
                        color:
                          Number(item.netQuantityRequired) > 0
                            ? "var(--color-danger)"
                            : "var(--color-text)",
                      }}
                      className={styles.s4}
              ...
              {plannedItems.length === 0 && (
                <div className="ui-empty-state">
                  No active requirements suggestions found. Click "Run MRP
                  Calculation" to inspect order pipelines.
                </div>
              )}
            </div>
          )}
