...
export default function APAutomationPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setSchedules(list<APSchedule>(schRes));
      setRuns(list<APRun>(runRes));
      setVendors(list<{ id: string; name: string }>(venRes));
      setBankAccounts(
    ...
    <RouteGuard permission="finance.payables.read">
      <div className="p-8 ui-stack-6">
        <div className="ui-flex-between">
          <div>
            ...
            <p className="ui-text-muted mt-1">
              3-Way matching, payment scheduling, and AP runs.
            </p>
          </div>
        ...
        <div className="ui-grid-2">
          {/* Payment Schedules */}
          <Card
            className={`border-primary/20 hover:border-primary/50 transition-colors ${styles.s3}`}
          >
            <div className={styles.s4}>
              <div className="ui-hstack-3">
                <div
                  className={`bg-indigo-100 dark:bg-indigo-900/30 ${styles.s5}`}
                >
                  <ShieldCheck
                    className={`text-indigo-700 dark:text-indigo-400 ${styles.s6}`}
                  />
                </div>
                <div className={styles.s7}>
                  <div>
                    ...
                    <p className="ui-text-sm-muted">
                      Pending verification schedules
                    </p>
                  </div>
                  <Button
                    size="sm"
          ...
          {/* Payment Runs */}
          <Card
            className={`border-primary/20 hover:border-primary/50 transition-colors ${styles.s13}`}
          >
            <div className={styles.s14}>
              <div className="ui-hstack-3">
                <div
                  className={`bg-green-100 dark:bg-green-900/30 ${styles.s15}`}
                >
                  <CreditCard
                    className={`text-green-700 dark:text-green-400 ${styles.s16}`}
                  />
                </div>
            ...
            <div className={styles.s21}>
              <Button
                ...
                onClick={() => setShowRunForm(!showRunForm)}
              >
                Create Payment Run <Plus className={styles.s22} />
              </Button>
            </div>
          </Card>
        </div>
