        ...
        <div className="ui-page-head">
          <div className="ui-page-head-content">
            ...
            <div className="ui-title-section">
              ...
              {pendingCount > 0 && (
                <span className="ui-badge ui-badge-yellow ml-2">
                  {pendingCount} pending
                </span>
              )}
            </div>
            <p className="ui-page-subtitle">
              Review and resolve purchase orders with price or quantity
              variances outside configured tolerances.
            </p>
          </div>
        ...
        <Card className="ui-list-card">
          {loading ? (
            <div className="ui-loading">
              <Loader2 className="animate-spin" size={20} /> Loading exceptions…
            </div>
          ) : exceptions.length === 0 ? (
            <div className="ui-empty-state">
              <CheckCircle size={40} className="ui-empty-icon text-green-500" />
              <p>
                No {statusFilter.toLowerCase()} exceptions. All matched invoices
                  ...
                  <div className="ui-list-row-main">
                    <div className="ui-list-row-title">
                      ...
                      <span className="font-medium">
                        {exc.varianceType.replace("_", " ")}
                      </span>
                      <span
                        className={`ui-badge ${STATUS_COLORS[exc.status] ?? "ui-badge-gray"}`}
                      >
                        {exc.status}
                      </span>
                    </div>
                    <div className="ui-list-row-meta">
                      ...
                      <span>
                        Expected: {exc.expectedValue ?? "—"} | Actual:{" "}
                        {exc.actualValue ?? "—"}
                      </span>
                    ...
                    {exc.resolutionNotes && (
                      <p className="ui-list-row-note">
                        Note: {exc.resolutionNotes}
                      </p>
                    )}
                  </div>
                  {exc.status === "PENDING" && (
