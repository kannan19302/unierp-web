  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [txRes, statsRes, rulesRes, runsRes, accountsRes, overviewRes] = await Promise.all([
        client.get<IntercompanyTransaction[] | { items?: IntercompanyTransaction[] }>(`${API_BASE}/intercompany/transactions`),
        client.get<IntercompanyStats>(`${API_BASE}/intercompany/stats`),
        client.get<EliminationRule[]>(`${API_BASE}/intercompany/elimination-rules`),
        client.get<EliminationRun[]>(`${API_BASE}/intercompany/elimination-runs`),
        client.get<Account[]>(`${API_BASE}/accounts`),
        client.get<{ entities?: Entity[] }>(`${API_BASE}/consolidation/overview`),
      ]);
      setTxs(Array.isArray(txRes) ? txRes : txRes.items || []);
      setStats(statsRes); setRules(rulesRes); setRuns(runsRes); setAccounts(accountsRes); setEntities(overviewRes.entities || []);
    } catch {
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    <div className="p-8 ui-stack-6">
      ...
      <div className="ui-flex-between ui-items-start">
        <div>
          <h1 className="text-3xl">Intercompany Auto-Netting & Eliminations</h1>
            ...
            <Card key={kpi.label} className="ui-card p-5">
              <div className="ui-flex-between">
                <div>
                  ...
                  <p style={{ color: kpi.color }} className={styles.s3}>{kpi.value}</p>
                </div>
                <div style={{ background: kpi.bg, color: kpi.color }} className={styles.s4}>{kpi.icon}</div>
              </div>
            </Card>
          ))}
        </div>
      ...
      <div className={styles.s5}>
        {[
          ...
          <button
            ...
            style={{ background: activeTab === tab.id ? 'var(--color-primary-light)' : 'transparent', color: activeTab === tab.id ? 'var(--color-primary)' : 'var(--color-text-secondary)', fontWeight: activeTab === tab.id ? 'var(--weight-semibold)' : 'var(--weight-medium)' }} className={styles.s6}
          >
            {tab.label}
          </button>
        ))}
      ...
      {activeTab === 'ledger' && (
        <Card className="ui-card">
          <div className={styles.s7}>
            ...
            <span className="ui-text-xs-muted">Compliance log of matched transaction offsets</span>
          </div>
          <ListPageTemplate
            columns={[
              { key: 'date', header: 'Transaction Date', render: (v) => <span className="font-semibold">{new Date(String(v)).toLocaleDateString()}</span> },
              ...
              { key: 'status', header: 'Status', render: (v) => (
                <span style={{ background: v === 'ELIMINATED' ? 'rgba(34,197,94,0.1)' : 'rgba(245,158,11,0.1)', color: v === 'ELIMINATED' ? '#22c55e' : '#f59e0b' }} className={styles.s8}>{String(v)}</span>
              ) },
              { key: 'eliminationJournalId', header: 'Elimination Journal', render: (v) => v ? <span className={styles.s9}><FileText size={14} /> ID: {String(v).substring(0, 8)}</span> : '—' },
              { key: 'id', header: 'Action', render: (v, row) => row.status === 'MATCHED' ? (
                <Button variant="primary" size="sm" disabled={actingId !== null} onClick={() => handleEliminate(String(v))} className={styles.s10}>
                  {actingId === v ? <Loader2 size={12} className="animate-spin" /> : 'Eliminate'}
                </Button>
              ) : (
                <span className={styles.s11}><CheckCircle2 size={12} /> Post Eliminated</span>
              ) },
            ] as ListColumn[]}
        ...
        <div className="ui-stack-6">
          ...
          {showRuleForm && (
            <Card className="ui-card p-6">
              <h3 className={styles.s12}>Create Intercompany Elimination Rule</h3>
              <form onSubmit={handleCreateRule} className="ui-stack-4">
                ...
                <div className="ui-grid-2">
                  <div className="ui-form-group">
                    <label className={styles.s13}>Source Organization (Seller)</label>
                    <select
                      className={`ui-input ${styles.s14}`}
                  ...
                  <div className="ui-form-group">
                    <label className={styles.s13}>Destination Organization (Buyer)</label>
                    <select
                      className={`ui-input ${styles.s14}`}
                ...
                <div className="ui-grid-2">
                  <div className="ui-form-group">
                    <label className={styles.s13}>Source GL Account (IC Receivables) *</label>
                    <select
                      className={`ui-input ${styles.s14}`}
                      required
                  ...
                  <div className="ui-form-group">
                    <label className={styles.s13}>Destination GL Account (IC Payables) *</label>
                    <select
                      className={`ui-input ${styles.s14}`}
                      required
            ...
            <ListPageTemplate
              columns={[
                { key: 'name', header: 'Name', render: (v, row) => (
                  <div><div className="font-semibold">{String(v)}</div>{Boolean(row.description) && <div className="ui-text-caption ui-text-tertiary">{String(row.description)}</div>}</div>
                ) },
                { key: 'sourceOrgId', header: 'Seller org', render: (v) => String(v || 'Any Seller') },
                { key: 'destinationOrgId', header: 'Buyer org', render: (v) => String(v || 'Any Buyer') },
                { key: 'sourceAccount', header: 'GL Accounts (Source → Destination)', render: (v, row) => (
                  <div>
                    <div className={styles.s17}><span className="ui-text-primary">({(v as any)?.code})</span> {(v as any)?.name}</div>
                    <div className={styles.s18}>→ <span className={styles.s19}>({(row.destinationAccount as any)?.code})</span> {(row.destinationAccount as any)?.name}</div>
                  </div>
                ) },
                { key: 'matchingCriteria', header: 'Criteria', render: (v, row) => <span className={styles.s20}>{String(v)} (±{String(row.toleranceDays)}d)</span> },
                { key: 'isActive', header: 'Status', render: (v) => (
                  <span style={{ background: v ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)', color: v ? '#22c55e' : '#ef4444' }} className={styles.s8}>{v ? 'Active' : 'Inactive'}</span>
                ) },
                { key: 'id', header: 'Action', render: (v) => (
                  <Button variant="outline" size="sm" onClick={() => handleDeleteRule(String(v))} className={styles.s21}><Trash2 size={14} /></Button>
                ) },
              ] as ListColumn[]}
        ...
        <div className="ui-stack-6">
          <Card className="ui-card p-6">
            ...
            <form onSubmit={handleExecuteRun} className={styles.s23}>
              ...
              <Button variant="primary" type="submit" disabled={runningBatch} className={styles.s24}>
          ...
          <Card className="ui-card">
            <div className={styles.s16}>
              <h3 className="ui-heading-base">Period Run Executions History</h3>
            </div>
            <ListPageTemplate
              columns={[
                { key: 'runDate', header: 'Run Date', render: (v) => new Date(String(v)).toLocaleString() },
                { key: 'periodStart', header: 'Period Range', render: (v, row) => <span className="font-semibold">{new Date(String(v)).toLocaleDateString()} – {new Date(String(row.periodEnd)).toLocaleDateString()}</span> },
                { key: 'totalEliminated', header: 'Total Eliminated', render: (v) => <span className="font-semibold">{fmt(Number(v))}</span> },
                { key: 'rulesAppliedCount', header: 'Rules Applied', render: (v) => `${String(v)} Rules` },
                { key: 'journalId', header: 'Journal Entry', render: (v, row) => v ? <span className={styles.s9}><FileText size={14} /> GL: {(row.journal as any)?.entryNumber || String(v).substring(0, 8)}</span> : '—' },
                { key: 'status', header: 'Status', render: (v) => (
                  <span style={{ background: v === 'POSTED' ? 'rgba(34,197,94,0.1)' : 'rgba(245,158,11,0.1)', color: v === 'POSTED' ? '#22c55e' : '#f59e0b' }} className={styles.s8}>{String(v)}</span>
                ) },
                { key: 'id', header: 'Action', render: (v, row) => row.status === 'DRAFT' ? (
                  <Button variant="primary" size="sm" disabled={actingId !== null} onClick={() => handlePostRun(String(v))} className={styles.s25}>
                    {actingId === v ? <Loader2 size={12} className="animate-spin" /> : <CheckSquare size={12} />} Approve &amp; Post
                  </Button>
                ) : (
                  <span className={styles.s11}><CheckCircle2 size={12} /> GL Posted</span>
                ) },
              ] as ListColumn[]}
