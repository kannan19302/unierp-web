...
export default function IntercompanyNettingPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [invoiceData, scheduleData, statsData] = await Promise.all([
        client.get<Invoice[] | { items?: Invoice[] }>('/api/v1/finance/invoices'),
        client.get<PaymentSchedule[] | { items?: PaymentSchedule[] }>(`${API_BASE}/treasury/payment-schedules`),
        client.get<IntercompanyStats>(`${API_BASE}/intercompany/stats`),
      ]);
      const invoiceList = Array.isArray(invoiceData) ? invoiceData : invoiceData.items || [];
      const scheduleList = Array.isArray(scheduleData) ? scheduleData : scheduleData.items || [];
      setInvoices(invoiceList.filter((invoice) => invoice.status !== 'PAID'));
      setSchedules(scheduleList.filter((schedule) => schedule.status !== 'PAID'));
      setStats(statsData);
    } catch {
    } finally {
      setLoading(false);
  ...
  const handleAutoMatch = async () => {
    setMatching(true);
    try {
      const result = await client.post<{ matchCount: number }>(`${API_BASE}/intercompany/auto-match`);
  ...
  return (
    ...
    <div className="p-8 ui-stack-6">
      ...
      <div className="ui-flex-between ui-items-start">
        ...
        <div className="ui-flex ui-gap-2">
          ...
          <Button variant="primary" onClick={handleAutoMatch} disabled={matching}>
            {matching ? <Loader2 size={16} className="animate-spin mr-2" /> : <Zap size={16} className="mr-2" />}
            Run Auto-Net Match
          </Button>
        </div>
      </div>
      ...
      {stats && (
        <StatCardRow stats={[
          { label: 'Unmatched AR Invoices', value: String(invoices.length), icon: <Info size={20} />, color: '#3b82f6' },
          { label: 'Unmatched AP Schedules', value: String(schedules.length), icon: <ArrowRightLeft size={20} />, color: '#f59e0b' },
          { label: 'Matched Pairs (Pending Netting)', value: String(stats.matchedCount), icon: <ShieldCheck size={20} />, color: '#22c55e' },
        ]} />
      )}
          ...
          <div className={styles.s3}>
            {(() => {
              const invoiceColumns: ListColumn[] = [
                { key: 'invoiceNumber', header: 'Invoice #', render: (v) => <span className="font-semibold">{v as string}</span> },
                { key: 'orgId', header: 'Subsidiary' },
                { key: 'totalAmount', header: 'Amount', render: (v) => <span className={styles.s4}>{fmt(Number(v))}</span> },
                { key: 'dueDate', header: 'Due Date', render: (v) => <span>{new Date(v as string).toLocaleDateString()}</span> },
              ];
              return (
                <ListPageTemplate
          ...
          <div className={styles.s3}>
            {(() => {
              const scheduleColumns: ListColumn[] = [
                { key: 'id', header: 'Schedule ID', render: (v) => <span>{(v as string).substring(0,8)}</span> },
                { key: 'orgId', header: 'Subsidiary' },
                { key: 'amount', header: 'Amount', render: (v) => <span className={styles.s5}>{fmt(Number(v))}</span> },
                { key: 'dueDate', header: 'Due Date', render: (v) => <span>{new Date(v as string).toLocaleDateString()}</span> },
              ];
              return (
                <ListPageTemplate
