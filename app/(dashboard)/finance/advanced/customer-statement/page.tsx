...
export default function CustomerStatementPage() {
  ...
  const fetchCustomers = useCallback(async () => {
    ...
    try {
      const data = await client.get<
        { data?: Customer[]; customers?: Customer[] } | Customer[]
      >("/crm/customers?limit=200");
      const list = Array.isArray(data)
        ? data
        : (data as { data?: Customer[] }).data ||
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatement(
        await client.get<Statement>(
          `/advanced-finance/customer-statement/${customerId}?${params}`,
        ),
      );
    } catch {
      alert("Unable to generate the customer statement.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-8 ui-stack-6">
        ...
        <div>
          ...
          <p className="ui-text-muted mt-1">
            Generate a full invoice/payment ledger for any customer within a
            period.
          </p>
        </div>
            ...
            <Card className="ui-card">
              <div className={styles.s6}>
                ...
                <div className="ui-flex ui-gap-2">
                  ...
                  >
                    <RefreshCw size={14} className={styles.s8} />
                    New Statement
                  </Button>
                  <Button variant="outline" size="sm" onClick={handleExportCsv}>
                    <Download size={14} className={styles.s8} />
                    Export CSV
                  </Button>
                </div>
              </div>
                  ...
                  {[
                    {
                      ...
                      value: fmt(statement.totalInvoiced),
                    ...
                    <div key={kpi.label} className={styles.s10}>
                      ...
                      <div>
                        ...
                        <p style={{ color: kpi.color }} className={styles.s12}>
                          {kpi.value}
                        </p>
                      </div>
                    </div>
