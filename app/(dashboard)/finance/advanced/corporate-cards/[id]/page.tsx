...
function ProgressBar({ pct }: { pct: number }) {
  ...
  return (
    <div className={styles.s1}>
      <div style={{ width: `${Math.min(pct, 100)}%`, backgroundColor: color }} className={styles.s2} />
    </div>
  );
}
...
export default function CorporateCardDetailPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [cardsRes, limitsRes, catLimitsRes, utilRes] = await Promise.all([
        client.get<CorporateCard[]>('/api/v1/advanced-finance/corporate-cards'),
        client.get<SpendLimit[]>(`/api/v1/advanced-finance/corporate-cards/${cardId}/limits`),
        client.get<CategoryLimit[]>(`/api/v1/advanced-finance/corporate-cards/${cardId}/category-limits`),
        client.get<Utilization>(`/api/v1/advanced-finance/corporate-cards/${cardId}/utilization`),
      ]);
  ...
  const loadAudit = useCallback(async (limitId: string) => {
    ...
    setAuditRows(await client.get<AuditRow[]>(`/api/v1/advanced-finance/corporate-cards/${cardId}/limits/${limitId}/audit`));
  }, [cardId, client]);

  const saveSpendLimit = async () => {
    await client.post(`/api/v1/advanced-finance/corporate-cards/${cardId}/limits`, { scopeType, scopeId: scopeId || undefined, period, amountCap: Number(amountCap) });
    setLimitModal(false); setScopeId(''); setAmountCap('');
    await load();
  };

  const deleteSpendLimit = async (id: string) => {
    await client.delete(`/api/v1/advanced-finance/corporate-cards/${cardId}/limits/${id}`);
    await load();
  };

  const saveCategoryLimit = async () => {
    await client.post(`/api/v1/advanced-finance/corporate-cards/${cardId}/category-limits`, { mccCategory, amountCap: Number(catAmountCap), period: catPeriod });
    setCategoryModal(false); setCatAmountCap('');
    await load();
  };

  const deleteCategoryLimit = async (id: string) => {
    await client.delete(`/api/v1/advanced-finance/corporate-cards/${cardId}/category-limits/${id}`);
    await load();
  };

  const toggleFreeze = async () => {
    const action = card?.isFrozen ? 'unfreeze' : 'freeze';
    await client.post(`/api/v1/advanced-finance/corporate-cards/${cardId}/${action}`, {});
    await load();
  };
  ...
  const submitIncreaseRequest = async () => {
    ...
    await client.post(`/api/v1/advanced-finance/corporate-cards/${cardId}/limits/${increaseLimitId}/request-increase`, { requestedCap: Number(requestedCap), reason: increaseReason || undefined });
    ...
    <RouteGuard permission="finance.corporate-card.read">
        ...
        actions={card ? (
          <ProtectedComponent permission="finance.corporate-card.freeze">
            <Button variant={card.isFrozen ? 'primary' : 'danger'} onClick={toggleFreeze}>
              {card.isFrozen ? <PlayCircle size={14} className="mr-2" /> : <Snowflake size={14} className="mr-2" />}
        ...
        <Card padding="none">
          <div className={styles.s5}>
            <ProtectedComponent permission="finance.corporate-card-limit.create">
              <Button variant="primary" onClick={() => setLimitModal(true)}><Plus size={14} className="mr-2" /> Add Spend Limit</Button>
            </ProtectedComponent>
          </div>
      ...
      {tab === 'categories' && (
        <Card padding="none">
          <div className={styles.s5}>
            <ProtectedComponent permission="finance.corporate-card-limit.update">
              <Button variant="primary" onClick={() => setCategoryModal(true)}><Plus size={14} className="mr-2" /> Add Category Limit</Button>
            </ProtectedComponent>
          </div>
      ...
      {tab === 'utilization' && (
        <div className="ui-stack-4">
          <Card padding="md">
            ...
            <div className="ui-stack-4">
              {utilization.spendLimits.length === 0 && <div className="ui-text-muted">No active spend limits.</div>}
          ...
          <Card padding="md">
            ...
            <div className="ui-stack-4">
              {utilization.categoryLimits.length === 0 && <div className="ui-text-muted">No active category limits.</div>}
