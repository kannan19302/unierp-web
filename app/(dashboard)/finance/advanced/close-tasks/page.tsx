...
export default function CloseTasksPage() {
  ...
  const activeTab = (searchParams?.get("subtab") || "checklist") as
    | "checklist"
    | "variance";
  ...
  const fetchPeriods = useCallback(async () => {
    try {
      const data = await client.get<FinancialPeriod[]>(
        "/advanced-finance/financial-periods",
      );
      setPeriods(data);
    ...
    async (periodId: string) => {
      ...
      try {
        const [tasks, flags, stats] = await Promise.all([
          client.get<CloseTask[]>(
            `/advanced-finance/close-tasks?periodId=${periodId}`,
          ),
          client.get<VarianceFlag[]>(
            `/advanced-finance/variance-flags?periodId=${periodId}`,
          ),
          client.get<CloseDashboardStats>(
            `/advanced-finance/close-tasks/dashboard?periodId=${periodId}`,
          ),
        ]);
        setTasks(tasks);
        setFlags(flags);
  ...
  const handleToggleTaskStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      await client.patch(`/advanced-finance/close-tasks/${id}`, {
        status: nextStatus,
      });
      fetchPeriodData(selectedPeriodId);
    } catch {
  ...
  const handleGenerateFromTemplate = async () => {
    ...
    setSuccess("");
    try {
      await client.post("/advanced-finance/close-tasks/generate", {
        financialPeriodId: selectedPeriodId,
      });
  ...
  const handleRunVarianceEngine = async () => {
    ...
    setSuccess("");
    try {
      const data = await client.post<{ flagsCreated: number }>(
        "/advanced-finance/variance-flags/run",
        {
          financialPeriodId: selectedPeriodId,
      ...
      setSuccess(
        `Variance scan complete! Found and flagged ${data.flagsCreated} anomalous accounts.`,
      );
      fetchPeriodData(selectedPeriodId);
    } catch {
      setError("Network error");
  ...
  const handleAcknowledgeFlag = async (id: string) => {
    try {
      await client.post(`/advanced-finance/variance-flags/${id}/acknowledge`, {
        notes: "Acknowledged during month-end reviews",
      });
  ...
  const handleResolveFlag = async (id: string) => {
    try {
      await client.post(`/advanced-finance/variance-flags/${id}/resolve`, {
        notes: "Resolved and audited",
      });
        ...
        <div className="ui-page-head">
          <div className="ui-page-head-content">
            ...
            <p className="ui-page-subtitle">
              Manage period-end reconciliation checklist tasks, assign close
              duties, and flag balance variance exceptions.
            </p>
          </div>
          ...
          <div className="ui-page-actions flex items-center gap-2">
            ...
            <Button
              variant="secondary"
              onClick={() => fetchPeriodData(selectedPeriodId)}
              disabled={loading}
            >
          ...
          <div className="ui-grid-3 mb-4">
            ...
            <Card className="ui-card p-4">
              ...
              <p className="text-xs text-gray-500 mt-3">
                Requires immediate attention from owners.
              </p>
            </Card>

            <Card className="ui-card p-4">
              <h3 className="text-xs text-gray-500 uppercase font-semibold">
                Unresolved Variance Flags
              </h3>
              <div className="flex items-end justify-between mt-2">
                <span
                ...
                <AlertTriangle
                  className={
                    stats.criticalFlags > 0 ? "text-red-500" : "text-amber-500"
                  }
                  size={24}
                />
              </div>
              <p className="text-xs text-gray-500 mt-3">
                {stats.criticalFlags} are marked critical (&gt;50% dev).
              </p>
            </Card>
          </div>
        )}
          ...
          <SubTabBar
            tabs={
              [
                {
                  id: "checklist",
                  label: "Closing Tasks Checklist",
                  href: "/finance/advanced/close-tasks?subtab=checklist",
                  icon: ClipboardList,
                },
                {
                  id: "variance",
                  label: "Reconciliation & Variance Flags",
                  href: "/finance/advanced/close-tasks?subtab=variance",
                  icon: AlertTriangle,
                },
              ] as SubTab[]
            }
            ...
            <Card className="ui-list-card">
              {loading ? (
                ...
                <div className="ui-empty-state">
                  ...
                  <p>
                    No checklist duties registered for this financial period.
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    Generate a default set or add custom closing steps to start.
                  </p>
                </div>
              ) : (
                <ListPageTemplate
          ...
          <>
            <Card className="ui-card p-4 mb-4">
              <h3 className="font-semibold text-sm mb-3">
                Reconciliation Variance Flag Scan
              </h3>
              <p className="text-xs text-gray-500 mb-4">
                Compare this period's ledger accounts against prior period
                actual balances. Generate exception flags when PoP deviation
              ...
              <div className="flex items-center gap-3">
                ...
                <Button
                  onClick={handleRunVarianceEngine}
                  disabled={actionLoading}
                >
                  <Play size={16} className="mr-1" /> Scan Ledgers & Flag
                  Variances
                </Button>
              </div>
            </Card>
            ...
            <Card className="ui-list-card">
              ...
              {loading ? (
                ...
                <div className="ui-empty-state">
                  ...
                  <p>
                    All accounts are within reasonable thresholds. No variance
                    anomalies flagged.
                  </p>
                </div>
              ) : (
                      ...
                      },
                      {
                        ...
                        render: (v, row) => (
                          <div className="flex gap-1">
                            {row.status === "OPEN" && (
                              <button
                                onClick={() => handleAcknowledgeFlag(String(v))}
                            ...
                            {row.status !== "RESOLVED" && (
                              <button
                                onClick={() => handleResolveFlag(String(v))}
