...
export default function BudgetingPage() {
  ...
  const fetchData = async () => {
    try {
      const [budRes, scnRes, accRes, configRes, reallocRes] = await Promise.all([
        ...
        client.get<BudgetScenario[]>('/api/v1/advanced-finance/forecast-scenarios'),
        client.get<GLAccount[]>('/api/v1/advanced-finance/accounts'),
        client.get<BudgetControlConfig>('/api/v1/advanced-finance/budget-control/config'),
        client.get<BudgetReallocation[]>('/api/v1/advanced-finance/budget-reallocations')
      ]);
      setBudgets(budRes); setScenarios(scnRes); setAccounts(accRes); setConfig(configRes); setReallocations(reallocRes);
    } catch { /* handled */ }
  ...
  const handleDeleteBudget = async () => {
    ...
    setDeleting(true);
    try {
      await client.delete(`/api/v1/advanced-finance/budgets/${deletingBudget.id}`);
      setDeleteOpen(false);
  ...
  const loadVsActuals = async (budget: BudgetAllocation) => {
    ...
    try {
      setVsActualData(await client.get<BudgetVsActualData>(`/api/v1/advanced-finance/budgets/${budget.id}/vs-actuals`));
    } catch { /* handled */ }
  ...
  const handleCreateScenario = async (e: React.FormEvent) => {
    ...
    try {
      await client.post('/api/v1/advanced-finance/forecast-scenarios', scenarioData);
  ...
  const executeReallocAction = async (id: string, action: 'submit' | 'approve' | 'reject') => {
    try {
      ...
      await client.request(`/api/v1/advanced-finance/budget-reallocations/${id}/${action}`, {
        method: 'POST',
        body
      });
      fetchData();
    ...
    <RouteGuard permission="finance.budget-scenarios.read">
      ...
      <div className={styles.s1}>
        ...
        <button
          ...
          className={`${styles.tab} ${activeTab === 'reallocations' ? styles.tabActive : ''}`}
        >
              ...
              {scenarios.length === 0 ? (
                ...
                scenarios.map(scn => (
                  <div key={scn.id} className={styles.s19}>
                    ...
                    {scn.description && <div className={styles.s22}>{scn.description}</div>}
                  </div>
                ))
              )}
            </div>
                    ...
                    budgets.map(b => (
                      <tr key={b.id} className="border-b">
                        ...
                        <td className={styles.s34}>
                          {b.costCenter ? (
                            <span className={styles.s35}><Layers size={12} /> {b.costCenter.name}</span>
                          ) : b.project ? (
                            <span className={styles.s36}><PieChart size={12} /> {b.project.name}</span>
                          ) : (
                            'Company-wide'
                          )}
                        </td>
                        ...
                        <td className={styles.s39}>
                          <div className={styles.s40}>
                            ...
                            <Button size="sm" variant="outline" onClick={() => confirmDelete(b)} title="Delete Budget" className="ui-text-danger">
                              <Trash2 size={12} />
                            </Button>
                          </div>
                        </td>
                  ...
                  reallocations.map(r => {
                    ...
                    return (
                      <tr key={r.id} className="border-b">
                        ...
                        <td className={styles.s53}>{r.description || 'N/A'}</td>
                        ...
                        <td className={styles.s60}>
                          <div className={styles.s61}>
                            ...
                            {r.status === 'SUBMITTED' && (
                              <>
                                <Button size="sm" variant="outline" className="ui-text-success" onClick={() => executeReallocAction(r.id, 'approve')}>
                                  <Check size={14} className="mr-1" /> Approve
                                </Button>
                                <Button size="sm" variant="outline" className="ui-text-danger" onClick={() => executeReallocAction(r.id, 'reject')}>
          ...
          <form onSubmit={handleUpdateConfig} className="ui-stack-6">
            <div className={styles.s64}>
              <FormField label="Enforcement Action Policy" required>
                ...
                <Select value={config.enforcementAction} onChange={e => setConfig({ ...config, enforcementAction: e.target.value as BudgetControlConfig['enforcementAction'] })}>
      ...
      <Modal open={deleteOpen} onClose={() => setDeleteOpen(false)} title="Delete Budget Allocation"
        ...
        footer={
          <>
            ...
            <Button variant="primary" onClick={handleDeleteBudget} disabled={deleting} className={styles.s71}>
      ...
      >
        {vsActualLoading ? (
          <div className={styles.s73}><Spinner size="lg" /></div>
        ) : vsActualData ? (
                ...
                <tbody>
                  {vsActualData.items.map((item, idx) => (
                    <tr key={idx} className="border-b">
                      <td className={styles.s86}>{item.accountCode} - {item.accountName}</td>
                      <td className={styles.s87}>{fmtBalance(item.budgetAmount)}</td>
                      <td className={styles.s88}>{fmtBalance(item.actualAmount)}</td>
                      <td className={`${styles.varianceCell} ${item.variance >= 0 ? styles.variancePositive : styles.varianceNegative}`}>
