...
export default function ConsolidationPage() {
  ...
  const fetchOverview = useCallback(async () => {
    try {
      setOverview(await client.get<ConsolidationOverview>('/api/v1/advanced-finance/consolidation/overview'));
    } finally {
      setLoading(false);
    }
  }, [client]);
  ...
  const runConsolidation = async () => {
    ...
    try {
      ...
      await client.post('/api/v1/advanced-finance/consolidation/run', { periodStart, periodEnd, eliminateIntercompany: true });
      await fetchOverview();
    } finally {
      setRunning(false);
    }
  ...
  const columns: Column<Entity>[] = [
    ...
    { key: 'revenue', header: 'Revenue', align: 'right' as const, render: (row) => <span className="text-sm">{fmtFull(row.revenue)}</span> },
    { key: 'expenses', header: 'Expenses', align: 'right' as const, render: (row) => <span className="ui-text-sm-muted">{fmtFull(row.expenses)}</span> },
    {
      key: 'netIncome', header: 'Net Income', align: 'right' as const,
      render: (row) => <span style={{ color: row.netIncome >= 0 ? 'var(--color-success)' : 'var(--color-danger)' }} className={styles.s2}>{fmtFull(row.netIncome)}</span>,
    },
    { key: 'assets', header: 'Total Assets', align: 'right' as const, render: (row) => <span className="text-sm">{fmtFull(row.assets)}</span> },
      ...
      <PageHeader title="Financial Consolidation" description="Multi-entity consolidated financial statements and inter-company eliminations"
        ...
        actions={
          <Button variant="primary" onClick={runConsolidation} disabled={running}>
            <GitMerge size={14} className="mr-2" /> {running ? 'Running…' : 'Run Consolidation'}
          </Button>
        }
      />
