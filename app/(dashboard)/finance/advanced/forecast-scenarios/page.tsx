...
export default function ForecastScenariosPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setScenarios(scnRes); setAccounts(accRes.filter(a => a.type === 'REVENUE' || a.type === 'EXPENSE'));
    } catch {
    } finally {
      setLoading(false);
    }
  ...
  const addBudgetLine = async (e: React.FormEvent) => {
    ...
    try {
        ...
        });
      {
        ...
        const list = await client.get<ForecastScenario[]>(`${API}/forecast-scenarios`);
        ...
        <div className="ui-stack-3">
          ...
          {scenarios.map(s => (
            <Card
              ...
              style={{ border: selected?.id === s.id ? '2px solid var(--color-primary)' : '1px solid var(--color-border)' }} className={styles.s9}
            >
              <div className="p-5">
        ...
        {selected ? (
                  ...
                  <form onSubmit={addBudgetLine} className={styles.s19}>
                    ...
                    <div className="ui-grid-3">
                      <div className="ui-form-group">
                        ...
                        <input className="ui-input" type="number" step="0.01" required placeholder="0.00" value={budgetForm.amount} onChange={e => setBudgetForm({ ...budgetForm, amount: e.target.value })} />
                      </div>
                      <div className="ui-form-group">
                        <label className="ui-label">From *</label>
                        <input className="ui-input" type="date" required value={budgetForm.startDate} onChange={e => setBudgetForm({ ...budgetForm, startDate: e.target.value })} />
                      </div>
                      <div className="ui-form-group">
                        <label className="ui-label">To *</label>
                        <input className="ui-input" type="date" required value={budgetForm.endDate} onChange={e => setBudgetForm({ ...budgetForm, endDate: e.target.value })} />
                      </div>
                    </div>
                    <div className="ui-flex-end ui-gap-2">
                ...
                {(selected.budgets || []).length === 0 ? (
                  ...
                  <div className="ui-stack-2">
                    {(selected.budgets || []).map(b => (
                      ...
                      >
                        <div>
                          <span className="font-medium">{b.account?.name || 'Account'}</span>
                    ...
                    <div className={styles.s24}>
                      ...
                      <span className="ui-text-primary">${totalBudget(selected).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                    </div>
                  </div>
                )}
              </div>
          ...
          <Card>
            <div className={styles.s25}>
              ...
              <p className="text-sm">Select a scenario to view its budget lines and projections.</p>
            </div>
          </Card>
        )}
      </div>
