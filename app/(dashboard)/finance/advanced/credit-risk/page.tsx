...
export default function CreditRiskPage() {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      setList(
        await client.get<CreditRiskEntry[]>("/advanced-finance/credit-risk"),
      );
    } catch {
      alert("Unable to load credit risk data.");
    } finally {
      setLoading(false);
    }
  }, [client]);
  ...
  const fetchSummary = useCallback(
    async (id: string) => {
      ...
      try {
        const data = await client.get<CreditSummary>(
          `/advanced-finance/customers/${id}/credit`,
        );
        setSummary(data);
        setEditData({
          creditLimit:
        ...
        alert("Unable to load the credit risk summary.");
      } finally {
  ...
  const handleSaveCredit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editData.creditLimit !== "")
        body.creditLimit = parseFloat(editData.creditLimit);
      if (editData.paymentTerms !== "")
        body.paymentTerms = parseInt(editData.paymentTerms);
      body.creditHold = editData.creditHold;
      if (editData.creditHoldReason)
        body.creditHoldReason = editData.creditHoldReason;
      body.riskRating = editData.riskRating;

      await client.patch(
        `/advanced-finance/customers/${selectedId}/credit`,
        body,
      );
      setEditMode(false);
      ...
      alert("Unable to update the credit limit.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleToggleHold = async (id: string, currentHold: boolean) => {
    if (
      !confirm(
        `${currentHold ? "Release" : "Place"} credit hold for this customer?`,
      )
    )
      return;
    try {
      await client.patch(`/advanced-finance/customers/${id}/credit`, {
        creditHold: !currentHold,
      });
      if (selectedId === id) fetchSummary(id);
      fetchList();
    } catch {
      alert(`Unable to ${currentHold ? "release" : "place"} the credit hold.`);
    }
  };
        ...
        <div className="ui-flex-between ui-items-start">
          <div>
            ...
            <p className="ui-text-muted mt-1">
              Customer credit limits, holds, risk ratings, and outstanding
              balance monitoring.
            </p>
          </div>
            ...
            <Card key={kpi.label} className="ui-card p-5">
              <div className="ui-flex-between">
                <div>
                  ...
                  <p style={{ color: kpi.color }} className={styles.s2}>
                    {kpi.value}
                  </p>
                </div>
                <div
                ...
                [
                  {
                    ...
                    render: (v, row) => (
                      <span
                        className={styles.s9}
                        onClick={() =>
                          handleSelectCustomer(String(row.customerId))
                        }
                      >
                        {Boolean(row.creditHold) && (
                          <Lock size={12} className={styles.s10} />
                        )}
                        {String(v)}
                      </span>
                    ),
                  ...
                  {!editMode ? (
                    ...
                    <form onSubmit={handleSaveCredit} className="ui-stack-3">
                      ...
                      <div className="ui-hstack-3">
                        ...
                        />
                        <label htmlFor="hold-check" className={styles.s23}>
                          Credit Hold
                        </label>
                      </div>
                      {editData.creditHold && (
                      ...
                      <Button type="submit" variant="primary" disabled={saving}>
                        {saving ? (
                          <Loader2
                            className={`animate-spin ${styles.s24}`}
                            size={14}
                          />
                        ) : null}
                        Save Changes
