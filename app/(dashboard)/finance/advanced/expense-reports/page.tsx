...
export default function ExpenseManagementPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await client.get<ExpenseReport[] | { data?: ExpenseReport[] }>('/advanced-finance/expense-reports');
  ...
  const refreshDetail = useCallback(async (id: string) => {
    setDetailReport(await client.get<ExpenseReport>(`/advanced-finance/expense-reports/${id}`));
  }, [client]);
  ...
  const runOcrScan = async () => {
    setOcrScanning(true);
    try {
      const data = await client.post<{ extracted?: { merchant?: string; amount?: number; date?: string; suggestedCategory?: string } }>('/advanced-finance/expenses/ocr-scan', { fileName: 'receipt.jpg', rawText: ocrText });
      if (data.extracted) {
  ...
  const addItem = async () => {
    ...
    try {
      await client.post(`/advanced-finance/expense-reports/${detailReport.id}/items`, {
  ...
  const columns: Column<ExpenseReport>[] = [
      ...
      render: (row) => (
        <div className={styles.s1} onClick={() => refreshDetail(row.id)}>
          <div className="ui-heading-sm">
            {row.title} {row.hasPolicyViolation && <ShieldAlert size={13} className={styles.s2} />}
          </div>
    ...
    { key: 'totalAmount', header: 'Amount', align: 'right' as const, render: (row) => <span className="font-semibold">{fmt(row.totalAmount)}</span> },
    { key: 'createdAt', header: 'Created', render: (row) => <span className="ui-text-xs-muted">{new Date(row.createdAt).toLocaleDateString()}</span> },
  ...
  return (
    <RouteGuard permission="finance.expense.read">
      ...
      <PageHeader title="Expense Management" description="Employee expense reports, OCR receipt capture, policy enforcement, and reimbursement"
        ...
        actions={<Button variant="primary" onClick={() => setCreateOpen(true)}><Plus size={14} className="mr-2" /> New Report</Button>}
      />
        ...
        {detailReport && (
          <div className="ui-stack-4">
            <div className="ui-flex-between">
              ...
              <span className={styles.s4}>{fmt(detailReport.totalAmount)}</span>
            </div>
                ...
                <div key={item.id} className={styles.s6}>
                  <div>
                    <div className="text-sm">
                      {item.description} {item.policyViolation && <span title={item.policyViolationReason || ''}><ShieldAlert size={12} className={styles.s2} /></span>}
                    </div>
                    <div className="ui-text-xs-tertiary">{item.category} · {item.merchant || 'N/A'} · {new Date(item.expenseDate).toLocaleDateString()}</div>
                  </div>
                ...
                <div className="ui-stack-2">
                  <FormField label="Scan receipt text (simulated OCR)">
                    <div className="ui-flex ui-gap-2">
                      ...
                      <Button variant="secondary" onClick={runOcrScan} disabled={ocrScanning || !ocrText}><Scan size={14} className="mr-1" /> Scan</Button>
                    </div>
                  </FormField>
