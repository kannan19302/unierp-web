        ...
        <div className="ui-flex-between ui-items-start">
          <div>
            ...
            <p className="ui-text-muted mt-1">
              Outstanding receivables by aging bucket — Current, 1–30, 31–60,
              61–90, 90+ days overdue.
            </p>
            {report && (
                ...
                <div className="ui-flex-between">
                  <div>
                    ...
                    <p className={styles.s6}>
                      {fmt(
                        report.buckets["1-30"].total +
                          report.buckets["31-60"].total +
                          report.buckets["61-90"].total +
                          report.buckets["90+"].total,
                      )}
                    </p>
                  </div>
              ...
              <Card className="ui-card p-5">
                ...
                <div className={styles.s13}>
                  {BUCKET_CONFIG.map(({ key, color }: any) => {
                    ...
                    const pct =
                      report.grandTotal > 0
                        ? (bucket.total / report.grandTotal) * 100
                        : 0;
                  ...
                  return (
                    <Card key={key} className={`ui-card ${styles.s15}`}>
                      <div
                        ...
                        onClick={() =>
                          setExpandedBucket(isExpanded ? null : key)
                        }
                      >
                        ...
                        <div className={styles.s16}>
                          <div className="text-right">
                            <p className={styles.bucketTotal} style={{ color }}>
                              {fmt(bucket.total)}
                            </p>
                            <p className="ui-text-xs-muted">
                              {bucket.count} invoice
                              {bucket.count !== 1 ? "s" : ""} · {pct.toFixed(1)}
                              % of total
                            </p>
                          </div>
                      ...
                      {isExpanded && bucket.invoices.length > 0 && (
                        ...
                        >
                          <ListPageTemplate
                            ...
                            data={
                              bucket.invoices as unknown as Record<
                                string,
                                unknown
                              >[]
                            }
