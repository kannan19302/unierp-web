...
export default function BudgetScenariosPage() {
  ...
  const fetchScenarios = useCallback(async () => {
    ...
    try {
      ...
      setScenarios(scenRes); setAccounts(accRes);
    } catch {
  ...
  const fetchScenarioLines = useCallback(async (scenarioId: string) => {
    ...
    setEditedCells({});
    try {
      const data = await client.get<{ lines: BudgetScenarioLine[] }>(`${API}/budget-scenarios/${scenarioId}`);
      setLines(data.lines);
    } catch {
  ...
  const handleClone = async () => {
    ...
    setActionLoading(true);
    try {
      await client.post(`${API}/budget-scenarios/${selectedScenario.id}/clone`, cloneForm);
      {
  ...
  const handleLockToggle = async () => {
    ...
    try {
      await client.post(`${API}/budget-scenarios/${selectedScenario.id}/${endpoint}`);
      {
        setSuccess(selectedScenario.isLocked ? 'Scenario unlocked' : 'Scenario approved and locked');
        // Refresh scenario
        setSelectedScenario(await client.get<BudgetScenario>(`${API}/budget-scenarios/${selectedScenario.id}`));
        fetchScenarios();
      }
    } catch {
  ...
  const handleDriverApply = async () => {
    ...
    setActionLoading(true);
    try {
      await client.post(`${API}/budget-scenarios/${selectedScenario.id}/driver`, {
  ...
  const handleSaveCell = async (accountId: string, month: number) => {
    ...
    try {
      await client.post(`${API}/budget-scenarios/${selectedScenario.id}/lines`, {
          accountId,
          month,
    ...
    <div className="ui-page-container">
      ...
      {!selectedScenario ? (
        <Card className="ui-list-card">
          {loading ? (
            <div className="ui-loading"><Loader2 className="animate-spin mr-2" size={20} /> Loading budget scenarios...</div>
          ) : scenarios.length === 0 ? (
            ...
            <ListPageTemplate
              columns={[
                { key: 'name', header: 'Scenario Name', render: (v, row) => (
                  <div onClick={() => setSelectedScenario(row as any)} className="cursor-pointer">
                    <div className="font-semibold text-blue-600 hover:underline">{String(v)}</div>
                    {Boolean(row.description) && <div className="text-xs text-gray-500">{String(row.description)}</div>}
                  </div>
                ) },
                { key: 'type', header: 'Type', render: (v) => <span className="text-xs font-semibold px-2 py-0.5 bg-gray-100 rounded">{String(v)}</span> },
                { key: 'fiscalYear', header: 'Fiscal Year' },
                { key: '_count', header: 'Lines Entered', render: (v) => <span className="font-medium text-gray-600">{(v as any)?.lines || 0}</span> },
                { key: 'isLocked', header: 'Status', render: (v) => <span className={`ui-badge ${v ? 'ui-badge-green' : 'ui-badge-gray'}`}>{v ? 'LOCKED / APPROVED' : 'DRAFT'}</span> },
                { key: 'id', header: 'Actions', render: (v) => (
                  <button onClick={(e) => { e.stopPropagation(); handleDelete(String(v)); }} className="ui-action-btn ui-action-btn-danger" title="Delete"><Trash2 size={14} /></button>
                ) },
              ] as ListColumn[]}
        ...
        <div className="flex flex-col gap-4">
          ...
          <Card className="ui-list-card overflow-x-auto">
            ...
            {linesLoading ? (
              <div className="ui-loading"><Loader2 className="animate-spin mr-2" size={20} /> Loading grid...</div>
            ) : (
              <table className="ui-table min-w-[1200px]">
                <thead>
                  <tr>
                    <th className="ui-th sticky left-0 bg-white z-10 w-[200px]">GL Account</th>
                    {Array.from({ length: 12 }, (_, i) => (
                      <th key={i + 1} className="ui-th text-right w-[90px]">Month {i + 1}</th>
                    ))}
