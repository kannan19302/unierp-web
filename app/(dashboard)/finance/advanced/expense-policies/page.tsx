...
interface Policy {
  id: string;
  category: string;
  maxAmountPerItem: string | number | null;
  receiptRequiredAbove: string | number;
  isActive: boolean;
}
interface MileageRate {
  id: string;
  ratePerMile: string | number;
...
interface CardTransaction {
  ...
  status: string;
  card?: { last4: string; provider: string };
}
...
export default function ExpensePoliciesPage() {
    ...
    try {
      const [
        ...
        client.get<CardTransaction[]>(
          "/advanced-finance/corporate-card-transactions/unmatched",
        ),
      ]);
  ...
  const savePolicy = async () => {
    await client.post("/advanced-finance/expense-policies", {
      category: pCategory,
      maxAmountPerItem: pMax ? Number(pMax) : null,
      receiptRequiredAbove: pReceiptAbove ? Number(pReceiptAbove) : 0,
    });
  ...
  const saveMileageRate = async () => {
    await client.post("/advanced-finance/mileage-rates", {
      ratePerMile: Number(mRate),
      effectiveDate: mDate,
    });
  ...
  const savePerDiemRate = async () => {
    await client.post("/advanced-finance/per-diem-rates", {
      location: dLocation,
      dailyRate: Number(dRate),
    });
    setPerDiemModal(false);
    setDLocation("");
    setDRate("");
    await load();
  };
  ...
  const saveCard = async () => {
    await client.post("/advanced-finance/corporate-cards", {
  ...
  const ignoreTransaction = async (id: string) => {
    await client.post(
      `/advanced-finance/corporate-card-transactions/${id}/ignore`,
    );
    await load();
  };
    ...
    <RouteGuard permission="finance.expense.read">
      <div className="ui-stack-6">
        ...
        {tab === "policies" && (
          <Card padding="none">
            <div className={styles.s1}>
              <Button variant="primary" onClick={() => setPolicyModal(true)}>
                <Plus size={14} className="mr-2" /> Add Policy
              </Button>
            </div>
            <DataTable
        ...
        {tab === "mileage" && (
          <Card padding="none">
            <div className={styles.s1}>
              <Button variant="primary" onClick={() => setMileageModal(true)}>
        ...
        {tab === "perdiem" && (
          <Card padding="none">
            <div className={styles.s1}>
              <Button variant="primary" onClick={() => setPerDiemModal(true)}>
                <Plus size={14} className="mr-2" /> Add Rate
              </Button>
            </div>
            <DataTable
        ...
        {tab === "cards" && (
          <>
            <Card padding="none">
              <div className={styles.s1}>
                <Button variant="primary" onClick={() => setCardModal(true)}>
                  <Plus size={14} className="mr-2" /> Register Card
                </Button>
              </div>
              <DataTable
                columns={cardColumns}
        ...
        <Modal
          ...
          title="Add Category Policy"
          size="sm"
          footer={
            <>
              <Button variant="secondary" onClick={() => setPolicyModal(false)}>
                Cancel
              </Button>
        ...
        <Modal
          ...
          footer={
            <>
              <Button
                variant="secondary"
                onClick={() => setMileageModal(false)}
              >
                Cancel
              </Button>
              <Button variant="primary" onClick={saveMileageRate}>
                Save
              </Button>
            </>
          }
        ...
        <Modal
          open={perDiemModal}
          onClose={() => setPerDiemModal(false)}
          title="Add Per Diem Rate"
          size="sm"
          footer={
            <>
              <Button
                variant="secondary"
                onClick={() => setPerDiemModal(false)}
              >
                Cancel
              </Button>
              <Button variant="primary" onClick={savePerDiemRate}>
                Save
              </Button>
            </>
          }
