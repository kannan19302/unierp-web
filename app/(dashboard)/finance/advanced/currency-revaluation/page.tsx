...
export default function CurrencyRevaluationPage() {
  ...
  const fetchHistory = async () => {
    try {
      setHistory(
        await client.get<Revaluation[]>(
          "/advanced-finance/currency-revaluations",
        ),
      );
    } catch {
  ...
  const runRevaluation = async () => {
    ...
    setError(null);
    try {
      await client.post("/advanced-finance/currency-revaluations/run", {
  ...
  return (
    <RouteGuard permission="finance.treasury.read">
      <div className="p-8 ui-stack-6">
        <div className="ui-flex-between">
          <div>
            ...
            <p className="ui-text-muted mt-1">
              Revalue open foreign-currency AR balances and recognise unrealized
              FX gain/loss.
            </p>
          </div>
        </div>
        ...
        {error && (
          <div
            className={`border-amber-300 bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-300 ${styles.s1}`}
          >
            {error}
          </div>
        )}
            ...
            <Card>
              <div className="p-5">
                <div className={styles.s5}>
                  <TrendingUp className="ui-text-success" /> Latest Unrealized
                  Gain
                </div>
                <div className={styles.s6}>
                  {latest.baseCurrency} {money(latest.totalGain)}
                </div>
              </div>
            </Card>
            <Card>
              <div className="p-5">
                <div className={styles.s5}>
                  <TrendingDown className="ui-text-danger" /> Latest Unrealized
                  Loss
                </div>
                <div className={styles.s7}>
                  {latest.baseCurrency} {money(latest.totalLoss)}
                </div>
              </div>
            </Card>
            <Card>
              <div className="p-5">
                <div className={styles.s5}>
                  <RefreshCw className="ui-text-primary" /> Net Adjustment
                </div>
                <div
        ...
        <Card>
          <div className="p-6">
            <h3 className={styles.s8}>Revaluation History</h3>
          </div>
          <div className={styles.s9}>
            {history.length === 0 && (
              <div className={styles.s10}>
                ...
                <p>
                  Run a revaluation above to recognise unrealized FX gains and
                  losses.
                </p>
              </div>
            )}
                ...
                <div key={r.id} className={styles.s12}>
                  ...
                  >
                    <div className="ui-hstack-3">
                      ...
                      <div>
                        ...
                        <div className="ui-text-xs-muted">
                          ...
                          base {r.baseCurrency} · {r.lines?.length || 0} items
                        </div>
                      </div>
                    </div>
                    <div className="ui-hstack-4">
                      ...
                      {r.journalId && (
                        <span className={styles.s14}>Posted</span>
                      )}
                    </div>
                  </button>
                  {isOpen && (
