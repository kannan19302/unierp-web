...
export default function FxRevaluationPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      setRuns(await client.get<FxRevaluationRun[]>(`${API_BASE}/fx-revaluation/runs`));
    } catch {
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateDraft = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const data = await client.post<FxRevaluationRun>(`${API_BASE}/fx-revaluation/runs`, {
          runDate,
          targetCurrency,
          notes,
        });
      if (data) {
        ...
        setDraftRun(await client.get<FxRevaluationRun>(`${API_BASE}/fx-revaluation/runs/${data.id}/details`));
  ...
  const handlePostRun = async (id: string) => {
    setSubmitting(true);
    try {
      const res = await client.post(`${API_BASE}/fx-revaluation/runs/${id}/post`);
      if (res) {
  ...
  const viewDetails = async (id: string) => {
    setLoading(true);
    try {
      setDraftRun(await client.get<FxRevaluationRun>(`${API_BASE}/fx-revaluation/runs/${id}/details`));
      {
        setShowPreviewModal(true);
      }
    } catch (e) {
  ...
  return (
    ...
    <div className="p-8 ui-stack-6">
      ...
      <Card className="ui-card">
        ...
        <ListPageTemplate
          columns={[
            { key: 'runDate', header: 'Revaluation Date', render: (v) => <span className="font-semibold">{new Date(String(v)).toLocaleDateString()}</span> },
            { key: 'targetCurrency', header: 'Target Currency' },
            { key: 'status', header: 'Status', render: (v) => (
              <span style={{ background: v === 'POSTED' ? 'rgba(34,197,94,0.1)' : 'rgba(245,158,11,0.1)', color: v === 'POSTED' ? '#22c55e' : '#f59e0b' }} className={styles.s6}>{String(v)}</span>
            ) },
            { key: 'journalId', header: 'Linked Journal', render: (v) => v ? <span className={styles.s7}><FileText size={14} /> GL ID: {String(v).substring(0, 8)}</span> : '—' },
            { key: 'notes', header: 'Notes', render: (v) => <span className="ui-text-muted">{String(v || '—')}</span> },
            { key: 'id', header: 'Action', render: (v) => <Button variant="outline" size="sm" onClick={() => viewDetails(String(v))}>View Details</Button> },
          ] as ListColumn[]}
        ...
        <div className={styles.s8}>
          <Card className={`ui-card ${styles.s11}`} >
            <div className={styles.s10}>
              <div>
                ...
                <p className={styles.s12}>
                  Run Date: {new Date(draftRun.runDate).toLocaleDateString()} • Status: <span className={styles.s13}>{draftRun.status}</span>
                </p>
              </div>
            ...
            <div className="p-5 ui-stack-4">
              <ListPageTemplate
                columns={[
                  { key: 'account', header: 'GL Account', render: (v, row) => <span><span className={styles.s13}>{(v as any)?.code || '1200-AR'}</span> — {(v as any)?.name || 'A/R'}</span> },
                  { key: 'entityType', header: 'Entity Type' },
                  { key: 'balanceInForeign', header: 'Foreign Balance', render: (v) => `${String(v)} ${draftRun!.targetCurrency}` },
                  { key: 'originalAmountBase', header: 'Original value (Base)', render: (v) => fmt(Number(v)) },
                  { key: 'revaluedAmountBase', header: 'Revalued value (Base)', render: (v) => fmt(Number(v)) },
                  { key: 'unrealizedGainLoss', header: 'Unrealized Gain/Loss', render: (v) => <span style={{ color: Number(v) >= 0 ? '#22c55e' : '#ef4444' }} className={styles.s14}>{Number(v) > 0 ? '+' : ''}{fmt(Number(v))}</span> },
                ] as ListColumn[]}
                data={((draftRun.details || []) as unknown as Record<string, unknown>[])}
                loading={false}
                emptyTitle="No lines"
