...
import { Card, Button, StatusBadge, PageHeader, ListPageTemplate, type ListColumn } from '@unerp/ui';
...
const reportMeta: Record<ReportType, { label: string; icon: React.ReactNode; description: string }> = {
  'pnl': { label: 'Profit & Loss', icon: <BarChart3 className={styles.s1} />, description: 'Revenue, expenses, and net profit for a period' },
  'balance-sheet': { label: 'Balance Sheet', icon: <PieChart className={styles.s1} />, description: 'Assets, liabilities, and equity as of a date' },
  'cash-flow': { label: 'Cash Flow', icon: <TrendingUp className={styles.s1} />, description: 'Operating, investing, and financing activities' },
  'trial-balance': { label: 'Trial Balance', icon: <BookOpen className={styles.s1} />, description: 'All accounts with debit/credit totals' },
  'aging': { label: 'AR/AP Aging', icon: <Clock className={styles.s1} />, description: 'Overdue invoices/purchase orders by aging bucket' },
};
...
export default function AdvancedReportsPage() {
  ...
  React.useEffect(() => {
    const fetchBooks = async () => {
      try {
        setBooks(await client.get<AccountingBook[]>('/api/v1/advanced-finance/accounting-books'));
      } catch {
      }
    };
    fetchBooks();
    ...
    return (
      <div className="ui-stack-6">
        ...
        <div>
          ...
          <ListPageTemplate
            columns={[
              { key: 'name', header: 'Account', render: (v, row) => <span>{String(v)} <span className={styles.s5}>({String(row.code)})</span></span> },
              { key: 'amount', header: 'Amount', render: (v) => formatCurrency(Number(v)) },
            ] as ListColumn[]}
            data={((d.revenueBreakdown || []) as unknown as Record<string, unknown>[])}
            loading={false} emptyTitle="No revenue" emptyDescription=""
          />
        </div>
        ...
        <div>
          ...
          <ListPageTemplate
            ...
            data={((d.expenseBreakdown || []) as unknown as Record<string, unknown>[])}
            loading={false} emptyTitle="No expenses" emptyDescription=""
          />
        </div>
        ...
        <div className={`rounded-xl p-5 ${d.netProfit >= 0 ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
          <div className="ui-flex-between">
            <span className="ui-heading-lg">Net {d.netProfit >= 0 ? 'Profit' : 'Loss'}</span>
    ...
    return (
      <div className="ui-stack-6">
        <div className={styles.s2}>
          ...
          <p className={styles.s3}>As of {new Date(d.asOfDate).toLocaleDateString()}</p>
        </div>
        ...
        <div>
          ...
          <ListPageTemplate
            ...
            data={(([...d.assets.current.accounts, ...d.assets.nonCurrent.accounts]) as unknown as Record<string, unknown>[])}
            loading={false} emptyTitle="No accounts" emptyDescription=""
          />
        </div>
        ...
        <div>
          ...
          <ListPageTemplate
            ...
            data={(([...d.liabilities.current.accounts, ...d.liabilities.nonCurrent.accounts]) as unknown as Record<string, unknown>[])}
            loading={false} emptyTitle="No accounts" emptyDescription=""
          />
        </div>
        ...
        <div className={styles.s9}>
          <div className="ui-flex-between">
            ...
            <span className={styles.s10}>{formatCurrency(d.totalLiabilitiesAndEquity)}</span>
          </div>
        </div>
      </div>
    );
  ...
  const renderTrialBalance = () => {
    ...
    return (
      <div className="ui-stack-6">
        ...
        <ListPageTemplate
          columns={[
            { key: 'code', header: 'Code', render: (v) => <span className="text-xs">{String(v)}</span> },
            { key: 'name', header: 'Account' },
            { key: 'type', header: 'Type', render: (v) => <StatusBadge status={String(v)} /> },
            { key: 'debitTotal', header: 'Debit Total', render: (v) => formatCurrency(Number(v)) },
            { key: 'creditTotal', header: 'Credit Total', render: (v) => formatCurrency(Number(v)) },
            { key: 'balance', header: 'Balance', render: (v) => formatCurrency(Number(v)) },
  ...
  const renderAging = () => {
    ...
    return (
      <div className="ui-stack-6">
        <div className={styles.s2}>
          <h2 className="text-2xl">{d.type === 'AR' ? 'Accounts Receivable' : 'Accounts Payable'} Aging Report</h2>
          <p className={styles.s3}>As of {new Date(d.asOfDate).toLocaleDateString()}</p>
        </div>
        ...
        <div className={styles.s12}>
          {Object.entries(d.bucketTotals).map(([bucket, data]) => (
            <div key={bucket} className={`rounded-lg p-4 border ${bucketColors[bucket] || 'bg-gray-50 border-gray-200'}`}>
              ...
              <div className="text-sm">{formatCurrency(data.totalOutstanding)}</div>
            </div>
          ))}
            ...
            <div key={bucket}>
              ...
              <ListPageTemplate
                columns={[
                  ...
                  { key: 'documentNumber', header: 'Document', render: (v) => <span className="text-xs">{String(v)}</span> },
                  { key: 'totalAmount', header: 'Total', render: (v) => formatCurrency(Number(v)) },
                  { key: 'outstanding', header: 'Outstanding', render: (v) => formatCurrency(Number(v)) },
                  { key: 'daysOverdue', header: 'Days Overdue' },
                ] as ListColumn[]}
                data={(items as unknown as Record<string, unknown>[])}
                loading={false} emptyTitle="No items" emptyDescription=""
              />
            </div>
          );
        })}
      ...
      return (
        <div className={styles.s17}>
          <div className={styles.s18}>
            <Loader2 className={`h-10 w-10 animate-spin mx-auto ${styles.s19}`}  />
          ...
          <Card className="ui-flex-col">
            ...
            <div className={styles.s24}>
              ...
              <Button onClick={generateReport} disabled={loading}>
                {loading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <FileText className="mr-2" />}
                Generate Report
              </Button>
            </div>
            ...
            <div className={styles.s30}>
              {!reportData && !loading && !error && (
                <div className={styles.s31}>
                  ...
                  <p className={styles.s32}>Select a report and click Generate</p>
                  <p className={styles.s33}>Choose from the left panel and set date filters</p>
                </div>
              )}
              {renderReport()}
            </div>
