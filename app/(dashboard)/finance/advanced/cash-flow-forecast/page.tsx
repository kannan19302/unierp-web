...
export default function CashFlowForecastPage() {
  ...
  const fetchForecast = useCallback(async () => {
    try {
      const path =
        selectedScenarioId && selectedScenarioId !== "baseline"
  ...
  const fetchScenarios = useCallback(async () => {
    try {
      setScenarios(
        await client.get<Scenario[]>(
          "/advanced-finance/cash-flow/forecast/scenarios",
        ),
      );
    } catch {}
  }, [client]);
  ...
  const handleExportCsv = async () => {
    try {
      const csvText = await client.text(
        "/advanced-finance/cash-flow/forecast/export",
      );
      ...
      link.setAttribute(
        "download",
        `13_week_cash_flow_forecast_${selectedScenarioId}.csv`,
      );
  ...
  const handleDeleteScenario = async (id: string) => {
    if (!confirm("Are you sure you want to delete this forecast scenario?"))
      return;
    try {
      await client.delete(
        `/advanced-finance/cash-flow/forecast/scenarios/${id}`,
      );
        ...
        <div className={styles.s1}>
          <div>
            ...
            <p className="ui-text-muted mt-1">
              Model rolling liquidity week-by-week by projecting AR Invoices, AP
              Payment Schedules, manual weekly adjustments, and scenarios.
            </p>
          </div>
        ...
        {forecastData && (
          <>
            ...
            <div className={`ui-grid-3 ${styles.s3}`}>
              {[
                {
                  label: "Forecast Pool Scenario",
                  value: forecastData.scenarioName,
                  icon: <Sliders size={20} />,
                  color: "var(--color-primary)",
                  bg: "var(--color-primary-light)",
                },
                {
                  label: "Starting Cash Pool",
                  value: fmt(forecastData.startingCash),
                  icon: <Calculator size={20} />,
                  color: "var(--color-primary)",
                  bg: "var(--color-primary-light)",
                },
                {
                  label: "Projected Ending cash (Week 13)",
                  value: fmt(forecastData.forecast[12]?.cumulativeBalance || 0),
            ...
            <Card className="ui-card">
              <div className={styles.s5}>
                <h3 className="ui-heading-base">
                  Weekly Forecast Calculations
                </h3>
                <span className="ui-text-xs-muted">
                  Click Adjust to register manual overrides (tax, payroll,
              ...
              <div className="builder-table-wrapper">
                <ListPageTemplate
                      ...
                      },
                      {
                        ...
                        render: (v, row) => (
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              setTargetWeek(row as any);
                              setOverrideAdjustment(Number(row.adjustments));
                              setOverrideComments(String(row.comments || ""));
                              setShowOverrideModal(true);
                            }}
                            className={styles.s9}
                          >
                            <Edit size={12} className={styles.s10} />
                            Adjust
                          </Button>
                        ),
                      },
                  ...
                  data={
                    forecastData.forecast as unknown as Record<
                      string,
                      unknown
                    >[]
                  }
            ...
            <Card className="ui-card">
              ...
              {scenarios.length === 0 ? (
                <div className={styles.s12}>
                  No custom scenarios created. Use Configure Scenario button to
                  ...
                  {scenarios.map((sc) => (
                    <div key={sc.id} className={styles.s13}>
                      ...
                      <div className="ui-hstack-4">
                        <span className="ui-text-xs-muted">
                          Inflow Multiplier:{" "}
                          <b>{Number(sc.inflowFactor).toFixed(2)}x</b> • Outflow
                          Multiplier:{" "}
                          <b>{Number(sc.outflowFactor).toFixed(2)}x</b>
                        </span>
                        <Button
                          variant="outline"
                          size="sm"
            ...
            <Card className={`ui-card ${styles.s16}`}>
              ...
              <form onSubmit={handleSaveOverride} className="p-5 ui-stack-4">
                <div className="ui-form-group">
                  <label className="ui-label ui-label">Week Starting</label>
                  <input
                    type="text"
                    className="ui-input"
                ...
                <div className="ui-form-group">
                  <label className="ui-label ui-label">Comments / Notes</label>
                  <textarea
          ...
          <div className={styles.s19}>
            <Card className={`ui-card ${styles.s20}`}>
              ...
              <form onSubmit={handleCreateScenario} className="p-5 ui-stack-4">
                <div className="ui-form-group">
                  <label className="ui-label ui-label">Scenario Name</label>
                  <input
                    type="text"
                    className="ui-input"
                    value={scenarioName}
