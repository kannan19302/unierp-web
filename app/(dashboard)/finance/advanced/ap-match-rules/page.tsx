...
export default function APMatchRulesPage() {
  ...
  const fetchRules = useCallback(async () => {
    ...
    try {
      setRules(
        await client.get<APMatchRule[]>(
          "/advanced-finance/payables/match-rules",
        ),
      );
    } catch {
      /* network error */
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingId)
        await client.patch(
          `/advanced-finance/payables/match-rules/${editingId}`,
          body,
        );
        ...
        <div className="ui-page-head">
          <div className="ui-page-head-content">
            ...
            <p className="ui-page-subtitle">
              Configure price and quantity tolerance thresholds for automated PO
        ...
        {showForm && (
          <Card className="ui-form-card mb-4">
            <h3 className="ui-form-title">
              {editingId ? "Edit Rule" : "New Match Rule"}
            </h3>
            <div className="ui-form-grid">
              <div className="ui-form-group">
                <label className="ui-label">
                  Vendor ID (leave blank for global default)
                </label>
                <input
                  className="ui-input"
                  value={form.vendorId}
        ...
        <Card className="ui-list-card">
          {loading ? (
            <div className="ui-loading">
              <Loader2 className="animate-spin" size={20} /> Loading rules…
            </div>
          ) : rules.length === 0 ? (
            <div className="ui-empty-state">
              <Settings size={40} className="ui-empty-icon" />
              <p>
                No AP match rules configured. Create a global rule to enable
                three-way matching.
              </p>
            </div>
          ) : (
