...
export default function TaxEnginePage() {
  ...
  const fetchData = async () => {
    try {
      const [rulesRes, wTaxesRes] = await Promise.all([
        client.get<TaxRule[]>("/advanced-finance/tax-rules"),
        client.get<WithholdingTax[]>("/advanced-finance/withholding-taxes"),
      ]);
      setRules(rulesRes);
  ...
  const handleCreateRule = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post("/advanced-finance/tax-rules", {
        name: ruleData.name,
  ...
  const handleCreateWithholding = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post("/advanced-finance/withholding-taxes", {
    ...
    <RouteGuard permission="finance.tax-engine.read">
      <div className="p-8 ui-stack-6">
        <div className="ui-flex-between">
          <div>
            ...
            <p className="ui-text-muted mt-1">
              Configure complex Tax Rules, GST components, and TDS logic.
            </p>
          </div>
        ...
        <div className="ui-grid-2">
          {/* Tax Rules */}
          <Card
            className={`border-primary/20 hover:border-primary/50 transition-colors ${styles.s2}`}
          >
            <div className={styles.s3}>
              <div className="ui-hstack-3">
                <div
                  className={`bg-purple-100 dark:bg-purple-900/30 ${styles.s4}`}
                >
                  <Percent
                    className={`text-purple-700 dark:text-purple-400 ${styles.s5}`}
                  />
                </div>
                <div>
                  <h3 className="ui-heading-lg">Sales & Purchase Tax Rules</h3>
                  <p className="ui-text-sm-muted">
                    e.g. Inter-state GST, Standard VAT
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className={styles.s7}>
              ...
              >
                Create Tax Rule <Plus className={styles.s8} />
              </Button>
            </div>
          </Card>
          ...
          >
            <div className={styles.s3}>
              <div className="ui-hstack-3">
                <div className={`bg-red-100 dark:bg-red-900/30 ${styles.s4}`}>
                  <ShieldAlert
                    className={`text-red-700 dark:text-red-400 ${styles.s5}`}
                  />
                </div>
                <div>
                  <h3 className="ui-heading-lg">Withholding Taxes (TDS)</h3>
                  <p className="ui-text-sm-muted">
                    Automated deductions on AP invoices
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className={styles.s7}>
              <Button
                variant="outline"
                className="w-full"
                onClick={() => setShowWithholdingForm(true)}
              >
                Add Withholding Tax <Plus className={styles.s8} />
              </Button>
            </div>
          </Card>
        </div>
