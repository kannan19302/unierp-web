...
export default function RecurringInvoicesPage() {
  ...
  const fetchSchedules = async () => {
    try {
      setSchedules(await client.get<RecurringSchedule[]>('/api/v1/advanced-finance/recurring-schedules'));
    } catch { /* empty */ }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      await client.post('/api/v1/advanced-finance/recurring-schedules', {
      ...
      setCreateOpen(false); fetchSchedules(); setForm({ frequency: 'MONTHLY', nextRunDate: '', description: '', customerName: '', amount: 0 });
    } catch { /* handled */ }
  ...
  const handleGenerate = async () => {
    ...
    setGenResult(null);
    try {
      setGenResult(await client.post<{ processed: number }>('/api/v1/advanced-finance/recurring/generate'));
    } catch { /* handled */ }
        ...
        actions={
          <div className="ui-flex ui-gap-2">
            <Button variant="outline" onClick={handleGenerate} disabled={generating}>
              <Play size={14} className="mr-2" /> {generating ? 'Generating...' : 'Run Now'}
            </Button>
