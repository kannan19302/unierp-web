...
export default function LeaseDetailPage() {
  ...
  const postMonth = async (row: ScheduleRow) => {
    ...
    setMsg(null);
    try {
      await apiPost(`/finance/leases/${id}/post-month`, {
        period: periodLabel(row.periodStart),
      });
      setMsg({
        text: `Posted ${periodLabel(row.periodStart)} successfully.`,
        type: "success",
      });
      load();
    } catch (e: any) {
  ...
  const terminate = async () => {
    ...
    try {
      ...
      setMsg({
        text: "Lease terminated and GL entry posted.",
        type: "success",
      });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <Link
            ...
            className="ui-text-muted hover:ui-text-primary"
          >
            <ArrowLeft size={18} />
          </Link>
          <div>
            <p className="text-xs ui-text-muted">Finance / Lease Accounting</p>
            <h1 className="text-xl font-bold ui-text-primary">
              {lease.leaseRef ?? lease.id}
            </h1>
            {lease.description && (
              <p className="text-sm ui-text-muted">{lease.description}</p>
            )}
          </div>
        </div>
      ...
      {msg && (
        <div
          className={`flex items-center gap-2 text-sm rounded p-3 ${msg.type === "success" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}
        >
          {msg.type === "success" ? (
            <CheckCircle size={14} />
          ) : (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            value:
              lease.initialRecognition != null
                ? fmt(Number(lease.initialRecognition))
                : "—",
          },
          {
            label: "Carrying Amount",
            value:
              lease.carryingAmount != null
                ? fmt(Number(lease.carryingAmount))
                : "—",
          },
          ...
          { label: "End Date", value: fmtDate(lease.endDate) },
          {
            ...
            value:
              lease.interestRate != null
                ? `${(Number(lease.interestRate) * 100).toFixed(2)}%`
                : "—",
          },
          {
      ...
      {showTerminate && (
        <Card className="p-4 border-red-200 bg-red-50 dark:bg-red-900/20 space-y-3">
          ...
          <p className="text-xs text-red-600">
            A GL entry will be posted to write off the remaining carrying
            amount.
          </p>
          <input
            type="date"
      ...
      <Card className="p-4">
        <h2 className="text-sm font-semibold ui-text-primary mb-4">
              ...
              },
              {
                ...
                render: (v) =>
                  v ? (
                    ...
                    <span className="text-xs ui-text-muted">Pending</span>
                  ),
              },
              {
                key: "id",
