...
export default function ARAutomationPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [levelsResponse, runsResponse, statsResponse] = await Promise.all([
        client.get<DunningLevel[] | { data?: DunningLevel[] }>(
          "/advanced-finance/dunning-levels",
        ),
        client.get<DunningRun[] | { data?: DunningRun[] }>(
          "/advanced-finance/dunning-runs",
        ),
        client.get<DunningStats>("/advanced-finance/dunning-stats"),
      ]);
      setLevels(
        Array.isArray(levelsResponse)
          ? levelsResponse
          : (levelsResponse.data ?? []),
      );
      setRuns(
        Array.isArray(runsResponse) ? runsResponse : (runsResponse.data ?? []),
      );
  ...
  const handleCreateLevel = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Unable to create the dunning level.");
    }
  };

  const handleDeleteLevel = async (id: string, name: string) => {
    if (!confirm(`Delete dunning level "${name}"? This cannot be undone.`))
      return;
    try {
      await client.delete(`/advanced-finance/dunning-levels/${id}`);
      fetchData();
    } catch {
      alert("Unable to delete the dunning level.");
    }
  };
  ...
  const handleExecuteDunning = async () => {
    if (
      !confirm(
        "Execute a dunning run now? This will apply late fees and send reminder emails to all overdue customers.",
      )
    )
      return;
    setRunningDunning(true);
    try {
      const result = await client.post<{ totalInvoices?: number }>(
        "/advanced-finance/dunning-runs",
      );
      alert(
        `Dunning run completed. ${result.totalInvoices ?? 0} invoice(s) processed.`,
      );
      fetchData();
    } catch {
      alert("Unable to complete the dunning run.");
    } finally {
  ...
  const handlePauseResume = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        `Dunning ${pauseAction === "pause" ? "paused" : "resumed"} for invoice.`,
      );
      setShowPauseForm(false);
      setPauseInvoiceId("");
    } catch {
      alert(`Unable to ${pauseAction} dunning for the invoice.`);
    }
  };
    ...
    <RouteGuard permission="finance.receivable.read">
      <div className="p-8 ui-stack-6">
        ...
        <div className="ui-flex-between ui-items-start">
          <div>
            ...
            <p className="ui-text-muted mt-1">
              Automated customer reminders, late fees, and collection cadences —
              NetSuite / Odoo parity.
            </p>
          </div>
          ...
          <div className={`ui-grid-3 ${styles.s1}`}>
            {[
              ...
              },
              {
                ...
                value: `$${stats.totalFeeCollected.toFixed(2)}`,
              ...
              <Card key={kpi.label} className="ui-card p-5">
                <div className="ui-flex-between">
                  <div>
                    ...
                    <p className={styles.kpiValue} style={{ color: kpi.color }}>
                      {kpi.value}
                    </p>
                  </div>
                  <div
        ...
        <div className="ui-flex ui-gap-2">
          ...
          >
            <Pause size={14} className={styles.s2} />
          ...
          >
            <RotateCcw size={14} className={styles.s3} />
          ...
          <Card className="ui-card">
            <div className="p-5">
              ...
              <form onSubmit={handlePauseResume} className={styles.s5}>
                ...
                <Button
                  type="submit"
                  variant={pauseAction === "pause" ? "danger" : "primary"}
                >
                  {pauseAction === "pause" ? "Pause Dunning" : "Resume Dunning"}
                </Button>
                <Button
        ...
        <div className={`ui-grid-2 ${styles.s8}`}>
          ...
          <Card className="ui-card ui-flex-col">
            <div className={styles.s9}>
              ...
              >
                <Plus size={14} className={styles.s12} />
                Add Level
              </Button>
            </div>
              ...
              {levels.length === 0 ? (
                <div className={styles.s14}>
                  ...
                  <p className={styles.s16}>
                    Add levels to start automated reminders.
                  </p>
                </div>
              ) : (
                <ListPageTemplate
              ...
              {runs.length === 0 ? (
                <div className={styles.s23}>
                  ...
                  <p className={styles.s25}>
                    Click Run Dunning to process overdue invoices.
                  </p>
                </div>
              ) : (
                <ListPageTemplate
