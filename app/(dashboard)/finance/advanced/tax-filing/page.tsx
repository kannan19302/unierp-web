...
export default function TaxFilingPage() {
  ...
  const fetchData = async () => {
    try {
      setFilings(await client.get<TaxFiling[]>('/api/v1/advanced-finance/tax-filings'));
    } catch {
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateFiling = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await client.post('/api/v1/advanced-finance/tax-filings', {
    ...
    <div className="p-8 ui-stack-6">
      <div className="ui-flex-between">
        <div>
          ...
          <p className="ui-text-muted mt-1">Generate regulatory compliance payloads and file returns.</p>
        </div>
            ...
            <form onSubmit={handleCreateFiling} className="ui-stack-4">
              <div className="ui-grid-3">
                <div className="ui-stack-2">
                  ...
                  <select className="ui-field-line" required value={filingData.filingType} onChange={e => setFilingData({ ...filingData, filingType: e.target.value })}>
                ...
                <div className="ui-stack-2">
                  ...
                  <input className="ui-field-line" type="date" required value={filingData.periodStart} onChange={e => setFilingData({ ...filingData, periodStart: e.target.value })} />
                </div>
                <div className="ui-stack-2">
                  <label className="ui-heading-sm">Period End</label>
                  <input className="ui-field-line" type="date" required value={filingData.periodEnd} onChange={e => setFilingData({ ...filingData, periodEnd: e.target.value })} />
                </div>
              </div>
      ...
      <Card className="ui-flex-col">
        <div className={styles.s2}>
          <div className="ui-hstack-3">
            <div className={`bg-indigo-100 dark:bg-indigo-900/30 ${styles.s3}`} >
              <UploadCloud className={`text-indigo-700 dark:text-indigo-400 ${styles.s4}`}  />
            </div>
            <div>
              <h3 className="ui-heading-lg">Statutory Filing Register</h3>
              <p className="ui-text-sm-muted">Historical and drafted compliance returns</p>
            </div>
          </div>
        </div>
        <div className={styles.s5}>
          {(() => {
            const filingColumns: ListColumn[] = [
              { key: 'filingType', header: 'Return Type', render: (v) => <span className="font-bold">{v as string}</span> },
              { key: 'periodStart', header: 'Period', render: (v, row) => <span className="ui-text-muted">{new Date(v as string).toLocaleDateString()} - {new Date(row['periodEnd'] as string).toLocaleDateString()}</span> },
              { key: 'status', header: 'Status', render: (v) => (v as string) === 'FILED' ? (
                <span className={`dark:text-green-400 bg-green-50 dark:bg-green-900/20 py-1 ${styles.s6}`} >
                  <CheckCircle className="h-3 w-3" /> FILED
                </span>
              ) : (
                <span className={`text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 py-1 ${styles.s7}`} >
                  {v as string}
                </span>
              ) },
              { key: 'id', header: 'Actions', render: (_, row) => <div className="text-right"><Button variant="ghost" size="sm" onClick={() => alert(JSON.stringify(row, null, 2))}>View Payload</Button></div> },
            ];
            return (
              <ListPageTemplate
