...
export default function BankReconMatchingPage() {
  ...
  const fetchTransactions = useCallback(async () => {
    setLoading(true);
    try {
      const body = await client.get<{ data: BankTransaction[] }>(
        `/advanced-finance/bank-feeds/transactions?status=${filterStatus}`,
      );
  ...
  const handleAutoMatch = async () => {
    ...
    try {
      const result = await client.post<{
        ...
        `/advanced-finance/bank-feeds/transactions/${selectedTx.id}/auto-match`,
      );
      if (result.matched) {
        setMatchResultMsg({
          type: "success",
        ...
        setMatchResultMsg({
          ...
          text:
            result.message ||
            "No automatic match found in the general ledger matching window.",
        });
      }
    } catch {
      setMatchResultMsg({
        type: "error",
        text: "Error executing auto-match algorithm.",
      });
    } finally {
  ...
  const handleManualMatch = async (e: React.FormEvent) => {
    ...
    try {
      await client.post(
        `/advanced-finance/bank-feeds/transactions/${selectedTx.id}/manual-match`,
        {
        ...
        <div className={styles.s1}>
          <div>
            ...
            <p className="ui-text-muted mt-1">
              Compare, match and reconcile downloaded bank transaction
              statements against GL entries and accounts receivable payments.
            </p>
          </div>
            ...
            {loading ? (
              ...
              <div className={styles.s7}>
                ...
                <p className={styles.s9}>
                  All feed downloads for this category are reconciled.
                </p>
              </div>
            ) : (
                    ...
                    >
                      <div>
                        ...
                        <p className="ui-text-xs-muted mt-1">
                          ...
                          {tx.connection?.bankName || "Direct Feed"}
                        </p>
                      </div>
          ...
          <div>
            {selectedTx ? (
              <Card className={`ui-card ${styles.s13}`}>
                    ...
                    <div className="ui-flex ui-gap-3">
                      ...
                      <div className={styles.s19}>
                        ...
                        <p className={styles.s20}>
                          {selectedTx.connection?.bankName ||
                            "Connected bank feed"}
                        </p>
                      </div>
                    </div>
                    <span
                      className={`${styles.selectedAmount} ${Number(selectedTx.amount) > 0 ? styles.amountPositive : styles.amountNegative}`}
                    >
                ...
                {filterStatus !== "UNMATCHED" && (
                  <div className={styles.s21}>
                    ...
                    <p className="ui-text-xs-muted mt-1">
                      ...
                      <span className={styles.s22}>{selectedTx.status}</span>
                    </p>
                    {selectedTx.matchedEntityId && (
                      <p className="ui-text-xs-muted mt-1">
                        Linked Ledger: {selectedTx.matchedEntityType} (
                        {selectedTx.matchedEntityId})
                      </p>
                    )}
                  </div>
                ...
                {filterStatus === "UNMATCHED" && (
                  <div className={styles.s23}>
                    {/* Action 1: Auto Match */}
                    <div>
                      <h4 className={styles.s24}>
                        Option 1: Automated Smart Reconciliation
                      </h4>
                      <p className={styles.s25}>
                        Scan our ledger journals and invoices to match by exact
                        amount and date timeline (+/- 7 days).
                      </p>
                      <Button
                        variant="primary"
                        onClick={handleAutoMatch}
                        disabled={matchingActionLoading}
                    ...
                    <div>
                      <h4 className={styles.s27}>
                        Option 2: Reference Linked Ledger Entry
                      </h4>
                      <p className={styles.s28}>
                        Manually map the download statement entry to a specific
                        Payment or GL Journal entry ID.
                      </p>
                      <form onSubmit={handleManualMatch} className="ui-stack-3">
                        <div className={`ui-grid-2 ${styles.s29}`}>
                          <div className="ui-form-group">
                            <label className={`ui-label ${styles.s30}`}>
                              Link Entity Type
                            </label>
                            <select
                          ...
                          <div className="ui-form-group">
                            <label className={`ui-label ${styles.s31}`}>
                              Entity UUID
                            </label>
                            <input
                              type="text"
                    ...
                    {/* Action 3: Ignore */}
                    <div>
                      <h4 className={styles.s33}>
                        Option 3: Disregard transaction
                      </h4>
                      <p className={styles.s34}>
                        Ignore transfers like internal inter-account cash
                        movements that are reconciled elsewhere.
                      </p>
                      <Button
                        variant="outline"
                    ...
                    {matchResultMsg && (
                      <div
                        className={`${styles.feedback} ${matchResultMsg.type === "success" ? styles.feedbackSuccess : matchResultMsg.type === "error" ? styles.feedbackError : styles.feedbackInfo}`}
                      >
              ...
              <Card className={`ui-card ${styles.s36}`}>
                <p>
                  Select a transaction from the feed list to match and
                  reconcile.
                </p>
              </Card>
            )}
