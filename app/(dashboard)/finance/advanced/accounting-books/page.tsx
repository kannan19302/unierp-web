...
export default function AccountingBooksPage() {
  ...
  const [trialBalance, setTrialBalance] = useState<{
    book: { name: string; standard: string };
  ...
  const loadRules = useCallback(async () => {
    try {
      const d = await client.get<unknown>(
        "/advanced-finance/accounting-books/rules",
      );
  ...
  const loadBooks = useCallback(async () => {
    try {
      const d = await client.get<AccountingBook[]>(
  ...
  const createBook = async () => {
    ...
    setError(null);
    try {
      await client.post("/advanced-finance/accounting-books", {
        name: newName.trim(),
  ...
  const viewTrialBalance = async (book: AccountingBook) => {
    ...
    try {
      const d = await client.get<{
        ...
        totalDebit: number;
        totalCredit: number;
      }>(`/advanced-finance/accounting-books/${book.id}/trial-balance`);
      setTrialBalance(d);
    } catch (e) {
  ...
  const runVariance = async () => {
    ...
    try {
      const d = await client.get<{
        ...
        `/advanced-finance/accounting-books/variance?book1=${book1Id}&book2=${book2Id}`,
      );
      setVariance(d);
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "Failed to run variance report",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <RouteGuard permission="finance.accounting.read">
      <div className="p-8 ui-stack-6">
        ...
        <div className={styles.s1}>
          <div>
            ...
            <p className="ui-text-muted mt-1">
              Manage parallel GAAP ledgers — post journals per book, view trial
              balances, and compare book-to-book variances.
            </p>
          </div>
        ...
        {error && <div className={styles.s5}>{error}</div>}
          ...
          <Card>
            <div className="p-5 ui-stack-4">
              <h3 className={styles.s12}>
                <Settings size={18} className="ui-text-primary" /> Configure
                Multi-Book Parallel Mapping Rule
              </h3>
              <div className={styles.s13}>
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="POST_DIRECTLY">
                      Post directly to same Account Code
                    </option>
                    <option value="EXCLUDE_ACCOUNT">
                      Exclude Account (Do not post)
                    </option>
                  </select>
                </div>
                ...
                <div>
                  <label className={styles.s24}>
                    Multiplier (Amount Scale)
                  </label>
                  <input
                    type="number"
                    step="0.0001"
        ...
        {books.length === 0 ? (
          <Card>
            <div className={styles.s28}>
              ...
              <p className="text-sm">
                No accounting books yet. Create your first book above.
              </p>
            </div>
          </Card>
        ) : (
              ...
              <Card key={b.id}>
                <div className="p-5">
                  <div className={styles.s31}>
                    ...
                    <Badge
                      variant={STANDARD_BADGE[b.standard] || "default"}
                      size="sm"
                    >
                      {b.standard}
                    </Badge>
                  ...
                  <p className={styles.s33}>
                    ...
                    {b.isPrimary && (
                      <span className={styles.s34}>
                        <CheckCircle size={12} className={styles.s35} />
                        Primary
                      </span>
                    )}
                  </p>
          ...
          <Card>
            <div className="p-5">
              <h3 className={styles.s37}>
                <ArrowLeftRight size={16} className="ui-text-primary" />{" "}
                Cross-Book Variance Report
              </h3>
              <div className={styles.s38}>
                ...
                <Button
                  onClick={runVariance}
                  disabled={
                    !book1Id || !book2Id || book1Id === book2Id || loading
                  }
                >
                  Run Report
                </Button>
            ...
            <div className="p-5">
              <div className="ui-flex-between mb-4">
                <h3 className="ui-heading-base">
                  Trial Balance — {selectedBook.name}{" "}
                  <Badge
                    variant={STANDARD_BADGE[selectedBook.standard] || "default"}
                    size="sm"
                  >
                    {selectedBook.standard}
                  </Badge>
                </h3>
                <button onClick={() => setView("list")} className={styles.s43}>
                  ✕ Close
                </button>
              </div>
              {loading ? (
                ...
                <>
                  <ListPageTemplate
                    ...
                    data={
                      trialBalance.rows as unknown as Record<string, unknown>[]
                    }
                    loading={false}
            ...
            <div className="p-5">
              <div className="ui-flex-between mb-4">
                ...
                <div className="ui-hstack-4">
                  <span className="ui-text-sm-muted">
                    Total variance:{" "}
                    <strong className={styles.s47}>
                      {variance.totalVariance.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                      })}
                    </strong>
                  </span>
              ...
              {loading ? (
                ...
                <ListPageTemplate
                  ...
                  data={
                    variance.variances as unknown as Record<string, unknown>[]
                  }
                  loading={false}
          ...
          <Card>
            <div className="p-5">
              <h3 className={styles.s53}>
                <Settings size={16} className="ui-text-primary" /> Parallel
                Ledger Posting & Mapping Rules
              </h3>
              {rules.length === 0 ? (
                <div className={styles.s54}>
                  <HelpCircle size={32} className={styles.s55} />
                  <p className="text-xs">
                    No mapping rules configured yet. Create one to automatically
                    map journal entries across books.
                  </p>
                </div>
              ) : (
