  ...
  const loadConfigAndCategories = useCallback(async () => {
    try {
      const [cfg, cats] = await Promise.all([
        ...
        storefrontGet<StorefrontPublicCategory[]>(
          `/store/${tenantSlug}/categories`,
        ),
      ]);
      setConfig(cfg);
      setCategories(cats);
    ...
    async (categoryId: string | null) => {
      try {
        const qs = categoryId
          ...
          : "";
        const page = await storefrontGet<PaginatedProducts>(
          `/store/${tenantSlug}/products${qs}`,
        );
        ...
        <Card>
          <div className={styles.s3}>
            ...
            <p className={styles.s6}>
              This storefront either doesn&apos;t exist or has not been enabled
              by its owner yet.
            </p>
          </div>
        </Card>
            ...
            {products.map((product) => (
              <Card key={product.listingId} padding="none">
                <div
                  ...
                  onClick={() =>
                    router.push(
                      `/store/${tenantSlug}/products/${product.listingId}`,
                    )
                  }
                ...
                >
                  ...
                  <div className={styles.s22}>
                    ...
                    <h3 className={styles.s23}>{product.name}</h3>
