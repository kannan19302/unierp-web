...
const BASE_URL = process.env.TARGET_URL || "http://localhost:4006";
...
async function run() {
  console.log("════════════════════════════════════════════════════════════════════════");
  console.log("🚀 UniERP Tenant Admin (SaaS Management) — E2E Integration Suite");
  ...
  console.log("📂 1. Tenant Admin SaaS Command Center");
  await checkRoute("/", "Tenant Admin Console", "Console renders header and SaaS management hub");

  console.log("\n🔑 2. Tenant Admin Authentication Portal");
  await checkRoute("/login", "Tenant Admin Console", "Login portal renders demo credentials notice");
  ...
  if (failedCount > 0) {
    ...
    console.log("🎉 All Tenant Admin End-to-End tests passed successfully!\n");
    process.exit(0);
  }
}
