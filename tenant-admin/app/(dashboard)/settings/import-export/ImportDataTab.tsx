...
const STEPS = [
  { title: "Select Model", description: "Choose target entity" },
  { title: "Upload File", description: "Upload CSV data" },
  { title: "Map Columns", description: "Match fields" },
  { title: "Validate", description: "Check data quality" },
  { title: "Import", description: "Execute import" },
];
...
export default function ImportDataTab() {
  ...
  const handleValidate = async () => {
    ...
    try {
      setValidation(
        await client.post<ValidationResult>("/admin/imports/validate", {
          model: targetModel,
          rows: mapped,
        }),
      );
          ...
          <div className="p-5">
            ...
            {step === 2 && (
              <div className="ui-stack-4">
                <div className="ui-flex-between">
                  <div>
                    <div className="ui-heading-sm">Column Mapping</div>
                    <div className="ui-text-xs-tertiary">
                      {parsedRows.length} rows detected, {sourceHeaders.length}{" "}
                      columns
                    </div>
                  </div>
                  <Badge variant="info">
                    {Object.keys(columnMap).length} mapped
                  </Badge>
                </div>
                      ...
                      columns={
                        [
                          ...
                          },
                          {
                            ...
                            header: "Field",
                            render: (r: ValidationError) => (
                              <code className={styles.s10}>{r.field}</code>
                            ),
                          },
                          {
                            key: "message",
                            header: "Error",
                            render: (r: ValidationError) => (
                              <span className={styles.s11}>{r.message}</span>
                            ),
                          },
            ...
            {step === 4 && result && (
              <div className={styles.s12}>
                ...
                <h3 className={styles.s13}>Import Complete</h3>
