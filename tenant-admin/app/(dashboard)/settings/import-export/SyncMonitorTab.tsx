...
const FALLBACK_ENTRIES: SyncEntry[] = [
  {
    ...
    payload: '{"title":"Offline Ticket #1"}',
    status: "PENDING",
    errorMessage: null,
  ...
  {
    id: "sync-3",
    clientId: "client-ipad-air",
    operation: "CREATE",
    entityType: "Attendance",
    payload: '{"checkIn":"09:00"}',
    status: "CONFLICT",
    errorMessage: "Duplicate record detected",
    createdAt: new Date(Date.now() - 14400000).toLocaleString(),
  },
];
...
export default function SyncMonitorTab() {
  ...
  const handleReconcile = async (
    id: string,
    status: "RECONCILED" | "CONFLICT" = "RECONCILED",
    errorMessage?: string,
  ) => {
    setReconciling(id);
    try {
      ...
      await client.request(`/admin/pwa-sync/reconcile/${id}`, {
        method: "PUT",
      ...
      showToast(
        status === "RECONCILED" ? "Entry reconciled" : "Marked as conflict",
        "success",
      );
    } catch {
      setEntries((prev: any) =>
  ...
  const statusIcon = (status: string) => {
    switch (status) {
      case "PENDING":
        return <Clock size={15} className="ui-text-warning" />;
      case "RECONCILED":
        return <CheckCircle size={15} className="ui-text-success" />;
      case "CONFLICT":
        return <AlertTriangle size={15} className="ui-text-danger" />;
      default:
        return null;
    }
  };
    ...
    <RouteGuard permission="settings.import-export.read">
      <div className="ui-stack-6">
        ...
        <div className="ui-grid-auto">
          <Card
            padding="lg"
            style={{ textAlign: "center", borderLeft: "4px solid #f59e0b" }}
          >
          ...
          <Card
            padding="lg"
            style={{ textAlign: "center", borderLeft: "4px solid #22c55e" }}
          >
          ...
          <Card
            padding="lg"
            style={{ textAlign: "center", borderLeft: "4px solid #ef4444" }}
          >
        ...
        <div className="ui-flex ui-gap-2">
          ...
          <button
            style={tabStyle("RECONCILED")}
            onClick={() => setFilterTab("RECONCILED")}
          >
            Reconciled ({reconciledCount})
          </button>
          <button
            style={tabStyle("CONFLICT")}
                    ...
                    return (
                      <div className={styles.s10}>
                        <Button
                          variant="outline"
                          onClick={() =>
                            setConflictModal(row as unknown as SyncEntry)
                          }
                          style={{
                            fontSize: "var(--text-xs)",
                            padding: "var(--space-1) var(--space-3)",
              ...
              <div className="p-5 ui-stack-4">
                <div>
                  <div className={styles.s15}>
                    {conflictModal.operation} {conflictModal.entityType} from{" "}
                    {conflictModal.clientId}
                  </div>
                    ...
                    {(() => {
                      try {
                        return JSON.stringify(
                          JSON.parse(conflictModal.payload),
                          null,
                          2,
                        );
                      } catch {
                        return conflictModal.payload;
                      }
                    })()}
                  </pre>
                </div>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleReconcile(
                        ...
                        "CONFLICT",
                        conflictModal.errorMessage ??
                          "Manually kept as conflict",
                      )
                    }
                    disabled={reconciling === conflictModal.id}
                  >
                    Keep Conflict
                  </Button>
                  <Button
                    variant="primary"
                    onClick={() =>
                      handleReconcile(conflictModal.id, "RECONCILED")
                    }
                    disabled={reconciling === conflictModal.id}
                  >
                    {reconciling === conflictModal.id
                      ? "Reconciling..."
                      : "Force Reconcile"}
                  </Button>
                </div>
