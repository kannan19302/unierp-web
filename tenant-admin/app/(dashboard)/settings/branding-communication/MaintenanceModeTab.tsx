...
export default function MaintenanceModeTab() {
  ...
  useEffect(() => {
    const fetchMaintenance = async () => {
      ...
      try {
        setMaintenance(
          await client.get<typeof maintenance>("/admin/platform/maintenance"),
        );
      } catch (e) {
        console.error(e);
      } finally {
      ...
      <div className={styles.s2}>
        ...
        <div>
          ...
          <p className={styles.s4}>
            Organization Owners and authorized tenant administrators bypass maintenance
            barriers and can log in normally to verify changes or resolve
            issues.
          </p>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <form onSubmit={handleSave} className={styles.s5}>
            ...
            <div className={styles.s10}>
              ...
              >
                {saving ? "Saving Status..." : "Apply Status Settings"}
              </button>
              {saved && (
                <span className={styles.s12}>
                  <CheckCircle size={12} /> Maintenance Mode saved
                </span>
              )}
            </div>
          </form>
