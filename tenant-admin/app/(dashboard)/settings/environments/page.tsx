...
export default function EnvironmentManagerPage() {
  ...
  const fetchEnvironments = async () => {
    ...
    try {
      setEnvironments(
        await client.get<SandboxEnvironment[]>("/admin/platform/environments"),
      );
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleSyncSandbox = async (type: string) => {
    ...
    try {
      ...
      {
        setFeedback(
          `Environment ${type} has been synchronized with Production records.`,
        );
        ...
        <div className="ui-flex-between">
          <div>
            ...
            <p className="ui-text-sm-muted">
              Configure and synchronize testing / staging sandbox environments
              with Production data schemas.
            </p>
          </div>
          <button
                ...
                <div className="ui-hstack-4">
                    ...
                    style={{
                      background:
                        env.status === "ACTIVE"
                          ...
                          : "rgba(var(--color-text-secondary-rgb), 0.1)",
                      color:
                        env.status === "ACTIVE"
                          ? "var(--color-success)"
                          : "var(--color-text-secondary)",
                  ...
                  {env.type !== "PROD" && (
                    <button
                      ...
                      style={{
                        cursor: syncingType !== null ? "wait" : "pointer",
                      }}
                      className={styles.s3}
                    >
                      <RefreshCw
                        size={12}
                        className={syncingType === env.type ? "spin" : ""}
                      />
                      Sync Data
                    </button>
                  )}
