...
export default function AutomationRulesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) await client.patch(url, form);
  ...
  const handleTest = async () => {
    ...
    try {
      const data = await client.post(
        `${API_BASE}/${testModal}/test`,
        JSON.parse(testInput) as unknown,
      );
  ...
  const openEdit = (r: AutomationRule) => {
    ...
    setForm({
      ...
      actions: r.actions ?? [],
      runOnce: r.runOnce,
      logExecution: r.logExecution,
      haltOnError: r.haltOnError,
    });
    setShowModal(true);
  };
  ...
  const updateCondition = (idx: number, patch: Partial<Condition>) => {
    ...
    c[idx] = { ...c[idx], ...patch } as Condition;
  ...
  const addAction = () =>
    setForm({
      ...form,
      actions: [...form.actions, { type: "send_email", config: "" }],
    });
  const updateAction = (idx: number, patch: Partial<RuleAction>) => {
    const a = [...form.actions];
    a[idx] = { ...a[idx], ...patch } as RuleAction;
      ...
      <div className={styles.p1}>
        ...
        <div className="ui-flex ui-gap-2">
          <button
            ...
            style={tabBtn("rules", "Rules", <Zap size={14} />)}
          >
            <Zap size={14} /> Rules
          </button>
          <button
            onClick={() => setTab("history")}
            style={tabBtn("history", "History", <History size={14} />)}
          >
                ...
                },
                {
                  ...
                  render: (v: any, row: any) => (
                    <div className="ui-flex-end ui-gap-2">
                      ...
                      <button
                        onClick={() =>
                          openEdit(row as unknown as AutomationRule)
                        }
                        style={{ ...btnGhost }}
                        className={styles.s3}
                        title="Edit"
                      ...
                      <button
                        onClick={() => setConfirmDeleteId(String(v))}
                        style={{ ...btnGhost }}
                        className={styles.s4}
                        title="Delete"
                      >
        ...
        <div style={overlay} onClick={() => setConfirmDeleteId(null)}>
          ...
          >
            ...
            <p className={styles.p5}>
              This will permanently remove this automation rule and all
              associated execution history.
            </p>
            <div className="ui-flex-end ui-gap-2">
              <button onClick={() => setConfirmDeleteId(null)} style={btnGhost}>
                Cancel
              </button>
              <button
      ...
      {testModal && (
        <div style={overlay} onClick={() => setTestModal(null)}>
          ...
          >
            ...
            {testResult && <div className={styles.s10}>{testResult}</div>}
          </div>
        </div>
      )}
          ...
          <div style={modal} onClick={(e: any) => e.stopPropagation()}>
            ...
            <div className="ui-stack-4">
              ...
              <div className={styles.p12}>
                <div style={{ ...labelStyle }} className={styles.s15}>
                  Settings
                </div>
            ...
            <div className={styles.p16}>
              <button onClick={() => setShowModal(false)} style={btnGhost}>
                Cancel
              </button>
